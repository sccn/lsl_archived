// lslboost heap: heap merge algorithms
//
...
#include <lslboost/heap/heap_concepts.hpp>
...
struct heap_merge_emulate
{
    ...
    typedef typename lslboost::mpl::if_c<Heap1::has_reserve,
                                      reserver,
...
struct heap_merge_same
{
    ...
    typedef typename lslboost::mpl::if_c<is_mergable,
...
{
    BOOST_CONCEPT_ASSERT((lslboost::heap::PriorityQueue<Heap1>));
    BOOST_CONCEPT_ASSERT((lslboost::heap::PriorityQueue<Heap2>));

    // if this assertion is triggered, the value_compare types are incompatible
    BOOST_STATIC_ASSERT((lslboost::is_same<typename Heap1::value_compare, typename Heap2::value_compare>::value));

    const bool same_heaps = lslboost::is_same<Heap1, Heap2>::value;

    typedef typename lslboost::mpl::if_c<same_heaps,
