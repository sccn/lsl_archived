// lslboost heap: helper classes for stable priority queues
//
...
#include <lslboost/heap/heap_merge.hpp>
...
template <typename T,
          ...
          typename StabilityCounterType = lslboost::uintmax_t,
          bool stable = false
         >
struct heap_base:
    Cmp,
    ...
    size_holder<constant_time_size, size_t>
{
    ...
    internal_type make_node(T const & val)
    {
        ...
        if (counter_ == std::numeric_limits<stability_counter_type>::max())
            BOOST_THROW_EXCEPTION(std::runtime_error("lslboost::heap counter overflow"));
...
class stable_heap_iterator:
    public lslboost::iterator_adaptor<stable_heap_iterator<T, ContainerIterator, Extractor>,
                                   ContainerIterator,
                                   T const,
                                   lslboost::random_access_traversal_tag>
{
    typedef lslboost::iterator_adaptor<stable_heap_iterator,
                                    ContainerIterator,
                                    T const,
                                    lslboost::random_access_traversal_tag> super_t;
...
struct make_heap_base
{
    ...
    typedef typename parameter::binding<Parspec, tag::stability_counter_type, lslboost::uintmax_t >::type stability_counter_type;
