...
std::pair<T, T> bisect(F f, T min, T max, Tol tol, lslboost::uintmax_t& max_iter, const Policy& pol)
{
   T fmin = f(min);
   T fmax = f(max);
   if(fmin == 0)
   ...
   // Error checking:
   //
   static const char* function = "lslboost::math::tools::bisect<%1%>";
   if(min >= max)
   {
      return lslboost::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "Arguments in wrong order in lslboost::math::tools::bisect (first arg=%1%)", min, pol));
   }
   if(fmin * fmax >= 0)
   {
      return lslboost::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "No change of sign in lslboost::math::tools::bisect, either there is no root to find, or there are multiple roots in the interval (f(min) = %1%).", fmin, pol));
   }
   ...
   static lslboost::uintmax_t max_count = 0;
...
inline std::pair<T, T> bisect(F f, T min, T max, Tol tol, lslboost::uintmax_t& max_iter)
{
...
{
   lslboost::uintmax_t m = (std::numeric_limits<lslboost::uintmax_t>::max)();
   return bisect(f, min, max, tol, m, policies::policy<>());
}

template <class F, class T>
T newton_raphson_iterate(F f, T guess, T min, T max, int digits, lslboost::uintmax_t& max_iter)
{
   ...
   do{
      ...
      delta2 = delta1;
      delta1 = delta;
      lslboost::math::tie(f0, f1) = f(result);
      if(0 == f0)
         break;
      if(f1 == 0)
      {
...
template <class F, class T>
T halley_iterate(F f, T guess, T min, T max, int digits, lslboost::uintmax_t& max_iter)
{
   BOOST_MATH_STD_USING
   ...
   do{
      ...
      delta2 = delta1;
      delta1 = delta;
      lslboost::math::tie(f0, f1, f2) = f(result);
      ...
      else
      {
         if(f2 != 0)
         {
            ...
            if(delta * f1 / f0 < 0)
            {
               ...
               // See https://svn.lslboost.org/trac/lslboost/ticket/8314.
               delta = f0 / f1;
...
template <class F, class T>
T schroeder_iterate(F f, T guess, T min, T max, int digits, lslboost::uintmax_t& max_iter)
{
   BOOST_MATH_STD_USING
