...
#include <lslboost/math/tools/solve.hpp>
#include <lslboost/math/tools/minima.hpp>
#include <lslboost/math/tools/roots.hpp>
#include <lslboost/math/tools/polynomial.hpp>
...
{
   typedef lslboost::function1<T, T const &> function_type;
public:
...
public:
   typedef lslboost::function1<T, T const &> function_type;
   typedef lslboost::numeric::ublas::vector<T> vector_type;
   typedef lslboost::numeric::ublas::matrix<T> matrix_type;
...
void remez_minimax<T>::init_chebyshev()
{
   ...
   //
   vector_type l_solution = lslboost::math::tools::solve(A, b);
         ...
         int bits)
{
   ...
   if(bits == 0)
   {
      ...
      m_precision = (std::min)(24, (lslboost::math::policies::digits<T, lslboost::math::policies::policy<> >() / 2) - 2);
   }
   else
   {
      // can't be more accurate than half the bits of T:
      m_precision = (std::min)(bits, (lslboost::math::policies::digits<T, lslboost::math::policies::policy<> >() / 2) - 2);
   }
...
T remez_minimax<T>::iterate()
{
   ...
   do{
      ...
      // Now go ahead and solve the expression to get our solution:
      //
      solution = lslboost::math::tools::solve(A, b);
   ...
   for(unsigned i = 1; i < control_points.size(); ++i)
   {
      ...
      lslboost::uintmax_t max_iter = 1000;
...
{
   lslboost::scoped_array<T> a(new T[orderN + 1]);
   if(pinned)
      a[0] = 0;
   unsigned terms = pinned ? orderN : orderN + 1;
   for(unsigned i = 0; i < terms; ++i)
      a[pinned ? i+1 : i] = solution[i];
   return lslboost::math::tools::polynomial<T>(&a[0], orderN);
}
...
polynomial<T> remez_minimax<T>::denominator()const
{
   ...
   lslboost::scoped_array<T> a(new T[terms]);
   a[0] = 1;
   for(unsigned i = 0; i < orderD; ++i)
      a[i+1] = solution[i + offsetD];
   return lslboost::math::tools::polynomial<T>(&a[0], orderD);
}
