// lslboost\math\tools\promotion.hpp
...
#include <lslboost/type_traits/is_floating_point.hpp> // for lslboost::is_floating_point;
#include <lslboost/type_traits/is_integral.hpp> // for lslboost::is_integral
#include <lslboost/type_traits/is_convertible.hpp> // for lslboost::is_convertible
#include <lslboost/type_traits/is_same.hpp>// for lslboost::is_same
#include <lslboost/type_traits/remove_cv.hpp>// for lslboost::remove_cv
// Boost Template meta programming:
#include <lslboost/mpl/if.hpp> // for lslboost::mpl::if_c.
#include <lslboost/mpl/and.hpp> // for lslboost::mpl::if_c.
#include <lslboost/mpl/or.hpp> // for lslboost::mpl::if_c.
#include <lslboost/mpl/not.hpp> // for lslboost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_<
          ...
          typename mpl::if_< typename mpl::and_<mpl::not_<is_floating_point<T2P> >, ::lslboost::is_convertible<T1P, T2P> >, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT_MSG((0 == ::lslboost::is_same<type, long double>::value), "Sorry, but this platform does not have sufficient long double support for the special functions to be reliably implemented.");
#endif
      };
