...
#  define BOOST_MATH_LOGGER_INCLUDE <lslboost/math/tools/iteration_logger.hpp>
...
void bracket(F f, T& a, T& b, T c, T& fa, T& fb, T& d, T& fd)
{
   ...
   //
   if(lslboost::math::sign(fa) * lslboost::math::sign(fc) < 0)
   {
      d = b;
      fd = fb;
      b = c;
                           ...
                           unsigned count)
{
   ...
   //
   T c;
   if(lslboost::math::sign(A) * lslboost::math::sign(fa) > 0)
   {
      c = a;
   }
   else
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, lslboost::uintmax_t& max_iter, const Policy& pol)
{
   //
   ...
   static const char* function = "lslboost::math::tools::toms748_solve<%1%>";

   lslboost::uintmax_t count = max_iter;
   ...
   b = bx;
   if(a >= b)
      return lslboost::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
   ...
   if(lslboost::math::sign(fa) * lslboost::math::sign(fb) > 0)
      return lslboost::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
...
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, lslboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, fax, fbx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, lslboost::uintmax_t& max_iter, const Policy& pol)
{
   max_iter -= 2;
...
template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, lslboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising, Tol tol, lslboost::uintmax_t& max_iter, const Policy& pol)
{
   BOOST_MATH_STD_USING
   static const char* function = "lslboost::math::tools::bracket_and_solve_root<%1%>";
   //
   ...
   if((fa < 0) == (guess < 0 ? !rising : rising))
   {
      ...
      {
         if(count == 0)
            return lslboost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", b, pol));
         //
   ...
   else
   {
      ...
      while((lslboost::math::sign)(fb) == (lslboost::math::sign)(fa))
      {
         ...
         if(count == 0)
            return lslboost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", a, pol));
         //
         // Heuristic: normally it's best not to increase the step sizes as we'll just end up
         // with a really wide range to search for the root.  However, if the initial guess was *really*
         // bad then we need to speed up the search otherwise we'll take forever if we're orders of
...
template <class F, class T, class Tol>
inline std::pair<T, T> bracket_and_solve_root(F f, const T& guess, const T& factor, bool rising, Tol tol, lslboost::uintmax_t& max_iter)
{
