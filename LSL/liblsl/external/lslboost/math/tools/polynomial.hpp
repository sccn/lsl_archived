...
T chebyshev_coefficient(unsigned n, unsigned m)
{
   ...
   if(r & 1)
      result = -result;
   result /= n - r;
   result *= lslboost::math::binomial_coefficient<T>(n - r, r);
   result *= ldexp(1.0f, m);
   return result;
}
...
public:
   ...
   polynomial(const polynomial<U>& p)
   {
      ...
      {
         m_data.push_back(lslboost::math::tools::real_cast<T>(p[i]));
      }
   }
   ...
   {
      return lslboost::math::tools::evaluate_polynomial(&m_data[0], z, m_data.size());;
   }
