...
#include <lslboost/tr1/random.hpp>
...
inline void unpack_and_append_tuple(Seq& s,
                                    const Item& data,
                                    const lslboost::integral_constant<int, N>&,
                                    const lslboost::false_type&)
{
...
inline void unpack_and_append_tuple(Seq& s,
                                    const Item& data,
                                    const lslboost::integral_constant<int, N>&,
                                    const lslboost::true_type&)
{
   // extract the N'th element, append, and recurse:
   typedef typename Seq::value_type value_type;
   value_type val = lslboost::math::get<N>(data);
   s.push_back(val);

   typedef lslboost::integral_constant<int, N+1> next_value;
   typedef lslboost::integral_constant<bool, (lslboost::math::tuple_size<Item>::value > N+1)> terminate;

   unpack_and_append_tuple(s, data, next_value(), terminate());
}

template <class Seq, class Item>
inline void unpack_and_append(Seq& s, const Item& data, const lslboost::true_type&)
{
   s.push_back(data);
}

template <class Seq, class Item>
inline void unpack_and_append(Seq& s, const Item& data, const lslboost::false_type&)
{
   // Item had better be a tuple-like type or we've had it!!!!
   typedef lslboost::integral_constant<int, 0> next_value;
   typedef lslboost::integral_constant<bool, (lslboost::math::tuple_size<Item>::value > 0)> terminate;
...
{
   typedef typename Seq::value_type value_type;
   unpack_and_append(s, data, ::lslboost::is_convertible<Item, value_type>());
}
   ...
   test_data& insert(F func, const parameter_info<T>& arg1)
   {
      ...
      while(a != b)
      {
         ...
         try{
            ...
            lslboost::math::tools::detail::unpack_and_append(row, func(*a));
            m_data.insert(row);
         }
...
#  pragma warning(disable: 4127)
   ...
   case periodic_in_range:
      ...
      do{
         ...
         try{
            info.z1 = lslboost::lexical_cast<T>(line);
            break;
         }
         catch(const lslboost::bad_lexical_cast&)
         {
      ...
      do{
         ...
         else
         {
            try
            {
               info.z2 = lslboost::lexical_cast<T>(line);
            }
            catch(const lslboost::bad_lexical_cast&)
            {
               std::cout << "Sorry, that was not valid input, try again [y/n]?";
      ...
      do{
         ...
         lslboost::algorithm::trim(line);
         try{
            info.n1 = lslboost::lexical_cast<int>(line);
            info.n2 = 0;
            if(info.n1 <= 0)
            {
   ...
   case power_series:
      ...
      do{
         ...
         lslboost::algorithm::trim(line);
         try{
            info.n2 = lslboost::lexical_cast<int>(line);
            break;
         }
         catch(const lslboost::bad_lexical_cast&)
         {
                         ...
                         const char* name)
{
   ...
   "   static const lslboost::array<lslboost::array<T, "
