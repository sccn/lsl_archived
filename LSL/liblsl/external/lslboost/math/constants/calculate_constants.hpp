...
namespace lslboost{ namespace math{ namespace constants{ namespace detail{
...
inline T constant_pi<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
   ...
   T lim;
   lim = lslboost::math::tools::epsilon<T>();
   ...
   do
   {
      ...
      B = ldexp(B, 1);
      result = A - B;
      bool neg = lslboost::math::sign(result) < 0;
      if(neg)
         result = -result;
      if(result <= lim)
         break;
...
{ // http://oeis.org/A006752/constant
 ...
 //"149374281672134266498119621763019776254769479356512926115106248574");
  ...
  // 9.159655941772190150546035149323841107, 74149374281672134266498119621763019776254769479356512926115106248574422619, -01);
   ...
   return lslboost::math::constants::pi<T, lslboost::math::policies::policy<> >()
      * log(2 + lslboost::math::constants::root_three<T, lslboost::math::policies::policy<> >())
       / 8
      + 3 * sum / 8;
}
...
T khinchin(int digits)
{
   ...
   return exp(sum / lslboost::math::constants::ln_two<T, lslboost::math::policies::policy<> >());
}
...
T zeta_series_derivative_2(unsigned digits)
{
   ...
   lslboost::math::itrunc((std::numeric_limits<T>::digits10 + 1) * 1.3);
...
inline T zeta_series_derivative_lead_2()
{
   ...
   return -2 * lslboost::math::constants::ln_two<T>();
}
...
inline T constant_glaisher<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
   ...
   v *= 6;
   v /= lslboost::math::constants::pi<T, forwarding_policy>() * lslboost::math::constants::pi<T, forwarding_policy>();
   v -= lslboost::math::constants::euler<T, forwarding_policy>();
   v -= log(2 * lslboost::math::constants::pi<T, forwarding_policy>());
   v /= -12;
   return exp(v);
