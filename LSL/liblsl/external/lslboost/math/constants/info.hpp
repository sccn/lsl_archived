...
#pragma warning(disable:4127)
   ...
   if(precision_type::value)
   {
      ...
      {
       os <<
       "lslboost::math::policies::precision<" << nameof<T>() << ", " << nameof<Policy>() << " reports that the compile time precision is \n" << precision_type::value << " binary digits.\n";
      }
      else if (std::numeric_limits<T>::radix == 10)
      {
         os <<
   ...
   {
      os <<
         "lslboost::math::policies::precision<" << nameof<T>() << ", Policy> \n"
         "reports that there is no compile type precision available.\n"
         "lslboost::math::tools::digits<" << nameof<T>() << ">() \n"
         "reports that the current runtime precision is \n" <<
         lslboost::math::tools::digits<T>() << " binary digits.\n";
   }
   ...
   case 0:
      ...
      if(lslboost::math::tools::digits<T>() > max_string_digits)
      {
...
{
   print_info_on_type<T, lslboost::math::policies::policy<> >(os);
}
