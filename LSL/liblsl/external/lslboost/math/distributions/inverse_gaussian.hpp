...
#include <lslboost/math/special_functions/erf.hpp> // for erf/erfc.
...
#include <lslboost/math/distributions/gamma.hpp> // for gamma function
// using lslboost::math::gamma_p;

#include <lslboost/math/tools/tuple.hpp>
//using std::tr1::tuple;
//using std::tr1::make_tuple;
#include <lslboost/math/tools/roots.hpp>
//using lslboost::math::tools::newton_raphson_iterate;
...
public:
   ...
   { // Default is a 1,1 inverse_gaussian distribution.
     static const char* function = "lslboost::math::inverse_gaussian_distribution<%1%>::inverse_gaussian_distribution";
...
{ // Probability Density Function
   ...
   RealType result = 0;
   static const char* function = "lslboost::math::pdf(const inverse_gaussian_distribution<%1%>&, %1%)";
...
{ // Cumulative Density Function.
   ...
   RealType scale = dist.scale();
   RealType mean = dist.mean();
   static const char* function = "lslboost::math::cdf(const inverse_gaussian_distribution<%1%>&, %1%)";
   RealType result = 0;
   if(false == detail::check_scale(function, scale, &result, Policy()))
   {
      return result;
...
{ 
  ...
  inverse_gaussian_quantile_functor(const lslboost::math::inverse_gaussian_distribution<RealType, Policy> dist, RealType const& p)
    : distribution(dist), prob(p)
  {
  }
  lslboost::math::tuple<RealType, RealType> operator()(RealType const& x)
  {
    ...
    return lslboost::math::make_tuple(fx, dx);
  }
  private:
  const lslboost::math::inverse_gaussian_distribution<RealType, Policy> distribution;
  RealType prob; 
};
...
{ 
    inverse_gaussian_quantile_complement_functor(const lslboost::math::inverse_gaussian_distribution<RealType, Policy> dist, RealType const& p)
    : distribution(dist), prob(p)
  {
  }
  lslboost::math::tuple<RealType, RealType> operator()(RealType const& x)
...
namespace detail
{
  ...
  { // guess at random variate value x for inverse gaussian quantile.
      ...
      using lslboost::math::policies::policy;
      // Error type.
      using lslboost::math::policies::overflow_error;
      // Action.
      using lslboost::math::policies::ignore_error;
    ...
    { // phi < 2 so much less symmetrical with long tail,
      ...
      using lslboost::math::gamma_distribution;
...
inline RealType quantile(const inverse_gaussian_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType mean = dist.mean();
   RealType scale = dist.scale();
   static const char* function = "lslboost::math::quantile(const inverse_gaussian_distribution<%1%>&, %1%)";
  ...
  lslboost::uintmax_t m = policies::get_max_root_iterations<Policy>(); // and max iterations.
  using lslboost::math::tools::newton_raphson_iterate;
  result =
...
inline RealType cdf(const complemented2_type<inverse_gaussian_distribution<RealType, Policy>, RealType>& c)
{
   ...
   RealType x = c.param;
   static const char* function = "lslboost::math::cdf(const complement(inverse_gaussian_distribution<%1%>&), %1%)";
...
inline RealType quantile(const complemented2_type<inverse_gaussian_distribution<RealType, Policy>, RealType>& c)
{
   ...
   RealType scale = c.dist.scale();
   RealType mean = c.dist.mean();
   static const char* function = "lslboost::math::quantile(const complement(inverse_gaussian_distribution<%1%>&), %1%)";
   RealType result = 0;
   if(false == detail::check_scale(function, scale, &result, Policy()))
      return result;
   if(false == detail::check_location(function, mean, &result, Policy()))
  ...
  lslboost::uintmax_t m = policies::get_max_root_iterations<Policy>();
  using lslboost::math::tools::newton_raphson_iterate;
  result =
