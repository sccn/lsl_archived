...
{ // Error checks:
  ...
  {
     if((sigma <= 0) || (!(lslboost::math::isfinite)(sigma)))
     {
...
public:
   ...
   {
      RealType err;
      detail::verify_sigma("lslboost::math::rayleigh_distribution<%1%>::rayleigh_distribution", l_sigma, &err, Policy());
...
inline RealType pdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result = 0;
   static const char* function = "lslboost::math::pdf(const rayleigh_distribution<%1%>&, %1%)";
...
inline RealType cdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "lslboost::math::cdf(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   ...
   result = -lslboost::math::expm1(-x * x / ( 2 * sigma * sigma), Policy());
   return result;
} // cdf
...
inline RealType quantile(const rayleigh_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "lslboost::math::quantile(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
      return result;
   ...
   result = sqrt(-2 * sigma * sigma * lslboost::math::log1p(-p, Policy()));
   return result;
} // quantile
...
inline RealType mean(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "lslboost::math::mean(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using lslboost::math::constants::root_half_pi;
...
inline RealType variance(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "lslboost::math::variance(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using lslboost::math::constants::four_minus_pi;
...
{
   using lslboost::math::constants::root_ln_four;
