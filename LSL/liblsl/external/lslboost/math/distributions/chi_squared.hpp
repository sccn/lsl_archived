...
public:
   ...
   chi_squared_distribution(RealType i) : m_df(i)
   {
      RealType result;
      detail::check_df(
         "lslboost::math::chi_squared_distribution<%1%>::chi_squared_distribution", m_df, &result, Policy());
...
{
   RealType degrees_of_freedom = dist.degrees_of_freedom();
   static const char* function = "lslboost::math::quantile(const chi_squared_distribution<%1%>&, %1%)";
   // Error check:
   RealType error_result;
   if(false ==
     (
   ...
   return 2 * lslboost::math::gamma_p_inv(degrees_of_freedom / 2, p, Policy());
} // quantile
...
inline RealType cdf(const complemented2_type<chi_squared_distribution<RealType, Policy>, RealType>& c)
{
   ...
   return lslboost::math::gamma_q(degrees_of_freedom / 2, chi_square / 2, Policy());
}
...
inline RealType quantile(const complemented2_type<chi_squared_distribution<RealType, Policy>, RealType>& c)
{
   ...
   return 2 * lslboost::math::gamma_q_inv(degrees_of_freedom / 2, q, Policy());
}
...
RealType chi_squared_distribution<RealType, Policy>::find_degrees_of_freedom(
   ...
   RealType hint)
{
   static const char* function = "lslboost::math::chi_squared_distribution<%1%>::find_degrees_of_freedom(%1%,%1%,%1%,%1%,%1%)";
