...
bool check_probabilities(char const* function, std::vector<RealT> const& probabilities, RealT* presult, PolicyT const& pol)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(lslboost::math::isfinite)(probabilities[i]))
        {
...
bool check_rates(char const* function, std::vector<RealT> const& rates, RealT* presult, PolicyT const& pol)
{
    ...
    {
        if (rates[i] <= 0
            || !(lslboost::math::isfinite)(rates[i]))
        {
            *presult = policies::raise_domain_error<RealT>(function,
...
{
    if (p < 0 || p > 1 || (lslboost::math::isnan)(p))
    {
...
RealT quantile_impl(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& p, bool comp)
{
    ...
    static const char* function = comp ? "lslboost::math::quantile(const lslboost::math::complemented2_type<lslboost::math::hyperexponential_distribution<%1%>, %1%>&)"
                                       : "lslboost::math::quantile(const lslboost::math::hyperexponential_distribution<%1%>&, %1%)";
...
class hyperexponential_distribution
{
    ...
    {
        RealT err;
        hyperexp_detail::check_dist("lslboost::math::hyperexponential_distribution<%1%>::hyperexponential_distribution",
                                    probs_,
                                    rates_,
                                    &err,
                                    PolicyT());
    ...
    public: template <typename ProbRangeT, typename RateRangeT>
            hyperexponential_distribution(ProbRangeT const& prob_range,
                                          ...
                                          typename lslboost::disable_if_c<lslboost::has_pre_increment<ProbRangeT>::value || lslboost::has_pre_increment<RateRangeT>::value>::type* = 0)
    : probs_(lslboost::begin(prob_range), lslboost::end(prob_range)),
      rates_(lslboost::begin(rate_range), lslboost::end(rate_range))
    {
        hyperexp_detail::normalize(probs_);
    ...
    public: template <typename RateIterT, typename RateIterT2>
            hyperexponential_distribution(RateIterT const& rate_first, 
                                          ...
                                          typename lslboost::enable_if_c<lslboost::has_pre_increment<RateIterT>::value || lslboost::has_pre_increment<RateIterT2>::value>::type* = 0)
...
public: hyperexponential_distribution(std::initializer_list<RealT> l1)
    ...
    : probs_(lslboost::size(rate_range), 1), // will be normalized below
      rates_(lslboost::begin(rate_range), lslboost::end(rate_range))
    {
        hyperexp_detail::normalize(probs_);
...
RealT pdf(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& x)
{
    ...
    if (!hyperexp_detail::check_x("lslboost::math::pdf(const lslboost::math::hyperexponential_distribution<%1%>&, %1%)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT cdf(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& x)
{
    ...
    if (!hyperexp_detail::check_x("lslboost::math::cdf(const lslboost::math::hyperexponential_distribution<%1%>&, %1%)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT cdf(complemented2_type<hyperexponential_distribution<RealT,PolicyT>, RealT> const& c)
{
    ...
    if (!hyperexp_detail::check_x("lslboost::math::cdf(lslboost::math::complemented2_type<const lslboost::math::hyperexponential_distribution<%1%>&, %1%>)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT variance(hyperexponential_distribution<RealT, PolicyT> const& dist)
{
    ...
    const RealT mean = lslboost::math::mean(dist);
