...
public:
   ...
   { // Constructor.
      RealType result;
      detail::check_df(
         "lslboost::math::students_t_distribution<%1%>::students_t_distribution", m_df, &result, Policy());
...
inline RealType pdf(const students_t_distribution<RealType, Policy>& dist, const RealType& t)
{
   ...
   if(false == detail::check_df(
      "lslboost::math::pdf(const students_t_distribution<%1%>&, %1%)", degrees_of_freedom, &error_result, Policy()))
      return error_result;
   ...
   if(basem1 < 0.125)
   {
      result = exp(-lslboost::math::log1p(basem1, Policy()) * (1+degrees_of_freedom) / 2);
   }
   else
   {
      result = pow(1 / (1 + basem1), (degrees_of_freedom + 1) / 2);
   }
   result /= sqrt(degrees_of_freedom) * lslboost::math::beta(degrees_of_freedom / 2, RealType(0.5f), Policy());
   return result;
} // pdf
...
inline RealType cdf(const students_t_distribution<RealType, Policy>& dist, const RealType& t)
{
   ...
   RealType error_result;
   if(false == detail::check_df(
      "lslboost::math::cdf(const students_t_distribution<%1%>&, %1%)", degrees_of_freedom, &error_result, Policy()))
      return error_result;
...
inline RealType quantile(const students_t_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   //
   return lslboost::math::detail::fast_students_t_quantile(degrees_of_freedom, probability, Policy());
} // quantile
...
inline RealType variance(const students_t_distribution<RealType, Policy>& dist)
{
   ...
   RealType error_result;
   if(false == detail::check_df(
      "lslboost::math::variance(students_t_distribution<%1%> const&, %1%)", dist.degrees_of_freedom(), &error_result, Policy()))
      return error_result;
