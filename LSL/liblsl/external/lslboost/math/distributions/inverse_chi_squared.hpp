...
#include <lslboost/math/special_functions/gamma.hpp> // for incomplete beta.
#include <lslboost/math/distributions/complement.hpp> // for complements.
#include <lslboost/math/distributions/detail/common_error_handling.hpp> // for error checks.
#include <lslboost/math/special_functions/fpclassify.hpp> // for isfinite
...
public:
   ...
   inverse_chi_squared_distribution(RealType df, RealType l_scale) : m_df(df), m_scale (l_scale)
   {
      RealType result;
      detail::check_df(
         "lslboost::math::inverse_chi_squared_distribution<%1%>::inverse_chi_squared_distribution",
         m_df, &result, Policy())
         && detail::check_scale(
"lslboost::math::inverse_chi_squared_distribution<%1%>::inverse_chi_squared_distribution",
...
RealType pdf(const inverse_chi_squared_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   static const char* function = "lslboost::math::pdf(const inverse_chi_squared_distribution<%1%>&, %1%)";
...
{
   static const char* function = "lslboost::math::cdf(const inverse_chi_squared_distribution<%1%>&, %1%)";
   RealType df = dist.degrees_of_freedom();
   RealType scale = dist.scale();
   RealType error_result;
   ...
   // RealType shape = df /2; // inv_gamma shape,
   // RealType scale = df * scale/2; // inv_gamma scale,
   // result = lslboost::math::gamma_q(shape, scale / x, Policy()); // inverse_gamma code.
   return lslboost::math::gamma_q(df / 2, (df * (scale / 2)) / x, Policy());
} // cdf

template <class RealType, class Policy>
inline RealType quantile(const inverse_chi_squared_distribution<RealType, Policy>& dist, const RealType& p)
{
   using lslboost::math::gamma_q_inv;
   RealType df = dist.degrees_of_freedom();
   RealType scale = dist.scale();

   static const char* function = "lslboost::math::quantile(const inverse_chi_squared_distribution<%1%>&, %1%)";
   // Error check:
   RealType error_result;
...
{ // Mean of inverse Chi-Squared distribution.
   ...
   static const char* function = "lslboost::math::mean(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 2)
      return policies::raise_domain_error<RealType>(
         function,
...
{ // Variance of inverse Chi-Squared distribution.
   RealType df = dist.degrees_of_freedom();
   RealType scale = dist.scale();
   static const char* function = "lslboost::math::variance(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 4)
   {
      return policies::raise_domain_error<RealType>(
         function,
...
{ // mode is not defined in Mathematica.
  ...
  // for origin of the formula used below.
   ...
   RealType df = dist.degrees_of_freedom();
   RealType scale = dist.scale();
   static const char* function = "lslboost::math::mode(const inverse_chi_squared_distribution<%1%>&)";
   if(df < 0)
      return policies::raise_domain_error<RealType>(
         function,
...
inline RealType skewness(const inverse_chi_squared_distribution<RealType, Policy>& dist)
{
   ...
   RealType df = dist.degrees_of_freedom();
   static const char* function = "lslboost::math::skewness(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 6)
      return policies::raise_domain_error<RealType>(
         function,
...
{
   RealType df = dist.degrees_of_freedom();
   static const char* function = "lslboost::math::kurtosis(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 8)
      return policies::raise_domain_error<RealType>(
         function,
