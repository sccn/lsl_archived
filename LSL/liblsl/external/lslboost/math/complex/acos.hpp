...
std::complex<T> acos(const std::complex<T>& z)
{
   ...
   // note that we have tweaked a_crossover as per: https://svn.lslboost.org/trac/lslboost/ticket/7290
   //
...
#pragma warning(disable:4127)
   ...
   else if((lslboost::math::isnan)(x))
   {
      if((lslboost::math::isinf)(y))
         return std::complex<T>(x, ((lslboost::math::signbit)(z.imag())) ? std::numeric_limits<T>::infinity() :  -std::numeric_limits<T>::infinity());
   ...
   else
   {
      ...
      //
      if((y == 0) && (x <= one))
         return std::complex<T>((x == 0) ? half_pi : std::acos(z.real()), (lslboost::math::changesign)(z.imag()));
      //
      ...
      else
      {
         ...
         if(y <= (std::numeric_limits<T>::epsilon() * std::fabs(xm1)))
         {
            ...
            {
               // This deviates from Hull et al's paper as per https://svn.lslboost.org/trac/lslboost/ticket/7290
