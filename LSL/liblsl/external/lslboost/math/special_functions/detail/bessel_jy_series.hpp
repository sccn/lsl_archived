...
inline T bessel_j_small_z_series(T v, T x, const Policy& pol)
{
   ...
   {
      prefix = pow(x / 2, v) / lslboost::math::tgamma(v+1, pol);
   }
   else
   {
      prefix = v * log(x / 2) - lslboost::math::lgamma(v+1, pol);
      prefix = exp(prefix);
   }
   if(0 == prefix)
      return prefix;
...
#endif
   policies::check_series_iterations<T>("lslboost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
   ...
   *pscale = scale;
   ...
   T result = lslboost::math::tools::sum_series(s, lslboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>("lslboost::math::bessel_y_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   if(!need_logs)
   {
      prefix = lslboost::math::tgamma(-v, pol) * lslboost::math::cos_pi(v) * p / constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = lslboost::math::lgamma(-v, &sgn, pol) + p;
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
   ...
   {
      T p = pow(z / 2, n);
      T result = -((lslboost::math::factorial<T>(n - 1) / constants::pi<T>()));
