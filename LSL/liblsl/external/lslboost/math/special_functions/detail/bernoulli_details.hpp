...
#include <lslboost/detail/lightweight_mutex.hpp>
#include <lslboost/utility/enable_if.hpp>
#include <lslboost/math/tools/toms748_solve.hpp>
...
#  define BOOST_MATH_ATOMIC_NS lslboost
...
typedef lslboost::atomic<int> atomic_counter_type;
typedef int atomic_integer_type;
#elif BOOST_ATOMIC_SHORT_LOCK_FREE == 2
typedef lslboost::atomic<short> atomic_counter_type;
typedef short atomic_integer_type;
#elif BOOST_ATOMIC_LONG_LOCK_FREE == 2
typedef lslboost::atomic<long> atomic_counter_type;
typedef long atomic_integer_type;
#elif BOOST_ATOMIC_LLONG_LOCK_FREE == 2
typedef lslboost::atomic<long long> atomic_counter_type;
typedef long long atomic_integer_type;
#else
#  define BOOST_MATH_NO_ATOMIC_INT
#endif
...
T b2n_asymptotic(int n)
{
   ...
   const T approximate_log_of_bernoulli_bn = 
        ((lslboost::math::constants::half<T>() + nx) * log(nx))
        + ((lslboost::math::constants::half<T>() - nx) * log(lslboost::math::constants::pi<T>()))
        + (((T(3) / 2) - nx) * lslboost::math::constants::ln_two<T>())
        + ((nx * (T(2) - (nx2 * 7) * (1 + ((nx2 * 30) * ((nx2 * 12) - 1))))) / (((nx2 * nx2) * nx2) * 2520));
   return ((n / 2) & 1 ? 1 : -1) * (approximate_log_of_bernoulli_bn > tools::log_max_value<T>() 
      ? policies::raise_overflow_error<T>("lslboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, nx, Policy())
...
T t2n_asymptotic(int n)
{
   ...
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("lslboost::math::tangent_t2n<%1%>(std::size_t)", 0, T(n), Policy());
   t2n *= p2;
   p2 -= 1;
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("lslboost::math::tangent_t2n<%1%>(std::size_t)", 0, Policy());
   t2n *= p2;
   return t2n;
}
//
...
struct max_bernoulli_root_functor
{
   ...
   double operator()(double n)
   {
      ...
      const double approximate_log_of_bernoulli_bn
         =   ((lslboost::math::constants::half<double>() + n) * log(n))
           + ((lslboost::math::constants::half<double>() - n) * log(lslboost::math::constants::pi<double>()))
           + (((double(3) / 2) - n) * lslboost::math::constants::ln_two<double>())
...
{
   long long t = lltrunc(lslboost::math::tools::log_max_value<T>());
   max_bernoulli_root_functor fun(t);
   lslboost::math::tools::equal_floor tol;
   lslboost::uintmax_t max_iter = lslboost::math::policies::get_max_root_iterations<Policy>();
   return static_cast<std::size_t>(lslboost::math::tools::toms748_solve(fun, sqrt(double(t)), double(t), tol, max_iter).first) / 2;
}
...
struct bernoulli_initializer
{
   struct init
   {
      init()
      {
         ...
         //
         lslboost::math::bernoulli_b2n<T>(2, Policy());
         try{
            lslboost::math::bernoulli_b2n<T>(max_bernoulli_b2n<T>::value + 1, Policy());
         } catch(const std::overflow_error&){}
         lslboost::math::tangent_t2n<T>(2, Policy());
      }
...
// lslboost::container::static_vector here, but that allocates on the stack, which may well
...
public:
   ...
   void tangent(std::size_t m)
   {
      ...
      for(std::size_t i = std::max<size_t>(2, prev_size); i < m; i++)
      {
         ...
         if(i >= min_overflow_index && (lslboost::math::tools::max_value<T>() / (i-1) < m_intermediates[1]) )
         {
            std::fill(tn.begin() + i, tn.end(), lslboost::math::tools::max_value<T>());
            break;
         }
         ...
         for(std::size_t j = 2; j <= i; j++)
         {
            overflow_check =
                  ...
                  (lslboost::math::tools::max_value<T>() / (i - j) < m_intermediates[j])
                  || (lslboost::math::tools::max_value<T>() / (i - j + 2) < m_intermediates[j-1])
                  || (lslboost::math::tools::max_value<T>() - m_intermediates[j] * (i - j) < m_intermediates[j-1] * (i - j + 2))
                  || ((lslboost::math::isinf)(m_intermediates[j]))
                );
   ...
   OutputIterator copy_bernoulli_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      if(start + n > bn.capacity())
      {
         ...
         for(; n; ++start, --n)
         {
            *out = policies::raise_overflow_error<T>("lslboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(start), pol);
            ++out;
         }
         return out;
      }
      ...
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("lslboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[i];
         ++out;
      }
      ...
      //
      lslboost::detail::lightweight_mutex::scoped_lock l(m_mutex);
      if(start + n >= bn.size())
      {
         std::size_t new_size = (std::min)((std::max)((std::max)(start + n, std::size_t(bn.size() + 20)), std::size_t(50)), std::size_t(bn.capacity()));
         tangent_numbers_series(new_size);
      ...
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("lslboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[static_cast<typename container_type::size_type>(i)];
         ++out;
      }
   ...
   OutputIterator copy_tangent_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      for(std::size_t i = start; i < start + n; ++i)
      {
         if(i >= m_overflow_limit)
            *out = policies::raise_overflow_error<T>("lslboost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol);
         else
         {
...
#if !defined(BOOST_HAS_THREADS)
#elif defined(BOOST_MATH_NO_ATOMIC_INT)
   lslboost::detail::lightweight_mutex m_mutex;
#else
   lslboost::detail::lightweight_mutex m_mutex;
