...
T erf_inv_imp(const T& p, const T& q, const Policy&, const lslboost::mpl::int_<64>*)
{
...
struct erf_roots
{
   lslboost::math::tuple<T,T,T> operator()(const T& guess)
   {
...
template <class T, class Policy>
T erf_inv_imp(const T& p, const T& q, const Policy& pol, const lslboost::mpl::int_<0>*)
{
   //
   ...
   if(policies::digits<T, Policy>() > 64)
   {
      ...
      policies::check_root_iterations<T>("lslboost::math::erf_inv<%1%>", max_iter, pol);
   }
   else
   {
      result = guess;
...
struct erf_inv_initializer
{
   struct init
   {
      ...
      static void do_init()
      {
         lslboost::math::erf_inv(static_cast<T>(0.25), Policy());
         lslboost::math::erf_inv(static_cast<T>(0.55), Policy());
         lslboost::math::erf_inv(static_cast<T>(0.95), Policy());
...
typename tools::promote_args<T>::type erfc_inv(T z, const Policy& pol)
{
   ...
   //
   static const char* function = "lslboost::math::erfc_inv<%1%>(%1%, %1%)";
   if((z < 0) || (z > 2))
...
typename tools::promote_args<T>::type erf_inv(T z, const Policy& pol)
{
   ...
   // Begin by testing for domain errors, and other special cases:
   //
   static const char* function = "lslboost::math::erf_inv<%1%>(%1%, %1%)";
   if((z < -1) || (z > 1))
