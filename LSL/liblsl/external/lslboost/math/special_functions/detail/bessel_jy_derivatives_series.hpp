...
inline T bessel_j_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T prefix;
   if (v < lslboost::math::max_factorial<T>::value)
   {
      prefix = pow(x / 2, v - 1) / 2 / lslboost::math::tgamma(v + 1, pol);
   }
   else
   {
      prefix = (v - 1) * log(x / 2) - constants::ln_two<T>() - lslboost::math::lgamma(v + 1, pol);
      prefix = exp(prefix);
   }
   if (0 == prefix)
      return prefix;
   ...
   lslboost::uintmax_t max_iter = lslboost::math::policies::get_max_series_iterations<Policy>();
...
#endif
   lslboost::math::policies::check_series_iterations<T>("lslboost::math::bessel_j_derivative_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
inline T bessel_y_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T p = log(x / 2);
   T scale = 1;
   bool need_logs = (v >= lslboost::math::max_factorial<T>::value) || (lslboost::math::tools::log_max_value<T>() / v < fabs(p));
   if (!need_logs)
   {
      gam = lslboost::math::tgamma(v, pol);
      p = pow(x / 2, v + 1) * 2;
      if (lslboost::math::tools::max_value<T>() * p < gam)
      {
         scale /= gam;
         gam = 1;
         if (lslboost::math::tools::max_value<T>() * p < gam)
      ...
      prefix = -gam / (lslboost::math::constants::pi<T>() * p);
   }
   else
   {
      gam = lslboost::math::lgamma(v, pol);
      p = (v + 1) * p + constants::ln_two<T>();
      prefix = gam - log(lslboost::math::constants::pi<T>()) - p;
      if (lslboost::math::tools::log_max_value<T>() < prefix)
      {
         prefix -= log(lslboost::math::tools::max_value<T>() / 4);
         scale /= (lslboost::math::tools::max_value<T>() / 4);
         if (lslboost::math::tools::log_max_value<T>() < prefix)
         {
            return -lslboost::math::policies::raise_overflow_error<T>(function, 0, pol);
         }
   ...
   T result = lslboost::math::tools::sum_series(s, lslboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   lslboost::math::policies::check_series_iterations<T>("lslboost::math::bessel_y_derivative_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   p = pow(x / 2, v - 1) / 2;
   if (!need_logs)
   {
      prefix = lslboost::math::tgamma(-v, pol) * lslboost::math::cos_pi(v) * p / lslboost::math::constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = lslboost::math::lgamma(-v, &sgn, pol) + (v - 1) * log(x / 2) - constants::ln_two<T>();
      prefix = exp(prefix) * sgn / lslboost::math::constants::pi<T>();
   }
   bessel_y_derivative_small_z_series_term_b<T, Policy> s2(v, x);
   max_iter = lslboost::math::policies::get_max_series_iterations<Policy>();
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
   T b = lslboost::math::tools::sum_series(s2, lslboost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T b = lslboost::math::tools::sum_series(s2, lslboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
