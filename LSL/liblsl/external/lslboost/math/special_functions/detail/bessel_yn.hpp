...
#include <lslboost/math/special_functions/detail/bessel_y0.hpp>
#include <lslboost/math/special_functions/detail/bessel_y1.hpp>
...
T bessel_yn(int n, T x, const Policy& pol)
{
    ...
    static const char* function = "lslboost::math::bessel_yn<%1%>(%1%,%1%)";
    ...
    if(x < policies::get_epsilon<T, Policy>())
    {
       ...
       if(tools::max_value<T>() * fabs(scale) < fabs(value))
          return lslboost::math::sign(scale) * lslboost::math::sign(value) * policies::raise_overflow_error<T>(function, 0, pol);
       value /= scale;
    }
    else if (n == 0)
    {
    ...
    else
    {
       ...
       int k = 1;
       BOOST_ASSERT(k < n);
       policies::check_series_iterations<T>("lslboost::math::bessel_y_n<%1%>(%1%,%1%)", n, pol);
       do
       {
           T fact = 2 * k / x;
