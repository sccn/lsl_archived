...
inline T didonato_FN(T p, T a, T x, unsigned N, T tolerance, const Policy& pol)
{
   ...
   //
   BOOST_MATH_STD_USING
   T u = log(p) + lslboost::math::lgamma(a + 1, pol);
...
T find_inverse_gamma(T a, T p, T q, const Policy& pol, bool* p_has_10_digits)
{
   ...
   *p_has_10_digits = false;
   ...
   {
      T g = lslboost::math::tgamma(a, pol);
      T b = q * g;
   ...
   else
   {
      ...
      else if (p > 0.5)
      {
         ...
         else
         {
            ...
            T lg = lslboost::math::lgamma(a, pol);
            T lb = log(q) + lg;
            if(lb < -D * 2.3)
            {
      ...
      else
      {
         ...
         if(w < 0.15f * ap1)
         {
            ...
            T v = log(p) + lslboost::math::lgamma(ap1, pol);
            z = exp((v + w) / a);
            s = lslboost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = lslboost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = lslboost::math::log1p(z / ap1 * (1 + z / ap2 * (1 + z / (a + 3))), pol);
            z = exp((v + z - s) / a);
...
struct gamma_p_inverse_func
{
   ...
   lslboost::math::tuple<T, T, T> operator()(const T& x)const
   {
      ...
      T f, f1;
      value_type ft;
      f = static_cast<T>(lslboost::math::detail::gamma_incomplete_imp(
      ...
      return lslboost::math::make_tuple(static_cast<T>(f - p), f1, f2);
   }
private:
   T a, p;
   bool invert;
...
T gamma_p_inv_imp(T a, T p, const Policy& pol)
{
   ...
   static const char* function = "lslboost::math::gamma_p_inv<%1%>(%1%, %1%)";
...
T gamma_q_inv_imp(T a, T q, const Policy& pol)
{
   ...
   static const char* function = "lslboost::math::gamma_q_inv<%1%>(%1%, %1%)";
