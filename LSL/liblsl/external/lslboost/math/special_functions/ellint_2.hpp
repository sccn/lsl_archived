...
T ellint_e_imp(T phi, T k, const Policy& pol)
{
    ...
    else
    {
       ...
       if(k2 > 1)
       {
          return policies::raise_domain_error<T>("lslboost::math::ellint_2<%1%>(%1%, %1%)", "The parameter k is out of range, got k = %1%", k, pol);
       }
       else if(rphi == 0)
       {
          result = 0;
...
inline typename tools::promote_args<T>::type ellint_2(T k, const Policy& pol, const mpl::true_&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_e_imp(static_cast<value_type>(k), pol), "lslboost::math::ellint_2<%1%>(%1%)");
}
...
{
   return lslboost::math::ellint_2(k, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_2(T1 k, T2 phi, const Policy& pol)
{
   ...
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_e_imp(static_cast<value_type>(phi), static_cast<value_type>(k), pol), "lslboost::math::ellint_2<%1%>(%1%,%1%)");
}
