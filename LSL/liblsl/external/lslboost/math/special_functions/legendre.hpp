...
{
   static const char* function = "lslboost::math::legrendre_p<%1%>(unsigned, %1%)";
   // Error handling:
   if((x < -1) || (x > 1))
   ...
   if(second)
   {
      ...
      p0 = (lslboost::math::log1p(x, pol) - lslboost::math::log1p(-x, pol)) / 2;
      p1 = x * p0 - 1;
   }
   else
   {
   ...
   {
      std::swap(p0, p1);
      p1 = lslboost::math::legendre_next(n, x, p0, p1);
      ++n;
   }
   return p1;
}
   ...
   legendre_p(int l, T x, const Policy& pol)
{
   ...
   static const char* function = "lslboost::math::legendre_p<%1%>(unsigned, %1%)";
   if(l < 0)
...
{
   return lslboost::math::legendre_p(l, x, policies::policy<>());
}
   ...
   legendre_q(unsigned l, T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::legendre_imp(l, static_cast<value_type>(x), pol, true), "lslboost::math::legendre_q<%1%>(unsigned, %1%)");
}
...
{
   return lslboost::math::legendre_q(l, x, policies::policy<>());
}
...
T legendre_p_imp(int l, int m, T x, T sin_theta_power, const Policy& pol)
{
   ...
   if((x < -1) || (x > 1))
      return policies::raise_domain_error<T>(
      "lslboost::math::legendre_p<%1%>(int, int, %1%)",
   ...
   {
      int sign = (m&1) ? -1 : 1;
      return sign * lslboost::math::tgamma_ratio(static_cast<T>(l+m+1), static_cast<T>(l+1-m), pol) * legendre_p_imp(l, -m, x, sin_theta_power, pol);
   }
   // Special cases:
   if(m > l)
      return 0;
   if(m == 0)
      return lslboost::math::legendre_p(l, x, pol);

   T p0 = lslboost::math::double_factorial<T>(2 * m - 1, pol) * sin_theta_power;
   ...
   {
      std::swap(p0, p1);
      p1 = lslboost::math::legendre_next(n, m, x, p0, p1);
      ++n;
   }
   return p1;
}
...
{
   return lslboost::math::legendre_p(l, m, x, policies::policy<>());
}
