...
inline T factorial(unsigned i, const Policy& pol)
{
   ...
   T result = lslboost::math::tgamma(static_cast<T>(i+1), pol);
...
T double_factorial(unsigned i, const Policy& pol)
{
   ...
   if(i & 1)
   {
      ...
      //
      T result = lslboost::math::tgamma(static_cast<T>(i) / 2 + 1, pol) / sqrt(constants::pi<T>());
   ...
   //
   return policies::raise_overflow_error<T>("lslboost::math::double_factorial<%1%>(unsigned)", 0, pol);
}
...
T rising_factorial_imp(T x, int n, const Policy& pol)
{
   ...
   if((x < 1) && (x + n < 0))
   {
      T val = lslboost::math::tgamma_delta_ratio(1 - x, static_cast<T>(-n), pol);
...
inline T falling_factorial_imp(T x, unsigned n, const Policy& pol)
{
   ...
   if(x < 0.5f)
   {
      ...
      if(n > max_factorial<T>::value - 2)
      {
         ...
         T t1 = x * lslboost::math::falling_factorial(x - 1, max_factorial<T>::value - 2);
         T t2 = lslboost::math::falling_factorial(x - max_factorial<T>::value + 1, n - max_factorial<T>::value + 1);
         if(tools::max_value<T>() / fabs(t1) < fabs(t2))
            return lslboost::math::sign(t1) * lslboost::math::sign(t2) * policies::raise_overflow_error<T>("lslboost::math::falling_factorial<%1%>", 0, pol);
         return t1 * t2;
      }
      return x * lslboost::math::falling_factorial(x - 1, n - 1);
   }
   if(x <= n - 1)
   {
      //
