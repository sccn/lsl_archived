...
#include <lslboost/math/special_functions/detail/polygamma.hpp>
#include <lslboost/math/special_functions/trigamma.hpp>
...
namespace lslboost { namespace math {
  ...
  inline typename tools::promote_args<T>::type polygamma(const int n, T x, const Policy& pol)
  {
     ...
     //
     if(n == 0)
        return lslboost::math::digamma(x, pol);
     if(n == 1)
        return lslboost::math::trigamma(x, pol);
     //
     ...
     return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        ...
        "lslboost::math::polygamma<%1%>(int, %1%)");
  }

  template<class T>
  inline typename tools::promote_args<T>::type polygamma(const int n, T x)
  {
      return lslboost::math::polygamma(n, x, policies::policy<>());
  }
