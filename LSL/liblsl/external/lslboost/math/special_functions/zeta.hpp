...
struct zeta_series_cache_size
{
   ...
   // to zeta for example.
   //
   typedef typename lslboost::math::policies::precision<T,Policy>::type precision_type;
...
T zeta_series_imp(T s, T sc, const Policy&)
{
   ...
   return sum * 1 / -lslboost::math::powm1(T(2), sc);
}
...
inline T zeta_series2_imp(T s, const Policy& pol)
{
   ...
   T result = tools::sum_series(
      ...
      max_iter);
   policies::check_series_iterations<T>("lslboost::math::zeta_series2<%1%>(%1%)", max_iter, pol);
   return result;
}
#endif
...
T zeta_polynomial_series(T s, T sc, Policy const &)
{
   ...
   //
   BOOST_MATH_STD_USING
   int n = itrunc(T(log(lslboost::math::tools::epsilon<T>()) / -2));
   T sum = 0;
   T two_n = ldexp(T(1), n);
   int ej_sign = 1;
   for(int j = 0; j < n; ++j)
...
{
   BOOST_MATH_STD_USING
   static const char* function = "lslboost::math::zeta<%1%>";
   if(sc == 0)
         ...
         if(v == s)
         {
            if(v < 0)
            {
               if(((-v) & 1) == 0)
                  return 0;
               int n = (-v + 1) / 2;
               if(n <= lslboost::math::max_bernoulli_b2n<T>::value)
                  return T((-v & 1) ? -1 : 1) * lslboost::math::unchecked_bernoulli_b2n<T>(n) / (1 - v);
            }
            else if((v & 1) == 0)
            {
               if(((v / 2) <= lslboost::math::max_bernoulli_b2n<T>::value) && (v <= lslboost::math::max_factorial<T>::value))
                  return T(((v / 2 - 1) & 1) ? -1 : 1) * ldexp(T(1), v - 1) * pow(constants::pi<T, Policy>(), v) *
                     lslboost::math::unchecked_bernoulli_b2n<T>(v / 2) / lslboost::math::unchecked_factorial<T>(v);
               return T(((v / 2 - 1) & 1) ? -1 : 1) * ldexp(T(1), v - 1) * pow(constants::pi<T, Policy>(), v) *
                  lslboost::math::bernoulli_b2n<T>(v / 2) / lslboost::math::factorial<T>(v);
            }
            else
               return zeta_imp_odd_integer(v, sc, pol, mpl::bool_<(Tag::value <= 113) && Tag::value>());
         }
      }
      catch(const lslboost::math::rounding_error&){} // Just fall through, s is too large to round
   ...
   else if(s < 0)
   {
      ...
      else
      {
         ...
         {
            T mult = lslboost::math::sin_pi(0.5f * sc, pol) * 2 * zeta_imp(s, sc, pol, tag);
            result = lslboost::math::lgamma(s, pol);
            ...
            if(tools::max_value<T>() / fabs(mult) < result)
               return lslboost::math::sign(mult) * policies::raise_overflow_error<T>(function, 0, pol);
            result *= mult;
         }
         else
         {
            result = lslboost::math::sin_pi(0.5f * sc, pol)
               * 2 * pow(2 * constants::pi<T>(), -s) 
               * lslboost::math::tgamma(s, pol) 
...
struct zeta_initializer
{
   struct init
   {
      ...
      static void do_init(const mpl::int_<0>&){ lslboost::math::zeta(static_cast<T>(5), Policy()); }
      static void do_init(const mpl::int_<53>&){ lslboost::math::zeta(static_cast<T>(5), Policy()); }
      static void do_init(const mpl::int_<64>&)
      {
         lslboost::math::zeta(static_cast<T>(0.5), Policy());
         lslboost::math::zeta(static_cast<T>(1.5), Policy());
         lslboost::math::zeta(static_cast<T>(3.5), Policy());
         lslboost::math::zeta(static_cast<T>(6.5), Policy());
         lslboost::math::zeta(static_cast<T>(14.5), Policy());
         lslboost::math::zeta(static_cast<T>(40.5), Policy());

         lslboost::math::zeta(static_cast<T>(5), Policy());
      }
      static void do_init(const mpl::int_<113>&)
      {
         ...
         lslboost::math::zeta(static_cast<T>(1.5), Policy());
         lslboost::math::zeta(static_cast<T>(3.5), Policy());
         lslboost::math::zeta(static_cast<T>(5.5), Policy());
         lslboost::math::zeta(static_cast<T>(9.5), Policy());
         lslboost::math::zeta(static_cast<T>(16.5), Policy());
         lslboost::math::zeta(static_cast<T>(25.5), Policy());
         lslboost::math::zeta(static_cast<T>(70.5), Policy());
...
inline typename tools::promote_args<T>::type zeta(T s, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::zeta_imp(
      ...
      forwarding_policy(),
      tag_type()), "lslboost::math::zeta<%1%>(%1%)");
}
