...
public:
   ...
   T operator()()
   {
      T result = term * lslboost::math::bernoulli_b2n<T>(k) / (2 * k);
      term /= xx;
      ++k;
      return result;
   }
...
inline T digamma_imp_large(T x, const Policy& pol, const mpl::int_<0>*)
{
   ...
   result = lslboost::math::tools::sum_series(s, lslboost::math::policies::get_epsilon<T, Policy>(), max_iter, -result);
   result = -result;
   policies::check_series_iterations<T>("lslboost::math::digamma<%1%>(%1%)", max_iter, pol);
   return result;
}
//
...
T digamma_imp(T x, const Tag* t, const Policy& pol)
{
   ...
   if(x <= -1)
   {
      ...
      if(remainder == 0)
      {
         return policies::raise_pole_error<T>("lslboost::math::digamma<%1%>(%1%)", 0, (1-x), pol);
      }
      result = constants::pi<T>() / tan(constants::pi<T>() * remainder);
   }
   if(x == 0)
      return policies::raise_pole_error<T>("lslboost::math::digamma<%1%>(%1%)", 0, x, pol);
   //
...
struct digamma_initializer
{
   struct init
   {
      ...
      {
         lslboost::math::digamma(T(1.5), Policy());
         lslboost::math::digamma(T(500), Policy());
      }
   ...
   digamma(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::digamma_imp(
      ...
      static_cast<const tag_type*>(0), forwarding_policy()), "lslboost::math::digamma<%1%>(%1%)");
}
