...
T float_next_imp(const T& val, const Policy& pol)
{
   ...
   int fpclass = (lslboost::math::fpclassify)(val);
...
inline typename tools::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
   ...
   return val < direction ? lslboost::math::float_next<result_type>(val, pol) : val == direction ? val : lslboost::math::float_prior<result_type>(val, pol);
}
...
T float_distance_imp(const T& a, const T& b, const Policy& pol)
{
   ...
   //
   frexp(((lslboost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
   ...
   //
   T mb, x, y, z;
   if(((lslboost::math::fpclassify)(a) == (int)FP_SUBNORMAL) || (b - a < tools::min_value<T>()))
   {
      //
