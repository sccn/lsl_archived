...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(!(lslboost::math::isfinite)(val) && (val > 0))
...
{
   return val < direction ? lslboost::math::float_next(val, pol) : val == direction ? val : lslboost::math::float_prior(val, pol);
}
...
T float_distance(const T& a, const T& b, const Policy& pol)
{
   ...
   if(a == b)
      return 0;
   if(a == 0)
      return 1 + fabs(float_distance(static_cast<T>(lslboost::math::sign(b) * detail::get_smallest_value<T>()), b, pol));
   if(b == 0)
      return 1 + fabs(float_distance(static_cast<T>(lslboost::math::sign(a) * detail::get_smallest_value<T>()), a, pol));
   if(lslboost::math::sign(a) != lslboost::math::sign(b))
      return 2 + fabs(float_distance(static_cast<T>(lslboost::math::sign(b) * detail::get_smallest_value<T>()), b, pol))
         + fabs(float_distance(static_cast<T>(lslboost::math::sign(a) * detail::get_smallest_value<T>()), a, pol));
   //
   ...
   //
   frexp(((lslboost::math::fpclassify)(a) == FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
...
{
   return lslboost::math::float_distance(a, b, policies::policy<>());
}
...
{
   return lslboost::math::float_advance(val, distance, policies::policy<>());
}
