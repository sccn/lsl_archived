...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   {
      prefix = prefix * pow(z / lslboost::math::constants::e<T>(), z);
      BOOST_MATH_INSTRUMENT_CODE(prefix);
...
T lgamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos& l, int*sign)
{
   ...
   if(z <= 0)
   {
      ...
      result = log(lslboost::math::constants::pi<T>()) - lgamma_imp(z, pol, l, 0) - log(t);
   }
   else if((z != 1) && (z != 2))
   {
      ...
      sum += detail::upper_gamma_fraction(z, limit, ::lslboost::math::policies::get_epsilon<T, Policy>());
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
   ...
   if((lslboost::math::fpclassify)(prefix) == (int)FP_INFINITE)
      policies::raise_overflow_error<T>("lslboost::math::detail::full_igamma_prefix<%1%>(%1%, %1%)", "Result of incomplete gamma function is too large to represent.", pol);
   ...
   case 2:
      {
         ...
         if(result != 0)
         {
            ...
            if(invert)
            {
               init_value = -a * (normalised ? 1 : lslboost::math::tgamma(a, pol)) / result;
            }
...
T tgamma_delta_ratio_imp(T z, T delta, const Policy& pol)
{
   ...
   if(z <= 0)
      policies::raise_domain_error<T>("lslboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", z, pol);
   if(z+delta <= 0)
      policies::raise_domain_error<T>("lslboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", z+delta, pol);
...
T tgamma_ratio_imp(T x, T y, const Policy& pol)
{
   ...
   if((x <= tools::min_value<T>()) || (lslboost::math::isinf)(x))
      policies::raise_domain_error<T>("lslboost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", x, pol);
   if((y <= tools::min_value<T>()) || (lslboost::math::isinf)(y))
      policies::raise_domain_error<T>("lslboost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", y, pol);
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
   ...
   //
   if(a <= 0)
      policies::raise_domain_error<T>("lslboost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument a to the incomplete gamma function must be greater than zero (got a=%1%).", a, pol);
   if(x < 0)
      policies::raise_domain_error<T>("lslboost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument x to the incomplete gamma function must be >= 0 (got x=%1%).", x, pol);
   //
   // Now special cases:
   //
   if(x == 0)
