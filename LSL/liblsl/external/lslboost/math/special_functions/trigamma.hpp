...
T trigamma_prec(T x, const mpl::int_<53>*, const Policy&)
{
   ...
   if(x <= 2)
   {
      return (offset + lslboost::math::tools::evaluate_polynomial(P_1_2, x) / tools::evaluate_polynomial(Q_1_2, x)) / (x * x);
   }
   else if(x <= 4)
   {
      T y = 1 / x;
...
T trigamma_prec(T x, const mpl::int_<64>*, const Policy&)
{
   ...
   if(x <= 2)
   {
      return (offset_1_2 + lslboost::math::tools::evaluate_polynomial(P_1_2, x) / tools::evaluate_polynomial(Q_1_2, x)) / (x * x);
   }
   else if(x <= 8)
   {
      T y = 1 / x;
...
T trigamma_prec(T x, const mpl::int_<113>*, const Policy&)
{
   ...
   if(x <= 2)
   {
      return (2 + lslboost::math::tools::evaluate_polynomial(P_1_2, x) / tools::evaluate_polynomial(Q_1_2, x)) / (x * x);
   }
   else if(x <= 4)
   {
      return (y_offset_2_4 + lslboost::math::tools::evaluate_polynomial(P_2_4, x) / tools::evaluate_polynomial(Q_2_4, x)) / (x * x);
   }
   else if(x <= 8)
   {
      T y = 1 / x;
...
T trigamma_imp(T x, const Tag* t, const Policy& pol)
{
   ...
   if(x <= 0)
   {
      ...
      if(floor(x) == x)
      {
         return policies::raise_pole_error<T>("lslboost::math::trigamma<%1%>(%1%)", 0, (1-x), pol);
      }
      T s = fabs(x) < fabs(z) ? lslboost::math::sin_pi(x, pol) : lslboost::math::sin_pi(z, pol);
      return -trigamma_imp(z, t, pol) + lslboost::math::pow<2>(constants::pi<T>()) / (s * s);
   }
   if(x < 1)
   {
      result = 1 / (x * x);
...
struct trigamma_initializer
{
   struct init
   {
      ...
      {
         lslboost::math::trigamma(T(2.5), Policy());
      }
   ...
   trigamma(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::trigamma_imp(
      ...
      static_cast<const tag_type*>(0), forwarding_policy()), "lslboost::math::trigamma<%1%>(%1%)");
}
