...
T ellint_d_imp(T phi, T k, const Policy& pol)
{
    ...
    if(phi >= tools::max_value<T>())
    {
       ...
       result = policies::raise_overflow_error<T>("lslboost::math::ellint_e<%1%>(%1%,%1%)", 0, pol);
    }
    ...
    else
    {
       ...
       //
       T rphi = lslboost::math::tools::fmod_workaround(phi, T(constants::half_pi<T>()));
       T m = lslboost::math::round((phi - rphi) / constants::half_pi<T>());
       int s = 1;
       ...
       if(k2 > 1)
       {
          return policies::raise_domain_error<T>("lslboost::math::ellint_d<%1%>(%1%, %1%)", "The parameter k is out of range, got k = %1%", k, pol);
       }
       else if(rphi == 0)
       {
          result = 0;
...
T ellint_d_imp(T k, const Policy& pol)
{
    ...
    if (abs(k) > 1)
    {
       return policies::raise_domain_error<T>("lslboost::math::ellint_e<%1%>(%1%)",
...
inline typename tools::promote_args<T>::type ellint_d(T k, const Policy& pol, const mpl::true_&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_d_imp(static_cast<value_type>(k), pol), "lslboost::math::ellint_d<%1%>(%1%)");
}
...
{
   return lslboost::math::ellint_d(k, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_d(T1 k, T2 phi, const Policy& pol)
{
   ...
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_d_imp(static_cast<value_type>(phi), static_cast<value_type>(k), pol), "lslboost::math::ellint_2<%1%>(%1%,%1%)");
}
