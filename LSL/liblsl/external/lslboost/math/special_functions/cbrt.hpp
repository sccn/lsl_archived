...
#include <lslboost/mpl/divides.hpp>
...
struct big_int_type
{
   operator lslboost::uintmax_t()const;
};
...
struct largest_cbrt_int_type
{
   typedef typename mpl::if_<
      lslboost::is_convertible<big_int_type, T>,
      lslboost::uintmax_t,
      unsigned int
   >::type type;
};
...
T cbrt_imp(T z, const Policy& pol)
{
   ...
   {
      return policies::raise_domain_error("lslboost::math::cbrt<%1%>(%1%)", "Argument to function must be finite but got %1%.", z, pol);
   }
