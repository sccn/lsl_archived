...
#include <lslboost/utility/string_ref_fwd.hpp>
...
namespace lslboost {
    ...
    public:
        ...
        const charT& at(size_t pos) const {
            if ( pos >= len_ )
                BOOST_THROW_EXCEPTION( std::out_of_range ( "lslboost::string_ref::at" ) );
            return ptr_[pos];
            }
...
namespace std {
    // Hashing
    template<> struct hash<lslboost::string_ref>;
    template<> struct hash<lslboost::u16string_ref>;
    template<> struct hash<lslboost::u32string_ref>;
    template<> struct hash<lslboost::wstring_ref>;
}
#endif
