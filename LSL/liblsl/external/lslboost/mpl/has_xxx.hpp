...
#include <lslboost/mpl/aux_/config/has_xxx.hpp>
#include <lslboost/mpl/aux_/config/msvc_typename.hpp>
...
template< typename T, typename name = ::lslboost::mpl::aux::has_xxx_tag > \
struct BOOST_PP_CAT(trait,_impl) : T \
{ \
    static lslboost::mpl::aux::no_tag \
    test(void(*)(::lslboost::mpl::aux::has_xxx_tag)); \
    \
    static lslboost::mpl::aux::yes_tag test(...); \
    \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(test(static_cast<void(*)(name)>(0))) \
            != sizeof(lslboost::mpl::aux::no_tag) \
        ); \
    typedef lslboost::mpl::bool_<value> type; \
}; \
\
template< typename T, typename fallback_ = lslboost::mpl::bool_<default_> > \
struct trait \
    : lslboost::mpl::if_c< \
          lslboost::mpl::aux::msvc_is_incomplete<T>::value \
        , lslboost::mpl::bool_<false> \
...
{ \
    BOOST_STATIC_CONSTANT(bool, value = false); \
    typedef lslboost::mpl::bool_<false> type; \
}; \
/**/
...
{ \
    ...
    { \
        template < class U > \
        static int  resolve( lslboost::mpl::aux::type_wrapper<U> const volatile * \
                           , lslboost::mpl::aux::type_wrapper<typename U::name >* = 0 ); \
        static char resolve( ... ); \
    }; \
    typedef lslboost::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT( bool, value = ( sizeof( trait_tester_impl::resolve( static_cast< t_ * >(0) ) ) == sizeof(int) ) ); \
}; \
template< typename T, typename fallback_ = lslboost::mpl::bool_<default_> > \
struct trait           \
{                      \
    BOOST_STATIC_CONSTANT( bool, value = (trait_tester< T, lslboost::is_class< T >::value >::value) );     \
    typedef lslboost::mpl::bool_< trait< T, fallback_ >::value > type; \
};
...
{ \
    ...
    { \
        template< typename U > \
        static lslboost::mpl::aux::yes_tag test( \
              lslboost::mpl::aux::type_wrapper<U> const volatile* \
            , lslboost::mpl::aux::type_wrapper<BOOST_MSVC_TYPENAME U::name>* = 0 \
            ); \
    \
        static lslboost::mpl::aux::no_tag test(...); \
    }; \
    \
    typedef lslboost::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(gcc_3_2_wknd::test(static_cast<t_*>(0))) \
            == sizeof(lslboost::mpl::aux::yes_tag) \
        ); \
    typedef lslboost::mpl::bool_<value> type; \
}; \
/**/
...
#     define BOOST_MPL_HAS_MEMBER_MULTI_ACCEPT(z, n, args) \
        template< typename V > \
        static lslboost::mpl::aux::yes_tag \
        BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
            lslboost::mpl::aux::type_wrapper< V > const volatile* \
...
#       define BOOST_MPL_HAS_MEMBER_TEST(args) \
          sizeof( \
              BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
                  static_cast< lslboost::mpl::aux::type_wrapper< U >* >(0) \
              ) \
          ) == sizeof(lslboost::mpl::aux::yes_tag) \
        /**/
#     else
...
#   define BOOST_MPL_HAS_MEMBER_IMPLEMENTATION( \
      ...
      template< \
          typename T \
        , typename fallback_ \
              = lslboost::mpl::bool_< BOOST_PP_ARRAY_ELEM(3, args) > \
      > \
...
#   define BOOST_MPL_HAS_XXX_TEMPLATE_NAMED_DEF(trait, name, default_) \
      template< typename T \
              , typename fallback_ = lslboost::mpl::bool_< default_ > > \
      struct trait { \
          BOOST_STATIC_CONSTANT(bool, value = fallback_::value); \
          typedef fallback_ type; \
      }; \
