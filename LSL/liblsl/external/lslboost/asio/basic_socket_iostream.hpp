...
//     : basic_iostream<char>(&this->lslboost::base_from_member<
...
# define BOOST_ASIO_PRIVATE_CTR_DEF(z, n, data) \
  ...
  explicit basic_socket_iostream(BOOST_PP_ENUM_BINARY_PARAMS(n, T, x)) \
    : std::basic_iostream<char>(&this->lslboost::base_from_member< \
        basic_socket_streambuf<Protocol, StreamSocketService, \
          Time, TimeTraits, TimerService> >::member) \
  { \
    tie(this); \
...
class basic_socket_iostream
  : public lslboost::base_from_member<
      basic_socket_streambuf<Protocol, StreamSocketService,
...
public:
    ...
    Time, TimeTraits, TimerService>* rdbuf() const
  {
    return const_cast<basic_socket_streambuf<Protocol, StreamSocketService,
      Time, TimeTraits, TimerService>*>(
        &this->lslboost::base_from_member<
          basic_socket_streambuf<Protocol, StreamSocketService,
            Time, TimeTraits, TimerService> >::member);
  }
   ...
   * @code tcp::iostream s("www.lslboost.org", "http");
   * if (!s)
   * {
   *   std::cout << "Error: " << s.error().message() << std::endl;
   * } @endcode
   */
  const lslboost::system::error_code& error() const
  {
