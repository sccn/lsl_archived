...
template <typename Protocol,
    ...
    typename Time = lslboost::posix_time::ptime,
    typename TimeTraits = lslboost::asio::time_traits<Time>,
    typename TimerService = deadline_timer_service<Time, TimeTraits> >
class basic_socket_streambuf
  : public std::streambuf,
    private lslboost::base_from_member<io_service>,
...
public:
  ...
  basic_socket_streambuf()
    : basic_socket<Protocol, StreamSocketService>(
        lslboost::base_from_member<lslboost::asio::io_service>::member),
      unbuffered_(false),
      timer_service_(0),
      ...
      const endpoint_type& endpoint)
  {
    ...
    {
      ec_ = lslboost::asio::error::operation_aborted;
      return 0;
    }
   ...
   * buffer.
   */
  const lslboost::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * lslboost::asio::error::operation_aborted.
   *
...
protected:
    ...
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        this->get_service().async_send(this->get_implementation(),
            lslboost::asio::buffer(&ch, sizeof(char_type)), 0, handler);
   ...
   * buffer.
   */
  virtual const lslboost::system::error_code& error() const
  {
    return ec_;
  }
...
private:
  ...
  void resolve_and_connect(const ResolverQuery& query)
  {
    ...
    {
      iterator_type end;
      ec_ = lslboost::asio::error::host_not_found;
      while (ec_ && i != end)
      {
        this->basic_socket<Protocol, StreamSocketService>::close(ec_);
  ...
  struct timer_handler
  {
    ...
    void operator()(const lslboost::system::error_code&)
    {
  ...
  void start_timer()
  {
    if (timer_state_ != timer_is_pending)
    {
      ...
      handler(lslboost::system::error_code());
    }
  }
