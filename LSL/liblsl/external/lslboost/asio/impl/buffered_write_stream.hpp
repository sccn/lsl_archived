...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(lslboost::system::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
  ...
  public:
    ...
    void operator()(const lslboost::system::error_code& ec, std::size_t)
    {
      ...
      else
      {
        ...
        const std::size_t bytes_copied = lslboost::asio::buffer_copy(
    ...
    BOOST_ASIO_MOVE_ARG(WriteHandler) handler)
{
  ...
  {
    next_layer_.async_write_some(lslboost::asio::const_buffers_1(0, 0),
