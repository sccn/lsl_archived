    ...
    lslboost::asio::basic_streambuf<Allocator>& b, char delim)
{
  ...
  lslboost::asio::detail::throw_error(ec, "read_until");
    ...
    lslboost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    // Look for a match.
    lslboost::match_results<iterator,
      typename std::vector<lslboost::sub_match<iterator> >::allocator_type>
        match_results;
    if (regex_search(start_pos, end, match_results, expr,
          lslboost::match_default | lslboost::match_partial))
    {
...
std::size_t read_until(SyncReadStream& s,
    ...
    typename lslboost::enable_if<is_match_condition<MatchCondition> >::type*)
{
  std::size_t search_position = 0;
  for (;;)
  {
...
namespace detail
{
  ...
  public:
    read_until_expr_op(AsyncReadStream& stream,
        lslboost::asio::basic_streambuf<Allocator>& streambuf,
        const lslboost::regex& expr, ReadHandler& handler)
      : stream_(stream),
        streambuf_(streambuf),
        expr_(expr),
        search_position_(0),
