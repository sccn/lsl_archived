...
template <typename Stream>
std::size_t buffered_read_stream<Stream>::fill(lslboost::system::error_code& ec)
{
  detail::buffer_resize_guard<detail::buffered_stream_storage>
    resize_guard(storage_);
  std::size_t previous_size = storage_.size();
    ...
    BOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  if (lslboost::asio::buffer_size(buffers) == 0 || !storage_.empty())
  {
    next_layer_.async_read_some(lslboost::asio::mutable_buffers_1(0, 0),
