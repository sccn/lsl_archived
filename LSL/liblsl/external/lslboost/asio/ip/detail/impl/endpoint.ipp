...
#include <lslboost/asio/ip/detail/endpoint.hpp>
  ...
  : data_()
{
  ...
  if (family == BOOST_ASIO_OS_DEF(AF_INET))
  {
    data_.v4.sin_family = BOOST_ASIO_OS_DEF(AF_INET);
    data_.v4.sin_port =
      lslboost::asio::detail::socket_ops::host_to_network_short(port_num);
    data_.v4.sin_addr.s_addr = BOOST_ASIO_OS_DEF(INADDR_ANY);
  }
  else
  {
  ...
  : data_()
{
  ...
  if (addr.is_v4())
  {
    ...
    data_.v4.sin_addr.s_addr =
      lslboost::asio::detail::socket_ops::host_to_network_long(
          static_cast<lslboost::asio::detail::u_long_type>(
            addr.to_v4().to_ulong()));
  }
  else
  {
    ...
    data_.v6.sin6_port =
      lslboost::asio::detail::socket_ops::host_to_network_short(port_num);
    data_.v6.sin6_flowinfo = 0;
    lslboost::asio::ip::address_v6 v6_addr = addr.to_v6();
    lslboost::asio::ip::address_v6::bytes_type bytes = v6_addr.to_bytes();
...
{
  if (new_size > sizeof(lslboost::asio::detail::sockaddr_storage_type))
  {
    lslboost::system::error_code ec(lslboost::asio::error::invalid_argument);
...
unsigned short endpoint::port() const
{
  if (is_v4())
  {
    return lslboost::asio::detail::socket_ops::network_to_host_short(
        data_.v4.sin_port);
  }
  else
  {
...
lslboost::asio::ip::address endpoint::address() const
{
  ...
  if (is_v4())
  {
    return lslboost::asio::ip::address_v4(
        lslboost::asio::detail::socket_ops::network_to_host_long(
          data_.v4.sin_addr.s_addr));
  }
  else
  {
    lslboost::asio::ip::address_v6::bytes_type bytes;
#if defined(BOOST_ASIO_HAS_STD_ARRAY)
    memcpy(bytes.data(), data_.v6.sin6_addr.s6_addr, 16);
#else // defined(BOOST_ASIO_HAS_STD_ARRAY)
    memcpy(bytes.elems, data_.v6.sin6_addr.s6_addr, 16);
#endif // defined(BOOST_ASIO_HAS_STD_ARRAY)
    return lslboost::asio::ip::address_v6(bytes, data_.v6.sin6_scope_id);
  }
}
...
#if !defined(BOOST_ASIO_NO_IOSTREAM)
std::string endpoint::to_string(lslboost::system::error_code& ec) const
{
