...
address_v4::address_v4(unsigned long addr)
{
  ...
  addr_.s_addr = lslboost::asio::detail::socket_ops::host_to_network_long(
      static_cast<lslboost::asio::detail::u_long_type>(addr));
}
...
{
  return lslboost::asio::detail::socket_ops::network_to_host_long(addr_.s_addr);
}
...
std::string address_v4::to_string(lslboost::system::error_code& ec) const
{
  char addr_str[lslboost::asio::detail::max_addr_v4_str_len];
  const char* addr =
    lslboost::asio::detail::socket_ops::inet_ntop(
        BOOST_ASIO_OS_DEF(AF_INET), &addr_, addr_str,
        lslboost::asio::detail::max_addr_v4_str_len, 0, ec);
  if (addr == 0)
    return std::string();
  return addr;
}
...
{
  address_v4 tmp;
  if (lslboost::asio::detail::socket_ops::inet_pton(
