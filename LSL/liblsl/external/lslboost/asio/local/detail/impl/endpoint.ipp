...
#include <lslboost/asio/local/detail/endpoint.hpp>
...
{
  if (new_size > sizeof(lslboost::asio::detail::sockaddr_un_type))
  {
  ...
  else
  {
    path_length_ = new_size
      - offsetof(lslboost::asio::detail::sockaddr_un_type, sun_path);
...
void endpoint::init(const char* path_name, std::size_t path_length)
{
  if (path_length > sizeof(data_.local.sun_path) - 1)
  {
    ...
    lslboost::system::error_code ec(lslboost::asio::error::name_too_long);
    lslboost::asio::detail::throw_error(ec);
  }

  using namespace std; // For memcpy.
  data_.local = lslboost::asio::detail::sockaddr_un_type();
