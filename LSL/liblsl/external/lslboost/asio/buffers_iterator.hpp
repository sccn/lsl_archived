...
namespace detail
{
  ...
  struct buffers_iterator_types_helper<false>
  {
    ...
    struct byte_type
    {
      typedef typename lslboost::add_const<ByteType>::type type;
    };
  };
  ...
  struct buffers_iterator_types
  {
    enum
    {
      is_mutable = lslboost::is_convertible<
...
public:
...
#endif
  {
    ...
    while (new_iter.current_ != new_iter.end_)
    {
      ...
      if (lslboost::asio::buffer_size(new_iter.current_buffer_) > 0)
        break;
      ++new_iter.current_;
    }
    return new_iter;
...
#endif
  {
    ...
    while (new_iter.current_ != new_iter.end_)
    {
      ...
      new_iter.position_ += lslboost::asio::buffer_size(buffer);
      ++new_iter.current_;
    }
    return new_iter;
  }
...
private:
  ...
  void increment()
  {
    ...
    if (current_buffer_position_ != lslboost::asio::buffer_size(current_buffer_))
      return;
    ...
    while (current_ != end_)
    {
      ...
      if (lslboost::asio::buffer_size(current_buffer_) > 0)
        return;
      ++current_;
    }
  }
    ...
    if (n > 0)
    {
      ...
      for (;;)
      {
        std::ptrdiff_t current_buffer_balance
          = lslboost::asio::buffer_size(current_buffer_)
          - current_buffer_position_;
