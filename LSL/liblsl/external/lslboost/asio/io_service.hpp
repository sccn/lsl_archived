...
#include <lslboost/asio/detail/service_registry_fwd.hpp>
#include <lslboost/asio/detail/wrapped_handler.hpp>
...
# include <lslboost/asio/detail/winsock_init.hpp>
  ...
  || defined(__osf__)
# include <lslboost/asio/detail/signal_init.hpp>
#endif
 ...
 *
 * @li lslboost::asio::ip::tcp::socket
 * @li lslboost::asio::ip::tcp::acceptor
 * @li lslboost::asio::ip::udp::socket
 * @li lslboost::asio::deadline_timer.
 *
 ...
 * creating an object of type lslboost::asio::io_service::work:
 *
 * @code lslboost::asio::io_service io_service;
 * lslboost::asio::io_service::work work(io_service);
 * ... @endcode
 *
 ...
 *
 * @code lslboost::asio::io_service io_service;
 * auto_ptr<lslboost::asio::io_service::work> work(
 *     new lslboost::asio::io_service::work(io_service));
 * ...
...
public:
   ...
   * without blocking.
   */
  BOOST_ASIO_DECL std::size_t run(lslboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t run_one(lslboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll(lslboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll_one(lslboost::system::error_code& ec);
   ...
   * @code io_service.dispatch(lslboost::bind(f, a1, ... an)); @endcode
   */
   ...
   *
   * @throws lslboost::system::system_error Thrown on failure. If the notification
   ...
   * function:
   * @code my_io_service.notify_fork(lslboost::asio::io_service::fork_prepare);
   * if (fork() == 0)
   * {
   *   // This is the child process.
   *   my_io_service.notify_fork(lslboost::asio::io_service::fork_child);
   * }
   * else
   * {
   *   // This is the parent process.
   *   my_io_service.notify_fork(lslboost::asio::io_service::fork_parent);
   * } @endcode
   *
   ...
   */
  BOOST_ASIO_DECL void notify_fork(lslboost::asio::io_service::fork_event event);
   ...
   *
   * @throws lslboost::asio::service_already_exists Thrown if a service of the
   * given type is already present in the io_service.
   *
   * @throws lslboost::asio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  lslboost::asio::detail::service_registry* service_registry_;
...
public:
   ...
   */
  explicit work(lslboost::asio::io_service& io_service);
...
protected:
   ...
   */
  BOOST_ASIO_DECL service(lslboost::asio::io_service& owner);
...
private:
  ...
  BOOST_ASIO_DECL virtual void fork_service(
      lslboost::asio::io_service::fork_event event);

  friend class lslboost::asio::detail::service_registry;
  struct key
  {
    key() : type_info_(0), id_(0) {}
    const std::type_info* type_info_;
    const lslboost::asio::io_service::id* id_;
  } key_;
...
class service_id
  : public lslboost::asio::io_service::id
{
};
...
{
public:
  static lslboost::asio::detail::service_id<Type> id;

  // Constructor.
  service_base(lslboost::asio::io_service& io_service)
    : lslboost::asio::io_service::service(io_service)
  {
  }
};

template <typename Type>
lslboost::asio::detail::service_id<Type> service_base<Type>::id;
...
#include <lslboost/asio/impl/io_service.hpp>
