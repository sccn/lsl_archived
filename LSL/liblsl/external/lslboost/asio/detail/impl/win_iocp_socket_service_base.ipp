...
#include <lslboost/asio/detail/win_iocp_socket_service_base.hpp>
...
lslboost::system::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
...
lslboost::system::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    lslboost::system::error_code& ec)
{
  if (!is_open(impl))
...
lslboost::system::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, lslboost::system::error_code& ec)
{
  if (is_open(impl))
  {
...
lslboost::system::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, lslboost::system::error_code& ec)
{
  if (is_open(impl))
  {
    ec = lslboost::asio::error::already_open;
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, lslboost::asio::error::already_open);
  else
  {
    lslboost::system::error_code ec;
    ...
    std::size_t addrlen, win_iocp_socket_connect_op_base* op)
{
      ...
      || family == BOOST_ASIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      if (op->ec_ && op->ec_ != lslboost::asio::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == lslboost::asio::error::in_progress
          || op->ec_ == lslboost::asio::error::would_block)
      {
