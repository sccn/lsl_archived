...
#include <lslboost/asio/detail/signal_blocker.hpp>
#include <lslboost/asio/detail/signal_set_service.hpp>
...
void lslboost_asio_signal_handler(int signal_number)
{
  ...
  ::signal(signal_number, lslboost_asio_signal_handler);
...
signal_set_service::signal_set_service(
  ...
  : io_service_(lslboost::asio::use_service<io_service_impl>(io_service)),
#if !defined(BOOST_ASIO_WINDOWS) \
  && !defined(BOOST_ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
    reactor_(lslboost::asio::use_service<reactor>(io_service)),
#endif // !defined(BOOST_ASIO_WINDOWS)
       //   && !defined(BOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
    next_(0),
...
void signal_set_service::fork_service(
    lslboost::asio::io_service::fork_event fork_ev)
{
#if !defined(BOOST_ASIO_WINDOWS) \
  && !defined(BOOST_ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
  ...
  switch (fork_ev)
  {
  case lslboost::asio::io_service::fork_prepare:
    {
    ...
    break;
  case lslboost::asio::io_service::fork_parent:
    ...
    break;
  case lslboost::asio::io_service::fork_child:
    if (state->fork_prepared_)
    {
      lslboost::asio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
lslboost::system::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl,
    int signal_number, lslboost::system::error_code& ec)
{
...
# if defined(BOOST_ASIO_HAS_SIGACTION)
      ...
      sa.sa_handler = lslboost_asio_signal_handler;
      sigfillset(&sa.sa_mask);
      if (::sigaction(signal_number, &sa, 0) == -1)
# else // defined(BOOST_ASIO_HAS_SIGACTION)
      if (::signal(signal_number, lslboost_asio_signal_handler) == SIG_ERR)
...
lslboost::system::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
    int signal_number, lslboost::system::error_code& ec)
{
  // Check that the signal number is valid.
...
lslboost::system::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
...
lslboost::system::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    lslboost::system::error_code& ec)
{
  ...
  op_queue<operation> ops;
  {
    ...
    {
      op->ec_ = lslboost::asio::error::operation_aborted;
      impl.queue_.pop();
      ops.push(op);
    }
  }
...
void signal_set_service::open_descriptors()
{
  ...
  {
    lslboost::system::error_code ec(errno,
        lslboost::asio::error::get_system_category());
    lslboost::asio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(BOOST_ASIO_WINDOWS)
       //   && !defined(BOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
