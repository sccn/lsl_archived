...
#include <lslboost/asio/detail/win_iocp_handle_service.hpp>
...
{
public:
  explicit overlapped_wrapper(lslboost::system::error_code& ec)
  {
    Internal = 0;
    InternalHigh = 0;
    Offset = 0;
...
win_iocp_handle_service::win_iocp_handle_service(
  ...
  : iocp_service_(lslboost::asio::use_service<win_iocp_io_service>(io_service)),
    mutex_(),
    impl_list_(0)
{
}
    ...
    win_iocp_handle_service::implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    lslboost::asio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
lslboost::system::error_code win_iocp_handle_service::assign(
    win_iocp_handle_service::implementation_type& impl,
    const native_handle_type& handle, lslboost::system::error_code& ec)
{
  if (is_open(impl))
  {
...
lslboost::system::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type& impl,
...
lslboost::system::error_code win_iocp_handle_service::cancel(
    win_iocp_handle_service::implementation_type& impl,
    lslboost::system::error_code& ec)
{
  if (!is_open(impl))
...
size_t win_iocp_handle_service::do_write(
    win_iocp_handle_service::implementation_type& impl, lslboost::uint64_t offset,
    const lslboost::asio::const_buffer& buffer, lslboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = lslboost::asio::error::bad_descriptor;
  ...
  BOOL ok = ::WriteFile(impl.handle_,
      lslboost::asio::buffer_cast<LPCVOID>(buffer),
      static_cast<DWORD>(lslboost::asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_write_op(
    win_iocp_handle_service::implementation_type& impl, lslboost::uint64_t offset,
    const lslboost::asio::const_buffer& buffer, operation* op)
{
  ...
  if (!is_open(impl))
  {
    iocp_service_.on_completion(op, lslboost::asio::error::bad_descriptor);
  }
  else if (lslboost::asio::buffer_size(buffer) == 0)
  {
    // A request to write 0 bytes on a handle is a no-op.
  ...
  else
  {
    ...
    BOOL ok = ::WriteFile(impl.handle_,
        lslboost::asio::buffer_cast<LPCVOID>(buffer),
        static_cast<DWORD>(lslboost::asio::buffer_size(buffer)),
...
size_t win_iocp_handle_service::do_read(
    win_iocp_handle_service::implementation_type& impl, lslboost::uint64_t offset,
    const lslboost::asio::mutable_buffer& buffer, lslboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = lslboost::asio::error::bad_descriptor;
  ...
  BOOL ok = ::ReadFile(impl.handle_,
      lslboost::asio::buffer_cast<LPVOID>(buffer),
      static_cast<DWORD>(lslboost::asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_read_op(
    win_iocp_handle_service::implementation_type& impl, lslboost::uint64_t offset,
    const lslboost::asio::mutable_buffer& buffer, operation* op)
{
  update_cancellation_thread_id(impl);
  iocp_service_.work_started();
