...
#include <lslboost/asio/detail/date_time_fwd.hpp>
...
private:
  ...
  static bool is_positive_infinity(
      const lslboost::date_time::base_time<T, TimeSystem>& time)
  {
  ...
  long to_msec(const Duration& d, long max_duration) const
  {
    if (d.ticks() <= 0)
      return 0;
    lslboost::int64_t msec = d.total_milliseconds();
    if (msec == 0)
      return 1;
  ...
  long to_usec(const Duration& d, long max_duration) const
  {
    if (d.ticks() <= 0)
      return 0;
    lslboost::int64_t usec = d.total_microseconds();
    if (usec == 0)
      return 1;
