...
#include <lslboost/coroutine/coroutine.hpp>
...
public:
  ...
  typedef lslboost::coroutines::push_coroutine<void> callee_type;
#else
  typedef lslboost::coroutines::coroutine<void()> callee_type;
#endif
...
#elif defined(BOOST_COROUTINES_UNIDRECT) || defined(BOOST_COROUTINES_V2)
  typedef lslboost::coroutines::pull_coroutine<void> caller_type;
#else
  typedef lslboost::coroutines::coroutine<void()>::caller_type caller_type;
#endif
   ...
   * } @endcode
   */
  basic_yield_context operator[](lslboost::system::error_code& ec) const
  {
...
/**
 * @defgroup spawn lslboost::asio::spawn
 *
 ...
 *
 * @code lslboost::asio::spawn(my_strand, do_echo);
 *
 * // ...
 *
 * void do_echo(lslboost::asio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           lslboost::asio::buffer(data), yield);
 *
 *       lslboost::asio::async_write(my_socket,
 *           lslboost::asio::buffer(data, length), yield);
 *     }
 *   }
...
void spawn(BOOST_ASIO_MOVE_ARG(Handler) handler,
    ...
    const lslboost::coroutines::attributes& attributes
      = lslboost::coroutines::attributes());
...
void spawn(lslboost::asio::io_service::strand strand,
    BOOST_ASIO_MOVE_ARG(Function) function,
    const lslboost::coroutines::attributes& attributes
      = lslboost::coroutines::attributes());
 ...
 */
template <typename Function>
void spawn(lslboost::asio::io_service& io_service,
    BOOST_ASIO_MOVE_ARG(Function) function,
    const lslboost::coroutines::attributes& attributes
      = lslboost::coroutines::attributes());
...
#include <lslboost/asio/impl/spawn.hpp>
