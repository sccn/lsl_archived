...
#include <lslboost/asio/detail/buffered_stream_storage.hpp>
...
public:
  ...
  /// error occurred.
  std::size_t flush(lslboost::system::error_code& ec)
  {
    std::size_t bytes_written = write(next_layer_,
        buffer(storage_.data(), storage_.size()),
        transfer_all(), ec);
  ...
  {
  public:
    flush_handler(lslboost::asio::io_service& io_service,
  ...
  {
    if (lslboost::asio::buffer_size(buffers) == 0)
      return 0;
  ...
  {
  public:
    write_some_handler(lslboost::asio::io_service& io_service,
    ...
    void operator()(const lslboost::system::error_code& ec, std::size_t)
    {
      if (ec)
      {
        std::size_t length = 0;
      ...
      else
      {
        ...
        std::size_t bytes_avail = lslboost::asio::buffer_size(buffers_);
        std::size_t length = bytes_avail < space_avail
          ? bytes_avail : space_avail;
        storage_.resize(orig_size + length);
        std::size_t bytes_copied = lslboost::asio::buffer_copy(
      ...
      WriteHandler handler)
  {
    ...
    else
    {
      ...
      get_io_service().post(detail::bind_handler(
            handler, lslboost::system::error_code(), bytes_copied));
    }
  }
  ...
  /// Determine the amount of data that may be read without blocking.
  std::size_t in_avail(lslboost::system::error_code& ec)
  {
...
private:
  ...
  std::size_t copy(const ConstBufferSequence& buffers)
  {
    std::size_t orig_size = storage_.size();
    std::size_t space_avail = storage_.capacity() - orig_size;
    std::size_t bytes_avail = lslboost::asio::buffer_size(buffers);
    std::size_t length = bytes_avail < space_avail ? bytes_avail : space_avail;
    storage_.resize(orig_size + length);
    return lslboost::asio::buffer_copy(
