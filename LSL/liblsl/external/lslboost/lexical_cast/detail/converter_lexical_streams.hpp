...
#include <lslboost/lexical_cast/detail/lcast_char_constants.hpp>
#include <lslboost/lexical_cast/detail/lcast_unsigned_converters.hpp>
#include <lslboost/lexical_cast/detail/inf_nan.hpp>
...
#include <lslboost/detail/basic_pointerbuf.hpp>
...
namespace lslboost {
    ...
    namespace detail // optimized stream wrappers
    {
        template< class CharT // a result of widest_char transformation
                ...
                >
        class lexical_istream_limited_src: lslboost::noncopyable {
...
/************************************ HELPER FUNCTIONS FOR OPERATORS << ( ... ) ********************************/
            ...
            inline bool shl_signed(const T n) {
                CharT* tmp_finish = buffer + CharacterBufferSize;
                typedef BOOST_DEDUCED_TYPENAME lslboost::make_unsigned<T>::type utype;
...
/************************************ OPERATORS << ( ... ) ********************************/
        public:
            ...
            template <class C>
            BOOST_DEDUCED_TYPENAME lslboost::disable_if<lslboost::is_const<C>, bool>::type 
            ...
            bool operator<<(lslboost::ulong_long_type n)   { return shl_unsigned(n); }
            ...
            bool operator<<(const lslboost::uint128_type& n)   { return shl_unsigned(n); }
            ...
            BOOST_DEDUCED_TYPENAME lslboost::disable_if<lslboost::is_const<C>, bool>::type
            operator<<(lslboost::array<C, N> const& input) BOOST_NOEXCEPT { 
                BOOST_STATIC_ASSERT_MSG(
                    (sizeof(lslboost::array<const C, N>) == sizeof(lslboost::array<C, N>)),
                    "lslboost::array<C, N> and lslboost::array<const C, N> must have exactly the same layout."
                );
                return ((*this) << reinterpret_cast<lslboost::array<const C, N> const& >(input)); 
            }
            ...
            bool operator<<(std::array<C, N> const& input) BOOST_NOEXCEPT { 
                BOOST_STATIC_ASSERT_MSG(
                    (sizeof(std::array<C, N>) == sizeof(lslboost::array<C, N>)),
                    "std::array and lslboost::array must have exactly the same layout. "
                    "Bug in implementation of std::array or lslboost::array."
                );
                return ((*this) << reinterpret_cast<lslboost::array<C, N> const& >(input)); 
            }
#endif
        ...
        template <class CharT, class Traits>
        class lexical_ostream_limited_src: lslboost::noncopyable {
...
/************************************ OPERATORS >> ( ... ) ********************************/
        public:
            ...
            template<class Alloc>
            bool operator>>(lslboost::container::basic_string<CharT,Traits,Alloc>& str) { 
                str.assign(start, finish); return true; 
            }
            ...
            template <std::size_t N>
            bool operator>>(lslboost::array<unsigned char, N>& output) BOOST_NOEXCEPT { 
                return ((*this) >> reinterpret_cast<lslboost::array<char, N>& >(output)); 
            }

            template <std::size_t N>
            bool operator>>(lslboost::array<signed char, N>& output) BOOST_NOEXCEPT { 
                return ((*this) >> reinterpret_cast<lslboost::array<char, N>& >(output)); 
            }
            ...
            bool operator>>(std::array<C, N>& output) BOOST_NOEXCEPT { 
                BOOST_STATIC_ASSERT_MSG(
                    (sizeof(lslboost::array<C, N>) == sizeof(lslboost::array<C, N>)),
                    "std::array<C, N> and lslboost::array<C, N> must have exactly the same layout."
                );
                return ((*this) >> reinterpret_cast<lslboost::array<C, N>& >(output));
            }
#endif
