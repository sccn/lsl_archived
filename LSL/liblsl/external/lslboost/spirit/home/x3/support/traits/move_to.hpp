...
#include <lslboost/fusion/include/move.hpp>
#include <lslboost/fusion/include/is_sequence.hpp>
...
namespace lslboost { namespace spirit { namespace x3 { namespace traits
{
    ...
    namespace detail
    {
        ...
        template <typename Iterator>
        inline void
        move_to(Iterator first, Iterator last, lslboost::iterator_range<Iterator>& rng, container_attribute)
        {
            rng = {first, last};
        }
    }
    ...
    {
        if (lslboost::addressof(src) != lslboost::addressof(dest))
            dest = std::move(src);
    }
