...
#include <lslboost/filesystem/fstream.hpp>

namespace lslboost { namespace spirit { namespace x3 { namespace testing
{
   ...
   {
       lslboost::regex e(re_prefix + std::string("(.*?)") + re_suffix);
       lslboost::match_results<Iterator> what;
       if (lslboost::regex_search(
            first, last, what, e
          , lslboost::match_default | lslboost::match_continuous))
        {
            re = what[1].str();
            first = what[0].second;
            return true;
    ...
    )
    {
       ...
       while (in_first != in_last && tem_first != tem_last)
       {
            ...
            {
                lslboost::match_results<iter_t> what;
                lslboost::regex e(re);
                if (!lslboost::regex_search(
    ...
    {
        lslboost::filesystem::ifstream file(p);
        if (!file)
            return "";
