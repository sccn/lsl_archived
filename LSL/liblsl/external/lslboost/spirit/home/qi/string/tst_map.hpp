...
#include <lslboost/spirit/home/qi/string/detail/tst.hpp>
#include <lslboost/unordered_map.hpp>
#include <lslboost/pool/object_pool.hpp>
...
namespace lslboost { namespace spirit { namespace qi
{
    ...
    struct tst_map
    {
        ...
        bool add(
            Iterator first
          , Iterator last
          , typename lslboost::call_traits<T>::param_type val)
        {
            if (first != last)
            {
                map_data x = {0, 0};
    ...
    private:
        ...
        T* new_data(typename lslboost::call_traits<T>::param_type val)
        {
