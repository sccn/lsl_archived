...
BOOST_SPIRIT_CLASSIC_NAMESPACE_BEGIN
    ...
    namespace impl {
        ...
        private:
#ifdef BOOST_SPIRIT_THREADSAFE
            static lslboost::mutex &mutex_instance();
            static void mutex_init();
#endif

            lslboost::shared_ptr<object_with_id_base_supply<IdT> > id_supply;
        };
        ...
        {
#ifdef BOOST_SPIRIT_THREADSAFE
            lslboost::mutex::scoped_lock lock(mutex);
#endif
            if (free_ids.size())
            {
        ...
        {
            {
                ...
                lslboost::call_once(been_here, mutex_init);
                lslboost::mutex &mutex = mutex_instance();
                lslboost::mutex::scoped_lock lock(mutex);
#endif
                static lslboost::shared_ptr<object_with_id_base_supply<IdT> >
                    static_supply;
...
#ifdef BOOST_SPIRIT_THREADSAFE
        template <typename TagT, typename IdT>
        inline lslboost::mutex &
        object_with_id_base<TagT, IdT>::mutex_instance()
        {
            static lslboost::mutex mutex;
            return mutex;
        }
#endif
