...
namespace impl
{
    ...
    private:
        lslboost::thread_specific_ptr<frame_ptr> tsp_frame;
...
private:
...
#ifdef PHOENIX_THREADSAFE
    static lslboost::thread_specific_ptr<holder_t*> &
    tsp_frame_instance()
    {
        static lslboost::thread_specific_ptr<holder_t*> the_instance;
        return the_instance;
    }
    ...
    closure_frame_holder_ref(holder_t* holder_ = 0)
    {
        ...
        static lslboost::once_flag been_here = BOOST_ONCE_INIT;
#else
        static lslboost::once_flag been_here;
#endif
        lslboost::call_once(been_here, tsp_frame_instance_init);
        lslboost::thread_specific_ptr<holder_t*> &tsp_frame = tsp_frame_instance();
        if (!tsp_frame.get())
