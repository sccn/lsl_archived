...
namespace utility { namespace impl {
    ...
    inline void
    detach(lslboost::shared_ptr<basic_chset<CharT> >& ptr)
    {
        if (!ptr.unique())
            ptr = lslboost::shared_ptr<basic_chset<CharT> >
                (new basic_chset<CharT>(*ptr));
    }

    template <typename CharT>
    inline void
    detach_clear(lslboost::shared_ptr<basic_chset<CharT> >& ptr)
    {
        if (ptr.unique())
            ptr->clear();
        else
    ...
    void chset_negated_set(lslboost::shared_ptr<basic_chset<CharT> > &ptr, chlit<CharT> const &ch,
            FakeT)
    {
    ...
    template <typename CharT, typename FakeT>
    void chset_negated_set(lslboost::shared_ptr<basic_chset<CharT> > &ptr,
