...
namespace lslboost { namespace spirit { namespace traits
{
    ...
    template <typename T>
    struct is_container<lslboost::optional<T> >
      : is_container<T>
    {};

#if !defined(BOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES)
    template<typename T>
    struct is_container<lslboost::variant<T> >
      : is_container<T>
    {};

    template<typename T0, typename T1, typename ...TN>
    struct is_container<lslboost::variant<T0, T1, TN...> >
      : mpl::bool_<is_container<T0>::value ||
            is_container<lslboost::variant<T1, TN...> >::value>
    {};
...
#define BOOST_SPIRIT_IS_CONTAINER(z, N, data)                                 \
    ...
    template <>
    struct is_container<lslboost::detail::variant::void_>
      : mpl::false_
    {};
...
#undef BOOST_SPIRIT_IS_CONTAINER
    ...
    template <typename T>
    struct container_value<lslboost::optional<T> >
      : container_value<T>
    {};
    ...
    template <typename T>
    struct optional_attribute<lslboost::optional<T> >
    {
        typedef T const& type;

        static type call(lslboost::optional<T> const& val)
        {
            return lslboost::get<T>(val);
        }

        static bool is_valid(lslboost::optional<T> const& val)
        {
            return !!val;
        }
    };
    ...
    {
        static bool call(lslboost::optional<Container>& c, T const& val)
        {
            if (!c)
                c = Container();
            return push_back(lslboost::get<Container>(c), val);
        }
    };
...
namespace lslboost { namespace spirit { namespace result_of
{
    ...
    template <typename T>
    struct optional_value<lslboost::optional<T> >
    {
        typedef T type;
    };

    template <typename T>
    struct optional_value<lslboost::optional<T> const>
    {
        typedef T const type;
    };
