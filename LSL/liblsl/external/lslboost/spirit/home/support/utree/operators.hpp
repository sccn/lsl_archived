...
#endif
#include <lslboost/spirit/home/support/utree/utree.hpp>
...
namespace lslboost { namespace spirit 
{
    ...
    struct utree_is_equal
    {
        ...
        bool dispatch(const A&, const B&, lslboost::mpl::false_) const
        {
            return false; // cannot compare different types by default
        }

        template <typename A, typename B>
        bool dispatch(const A& a, const B& b, lslboost::mpl::true_) const
        {
        ...
        bool operator()(const A& a, const B& b) const
        {
            return dispatch(a, b,
                lslboost::mpl::and_<
                    lslboost::is_arithmetic<A>,
                    lslboost::is_arithmetic<B> >());
        }
    ...
    struct utree_print
    {
        ...
        void operator()(lslboost::iterator_range<Iterator> const& range) const
        {
            typedef typename lslboost::iterator_range<Iterator>::const_iterator iterator;
            (*this)('(');
            for (iterator i = range.begin(); i != range.end(); ++i)
            {
                lslboost::spirit::utree::visit(*i, *this);
            }
            (*this)(')');
        }
    ...
    struct logical_function
    {
        ...
        {
            return dispatch(a, lslboost::is_arithmetic<A>());
        }
    ...
    struct arithmetic_function
    {
        ...
        template <typename A, typename B>
        utree dispatch(A const&, B const&, lslboost::mpl::false_) const
        {
            return utree(); // cannot apply to non-arithmetic types
        }

        template <typename A, typename B>
        utree dispatch(A const& a, B const& b, lslboost::mpl::true_) const
        {
            return Base::eval(a, b); // for arithmetic types
        }
        ...
        template <typename A>
        utree dispatch(A const&, lslboost::mpl::false_) const
        {
            return utree(); // cannot apply to non-arithmetic types
        }

        template <typename A>
        utree dispatch(A const& a, lslboost::mpl::true_) const
        {
    ...
    struct integral_function
    {
        ...
        utree operator()(A const& a, B const& b) const
        {
            return dispatch(a, b,
                lslboost::mpl::and_<
                    lslboost::is_integral<A>,
                    lslboost::is_integral<B> >());
        }
        ...
        utree operator()(A const& a) const
        {
            return dispatch(a, lslboost::is_integral<A>());
        }
    };
