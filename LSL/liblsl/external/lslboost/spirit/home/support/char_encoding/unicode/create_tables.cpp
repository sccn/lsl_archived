...
#include <lslboost/spirit/include/qi.hpp>
#include <lslboost/spirit/include/phoenix.hpp>
...
using lslboost::spirit::qi::parse;
using lslboost::spirit::qi::hex;
using lslboost::spirit::qi::char_;
using lslboost::spirit::qi::eol;
using lslboost::spirit::qi::rule;
using lslboost::spirit::qi::omit;
using lslboost::spirit::qi::_1;
using lslboost::spirit::qi::_val;
using lslboost::phoenix::push_back;
using lslboost::phoenix::ref;
...
void print_head(Out& out)
{
    out 
        ...
        << "#include <lslboost/cstdint.hpp>\n"
        << "\n"
        << "namespace lslboost { namespace spirit { namespace ucd { namespace detail\n"
        << "{"
        ;
}
...
void print_tail(Out& out)
{
    out 
        << "\n"
        << "}}}} // namespace lslboost::spirit::unicode::detail\n"
        ;
}
...
char const* get_int_type_name(int size)
{
    switch (size)
    {
        case 1: return "::lslboost::uint8_t";
        case 2: return "::lslboost::uint16_t";
        case 4: return "::lslboost::uint32_t";
        case 5: return "::lslboost::uint64_t";
...
void print_file(Out& out, Builder& builder, int field_width, char const* name)
{
    ...
    out
        << "\n"
        << "    static const ::lslboost::uint8_t " << name << "_stage1[] = {\n"
        << "\n"
        ;
    ...
    for (int i = 0; i < int(stage2.size()); ++i)
    {
        ...
        print_table(out, 
            lslboost::iterator_range<value_type const*>(p, p+block_size), !last, field_width);
    }
    ...
    out 
        << "\n"
        << "    inline " << int_name << ' ' << name << "_lookup(::lslboost::uint32_t ch)\n"
        << "    {\n"
        << "        ::lslboost::uint32_t block_offset = " << name << "_stage1[ch / " << block_size << "] * " << block_size << ";\n"
