...
namespace lslboost { namespace spirit { namespace traits
{
    ...
    struct extract_from_attribute<lslboost::optional<Attribute>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(lslboost::optional<Attribute> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(lslboost::get<Attribute>(attr), ctx);
        }
    };

    template <typename Attribute, typename Exposed>
    struct extract_from_attribute<lslboost::optional<Attribute const>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(lslboost::optional<Attribute const> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(lslboost::get<Attribute const>(attr), ctx);
        }
    };

    // This handles attributes wrapped inside a lslboost::ref().
    template <typename Attribute, typename Exposed>
