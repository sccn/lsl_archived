...
#include <lslboost/spirit/home/support/detail/lexer/generator.hpp>
...
#include <lslboost/spirit/home/lex/lexer/lexertl/token.hpp>
#include <lslboost/spirit/home/lex/lexer/lexertl/functor.hpp>
#include <lslboost/spirit/home/lex/lexer/lexertl/functor_data.hpp>
#include <lslboost/spirit/home/lex/lexer/lexertl/iterator.hpp>
...
namespace lslboost { namespace spirit { namespace lex { namespace lexertl
{
    ...
    namespace detail
    {
        ...
        //
        ///////////////////////////////////////////////////////////////////////
        inline lslboost::lexer::regex_flags map_flags(unsigned int flags)
        {
            unsigned int retval = lslboost::lexer::none;
            if (flags & match_flags::match_not_dot_newline)
                retval |= lslboost::lexer::dot_not_newline;
            if (flags & match_flags::match_icase)
                retval |= lslboost::lexer::icase;

            return lslboost::lexer::regex_flags(retval);
        }
    }
    ...
    public:
        ...
        typedef lslboost::lexer::basic_rules<char_type> basic_rules_type;
    ...
    private:
        ...
        struct iterator_data_type
        {
            ...
            iterator_data_type(
                    lslboost::lexer::basic_state_machine<char_type> const& sm
                  , lslboost::lexer::basic_rules<char_type> const& rules
                  , semantic_actions_type const& actions)
              : state_machine_(sm), rules_(rules), actions_(actions)
            {}

            lslboost::lexer::basic_state_machine<char_type> const& state_machine_;
            lslboost::lexer::basic_rules<char_type> const& rules_;
    ...
    public:
        ...
        lslboost::lexer::rules const& get_rules() const { return rules_; }

        void clear(char_type const* state)
        {
            std::size_t s = rules_.state(state);
            if (lslboost::lexer::npos != s)
                rules_.clear(state);
            initialized_dfa_ = false;
        }
        std::size_t add_state(char_type const* state)
        {
            ...
            if (lslboost::lexer::npos == stateid) {
...
//         }
        ...
        bool init_dfa(bool minimize = false) const
        {
            if (!initialized_dfa_) {
                ...
                typedef lslboost::lexer::basic_generator<char_type> generator;
                generator::build (rules_, state_machine_);
                if (minimize)
                    generator::minimise (state_machine_);

#if defined(BOOST_SPIRIT_LEXERTL_DEBUG)
                lslboost::lexer::debug::dump(state_machine_, std::cerr);
#endif
...
//                 std::swap(rules, rules_);
    ...
    private:
        ...
        mutable lslboost::lexer::basic_state_machine<char_type> state_machine_;
        lslboost::lexer::regex_flags flags_;
