...
#include <lslboost/exception/get_error_info.hpp>
...
#include <lslboost/exception/current_exception_cast.hpp>
namespace
lslboost
    {
    namespace
    exception_detail
        {
        std::string diagnostic_information_impl( lslboost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        lslboost::exception const * be=current_exception_cast<lslboost::exception const>();
...
lslboost
    ...
    exception_detail
        ...
        inline
        exception const *
        get_lslboost_exception( exception const * e )
            {
            return e;
            }
        ...
        inline
        std::string
        diagnostic_information_impl( lslboost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<lslboost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information( T const & e, bool verbose=true )
        {
        return exception_detail::diagnostic_information_impl(exception_detail::get_lslboost_exception(&e),exception_detail::get_std_exception(&e),true,verbose);
        }
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) throw()
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce lslboost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
