...
#include <lslboost/thread/concurrent_queues/detail/sync_queue_base.hpp>
#include <lslboost/thread/concurrent_queues/queue_op_status.hpp>
#include <lslboost/thread/condition_variable.hpp>
#include <lslboost/thread/csbl/devector.hpp>
...
namespace concurrent
{
  ...
  private:
    ...
    {
      elem = lslboost::move(super::data_.front());
      super::data_.pop_front();
    }
    inline value_type pull_front(unique_lock<mutex>& )
    {
      value_type e = lslboost::move(super::data_.front());
      super::data_.pop_front();
      return lslboost::move(e);
    }
    ...
    {
      super::data_.push_back(lslboost::move(elem));
      super::notify_not_empty_if_needed(lk);
    }
  };
...
//  }
  ...
  {
    if (super::closed(lk)) return queue_op_status::closed;
    push_back(lslboost::move(elem), lk);
    return queue_op_status::success;
  }
  ...
  {
    unique_lock<mutex> lk(super::mtx_);
    return wait_push_back(lslboost::move(elem), lk);
  }
