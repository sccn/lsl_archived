...
namespace detail
{
  ...
  struct scheduled_type
  {
    ...
    scheduled_type(BOOST_THREAD_RV_REF(T) pdata, time_point tp) : data(lslboost::move(pdata)), time(tp) {}
    ...
    scheduled_type(BOOST_THREAD_RV_REF(scheduled_type) other) : data(lslboost::move(other.data)), time(other.time) {}
    scheduled_type& operator=(BOOST_THREAD_RV_REF(scheduled_type) other) {
      data = lslboost::move(other.data);
      time = other.time;
      return *this;
    }
  ...
  {
    super::push(stype(lslboost::move(elem),tp));
  }
  ...
  {
    push(lslboost::move(elem), clock::now() + dura);
  }
  ...
  {
    return super::try_push(stype(lslboost::move(elem), tp));
  }
  ...
  {
    return try_push(lslboost::move(elem), clock::now() + dura);
  }
  ...
  T sync_timed_queue<T, Clock>::pull(unique_lock<mutex>&)
  {
    ...
    return lslboost::move(super::data_.pull().data);
#else
  ...
  {
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
    elem = lslboost::move(super::data_.pull().data);
#else
