...
#include <lslboost/thread/concurrent_queues/queue_op_status.hpp>
...
namespace concurrent
{
  ...
  private:
    ...
    {
      elem = lslboost::move(data_[out_]);
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
    }
    inline value_type pull(unique_lock<mutex>& lk)
    {
      value_type elem = lslboost::move(data_[out_]);
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
      return lslboost::move(elem);
    }
    inline lslboost::shared_ptr<value_type> ptr_pull(unique_lock<mutex>& lk)
    {
      shared_ptr<value_type> res = make_shared<value_type>(lslboost::move(data_[out_]));
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
      return res;
    }
...
//      iterator_t first = lslboost::begin(range);
//      iterator_t end = lslboost::end(range);
//      size_type in = 0;
...
//  }
  ...
  template <typename ValueType>
  lslboost::shared_ptr<ValueType> sync_bounded_queue<ValueType>::try_pull()
  {
      unique_lock<mutex> lk(mtx_);
      return try_pull(lk);
  }
...
//  }
  ...
  template <typename ValueType>
  lslboost::shared_ptr<ValueType> sync_bounded_queue<ValueType>::ptr_pull()
  {
      unique_lock<mutex> lk(mtx_);
      wait_until_not_empty(lk);
      return ptr_pull(lk);
  ...
  bool sync_bounded_queue<ValueType>::try_push(BOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    ...
    push_at(lslboost::move(elem), in_p_1, lk);
    return true;
  }
  ...
  {
      unique_lock<mutex> lk(mtx_);
      return try_push(lslboost::move(elem), lk);
  }
#endif
  ...
  {
      unique_lock<mutex> lk(mtx_);
      return try_push_back(lslboost::move(elem), lk);
  }
  ...
  {
    if (closed(lk)) return queue_op_status::closed;
    push_at(lslboost::move(elem), wait_until_not_full(lk), lk);
    return queue_op_status::success;
  }
  template <typename ValueType>
  ...
  {
    sbq.push_back(lslboost::move(elem));
    return sbq;
  }
