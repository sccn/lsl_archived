...
#define BOOST_ONCE_INIT {0,0}
    ...
    void call_once(once_flag& flag,Function f)
    {
        ...
        while((status=::lslboost::detail::interlocked_read_acquire(&flag.status))
              !=function_complete_flag_value)
        {
            ...
            if(!status)
            {
                try
                {
                    ...
                    if(event_handle)
                    {
                        ::lslboost::detail::win32::ResetEvent(event_handle);
                    }
                    f();
                    if(!counted)
                    {
                    ...
                    if(event_handle)
                    {
                        ::lslboost::detail::win32::SetEvent(event_handle);
                    }
                    break;
                }
                catch(...)
            ...
            if(!counted)
            {
                BOOST_INTERLOCKED_INCREMENT(&flag.count);
                counted=true;
                status=::lslboost::detail::interlocked_read_acquire(&flag.status);
            ...
            BOOST_VERIFY(!::lslboost::detail::win32::WaitForSingleObject(
                             event_handle,::lslboost::detail::win32::infinite));
        }
    }
}
