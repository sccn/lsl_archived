...
#define BOOST_DEFINE_HAS_MEMBER_CALLED(member_name)                     \
        template<typename T, bool=lslboost::is_class<T>::value>            \
    ...
    public:
        ...
        void lock()
        {
            if(m==0)
            {
                lslboost::throw_exception(lslboost::lock_error(system::errc::operation_not_permitted, "lslboost unique_lock has no mutex"));
            }
            if(owns_lock())
            {
                lslboost::throw_exception(lslboost::lock_error(system::errc::resource_deadlock_would_occur, "lslboost unique_lock owns already the mutex"));
            }
            m->lock();
            is_locked=true;
        }
        ...
        void unlock()
        {
          if(m==0)
          {
            ...
            if(!owns_lock())
            {
                lslboost::throw_exception(lslboost::lock_error(system::errc::operation_not_permitted, "lslboost unique_lock doesn't own the mutex"));
            }
            m->unlock();
            is_locked=false;
        }
    ...
    public:
        ...
        void lock()
        {
            if(m==0)
            {
              lslboost::throw_exception(lslboost::lock_error(system::errc::operation_not_permitted, "lslboost shared_lock has no mutex"));
            }
            if(owns_lock())
            {
              lslboost::throw_exception(lslboost::lock_error(system::errc::resource_deadlock_would_occur, "lslboost shared_lock owns already the mutex"));
            }
            m->lock_shared();
            is_locked=true;
        }
        ...
        void unlock()
        {
            ...
            if(!owns_lock())
            {
              lslboost::throw_exception(lslboost::lock_error(system::errc::operation_not_permitted, "lslboost shared_lock doesn't own the mutex"));
            }
            m->unlock_shared();
            is_locked=false;
        }
    ...
    public:
        ...
        void lock()
        {
          if(m==0)
          {
            ...
            if(owns_lock())
            {
              lslboost::throw_exception(lslboost::lock_error(system::errc::resource_deadlock_would_occur, "lslboost upgrade_lock owns already the mutex"));
            }
            m->lock_upgrade();
            is_locked=true;
        }
        ...
        void unlock()
        {
          if(m==0)
          {
            ...
            if(!owns_lock())
            {
              lslboost::throw_exception(lslboost::lock_error(system::errc::operation_not_permitted, "lslboost upgrade_lock doesn't own the mutex"));
            }
            m->unlock_upgrade();
            is_locked=false;
        }
    ...
    namespace detail
    {
        ...
        {
            lslboost::unique_lock<MutexType1> l1(m1,lslboost::try_to_lock);
            if(!l1)
            {
                return 1;
            }
        ...
        {
            lslboost::unique_lock<MutexType1> l1(m1);
            if(!m2.try_lock())
            {
                return 1;
            }
    ...
    namespace detail
    {
...

        {
            ...
            Iterator const failed=lslboost::try_lock(++begin,end);
            if(failed==end)
            {
                guard.release();
            }
    ...
    namespace detail
    {
        ...
        void lock_impl(Iterator begin,Iterator end,is_mutex_type_wrapper<false>)
        {
            ...
            for(;;)
            {
                ...
                {
                    begin_lock.lock();
                    Iterator const failed_lock=lslboost::try_lock(next,end);
                    if(failed_lock==end)
                    {
                        begin_lock.release();
                        return;
                ...
                else
                {
                    ...
                    {
                        Iterator const failed_lock=lslboost::try_lock(second,next);
                        if(failed_lock==next)
                        {
                            begin_lock.release();
                            guard.release();
