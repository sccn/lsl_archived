...
//    Adapt to lslboost from CCIA C++11 implementation
...
#include <lslboost/thread/executors/executor.hpp>
...
namespace executors
{
  ...
  public:
    ...
    executor_adaptor(BOOST_THREAD_RV_REF(Args) ... args) : ex(lslboost::forward<Args>(args)...) {}
#else
    /**
     * executor_adaptor constructor
     */
    ...
    void submit(BOOST_THREAD_RV_REF(work) closure)  {
      return ex.submit(lslboost::move(closure));
    }
...
//    }
    ...
    {
      submit(work(lslboost::forward<Closure>(closure)));
    }
