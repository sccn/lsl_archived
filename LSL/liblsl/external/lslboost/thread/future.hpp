...
#include <lslboost/thread/detail/invoker.hpp>
#include <lslboost/thread/detail/invoke.hpp>
#include <lslboost/thread/detail/is_convertible.hpp>
#include <lslboost/thread/exceptional_ptr.hpp>
#include <lslboost/thread/futures/future_error.hpp>
#include <lslboost/thread/futures/future_error_code.hpp>
#include <lslboost/thread/futures/future_status.hpp>
#include <lslboost/thread/futures/is_future_type.hpp>
#include <lslboost/thread/futures/launch.hpp>
#include <lslboost/thread/futures/wait_for_all.hpp>
#include <lslboost/thread/futures/wait_for_any.hpp>
...
#else
#include <lslboost/thread/csbl/memory/unique_ptr.hpp>
#endif
...
#include <lslboost/thread/csbl/tuple.hpp>
#include <lslboost/thread/csbl/vector.hpp>
#endif
...
namespace lslboost
{
    namespace detail
    {
        ...
        struct shared_state_base : enable_shared_from_this<shared_state_base>
        {
            ...
            // This declaration should be only included conditionally, but is included to maintain the same layout.
            virtual void launch_continuation(lslboost::unique_lock<lslboost::mutex>&, shared_ptr<shared_state_base>)
            {
            }
            ...
            virtual void block_if_needed(lslboost::unique_lock<lslboost::mutex>&)
            {}
            ...
            void inc(lslboost::unique_lock<lslboost::mutex>&) { ++cnt_; }
            void inc() { lslboost::unique_lock<lslboost::mutex> lk(this->mutex); inc(lk); }

            void dec(lslboost::unique_lock<lslboost::mutex>& lk) {
              if (--cnt_ == 0) {
                block_if_needed(lk);
              }
            }
            void dec() { lslboost::unique_lock<lslboost::mutex> lk(this->mutex); dec(lk); }
            ...
            void do_continuation(lslboost::unique_lock<lslboost::mutex>& lock)
            {
                if (! continuations.empty()) {
                  ...
                  for (continuations_type::iterator it = the_continuations.begin(); it != the_continuations.end(); ++it) {
                    lslboost::unique_lock<lslboost::mutex> cont_lock((*it)->mutex);
            ...
            future_state::state get_state(lslboost::unique_lock<lslboost::mutex>& lk) const
            {
                if(!done)
                {
        ...
        public:
          ...
          virtual void block_if_needed(lslboost::unique_lock<lslboost::mutex>& lk)
          {
            this->wait(lk, false);
          }
        ...
        struct future_async_shared_state: future_async_shared_state_base<Rp>
        {
          ...
          {
            shared_ptr<lslboost::detail::shared_state_base> that = this->shared_from_this();
            this->thr_ = thread(&future_async_shared_state::run, that, lslboost::forward<Fp>(f));
          }

          static void run(shared_ptr<lslboost::detail::shared_state_base> that_, BOOST_THREAD_FWD_REF(Fp) f)
          {
        ...
        struct future_async_shared_state<void, Fp>: public future_async_shared_state_base<void>
        {
          void init(BOOST_THREAD_FWD_REF(Fp) f)
          {
            this->thr_ = thread(&future_async_shared_state::run, this->shared_from_this(), lslboost::move(f));
          }
...
#else
    template<typename F1, typename... Fs>
    typename lslboost::enable_if<is_future_type<F1>, unsigned>::type wait_for_any(F1& f1, Fs&... fs)
    {
      detail::future_waiter waiter;
      waiter.add(f1, fs...);
      return waiter.wait();
...
namespace detail {
    ...
    public:
      ...
      future_executor_shared_state(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f) {
        this->set_executor();
        shared_state_nullary_task<Rp,Fp> t(this, lslboost::forward<Fp>(f));
    ...
    make_future_executor_shared_state(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f) {
      shared_ptr<future_executor_shared_state<Rp, Executor> >
          h(new future_executor_shared_state<Rp, Executor>(ex, lslboost::forward<Fp>(f)));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
...
namespace detail
{
  ...
  public:
    ...
    void launch_continuation(lslboost::unique_lock<lslboost::mutex>&, shared_ptr<shared_state_base> that) {
  ...
  struct run_it {
    shared_ptr<lslboost::detail::shared_state_base> that_;

    run_it(shared_ptr<lslboost::detail::shared_state_base> that) : that_ (that) {}
    void operator()()
    {
  ...
  public:
    ...
    : ex(&ex), parent(lslboost::move(f)),
      continuation(lslboost::move(c)),
      centinel(parent.future_)  {
      this->set_executor();
    }

    void launch_continuation(lslboost::unique_lock<lslboost::mutex>& lck, shared_ptr<shared_state_base> that ) {
      relocker relock(lck);
  ...
  public:
    ...
    virtual void launch_continuation(lslboost::unique_lock<lslboost::mutex>&lk, shared_ptr<shared_state_base> ) {
  ...
  make_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<future_executor_continuation_shared_state<Ex,F,Rp, Fp> >
        h(new future_executor_continuation_shared_state<Ex, F,Rp, Fp>(ex, lslboost::move(f), lslboost::forward<Fp>(c)));
    lock.lock();
    h->parent.future_->set_continuation_ptr(h, lock);
    lock.unlock();
  ...
  make_shared_future_deferred_continuation_shared_state(
    ...
    shared_ptr<shared_future_deferred_continuation_shared_state<F, Rp, Fp> >
        h(new shared_future_deferred_continuation_shared_state<F, Rp, Fp>(f, lslboost::forward<Fp>(c)));
    lock.lock();
    h->parent.future_->set_continuation_ptr(h, lock);
    lock.unlock();
  ...
  make_shared_future_async_continuation_shared_state(
    ...
    shared_ptr<shared_future_async_continuation_shared_state<F,Rp, Fp> >
        h(new shared_future_async_continuation_shared_state<F,Rp, Fp>(f, lslboost::forward<Fp>(c)));
    lock.lock();
    h->parent.future_->set_continuation_ptr(h, lock);
    lock.unlock();
  ...
  make_shared_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<shared_future_executor_continuation_shared_state<Ex, F, Rp, Fp> >
        h(new shared_future_executor_continuation_shared_state<Ex, F, Rp, Fp>(ex, f, lslboost::forward<Fp>(c)));
    lock.lock();
    h->parent.future_->set_continuation_ptr(h, lock);
    lock.unlock();
  ...
  BOOST_THREAD_FUTURE<R>::then(Ex& ex, BOOST_THREAD_FWD_REF(F) func) {
    ...
    lslboost::unique_lock<lslboost::mutex> lock(this->future_->mutex);
    lock.unlock();
    return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_future_executor_continuation_shared_state<Ex, BOOST_THREAD_FUTURE<R>, future_type, F>(ex,
                  lock, lslboost::move(*this), lslboost::forward<F>(func)
              )));
  }
#endif
  ...
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)  const
  {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      lock.unlock();
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type, F>(
                  lock, *this, lslboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      this->future_->wait_internal(lock);
      lock.unlock();
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_shared_future_deferred_continuation_shared_state<shared_future<R>, future_type, F>(
                  lock, *this, lslboost::forward<F>(func)
              )));
    } else {
      lock.unlock();
  ...
  shared_future<R>::then(Ex& ex, BOOST_THREAD_FWD_REF(F) func)  const
  {
    ...
    lslboost::unique_lock<lslboost::mutex> lock(this->future_->mutex);
    lock.unlock();
    return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_shared_future_executor_continuation_shared_state<Ex, shared_future<R>, future_type, F>(ex,
                  lock, *this, lslboost::forward<F>(func)
              )));
  }
#endif
  ...
  shared_future<R>::then(BOOST_THREAD_FWD_REF(F) func)  const {
    ...
    if (underlying_cast<int>(this->launch_policy(lock)) & int(launch::async)) {
      lock.unlock();
      return lslboost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type, F>(
          lock, *this, lslboost::forward<F>(func));
    } else if (underlying_cast<int>(this->launch_policy(lock)) & int(launch::deferred)) {
      this->future_->wait_internal(lock);
      lock.unlock();
      return lslboost::detail::make_shared_future_deferred_continuation_shared_state<shared_future<R>, future_type, F>(
          lock, *this, lslboost::forward<F>(func));
    } else {
      lock.unlock();
      return lslboost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type, F>(
...
namespace detail
{
  ...
  public:
    ...
    typename F::value_type parent_value(lslboost::unique_lock<lslboost::mutex>& ) {
        typename F::value_type r = parent.get();
        r.set_exceptional_if_invalid();
        return lslboost::move(r);
    }

    virtual void wait(lslboost::unique_lock<lslboost::mutex>& lk, bool ) { // todo see if rethrow must be used
        parent_value(lk).wait();
    }
    virtual Rp get(lslboost::unique_lock<lslboost::mutex>& lk) {
    ...
    virtual void set_continuation_ptr(continuation_ptr_type continuation, lslboost::unique_lock<lslboost::mutex>& lock)
    {
      lslboost::unique_lock<lslboost::mutex> lk(parent.future_->mutex);
