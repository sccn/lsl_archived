    ...
    public:
      BOOST_THREAD_NO_COPYABLE(condition_variable)
        condition_variable()
        {
            ...
            if(res)
            {
                lslboost::throw_exception(thread_resource_error(res, "lslboost:: condition_variable constructor failed in pthread_mutex_init"));
            }
            int const res2=pthread_cond_init(&cond,NULL);
            if(res2)
            {
                BOOST_VERIFY(!pthread_mutex_destroy(&internal_mutex));
                lslboost::throw_exception(thread_resource_error(res2, "lslboost:: condition_variable constructor failed in pthread_cond_init"));
            }
        }
        ~condition_variable()
        {
        ...
        inline bool timed_wait(
            unique_lock<mutex>& m,
            lslboost::system_time const& wait_until)
        {
        ...
        bool timed_wait(
            unique_lock<mutex>& m,
            lslboost::system_time const& wait_until,predicate_type pred)
        {
            while (!pred())
            {
