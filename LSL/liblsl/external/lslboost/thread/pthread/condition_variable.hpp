...
namespace lslboost
{
    ...
    inline void condition_variable::wait(unique_lock<mutex>& m)
    {
        ...
        if(res)
        {
            lslboost::throw_exception(condition_error(res, "lslboost:: condition_variable constructor failed in pthread_cond_wait"));
        }
    }
    ...
    {
        if (!m.owns_lock())
            lslboost::throw_exception(condition_error(EPERM, "condition_variable do_timed_wait: mutex not locked"));
        ...
        if(cond_res)
        {
            lslboost::throw_exception(condition_error(cond_res, "condition_variable failed in pthread_cond_timedwait"));
        }
        return true;
    }
    ...
    public:
        ...
        condition_variable_any()
        {
            ...
            if(res)
            {
                lslboost::throw_exception(thread_resource_error(res, "condition_variable_any failed in pthread_mutex_init"));
            }
            int const res2=pthread_cond_init(&cond,NULL);
            if(res2)
            {
                BOOST_VERIFY(!pthread_mutex_destroy(&internal_mutex));
                lslboost::throw_exception(thread_resource_error(res, "condition_variable_any failed in pthread_cond_init"));
            }
        }
        ~condition_variable_any()
        {
        ...
        void wait(lock_type& m)
        {
            ...
            if(res)
            {
                lslboost::throw_exception(condition_error(res, "condition_variable_any failed in pthread_cond_wait"));
            }
        }
    ...
    private: // used by lslboost::thread::try_join_until
          ...
          struct timespec const &timeout)
        {
          ...
          if(res)
          {
              lslboost::throw_exception(condition_error(res, "condition_variable_any failed in pthread_cond_timedwait"));
          }
          return true;
        }
