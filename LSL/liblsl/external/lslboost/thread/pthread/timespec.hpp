...
namespace lslboost
{
  ...
  {
#if defined BOOST_THREAD_USES_DATETIME
    inline struct timespec to_timespec(lslboost::system_time const& abs_time)
    {
    ...
    inline timespec to_timespec(lslboost::intmax_t const& ns)
    {
      lslboost::intmax_t s = ns / 1000000000l;
      struct timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline lslboost::intmax_t to_nanoseconds_int_max(timespec const& ts)
    {
      return static_cast<lslboost::intmax_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
