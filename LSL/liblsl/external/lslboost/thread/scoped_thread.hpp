...
namespace lslboost
{
   ...
   *
   * thread std/lslboost::thread destructor terminates the program if the thread is not joinable.
   * Having a wrapper that can join the thread before destroying it seems a natural need.
   *
   * Example:
   *
   *     lslboost::strict_scoped_thread<> t((lslboost::thread(F)));
   *
   */
  ...
  public:
    ...
    explicit strict_scoped_thread(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Args)... args) :
      t_(lslboost::forward<F>(f), lslboost::forward<Args>(args)...) {}
#else
    template <class F>
    ...
    strict_scoped_thread(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2, BOOST_THREAD_FWD_REF(A3) a3) :
      t_(lslboost::forward<F>(f), lslboost::forward<A1>(a1), lslboost::forward<A2>(a2), lslboost::forward<A3>(a3)) {}
#endif
   ...
   * Example:
   *
   *     lslboost::scoped_thread<> t((lslboost::thread(F)));
   *     t.interrupt();
   *
   */
  template <class CallableThread = join_if_joinable>
...
//    }
    ...
    t_(lslboost::move(BOOST_THREAD_RV(x).t_))
    {}
