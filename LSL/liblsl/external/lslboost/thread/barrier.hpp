...
namespace lslboost
{
    ...
    public:
        ...
        {
            if (count == 0)
                lslboost::throw_exception(thread_exception(system::errc::invalid_argument, "barrier constructor: count cannot be zero."));
        }

        bool wait()
        {
            lslboost::mutex::scoped_lock lock(m_mutex);
