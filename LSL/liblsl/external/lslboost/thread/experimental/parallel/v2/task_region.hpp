...
#include <lslboost/thread/executors/basic_thread_pool.hpp>
#endif
#include <lslboost/thread/experimental/exception_list.hpp>
#include <lslboost/thread/experimental/parallel/v2/inline_namespace.hpp>
  ...
  namespace detail
  {
    void handle_task_region_exceptions(exception_list& errors)
    {
      ...
      catch (exception_list const& el)
      {
        ...
        {
          lslboost::exception_ptr const& e = *it;
          try {
            rethrow_exception(e);
          }
          catch (...)
      ...
      {
        errors.add(lslboost::current_exception());
      }
    }
    ...
    void wait_all()
    {
      ...
      for (group_type::iterator it = group.begin(); it != group.end(); ++it)
      {
        ...
        if (f.has_exception())
        {
          try
          {
            lslboost::rethrow_exception(f.get_exception_ptr());
          }
          catch (...)
          {
  ...
  public:
    ...
    void run(BOOST_THREAD_FWD_REF(F) f)
    {
#if defined BOOST_THREAD_TASK_REGION_HAS_SHARED_CANCELED
      {
        ...
        if (canceled) {
          lslboost::throw_exception(task_canceled_exception());
        }
      }
#if defined BOOST_THREAD_PROVIDES_EXECUTORS
