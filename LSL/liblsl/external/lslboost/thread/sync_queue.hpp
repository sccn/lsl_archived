...
#include <lslboost/thread/sync_bounded_queue.hpp>
#include <lslboost/container/deque.hpp>
...
namespace lslboost
{
  ...
  private:
    ...
    lslboost::container::deque<ValueType> data_;
    bool closed_;
    ...
    {
      elem = lslboost::move(data_.front());
      data_.pop_front();
    }
    inline lslboost::shared_ptr<value_type> ptr_pull(unique_lock<mutex>& )
    {
      shared_ptr<value_type> res = make_shared<value_type>(lslboost::move(data_.front()));
      data_.pop_front();
      return res;
    }
...
//  }
  ...
  template <typename ValueType>
  lslboost::shared_ptr<ValueType> sync_queue<ValueType>::try_pull()
  {
    try
    {
      unique_lock<mutex> lk(mtx_);
  ...
  template <typename ValueType>
  lslboost::shared_ptr<ValueType> sync_queue<ValueType>::ptr_pull()
  {
    try
    {
      unique_lock<mutex> lk(mtx_);
  ...
  {
    throw_if_closed(lk);
    push(lslboost::forward<ValueType>(elem), lk);
    return true;
  }
  ...
  {
    sbq.push(lslboost::forward<ValueType>(elem));
    return sbq;
  }
