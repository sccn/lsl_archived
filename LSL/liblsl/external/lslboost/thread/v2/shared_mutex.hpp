...
public:
    ...
    template <class Rep, class Period>
        bool try_lock_for(const lslboost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
        try_lock_until(
                      const lslboost::chrono::time_point<Clock, Duration>& abs_time);
    void unlock();
    ...
    template <class Rep, class Period>
        bool
        try_lock_shared_for(const lslboost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
...
public:
    ...
    template <class Rep, class Period>
        ...
        try_lock_upgrade_for(
                            const lslboost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
...
namespace lslboost {
  namespace thread_v2 {
    ...
    class shared_mutex
    {
      typedef ::lslboost::mutex              mutex_t;
      typedef ::lslboost::condition_variable cond_t;
    ...
    public:
      ...
      bool try_lock_for(const lslboost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_lock_until(lslboost::chrono::steady_clock::now() + rel_time);
      }
      template <class Clock, class Duration>
      bool
      try_lock_until(
      ...
      try_lock_shared_for(const lslboost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_lock_shared_until(lslboost::chrono::steady_clock::now() +
            rel_time);
      }
      template <class Clock, class Duration>
      bool
        ...
        const lslboost::chrono::time_point<Clock, Duration>& abs_time)
    {
      ...
      if (state_ & write_entered_)
      {
        while (true)
        {
          lslboost::cv_status status = gate1_.wait_until(lk, abs_time);
          if ((state_ & write_entered_) == 0)
            break;
          if (status == lslboost::cv_status::timeout)
            return false;
        }
      }
      ...
      if (state_ & n_readers_)
      {
        while (true)
        {
          lslboost::cv_status status = gate2_.wait_until(lk, abs_time);
    ...
    public:
      ...
      try_lock_upgrade_for(
          const lslboost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_lock_upgrade_until(lslboost::chrono::steady_clock::now() +
            rel_time);
      }
      template <class Clock, class Duration>
      bool
          ...
          const lslboost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_unlock_shared_and_lock_until(
            lslboost::chrono::steady_clock::now() + rel_time);
      }
      template <class Clock, class Duration>
      bool
      try_unlock_shared_and_lock_until(
