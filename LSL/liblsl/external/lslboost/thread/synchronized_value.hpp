...
#include <tuple> // todo change to <lslboost/tuple.hpp> once Boost.Tuple or Boost.Fusion provides Move semantics on C++98 compilers.
#include <functional>
#endif
...
namespace lslboost
{
  ...
  protected:
    ...
    lslboost::unique_lock<mutex_type> lk_;
    T const& value_;
  ...
  public:
    ...
    : lk_(lslboost::move(BOOST_THREAD_RV(other).lk_)),value_(BOOST_THREAD_RV(other).value_)
    {
    }
  ...
  public:
    ...
    : base_type(lslboost::move(static_cast<base_type&>(other)))
    {
    }
  ...
  public:
    ...
    : base_type(lslboost::move(static_cast<base_type&>(other))), value_(BOOST_THREAD_RV(other).value_)
    {
    }
  ...
  public:
    ...
    void swap(synchronized_value & rhs)
    {
      ...
      unique_lock<mutex_type> lk2(rhs.mtx_, defer_lock);
      lock(lk1,lk2);
      lslboost::swap(value_, rhs.value_);
    }
    /**
    ...
    template <typename F>
    inline
    typename lslboost::result_of<F(value_type&)>::type
    ...
    template <typename F>
    inline
    typename lslboost::result_of<F(value_type const&)>::type
...
//  struct hash<synchronized_value<T,L> >;
  ...
  synchronize(SV1& sv1, SV2& sv2)
  {
    lslboost::lock(sv1.mtx_, sv2.mtx_);
    typedef std::tuple<
        typename synchronized_value_strict_lock_ptr<SV1>::type,
        typename synchronized_value_strict_lock_ptr<SV2>::type
        > t_type;
