...
#include <lslboost/numeric/interval/detail/interval_prototype.hpp>
#include <lslboost/numeric/interval/detail/bugs.hpp>
#include <lslboost/numeric/interval/detail/test_input.hpp>
#include <lslboost/numeric/interval/rounded_arith.hpp>
#include <algorithm>
                                   ...
                                   const interval<T, Policies>& y)
{
  ...
  const T& yl = y.lower();
  const T& yu = y.upper();
  if (::lslboost::numeric::interval_lib::user::is_neg(xu))
    if (::lslboost::numeric::interval_lib::user::is_neg(yu))
      return I(rnd.div_down(xu, yl), rnd.div_up(xl, yu), true);
    else
      return I(rnd.div_down(xl, yl), rnd.div_up(xu, yu), true);
  else if (::lslboost::numeric::interval_lib::user::is_neg(xl))
    if (::lslboost::numeric::interval_lib::user::is_neg(yu))
...
interval<T, Policies> div_non_zero(const T& x, const interval<T, Policies>& y)
{
  ...
  const T& yl = y.lower();
  const T& yu = y.upper();
  if (::lslboost::numeric::interval_lib::user::is_neg(x))
...
{
  // assert(::lslboost::numeric::interval_lib::user::is_pos(yu));
  if (::lslboost::numeric::interval_lib::user::is_zero(x.lower()) &&
      ::lslboost::numeric::interval_lib::user::is_zero(x.upper()))
    return x;
  typename Policies::rounding rnd;
  typedef interval<T, Policies> I;
  const T& xl = x.lower();
...
interval<T, Policies> div_positive(const T& x, const T& yu)
{
  // assert(::lslboost::numeric::interval_lib::user::is_pos(yu));
  typedef interval<T, Policies> I;
  if (::lslboost::numeric::interval_lib::user::is_zero(x))
...
{
  // assert(::lslboost::numeric::interval_lib::user::is_neg(yl));
  if (::lslboost::numeric::interval_lib::user::is_zero(x.lower()) &&
      ::lslboost::numeric::interval_lib::user::is_zero(x.upper()))
    return x;
  typename Policies::rounding rnd;
...
{
  // assert(::lslboost::numeric::interval_lib::user::is_neg(y.lower()) && ::lslboost::numeric::interval_lib::user::is_pos(y.upper()));
  if (::lslboost::numeric::interval_lib::user::is_zero(x.lower()) && ::lslboost::numeric::interval_lib::user::is_zero(x.upper()))
    { b = false; return x; }
  typename Policies::rounding rnd;
  typedef interval<T, Policies> I;
  const T& xl = x.lower();
...
interval<T, Policies> div_zero_part2(const interval<T, Policies>& x,
                                     const interval<T, Policies>& y)
{
  // assert(::lslboost::numeric::interval_lib::user::is_neg(y.lower()) && ::lslboost::numeric::interval_lib::user::is_pos(y.upper()) && (div_zero_part1(x, y, b), b));
  typename Policies::rounding rnd;
  typedef interval<T, Policies> I;
  typedef typename Policies::checking checking;
  if (::lslboost::numeric::interval_lib::user::is_neg(x.upper()))
