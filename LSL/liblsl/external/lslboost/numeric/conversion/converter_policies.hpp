...
#include <lslboost/config/no_tr1/cmath.hpp> // for std::floor and std::ceil
...
#else
    if ( r == cNegOverflow )
      ::lslboost::throw_exception(negative_overflow()) ;
    else if ( r == cPosOverflow )
           ::lslboost::throw_exception(positive_overflow()) ;
#endif
  }
} ;
