/*
 [auto_generated]
 lslboost/numeric/odeint/util/split_adaptor.hpp
...
{
    const std::pair<std::size_t, std::size_t> off = split_offsets(lslboost::size(rng), index, parts);
    return make_iterator_range( lslboost::begin(rng) + off.first, lslboost::begin(rng) + off.second );
}
