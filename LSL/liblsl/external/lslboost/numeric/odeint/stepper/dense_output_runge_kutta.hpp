/*
 [auto_generated]
 lslboost/numeric/odeint/stepper/dense_output_runge_kutta.hpp
...
public:
    ...
    void calc_state( time_type t , StateOut &x ) const
    {
        if( t == current_time() )
        {
            lslboost::numeric::odeint::copy( get_current_state() , x );
        }
     ...
     * \param t The time at which the solution should be calculated, has to be
     * in the current time interval.
     * \param x The output variable where the result is written into, can be a lslboost range.
     */
    template< class StateOut >
