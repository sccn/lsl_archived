/*
 [auto_generated]
 lslboost/numeric/odeint/stepper/bulirsch_stoer_dense_out.hpp
...
public:
    ...
    void initialize( const StateType &x0 , const time_type &t0 , const time_type &dt0 )
    {
        ...
        lslboost::numeric::odeint::copy( x0 , get_current_state() );
        m_t = t0;
        m_dt = dt0;
        reset();
    }
...
private:
    ...
    /* k is the order to which the result was approximated */
    {
        ...
        for( int j = 0 ; j<=k ; j++ )
        {
            /* not working with lslboost units... */
        ...
        {
            lslboost::numeric::odeint::copy( m_diffs[2*k+1][0].m_v , m_err.m_v );
                ...
                if( i >= 0 )
                {
                    m_algebra.for_each3( m_diffs[kappa][j_diffs].m_v , m_diffs[kappa][j_diffs].m_v , m_derivs[j][i].m_v ,
                                         typename operations_type::template scale_sum2< value_type , value_type >( 1.0 ,
                                                                                                                   sign * fac * lslboost::math::binomial_coefficient< value_type >( kappa , c ) ) );
                }
                else
                {
    ...
    void do_interpolation( time_type t , StateOut &out ) const
    {
        ...
        lslboost::numeric::odeint::copy( m_mp_states[0].m_v , out );
