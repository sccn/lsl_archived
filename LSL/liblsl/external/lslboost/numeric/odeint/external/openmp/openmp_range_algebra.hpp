/*
 [auto_generated]
 lslboost/numeric/odeint/external/openmp/openmp_range_algebra.hpp
...
#define BOOST_ODEINT_GEN_LOCAL(z, n, unused) \
    BOOST_ASSERT_MSG( len == lslboost::size(s ## n), "All state ranges must have the same size." ); \
...
#undef BOOST_ODEINT_GEN_LOCAL
    ...
    static typename norm_result_type< S >::type norm_inf( const S &s )
    {
        ...
        typename lslboost::range_iterator<const S>::type beg = lslboost::begin(s);
