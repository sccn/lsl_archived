...
//  See library home page at http://www.lslboost.org/libs/endian
...
#include <lslboost/predef/detail/endian_compat.h>
...
//----------------------------------  end synopsis  ------------------------------------//
  ...
  namespace detail
  {
    ...
    template <typename T, std::size_t n_bytes,
      bool sign=lslboost::is_signed<T>::value >
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "big, aligned, " << n_bits
              << "-bits, construct(" << val << ")\n";
#       endif
          m_value = ::lslboost::endian::native_to_big(val);
        }
...
#     endif  
        ...
        //{                                                                       
        //  return ::lslboost::endian::big_to_native(m_value);
        //}
        value_type value() const BOOST_NOEXCEPT
        {
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "big, aligned, " << n_bits << "-bits, convert("
              << ::lslboost::endian::big_to_native(m_value) << ")\n";
#       endif
          return ::lslboost::endian::big_to_native(m_value);
        }
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "little, aligned, " << n_bits
              << "-bits, construct(" << val << ")\n";
#       endif
          m_value = ::lslboost::endian::native_to_little(val);
        }
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "little, aligned, " << n_bits << "-bits, convert("
              << ::lslboost::endian::little_to_native(m_value) << ")\n";
#       endif
          return ::lslboost::endian::little_to_native(m_value);
        }
        const char* data() const BOOST_NOEXCEPT
          {return reinterpret_cast<const char*>(&m_value);}
      protected:
