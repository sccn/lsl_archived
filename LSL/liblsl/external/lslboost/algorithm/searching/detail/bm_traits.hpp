...
namespace lslboost { namespace algorithm { namespace detail {
...
//  Special case small numeric values; use an array
    ...
    private:
        typedef typename lslboost::make_unsigned<key_type>::type unsigned_key_type;
        typedef lslboost::array<value_type, 1U << (CHAR_BIT * sizeof(key_type))> skip_map;
        skip_map skip_;
        const value_type k_default_value;
    public:
        ...
        void PrintSkipTable () const {
            std::cout << "BM(H) Skip Table <lslboost:array>:" << std::endl;
            for ( typename skip_map::const_iterator it = skip_.begin (); it != skip_.end (); ++it )
    ...
    struct BM_traits {
        ...
        typedef lslboost::algorithm::detail::skip_table<key_type, value_type, 
                lslboost::is_integral<key_type>::value && (sizeof(key_type)==1)> skip_table_t;
        };
