...
///
/// Suggested by olafvdspek in https://svn.lslboost.org/trac/lslboost/ticket/3215
...
#include <lslboost/mpl/identity.hpp>      // for identity
#include <lslboost/utility/enable_if.hpp> // for lslboost::disable_if

namespace lslboost { namespace algorithm {

/// \fn clamp ( T const& val, 
///               typename lslboost::mpl::identity<T>::type const & lo, 
///               typename lslboost::mpl::identity<T>::type const & hi, Pred p )
...
/// \fn clamp ( T const& val, 
///               typename lslboost::mpl::identity<T>::type const & lo, 
///               typename lslboost::mpl::identity<T>::type const & hi )
/// \return the value "val" brought into the range [ lo, hi ].
...
///       typename std::iterator_traits<typename lslboost::range_iterator<const Range>::type>::value_type const & lo,
///       typename std::iterator_traits<typename lslboost::range_iterator<const Range>::type>::value_type const & hi )
/// \return clamp the sequence of values [first, last) into [ lo, hi ]
/// 
...
///
  ...
  typename lslboost::disable_if_c<lslboost::is_same<Range, OutputIterator>::value, OutputIterator>::type
  clamp_range ( const Range &r, OutputIterator out,
    typename std::iterator_traits<typename lslboost::range_iterator<const Range>::type>::value_type const & lo, 
    typename std::iterator_traits<typename lslboost::range_iterator<const Range>::type>::value_type const & hi )
  {
    return clamp_range ( lslboost::begin ( r ), lslboost::end ( r ), out, lo, hi );
  } 
...
//  In that case, the user will get the two iterator version.
  ...
  clamp_range ( const Range &r, OutputIterator out,
    ...
    Pred p )
  {
    return clamp_range ( lslboost::begin ( r ), lslboost::end ( r ), out, lo, hi, p );
  } 
