...
class basic_string_base
{
 ...
 public:
   ...
   basic_string_base(BOOST_RV_REF(basic_string_base) b)
      :  members_(lslboost::move(b.alloc()))
   { 
      this->init();
      this->swap_data(b);
   }
   ...
   typedef typename lslboost::aligned_storage< sizeof(long_t),
   ...
   void swap_data(basic_string_base& other)
   {
      if(this->is_short()){
         ...
         if(other.is_short()){
            ...
            lslboost::container::swap_dispatch(this->members_.m_repr.long_repr(), other.members_.m_repr.long_repr());
         }
      }
   }
};
