...
#include <lslboost/container/detail/minimal_char_traits_header.hpp>
...
class basic_string_base
{
 ...
 public:
   ...
   typedef ::lslboost::intrusive::pointer_traits<pointer> pointer_traits;
      ...
      :  public Allocator
   {
      ...
      explicit members_holder(BOOST_FWD_REF(AllocatorConvertible) a)
         :  Allocator(lslboost::forward<AllocatorConvertible>(a))
      {}
   ...
   typedef container_detail::integral_constant<unsigned,
      lslboost::container::container_detail::version<Allocator>::value> alloc_version;
   ...
   void swap_data(basic_string_base& other)
   {
      if(this->is_short()){
         ...
         if(other.is_short()){
            ...
            lslboost::adl_move_swap(this->members_.m_repr.long_repr(), other.members_.m_repr.long_repr());
         }
      }
   }
};
   ...
   :  private container_detail::basic_string_base<Allocator>
{
      ...
      )
   {
      const size_type n_pos = p - this->cbegin();
      if (first != last) {
         const size_type n = lslboost::container::iterator_distance(first, last);
         const size_type old_size = this->priv_size();
         ...
         if(enough_capacity){
            ...
            if (elems_after >= n) {
               ...
               lslboost::container::iterator_advance(mid, elems_after + 1);
   ...
   {
      difference_type n = lslboost::container::iterator_distance(j1, j2);
      ...
      if (len >= n) {
         ...
         ForwardIter m = j1;
         lslboost::container::iterator_advance(m, len);
   ...
   {
      typedef typename lslboost::container::iterator_traits<InputIter>::iterator_category Category;
...
struct has_trivial_destructor_after_move<lslboost::container::basic_string<C, T, Allocator> >
{
   typedef typename ::lslboost::container::allocator_traits<Allocator>::pointer pointer;
