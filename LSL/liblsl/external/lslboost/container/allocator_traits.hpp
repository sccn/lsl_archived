...
#include <lslboost/container/detail/type_traits.hpp>  //is_empty
...
#define BOOST_INTRUSIVE_HAS_MEMBER_FUNCTION_CALLABLE_WITH_NS_BEG namespace lslboost { namespace container { namespace container_detail {
...
#include <lslboost/intrusive/detail/has_member_function_callable_with.hpp>
...
struct allocator_traits
{
   ...
   typedef typename allocator_type::value_type value_type;
   ...
   #else
      //pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(lslboost::container::container_detail::, Allocator,
         pointer, value_type*)
            pointer;
      //const_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(lslboost::container::container_detail::, Allocator,
      ...
      //rebind_alloc & rebind_traits
         ...
         //C++11
         template <typename T> using rebind_alloc  = typename lslboost::intrusive::pointer_rebind<Allocator, T>::type;
         template <typename T> using rebind_traits = allocator_traits< rebind_alloc<T> >;
      #else    // #if !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES)
         //Some workaround for C++03 or C++11 compilers with no template aliases
         template <typename T>
         struct rebind_alloc : lslboost::intrusive::pointer_rebind<Allocator,T>::type
         {
            typedef typename lslboost::intrusive::pointer_rebind<Allocator,T>::type Base;
            #if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
               template <typename... Args>
               rebind_alloc(BOOST_FWD_REF(Args)... args) : Base(lslboost::forward<Args>(args)...) {}
         ...
         struct rebind_traits
            : allocator_traits<typename lslboost::intrusive::pointer_rebind<Allocator, T>::type>
         {};
      ...
      template <class T>
      struct portable_rebind_alloc
      {  typedef typename lslboost::intrusive::pointer_rebind<Allocator, T>::type type;  };
   ...
   select_on_container_copy_construction(const Allocator &a)
   {
      ...
      {
         static const bool value = ::lslboost::move_detail::and_
   ...
   {  return a;  }
      ...
      {  ::new((void*)p, lslboost_container_new_t()) T(::lslboost::forward<Args>(args)...); }
   #else // #if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
      public:
      ...
      {  ::new((void*)p, lslboost_container_new_t()) T(BOOST_MOVE_FWD##N); }\
      //
   ...
   template<class T>
   static void priv_construct(container_detail::false_type, Allocator &, T *p, const ::lslboost::container::default_init_t&)
   {  ::new((void*)p) T; }
