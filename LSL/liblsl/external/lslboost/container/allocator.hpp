...
class allocator
{
   ...
   pointer allocate(size_type count, const void * hint= 0)
   {
      ...
      void *ret = lslboost_cont_malloc(count*sizeof(T));
      if(!ret)
         lslboost::container::throw_bad_alloc();
   ...
   void allocate_many(size_type elem_size, std::size_t n_elements, multiallocation_chain &chain)
   {
      ...
      if(!lslboost_cont_multialloc_nodes(n_elements, elem_size*sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, &ch)){
         lslboost::container::throw_bad_alloc();
      }
      ...
      if(!lslboost_cont_multialloc_nodes(n_elements, elem_size*sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, reinterpret_cast<lslboost_cont_memchain *>(&chain))){
         lslboost::container::throw_bad_alloc();
      }
   }
   ...
   void allocate_many(const size_type *elem_sizes, size_type n_elements, multiallocation_chain &chain)
   {
      ...
      lslboost_cont_memchain ch;
      BOOST_CONTAINER_MEMCHAIN_INIT(&ch);
      if(!lslboost_cont_multialloc_arrays(n_elements, elem_sizes, sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, &ch)){
         lslboost::container::throw_bad_alloc();
      }
      chain.incorporate_after(chain.before_begin()
                             ,(T*)BOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ch)
                             ...
                             ,BOOST_CONTAINER_MEMCHAIN_SIZE(&ch) );
      /*
      if(!lslboost_cont_multialloc_arrays(n_elements, elem_sizes, sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, reinterpret_cast<lslboost_cont_memchain *>(&chain))){
         lslboost::container::throw_bad_alloc();
      }*/
   }
