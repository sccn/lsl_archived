...
#include <lslboost/container/detail/adaptive_node_pool.hpp>
#include <lslboost/container/detail/multiallocation_chain.hpp>
#include <lslboost/container/detail/mpl.hpp>
#include <lslboost/container/detail/alloc_lib_auto_link.hpp>
#include <lslboost/container/detail/singleton.hpp>
...
class adaptive_pool
{
   ...
   pointer allocate(size_type count, const void * = 0)
   {
      ...
      if(Version == 1 && count == 1){
         ...
         return static_cast<pointer>(lslboost_cont_malloc(count*sizeof(T)));
      }
   }
   ...
   void allocate_many(size_type elem_size, std::size_t n_elements, multiallocation_chain &chain)
   {
      ...
      lslboost_cont_memchain ch;
      BOOST_CONTAINER_MEMCHAIN_INIT(&ch);
      if(BOOST_UNLIKELY(!lslboost_cont_multialloc_nodes(n_elements, elem_size*sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, &ch))){
         lslboost::container::throw_bad_alloc();
      }
      ...
      if(BOOST_UNLIKELY(!lslboost_cont_multialloc_nodes
            (n_elements, elem_size*sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, reinterpret_cast<lslboost_cont_memchain *>(&chain)))){
         lslboost::container::throw_bad_alloc();
      }
   }
   ...
   void allocate_many(const size_type *elem_sizes, size_type n_elements, multiallocation_chain &chain)
   {
      ...
      lslboost_cont_memchain ch;
      BOOST_CONTAINER_MEMCHAIN_INIT(&ch);
      if(BOOST_UNLIKELY(!lslboost_cont_multialloc_arrays(n_elements, elem_sizes, sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, &ch))){
         lslboost::container::throw_bad_alloc();
      }
      chain.incorporate_after(chain.before_begin()
                             ,(T*)BOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ch)
                             ,(T*)BOOST_CONTAINER_MEMCHAIN_LASTMEM(&ch)
                             ,BOOST_CONTAINER_MEMCHAIN_SIZE(&ch) );*/
      if(BOOST_UNLIKELY(!lslboost_cont_multialloc_arrays
         (n_elements, elem_sizes, sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS, reinterpret_cast<lslboost_cont_memchain *>(&chain)))){
         lslboost::container::throw_bad_alloc();
      }
   }
   ...
   {/*
      ...
      lslboost_cont_multidealloc(&ch);*/
      lslboost_cont_multidealloc(reinterpret_cast<lslboost_cont_memchain *>(&chain));
   }
      ...
      ,size_type &prefer_in_recvd_out_size, pointer &reuse_ptr)
   {
      ...
      lslboost_cont_command_ret_t ret = {0 , 0};
      ...
      std::size_t r_size;
      {
         ...
         ret = lslboost_cont_allocation_command(command, sizeof(T), l_size, p_size, &r_size, reuse_ptr_void);
