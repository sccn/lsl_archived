...
# define BOOST_NO_MT
  ...
  #error Unable to determine platform mutex type; #define BOOST_NO_MT to assume single-threaded
   ...
   /* Note CAS_LOCK defined to return 0 on success */
      ...
      static FORCEINLINE int lslboost_container_x86_cas_lock(int *sl) {
         int ret;
         int val = 1;
         int cmp = 0;
      ...
      static FORCEINLINE void lslboost_container_x86_clear_lock(int* sl) {
         assert(*sl != 0);
         int prev = 0;
         int ret;
      ...
      #define BOOST_CONTAINER_CAS_LOCK(sl)     lslboost_container_x86_cas_lock(sl)
      #define BOOST_CONTAINER_CLEAR_LOCK(sl)   lslboost_container_x86_clear_lock(sl)
   ...
   inline int lslboost_interprocess_spin_acquire_lock(int *sl) {
      int spins = 0;
   ...
   #define BOOST_CONTAINER_ACQUIRE_LOCK(sl)      (BOOST_CONTAINER_CAS_LOCK(sl)? lslboost_interprocess_spin_acquire_lock(sl) : 0)
