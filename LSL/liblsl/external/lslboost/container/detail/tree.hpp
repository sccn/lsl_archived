...
#include <lslboost/container/options.hpp>

// container/detail
#include <lslboost/container/detail/algorithm.hpp> //algo_equal(), algo_lexicographical_compare
#include <lslboost/container/detail/compare_functors.hpp>
...
#include <lslboost/container/detail/node_alloc_holder.hpp>
...
#include <lslboost/intrusive/detail/tree_value_compare.hpp>    //tree_value_compare
// move
...
#include <lslboost/container/detail/std_fwd.hpp>

namespace lslboost {
namespace container {
namespace container_detail {

using lslboost::intrusive::tree_value_compare;

template<class VoidPointer, lslboost::container::tree_type_enum tree_type_value, bool OptimizeSize>
struct intrusive_tree_hook;

template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, lslboost::container::red_black_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, lslboost::container::avl_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, lslboost::container::scapegoat_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, lslboost::container::splay_tree, OptimizeSize>
{
   typedef typename container_detail::bi::make_bs_set_base_hook
      < container_detail::bi::void_pointer<VoidPointer>
      , container_detail::bi::link_mode<container_detail::bi::normal_link>
...
{
   typedef pair<typename lslboost::move_detail::remove_const<T1>::type, T2> type;
};

//The node to be store in the tree
template <class T, class VoidPointer, lslboost::container::tree_type_enum tree_type_value, bool OptimizeSize>
struct tree_node
   :  public intrusive_tree_hook<VoidPointer, tree_type_value, OptimizeSize>::type
{
   ...
   {
      const_cast<T1&>(m_data.first) = ::lslboost::move(p.first);
      m_data.second = ::lslboost::move(p.second);
   }
...
template< class NodeType, class NodeCompareType
        ...
        , lslboost::container::tree_type_enum tree_type_value>
struct intrusive_tree_dispatch;

template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, lslboost::container::red_black_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, lslboost::container::avl_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, lslboost::container::scapegoat_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, lslboost::container::splay_tree>
{
...
template<class Allocator, class ValueCompare, lslboost::container::tree_type_enum tree_type_value, bool OptimizeSize>
...
template<lslboost::container::tree_type_enum tree_type_value>
      ...
      >
{
   ...
   typedef lslboost::container::reverse_iterator<iterator>                 reverse_iterator;
   typedef lslboost::container::reverse_iterator<const_iterator>           const_reverse_iterator;
      ...
      : AllocHolder(value_compare(comp), a)
   {
      if(unique_insertion){
         ...
         this->allocate_many_and_construct
            ( first, lslboost::container::iterator_distance(first, last)
      ...
      :  AllocHolder(x.value_comp(), a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->icont().clone_from
            (lslboost::move(x.icont()), typename AllocHolder::move_cloner(*this), Destroyer(this->node_alloc()));
      }
   }
   ...
   tree& operator=(BOOST_COPY_ASSIGN_REF(tree) x)
   {
      if (&x != this){
         ...
         //automatically
         Icont other_tree(::lslboost::move(this->icont()));
                                 ...
                                 && lslboost::container::container_detail::is_nothrow_move_assignable<Compare>::value )
   {
      ...
      if(propagate_alloc || allocators_equal){
         ...
         //Obtain resources
         this->icont() = lslboost::move(x.icont());
      }
   ...
   {
      NodePtr tmp = AllocHolder::create_node(lslboost::forward<MovableConvertible>(v));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
      iterator ret(this->icont().insert_unique_commit(*tmp, data));
      destroy_deallocator.release();
      return ret;
   ...
   std::pair<iterator,bool> insert_unique(BOOST_FWD_REF(MovableConvertible) v)
   {
      ...
      std::pair<iterator,bool> ret =
         this->insert_unique_check(KeyOfValue()(v), data);
      if(ret.second){
         ret.first = this->insert_unique_commit(lslboost::forward<MovableConvertible>(v), data);
      }
      return ret;
   }
   ...
   {  return this->emplace_unique_impl(AllocHolder::create_node(lslboost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_hint_unique(const_iterator hint, BOOST_FWD_REF(Args)... args)
   {  return this->emplace_unique_hint_impl(hint, AllocHolder::create_node(lslboost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_equal(BOOST_FWD_REF(Args)... args)
   {
      NodePtr tmp(AllocHolder::create_node(lslboost::forward<Args>(args)...));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
   ...
   iterator insert_unique(const_iterator hint, BOOST_FWD_REF(MovableConvertible) v)
   {
      ...
      if(!ret.second)
         return ret.first;
      return this->insert_unique_commit(lslboost::forward<MovableConvertible>(v), data);
   }
...
struct has_trivial_destructor_after_move
   < 
      ::lslboost::container::container_detail::tree
