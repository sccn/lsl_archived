...
} lslboost_cont_memchain_node;


/*!An forward iterator to traverse the elements of a memory chain container.*/
typedef struct multialloc_it_impl
{
   lslboost_cont_memchain_node *node_ptr;
} lslboost_cont_memchain_it;

/*!Memory chain: A container holding memory portions allocated by lslboost_cont_multialloc_nodes
   and lslboost_cont_multialloc_arrays functions.*/
typedef struct lslboost_cont_memchain_impl
{
   size_t                   num_mem;
   lslboost_cont_memchain_node  root_node;
   lslboost_cont_memchain_node *last_node_ptr;
} lslboost_cont_memchain;
...
#define BOOST_CONTAINER_MEMCHAIN_END_IT(PMEMCHAIN)     {(lslboost_cont_memchain_node *)0 }
...
#define BOOST_CONTAINER_MEMCHAIN_INIT_FROM(PMEMCHAIN, FIRST, LAST, NUM)\
   (PMEMCHAIN)->last_node_ptr = (lslboost_cont_memchain_node *)(LAST), \
   (PMEMCHAIN)->root_node.next_node_ptr  = (lslboost_cont_memchain_node *)(FIRST), \
...
#define BOOST_CONTAINER_MEMCHAIN_PUSH_BACK(PMEMCHAIN, MEM)\
   do{\
      lslboost_cont_memchain *____chain____ = (PMEMCHAIN);\
      lslboost_cont_memchain_node *____tmp_mem____ = (lslboost_cont_memchain_node *)(MEM);\
...
#define BOOST_CONTAINER_MEMCHAIN_PUSH_FRONT(PMEMCHAIN, MEM)\
   do{\
      lslboost_cont_memchain *____chain____ = (PMEMCHAIN);\
      lslboost_cont_memchain_node *____tmp_mem____   = (lslboost_cont_memchain_node *)(MEM);\
      lslboost_cont_memchain *____root____  = &((PMEMCHAIN)->root_node);\
      if(!____chain____->root_node.next_node_ptr){\
         ____chain____->last_node_ptr = ____tmp_mem____;\
      }\
      lslboost_cont_memchain_node *____old_first____ = ____root____->next_node_ptr;\
...
#define BOOST_CONTAINER_MEMCHAIN_ERASE_AFTER(PMEMCHAIN, BEFORE_IT)\
   do{\
      lslboost_cont_memchain *____chain____ = (PMEMCHAIN);\
      lslboost_cont_memchain_node *____prev_node____  = (BEFORE_IT).node_ptr;\
      lslboost_cont_memchain_node *____erase_node____ = ____prev_node____->next_node_ptr;\
...
#define BOOST_CONTAINER_MEMCHAIN_POP_FRONT(PMEMCHAIN)\
   do{\
      lslboost_cont_memchain *____chain____ = (PMEMCHAIN);\
      lslboost_cont_memchain_node *____prev_node____  = &____chain____->root_node;\
      lslboost_cont_memchain_node *____erase_node____ = ____prev_node____->next_node_ptr;\
      if(____chain____->last_node_ptr == ____erase_node____){\
         ____chain____->last_node_ptr = &____chain____->root_node;\
      }\
...
#define BOOST_CONTAINER_MEMCHAIN_SPLICE_BACK(PMEMCHAIN, PMEMCHAIN2)\
   do{\
      lslboost_cont_memchain *____chain____  = (PMEMCHAIN);\
      lslboost_cont_memchain *____chain2____ = (PMEMCHAIN2);\
...
#define BOOST_CONTAINER_MEMCHAIN_INCORPORATE_AFTER(PMEMCHAIN, BEFORE_IT, FIRST, BEFORELAST, NUM)\
   do{\
      lslboost_cont_memchain *____chain____  = (PMEMCHAIN);\
      lslboost_cont_memchain_node *____pnode____  = (BEFORE_IT).node_ptr;\
      lslboost_cont_memchain_node *____next____   = ____pnode____->next_node_ptr;\
      lslboost_cont_memchain_node *____first____  = (lslboost_cont_memchain_node *)(FIRST);\
      lslboost_cont_memchain_node *____blast____  = (lslboost_cont_memchain_node *)(BEFORELAST);\
...
BOOST_CONTAINER_DECL size_t lslboost_cont_size(const void *p);

BOOST_CONTAINER_DECL void* lslboost_cont_malloc(size_t bytes);

BOOST_CONTAINER_DECL void  lslboost_cont_free(void* mem);

BOOST_CONTAINER_DECL void* lslboost_cont_memalign(size_t bytes, size_t alignment);

/*!Indicates the all elements allocated by lslboost_cont_multialloc_nodes or lslboost_cont_multialloc_arrays
   must be contiguous.*/
#define DL_MULTIALLOC_ALL_CONTIGUOUS        ((size_t)(-1))

/*!Indicates the number of contiguous elements allocated by lslboost_cont_multialloc_nodes or lslboost_cont_multialloc_arrays
   should be selected by those functions.*/
#define DL_MULTIALLOC_DEFAULT_CONTIGUOUS    ((size_t)(0))

BOOST_CONTAINER_DECL int lslboost_cont_multialloc_nodes
   (size_t n_elements, size_t elem_size, size_t contiguous_elements, lslboost_cont_memchain *pchain);

BOOST_CONTAINER_DECL int lslboost_cont_multialloc_arrays
   (size_t n_elements, const size_t *sizes, size_t sizeof_element, size_t contiguous_elements, lslboost_cont_memchain *pchain);

BOOST_CONTAINER_DECL void lslboost_cont_multidealloc(lslboost_cont_memchain *pchain);

BOOST_CONTAINER_DECL size_t lslboost_cont_footprint();

BOOST_CONTAINER_DECL size_t lslboost_cont_allocated_memory();

BOOST_CONTAINER_DECL size_t lslboost_cont_chunksize(const void *p);

BOOST_CONTAINER_DECL int lslboost_cont_all_deallocated();

typedef struct lslboost_cont_malloc_stats_impl
{
   size_t max_system_bytes;
   size_t system_bytes;
   size_t in_use_bytes;
} lslboost_cont_malloc_stats_t;

BOOST_CONTAINER_DECL lslboost_cont_malloc_stats_t lslboost_cont_malloc_stats();

BOOST_CONTAINER_DECL size_t lslboost_cont_in_use_memory();

BOOST_CONTAINER_DECL int lslboost_cont_trim(size_t pad);

BOOST_CONTAINER_DECL int lslboost_cont_mallopt
   (int parameter_number, int parameter_value);

BOOST_CONTAINER_DECL int lslboost_cont_grow
   (void* oldmem, size_t minbytes, size_t maxbytes, size_t *received);

BOOST_CONTAINER_DECL int lslboost_cont_shrink
   (void* oldmem, size_t minbytes, size_t maxbytes, size_t *received, int do_commit);

BOOST_CONTAINER_DECL void* lslboost_cont_alloc
   (size_t minbytes, size_t preferred_bytes, size_t *received_bytes);

BOOST_CONTAINER_DECL int lslboost_cont_malloc_check();
...
typedef struct lslboost_cont_command_ret_impl
{
   void *first;
   int   second;
}lslboost_cont_command_ret_t;

BOOST_CONTAINER_DECL lslboost_cont_command_ret_t lslboost_cont_allocation_command
...
BOOST_CONTAINER_DECL int lslboost_cont_mallopt(int param_number, int value);
