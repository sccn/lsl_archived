   ...
   :  public advanced_insert_aux_int<Iterator>
{
   ...
   {  ::lslboost::copy_or_move(this->first_, this->last_, p);  }

   virtual void uninitialized_copy_remaining_to(Iterator p)
   {  ::lslboost::container::uninitialized_copy_or_move_alloc(this->a_, this->first_, this->last_, p);  }
   ...
   virtual void uninitialized_copy_some_and_update(Iterator pos, difference_type division_count, bool first_n)
   {
      ...
      if(first_n){
         ::lslboost::container::uninitialized_copy_or_move_alloc(this->a_, this->first_, mid, pos);
         this->first_ = mid;
      }
      else{
         ::lslboost::container::uninitialized_copy_or_move_alloc(this->a_, mid, this->last_, pos);
         this->last_ = mid;
      }
   }
   ...
   virtual void copy_some_and_update(Iterator pos, difference_type division_count, bool first_n)
   {
      ...
      std::advance(mid, division_count);
      if(first_n){
         ::lslboost::copy_or_move(this->first_, mid, pos);
         this->first_ = mid;
      }
      else{
         ::lslboost::copy_or_move(mid, this->last_, pos);
         this->last_ = mid;
      }
   }
   A &a_;
   ...
   void priv_copy_remaining_to(const index_tuple<IdxPack...>&, Iterator p)
   {
      if(!this->used_){
         ...
         alloc_traits::construct(this->a_, vp,
            ::lslboost::container::container_detail::stored_ref<Args>::forward(get<IdxPack>(this->args_))...);
