...
#include <lslboost/container/new_allocator.hpp> //new_allocator
#include <lslboost/container/throw_exception.hpp>
// container detail
#include <lslboost/container/detail/advanced_insert_int.hpp>
#include <lslboost/container/detail/algorithm.hpp> //equal()
#include <lslboost/container/detail/alloc_helpers.hpp>
#include <lslboost/container/detail/allocation_type.hpp>
#include <lslboost/container/detail/copy_move_algo.hpp>
...
#include <lslboost/container/detail/next_capacity.hpp>
#include <lslboost/container/detail/to_raw_pointer.hpp>
#include <lslboost/container/detail/type_traits.hpp>
#include <lslboost/container/detail/version_type.hpp>
// intrusive
...
#endif
#include <lslboost/move/detail/move_helpers.hpp>
// other
   ...
   : public Allocator
{
   ...
   pointer allocation_command(lslboost::container::allocation_type command,
   ...
   pointer priv_allocation_command(version_1, lslboost::container::allocation_type command,
                         size_type ,
...
class vector
{
   ...
   void emplace_back(BOOST_FWD_REF(Args)...args)
   {
      if (BOOST_LIKELY(this->room_enough())){
         ...
         allocator_traits_type::construct(this->m_holder.alloc(), this->back_raw(), ::lslboost::forward<Args>(args)...);
   ...
   iterator erase(const_iterator first, const_iterator last)
   {
      if (first != last){
         ...
         T* const ptr = container_detail::to_raw_pointer(lslboost::container::move(last_ptr, old_end_ptr, first_ptr));
      ...
      if(BOOST_LIKELY(n)){
         ...
         if(BOOST_LIKELY(s)){
            ...
            if(new_buffer){
               ...
               lslboost::uintptr_t const addr = lslboost::uintptr_t(container_detail::to_raw_pointer(m_holder.start()) + s + n);
      ...
      (UniqueBool, FwdIt first, size_type n, Compare comp, pointer new_storage, size_type const new_cap)
   {
      ...
      lslboost::container::destroy_alloc_n(a, container_detail::to_raw_pointer(old_p), old_size);
         ...
         >::type * = 0)
   {
      ...
      //Else do a one by one move
         ...
         this->assign( lslboost::make_move_iterator(container_detail::iterator_to_raw_pointer(x.begin()))
                     , lslboost::make_move_iterator(container_detail::iterator_to_raw_pointer(x.end()  ))
                     );
      }
   ...
   void priv_swap(Vector &x, container_detail::false_type)  //version_N
   {
      ...
      if(are_swap_propagable( this->get_stored_allocator(), this->m_holder.start()
         ...
         sml.insert( sml.cend()
                   , lslboost::make_move_iterator(container_detail::iterator_to_raw_pointer(big.nth(common_elements)))
                   , lslboost::make_move_iterator(container_detail::iterator_to_raw_pointer(big.end()))
                   );
      }
      ...
      if(s_before >= before_plus_new){
         ...
         //s_before is so big that divides old_end
            ...
            if(!value_traits::trivial_dctr){
               ...
               ::lslboost::container::move(pos + raw_gap, old_finish, old_start);
         ...
         if (s_before <= elemsbefore) {
            ...
            if(do_after){
               ...
               //Displace old_end
               T* const move_end = ::lslboost::container::move(pos, old_finish, move_start);
