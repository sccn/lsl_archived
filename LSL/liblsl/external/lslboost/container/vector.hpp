...
#include <lslboost/move/move_helpers.hpp>
...
class vector_const_iterator
{
   ...
   typedef typename lslboost::intrusive::pointer_traits<Pointer>::template
...
struct vector_value_traits
{
   ...
   static const bool trivial_dctr_after_move = trivial_dctr;
      //::lslboost::has_trivial_destructor_after_move<value_type>::value || trivial_dctr;
...
{
   typedef lslboost::container::allocator_traits<A>         allocator_traits_type;
   ...
   vector_alloc_holder()
      BOOST_CONTAINER_NOEXCEPT_IF(::lslboost::has_nothrow_default_constructor<A>::value)
      : members_()
   {}
   ...
   explicit vector_alloc_holder(BOOST_FWD_REF(AllocConvertible) a) BOOST_CONTAINER_NOEXCEPT
      : members_(lslboost::forward<AllocConvertible>(a))
   {}
   ...
   typedef container_detail::integral_constant<unsigned,
      lslboost::container::container_detail::version<A>::value> alloc_version;
      ...
      : public A
   {
      ...
      explicit members_holder(BOOST_FWD_REF(Alloc) alloc)
         :  A(lslboost::forward<Alloc>(alloc)), m_start(0), m_size(0), m_capacity(0)
      {}
...
class vector : private container_detail::vector_alloc_holder<A>
{
   ...
   void reserve(size_type new_cap)
   {
      if (this->capacity() < new_cap){
         ...
         //If there is no forward expansion, move objects
            ...
            container_detail::advanced_insert_aux_proxy<A, lslboost::move_iterator<T*>, T*>
               proxy(this->alloc(), ::lslboost::make_move_iterator(dummy_it), ::lslboost::make_move_iterator(dummy_it));
      ...
      BOOST_CONTAINER_NOEXCEPT
   {
      if (&x != this){
         ...
         //If unequal allocators, then do a one by one move
            ...
            this->assign( lslboost::make_move_iterator(container_detail::to_raw_pointer(x.members_.m_start))
                        , lslboost::make_move_iterator(container_detail::to_raw_pointer(x.members_.m_start + x.members_.m_size)));
         }
      }
      return *this;
   }
   ...
   void emplace_back(Args &&...args)
   {
      ...
      if (this->members_.m_size < this->members_.m_capacity){
         ...
         allocator_traits_type::construct(this->alloc(), back_pos, ::lslboost::forward<Args>(args)...);
         ++this->members_.m_size;
      }
      else{
         typedef container_detail::advanced_insert_aux_emplace<A, T*, Args...> type;
         type &&proxy = type(this->alloc(), ::lslboost::forward<Args>(args)...);
   ...
   iterator erase(const_iterator position)
   {
      ...
      ::lslboost::move(pos + 1, beg + this->members_.m_size, pos);
      --this->members_.m_size;
      //Destroy last element
   ...
   iterator erase(const_iterator first, const_iterator last)
   {
      if (first != last){   // worth doing, copy down over hole
         ...
         T* ptr = container_detail::to_raw_pointer(lslboost::move
   ...
   size_type priv_insert_ordered_at_shift_range(size_type first_pos, size_type last_pos, size_type limit_pos, size_type shift_count)
   {
      ...
      if((last_pos + shift_count) <= limit_pos){
         //All move assigned
         lslboost::move_backward(begin_ptr + first_pos, begin_ptr + last_pos, begin_ptr + last_pos + shift_count);
      }
      //Case B:
      ...
      //Case C:
         ...
         lslboost::move_backward(begin_ptr + first_pos, boundary_ptr, limit_ptr);
      }
      return hole_size;
   }
          ...
          T* pos, const size_type n, advanced_insert_aux_int_t &interf)
   {
      ...
      if(difference_type(s_before) >= difference_type(elemsbefore + n)){
         ...
         ::lslboost::container::uninitialized_move_alloc(this->alloc(), old_start, pos, new_start);
         ...
         //s_before is so big that divides old_end
            ...
            T *to_destroy = ::lslboost::move(pos + raw_gap, old_finish, old_start);
         ...
         if (s_before <= elemsbefore) {
            ...
            T* next = ::lslboost::move(start_n, pos, old_start);
            if(do_after){
   ...
   void priv_assign_aux(FwdIt first, FwdIt last, std::forward_iterator_tag)
   {
      ...
      if(same_buffer_start){
         ...
         if (this->size() >= n){
            ...
            ::lslboost::container::uninitialized_copy_or_move_alloc(this->alloc(), mid, last, end);
            this->members_.m_size = n;
         }
      }
      else if(!ret.second){
         typename value_traits::ArrayDeallocator scoped_alloc(ret.first, this->alloc(), real_cap);
         ::lslboost::container::uninitialized_copy_or_move_alloc(this->alloc(), first, last, container_detail::to_raw_pointer(ret.first));
         scoped_alloc.release();
         ...
         ::lslboost::container::uninitialized_copy_or_move_alloc
...
template <class T, class A>
struct has_trivial_destructor_after_move<lslboost::container::vector<T, A> >
{
