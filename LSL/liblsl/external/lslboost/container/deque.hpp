...
class deque : protected deque_base<Allocator>
{
   ...
   void emplace_front(Args&&... args)
   {
      if(this->priv_push_front_simple_available()){
         ...
         this->priv_insert_front_aux_impl(1, type(this->alloc(), lslboost::forward<Args>(args)...));
      }
   }
   ...
   void emplace_back(Args&&... args)
   {
      if(this->priv_push_back_simple_available()){
         ...
         typedef container_detail::insert_non_movable_emplace_proxy<Allocator, iterator, Args...> type;
         this->priv_insert_back_aux_impl(1, type(this->alloc(), lslboost::forward<Args>(args)...));
      }
   }
   ...
   iterator emplace(const_iterator p, Args&&... args)
   {
      if(p == this->cbegin()){
         ...
         return this->priv_insert_aux_impl(p, 1, type(this->alloc(), lslboost::forward<Args>(args)...));
      }
   }
   ...
   iterator erase(const_iterator pos) BOOST_CONTAINER_NOEXCEPT
   {
      ...
      if (index < (this->size()/2)) {
         lslboost::move_backward(this->begin(), pos.unconst(), next);
         pop_front();
      }
      else {
         lslboost::move(next, this->end(), pos.unconst());
         pop_back();
      }
   ...
   iterator erase(const_iterator first, const_iterator last) BOOST_CONTAINER_NOEXCEPT
   {
      if (first == this->members_.m_start && last == this->members_.m_finish) {
         ...
         if (elems_before < (this->size() - n) - elems_before) {
            lslboost::move_backward(begin(), first.unconst(), last.unconst());
            ...
            lslboost::move(last.unconst(), end(), first.unconst());
   ...
   iterator priv_insert(const_iterator position, BOOST_FWD_REF(U) x)
   {
      if (position == cbegin()){
         ...
         return priv_insert_aux_impl
            (position, (size_type)1, container_detail::get_insert_value_proxy<iterator>(this->alloc(), ::lslboost::forward<U>(x)));
      }
   }
   ...
   void priv_push_front(BOOST_FWD_REF(U) x)
   {
      if(this->priv_push_front_simple_available()){
         ...
         priv_insert_aux_impl
            (this->cbegin(), (size_type)1, container_detail::get_insert_value_proxy<iterator>(this->alloc(), ::lslboost::forward<U>(x)));
      }
   }
   ...
   void priv_push_back(BOOST_FWD_REF(U) x)
   {
      if(this->priv_push_back_simple_available()){
         ...
         priv_insert_aux_impl
            (this->cend(), (size_type)1, container_detail::get_insert_value_proxy<iterator>(this->alloc(), ::lslboost::forward<U>(x)));
      ...
      if (elemsbefore < length / 2) {
         ...
         if(!elemsbefore){
            ...
            if (elemsbefore >= n) {
               ...
               this->members_.m_start = new_start;
               lslboost::move(start_n, pos, old_start);
   ...
   void priv_reallocate_map(size_type nodes_to_add, bool add_at_front)
   {
      ...
      if (this->members_.m_map_size > 2 * new_num_nodes) {
         ...
         if (new_nstart < this->members_.m_start.m_node)
            lslboost::move(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1, new_nstart);
         else
            lslboost::move_backward
