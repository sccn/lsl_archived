...
#include <lslboost/container/detail/algorithm.hpp> //algo_equal(), algo_lexicographical_compare
...
#include <lslboost/container/detail/min_max.hpp>
...
class deque_iterator
{
   ...
   typedef typename lslboost::intrusive::pointer_traits<Pointer>::
...
class deque_base
{
   ...
   explicit deque_base(BOOST_RV_REF(deque_base) x)
      :  members_( lslboost::move(x.ptr_alloc())
                 , lslboost::move(x.alloc()) )
   {}
   ...
   {
      ::lslboost::adl_move_swap(this->members_.m_start, x.members_.m_start);
      ::lslboost::adl_move_swap(this->members_.m_finish, x.members_.m_finish);
      ::lslboost::adl_move_swap(this->members_.m_map, x.members_.m_map);
      ::lslboost::adl_move_swap(this->members_.m_map_size, x.members_.m_map_size);
   }
...
//      }
      ...
      ,  public allocator_type
   {
      ...
      members_holder(BOOST_FWD_REF(PtrAllocConvertible) pa, BOOST_FWD_REF(ValAllocConvertible) va)
         : map_allocator_type(lslboost::forward<PtrAllocConvertible>(pa))
         , allocator_type    (lslboost::forward<ValAllocConvertible>(va))
         , m_map(0), m_map_size(0)
         , m_start(), m_finish(m_start)
      {}
...
class deque : protected deque_base<Allocator>
{
      ...
      :  Base(allocator_traits_type::select_on_container_copy_construction(x.alloc()))
   {
      if(x.size()){
         ...
         lslboost::container::uninitialized_copy_alloc
      ...
      :  Base(a)
   {
      if(x.alloc() == a){
         ...
         if(x.size()){
            this->priv_initialize_map(x.size());
            lslboost::container::uninitialized_copy_alloc
               ( this->alloc(), lslboost::make_move_iterator(x.begin())
               , lslboost::make_move_iterator(x.end()), this->members_.m_start);
         }
      }
   }
                                  ...
                                  || allocator_traits_type::is_always_equal::value)
   {
      ...
      //Else do a one by one move
         ...
         this->assign( lslboost::make_move_iterator(x.begin())
                     , lslboost::make_move_iterator(x.end()));
      }
      return *this;
   }
   ...
   {
      const size_type len = lslboost::container::iterator_distance(first, last);
      if (len > size()) {
         FwdIt mid = first;
         lslboost::container::iterator_advance(mid, this->size());
         lslboost::container::copy(first, mid, begin());
         this->insert(this->cend(), mid, last);
      }
      else{
         this->erase(lslboost::container::copy(first, last, this->begin()), cend());
      }
   }
   #endif
   ...
   void emplace_front(BOOST_FWD_REF(Args)... args)
   {
      if(this->priv_push_front_simple_available()){
         allocator_traits_type::construct
            ...
            , lslboost::forward<Args>(args)...);
         this->priv_push_front_simple_commit();
      }
      else{
         typedef container_detail::insert_nonmovable_emplace_proxy<Allocator, iterator, Args...> type;
         this->priv_insert_front_aux_impl(1, type(lslboost::forward<Args>(args)...));
      }
   }
   ...
   void emplace_back(BOOST_FWD_REF(Args)... args)
   {
      if(this->priv_push_back_simple_available()){
         ...
         typedef container_detail::insert_nonmovable_emplace_proxy<Allocator, iterator, Args...> type;
         this->priv_insert_back_aux_impl(1, type(lslboost::forward<Args>(args)...));
      }
   }
   ...
   {
      if(p == this->cbegin()){
         this->emplace_front(lslboost::forward<Args>(args)...);
         return this->begin();
      }
      else if(p == this->cend()){
         this->emplace_back(lslboost::forward<Args>(args)...);
         return (this->end()-1);
      }
      else{
         typedef container_detail::insert_emplace_proxy<Allocator, iterator, Args...> type;
         return this->priv_insert_aux_impl(p, 1, type(lslboost::forward<Args>(args)...));
      }
   }
      ...
      )
   {
      ...
      return priv_insert_aux_impl(p, lslboost::container::iterator_distance(first, last), proxy);
   }
   #endif
   ...
   iterator erase(const_iterator pos) BOOST_NOEXCEPT_OR_NOTHROW
   {
      ...
      if (index < (this->size()/2)) {
         lslboost::container::move_backward(this->begin(), pos.unconst(), next);
         pop_front();
      }
      else {
         lslboost::container::move(next, this->end(), pos.unconst());
         pop_back();
      }
   ...
   iterator erase(const_iterator first, const_iterator last) BOOST_NOEXCEPT_OR_NOTHROW
   {
      if (first == this->members_.m_start && last == this->members_.m_finish) {
         ...
         if (elems_before < (this->size() - n) - elems_before) {
            lslboost::container::move_backward(begin(), first.unconst(), last.unconst());
            ...
            lslboost::container::move(last.unconst(), end(), first.unconst());
   ...
   //! <b>Complexity</b>: Linear to the number of elements in the container.
   friend bool operator<(const deque& x, const deque& y)
   {  return ::lslboost::container::algo_lexicographical_compare(x.begin(), x.end(), y.begin(), y.end());  }
   ...
   {
      if (p == cbegin()){
         this->push_front(::lslboost::forward<U>(x));
         return begin();
      }
      else if (p == cend()){
         this->push_back(::lslboost::forward<U>(x));
         return --end();
      }
      else {
         return priv_insert_aux_impl
            ( p, (size_type)1
            , container_detail::get_insert_value_proxy<iterator, Allocator>(::lslboost::forward<U>(x)));
      }
   }
   ...
   void priv_push_front(BOOST_FWD_REF(U) x)
   {
      if(this->priv_push_front_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_front_simple_pos(), ::lslboost::forward<U>(x));
         this->priv_push_front_simple_commit();
      }
      else{
   ...
   void priv_push_back(BOOST_FWD_REF(U) x)
   {
      if(this->priv_push_back_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_back_simple_pos(), ::lslboost::forward<U>(x));
         this->priv_push_back_simple_commit();
      }
      else{
         priv_insert_aux_impl
      ...
      if (elemsbefore < length / 2) {
         ...
         if(!elemsbefore){
            ...
            if (elemsbefore >= n) {
               ...
               this->members_.m_start = new_start;
               lslboost::container::move(start_n, pos, old_start);
   ...
   void priv_fill_initialize(const value_type& value)
   {
      ...
      BOOST_TRY {
         for ( ; cur < this->members_.m_finish.m_node; ++cur){
            lslboost::container::uninitialized_fill_alloc
   ...
   {
      size_type n = 0;
      n = lslboost::container::iterator_distance(first, last);
      ...
      BOOST_TRY {
         for (; cur_node < this->members_.m_finish.m_node; ++cur_node) {
            FwdIt mid = first;
            lslboost::container::iterator_advance(mid, this->s_buffer_size());
            ::lslboost::container::uninitialized_copy_alloc(this->alloc(), first, mid, *cur_node);
            first = mid;
         }
         ::lslboost::container::uninitialized_copy_alloc(this->alloc(), first, last, this->members_.m_finish.m_first);
      }
      BOOST_CATCH(...){
   ...
   void priv_reallocate_map(size_type nodes_to_add, bool add_at_front)
   {
      ...
      if (this->members_.m_map_size > 2 * new_num_nodes) {
         ...
         if (new_nstart < this->members_.m_start.m_node)
            lslboost::container::move(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1, new_nstart);
         else
            lslboost::container::move_backward
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<lslboost::container::deque<T, Allocator> >
{
   typedef typename ::lslboost::container::allocator_traits<Allocator>::pointer pointer;
