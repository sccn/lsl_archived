...
class deque : protected deque_base<T, A>
{
   ...
   typedef lslboost::move_iterator<r_iterator>     move_it;
   ...
   iterator erase(const_iterator pos) BOOST_CONTAINER_NOEXCEPT
   {
      ...
      if (size_type(index) < (this->size() >> 1)) {
         lslboost::move_backward(begin(), iterator(pos), iterator(next));
         pop_front();
      }
      else {
         lslboost::move(iterator(next), end(), iterator(pos));
         pop_back();
      }
   ...
   iterator erase(const_iterator first, const_iterator last) BOOST_CONTAINER_NOEXCEPT
   {
      if (first == this->members_.m_start && last == this->members_.m_finish) {
         ...
         if (elems_before < static_cast<difference_type>(this->size() - n) - elems_before) {
            lslboost::move_backward(begin(), iterator(first), iterator(last));
            ...
            lslboost::move(iterator(last), end(), iterator(first));
   ...
   {
      if (position == cbegin()) {
         this->push_front(lslboost::move(mx));
         return begin();
      }
      else if (position == cend()) {
         this->push_back(lslboost::move(mx));
         return(end()-1);
      }
      else {
   ...
   void priv_push_front(BOOST_RV_REF(value_type) t)
   {
      if(this->priv_push_front_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_front_simple_pos(), lslboost::move(t));
         this->priv_push_front_simple_commit();
      }
      else{
   ...
   void priv_push_back(BOOST_RV_REF(T) t)
   {
      if(this->priv_push_back_simple_available()){
         allocator_traits_type::construct
            ( this->alloc(), this->priv_push_back_simple_pos(), lslboost::move(t));
         this->priv_push_back_simple_commit();
      }
      else{
   ...
   void priv_insert_aux(const_iterator pos, InpIt first, InpIt last, std::input_iterator_tag)
   {
      for(;first != last; ++first){
         this->insert(pos, lslboost::move(value_type(*first)));
      }
   }
  ...
  // or not the type is an integer.
   ...
   void priv_assign_aux(FwdIt first, FwdIt last, std::forward_iterator_tag)
   {
      ...
      if (len > size()) {
         ...
         lslboost::copy_or_move(first, mid, begin());
         this->insert(cend(), mid, last);
      }
      else
         this->erase(lslboost::copy_or_move(first, last, begin()), cend());
   }
...
template <class T, class A>
struct has_trivial_destructor_after_move<lslboost::container::deque<T, A> >
{
