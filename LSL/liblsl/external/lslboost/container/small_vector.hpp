...
//!
//! `lslboost::container::vector` supports partially propagable allocators
...
//! `lslboost::container::vector< T, small_vector_allocator<Allocator> >`
   ...
   : public Allocator
{
   ...
   struct rebind
   {
      ...
      explicit small_vector_allocator(BOOST_FWD_REF(Args) ...args)
         : Allocator(::lslboost::forward<Args>(args)...)
      {}
   #else
   ...
   small_vector_allocator(BOOST_RV_REF(small_vector_allocator) other) BOOST_NOEXCEPT_OR_NOTHROW
      : Allocator(::lslboost::move(other.as_base()))
   {}
   ...
   {  return static_cast<small_vector_allocator&>(this->Allocator::operator=(::lslboost::move(other.as_base())));  }
   ...
   {  lslboost::adl_move_swap(l.as_base(), r.as_base());  }
...
//! </code>
//!
//! All `lslboost::container:vector` member functions are inherited. See `vector` documentation for details.
//!
   ...
   : public vector<T, small_vector_allocator<SecondaryAllocator> >
{
   ...
   {
      return lslboost::intrusive::pointer_traits<pointer>::pointer_to
   ...
   explicit small_vector_base(initial_capacity_t, std::size_t capacity, BOOST_FWD_REF(AllocFwd) a)
      : base_type(initial_capacity_t(), this->internal_storage(), capacity, ::lslboost::forward<AllocFwd>(a))
   {}
   ...
   #endif
{
   ...
   small_vector(BOOST_RV_REF(small_vector) other)
      : base_type(initial_capacity_t(), internal_capacity(), ::lslboost::move(other.get_stored_allocator()))
