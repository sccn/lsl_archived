...
class flat_set
{
   ...
   flat_set(BOOST_RV_REF(flat_set) mx)
      : m_flat_tree(lslboost::move(mx.m_flat_tree))
   {}
   ...
   flat_set(BOOST_RV_REF(flat_set) mx, const allocator_type &a)
      : m_flat_tree(lslboost::move(mx.m_flat_tree), a)
   {}
   ...
   template<class U>
   std::pair<iterator, bool> insert(const U &u, typename container_detail::enable_if_c<container_detail::is_same<T, U>::value && !::lslboost::has_move_emulation_enabled<U>::value >::type* =0)
   ...
   {  return m_flat_tree.insert_unique(lslboost::move(x));  }
   ...
   template<class U>
   iterator insert(const_iterator position, const U &u, typename container_detail::enable_if_c<container_detail::is_same<T, U>::value && !::lslboost::has_move_emulation_enabled<U>::value >::type* =0)
   ...
   {  return m_flat_tree.insert_unique(position, lslboost::move(x)); }
   ...
   {  return m_flat_tree.emplace_unique(lslboost::forward<Args>(args)...); }
   ...
   {  return m_flat_tree.emplace_hint_unique(hint, lslboost::forward<Args>(args)...); }
...
struct has_trivial_destructor_after_move<lslboost::container::flat_set<T, C, A> >
{
...
class flat_multiset
{
   ...
   template<class U>
   iterator insert(const U &u, typename container_detail::enable_if_c<container_detail::is_same<T, U>::value && !::lslboost::has_move_emulation_enabled<U>::value >::type* =0)
   {  return priv_insert(u); }
   #endif
   ...
   {  return m_flat_tree.insert_equal(lslboost::move(x));   }
   ...
   iterator insert( const_iterator position, const U &u
                  , typename container_detail::enable_if_c<container_detail::is_same<T, U>::value && !::lslboost::has_move_emulation_enabled<U>::value >::type* =0)
   {  return priv_insert(position, u); }
   #endif
   ...
   //! <b>Note</b>: If an element is inserted it might invalidate elements.
   iterator insert(const_iterator position, BOOST_RV_REF(value_type) x)
   {  return m_flat_tree.insert_equal(position, lslboost::move(x));  }
   ...
   {  return m_flat_tree.emplace_equal(lslboost::forward<Args>(args)...); }
   ...
   template <class... Args>
   iterator emplace_hint(const_iterator hint, Args&&... args)
   {  return m_flat_tree.emplace_hint_equal(hint, lslboost::forward<Args>(args)...); }
...
//!specialization for optimizations
template <class T, class C, class A>
struct has_trivial_destructor_after_move<lslboost::container::flat_multiset<T, C, A> >
{
   static const bool value = has_trivial_destructor<A>::value && has_trivial_destructor<C>::value;
};
*/
