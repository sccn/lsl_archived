...
//! \tparam MapOptions is an packed option type generated using using lslboost::container::tree_assoc_options.
   ...
   ///@endcond
{
   ...
   //! Effects: If there is no key equivalent to x in the map, inserts
   //! value_type(lslboost::move(x), T()) into the map (the key is move-constructed)
   //!
   //! Returns: A reference to the mapped_type corresponding to x in *this.
   //!
   //! Complexity: Logarithmic.
   ...
   { return this->base_t::insert_unique(lslboost::move(x)); }
   ...
   { return this->base_t::insert_unique(p, lslboost::move(x)); }
   ...
   mapped_type& priv_subscript(const key_type &k)
   {
      ...
      if (i == this->end() || this->key_comp()(k, (*i).first)){
         ...
         movable_value_type val(k, lslboost::move(m.m_t));
         i = insert(i, lslboost::move(val));
      }
      return (*i).second;
   }
   ...
   mapped_type& priv_subscript(BOOST_RV_REF(key_type) mk)
   {
      ...
      if (i == this->end() || this->key_comp()(k, (*i).first)){
         container_detail::value_init<mapped_type> m;
         movable_value_type val(lslboost::move(k), lslboost::move(m.m_t));
         i = insert(i, lslboost::move(val));
      }
      return (*i).second;
   }
...
struct has_trivial_destructor_after_move<lslboost::container::map<Key, T, Compare, Allocator> >
{
   typedef typename ::lslboost::container::allocator_traits<Allocator>::pointer pointer;
...
//! \tparam Allocator is the allocator to allocate the <code>value_type</code>s
//!   (e.g. <i>allocator< std::pair<const Key, T> > </i>).
//! \tparam MultiMapOptions is an packed option type generated using using lslboost::container::tree_assoc_options.
template < class Key, class T, class Compare = std::less<Key>
   ...
   ///@endcond
{
   ...
   { return this->base_t::insert_equal(lslboost::move(x)); }
   ...
   //!   is inserted right before p.
   iterator insert(const_iterator p, BOOST_RV_REF(nonconst_value_type) x)
   { return this->base_t::insert_equal(p, lslboost::move(x)); }
...
//!specialization for optimizations
template <class Key, class T, class Compare, class Allocator>
struct has_trivial_destructor_after_move<lslboost::container::multimap<Key, T, Compare, Allocator> >
{
   typedef typename ::lslboost::container::allocator_traits<Allocator>::pointer pointer;
   static const bool value = ::lslboost::has_trivial_destructor_after_move<Allocator>::value &&
                             ::lslboost::has_trivial_destructor_after_move<pointer>::value &&
