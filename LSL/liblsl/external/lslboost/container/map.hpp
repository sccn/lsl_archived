...
class map
{
   ...
   map(BOOST_RV_REF(map) x, const allocator_type &a)
      : m_tree(lslboost::move(x.m_tree), a)
   {
      //Allocator type must be std::pair<CONST Key, T>
      BOOST_STATIC_ASSERT((container_detail::is_same<std::pair<const Key, T>, typename Allocator::value_type>::value));
   }
   ...
   {  m_tree = lslboost::move(x.m_tree);   return *this;  }
   ...
   { return m_tree.insert_unique(lslboost::move(x)); }
   ...
   { return m_tree.insert_unique(position, lslboost::move(x)); }
   ...
   {  return m_tree.emplace_unique(lslboost::forward<Args>(args)...); }
   ...
   {  return m_tree.emplace_hint_unique(hint, lslboost::forward<Args>(args)...); }
...
struct has_trivial_destructor_after_move<lslboost::container::map<K, T, C, Allocator> >
{
...
class multimap
{
   ...
   {  return m_tree.emplace_equal(lslboost::forward<Args>(args)...); }
   ...
   template <class... Args>
   iterator emplace_hint(const_iterator hint, Args&&... args)
   {  return m_tree.emplace_hint_equal(hint, lslboost::forward<Args>(args)...); }
   ...
   { return m_tree.insert_equal(lslboost::move(x)); }
   ...
   //!   is inserted right before p.
   iterator insert(const_iterator position, BOOST_RV_REF(nonconst_value_type) x)
   { return m_tree.insert_equal(position, lslboost::move(x)); }
...
//!specialization for optimizations
template <class K, class T, class C, class Allocator>
struct has_trivial_destructor_after_move<lslboost::container::multimap<K, T, C, Allocator> >
{
   static const bool value = has_trivial_destructor_after_move<Allocator>::value && has_trivial_destructor_after_move<C>::value;
};
