...
class set
{
   ...
   {  return m_tree.insert_unique(p, lslboost::move(x)); }
...
struct has_trivial_destructor_after_move<lslboost::container::set<T, C, A> >
{
...
class multiset
{
   ...
   //!   is inserted right before p.
   iterator insert(const_iterator p, BOOST_RV_REF(value_type) x)
   {  return m_tree.insert_equal(p, lslboost::move(x));  }
...
//!specialization for optimizations
template <class T, class C, class A>
struct has_trivial_destructor_after_move<lslboost::container::multiset<T, C, A> >
{
   static const bool value = has_trivial_destructor<A>::value && has_trivial_destructor<C>::value;
};
*/
