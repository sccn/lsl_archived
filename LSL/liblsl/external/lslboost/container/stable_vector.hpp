...
#include <lslboost/container/detail/construct_in_place.hpp>
#include <lslboost/container/detail/iterator.hpp>
#include <lslboost/container/detail/iterators.hpp>
#include <lslboost/container/detail/placement_new.hpp>
      ...
      >
{
   ...
   public:
   typename ::lslboost::intrusive::pointer_traits<Pointer>::element_type value;
};

template<class VoidPtr, class VoidAllocator>
struct index_traits
{
   typedef lslboost::intrusive::
      pointer_traits
   ...
   typedef ::lslboost::container::vector
...
{
   typedef lslboost::intrusive::pointer_traits<Pointer>                                non_const_ptr_traits;
   public:
   ...
   typedef typename ::lslboost::container::container_detail::if_c
      < IsConst
      , typename non_const_ptr_traits::template
         rebind_pointer<const value_type>::type
      , Pointer
      >::type                                                                       pointer;
   typedef lslboost::intrusive::pointer_traits<pointer>                                ptr_traits;
...
class stable_vector
{
   ...
   typedef ::lslboost::container::container_detail::integral_constant
      <unsigned, lslboost::container::container_detail::
   ...
   stable_vector(BOOST_RV_REF(stable_vector) x)
      : internal_data(lslboost::move(x.priv_node_alloc())), index(lslboost::move(x.index))
   {
      ...
      : internal_data(a), index(a)
   {
      if(this->priv_node_alloc() == x.priv_node_alloc()){
         ...
         stable_vector_detail::clear_on_destroy<stable_vector> cod(*this);
         this->insert(this->cend(), lslboost::make_move_iterator(x.begin()), lslboost::make_move_iterator(x.end()));
         STABLE_VECTOR_CHECK_INVARIANT;
         cod.release();
      }
   }
   ...
   void emplace_back(Args &&...args)
   {
      ...
      EmplaceFunctor &&ef = EmplaceFunctor(lslboost::forward<Args>(args)...);
   ...
   {
      const size_type num_new = static_cast<size_type>(lslboost::container::iterator_distance(first, last));
   ...
   void priv_push_back(BOOST_MOVE_CATCH_FWD(U) x)
   {
      if(BOOST_LIKELY(this->priv_capacity_bigger_than_size())){
         ...
         BOOST_ASSERT(!!p);
         {
            ...
            //This might throw
            this->priv_build_node_from_convertible(p, ::lslboost::forward<U>(x));
            rollback.release();
         }
         ...
         this->insert(this->cend(), ::lslboost::forward<U>(x));
      }
   }
   ...
   iterator priv_insert(const_iterator p, BOOST_RV_REF(T) x)
   {
      ...
      typedef lslboost::move_iterator<repeat_it>      repeat_move_it;
   ...
   {
      //This can throw
      lslboost::container::construct_in_place
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , it);
      //This does not throw
      ::new(static_cast<node_base_type*>(container_detail::to_raw_pointer(p)), lslboost_container_new_t())
   ...
   {
      //This can throw
      lslboost::container::allocator_traits<node_allocator_type>::construct
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , ::lslboost::forward<ValueConvertible>(value_convertible));
      //This does not throw
      ::new(static_cast<node_base_type*>(container_detail::to_raw_pointer(p)), lslboost_container_new_t()) node_base_type;
   }

   void priv_swap_members(stable_vector &x)
   {
      lslboost::adl_move_swap(this->internal_data.pool_size, x.internal_data.pool_size);
      ...
      : public node_allocator_type
   {
      ...
      explicit ebo_holder(BOOST_FWD_REF(AllocatorRLValue) a)
         : node_allocator_type(lslboost::forward<AllocatorRLValue>(a))
         , pool_size(0)
         , end_node()
      {}
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<lslboost::container::stable_vector<T, Allocator> >
{
   typedef typename ::lslboost::container::allocator_traits<Allocator>::pointer pointer;
