                         ...
                         , Reference>
{
   ...
   friend class iterator<T, const T, typename lslboost::intrusive::pointer_traits<Pointer>::template rebind_pointer<T>::type>;
   ...
   iterator(const iterator<T, T&, typename lslboost::intrusive::pointer_traits<Pointer>::template rebind_pointer<T>::type>& x)
      : pn(x.pn)
   {}
...
{
   typedef typename lslboost::intrusive::pointer_traits
...
class stable_vector
{
   ...
   typedef ::lslboost::container::vector<void_ptr,
   ...
   node_type_ptr_t allocate_one(AllocatorVersion,
      typename lslboost::container::container_detail::enable_if_c
         <lslboost::container::container_detail::is_same<AllocatorVersion, allocator_v1>
            ::value>::type * = 0)
   ...
   stable_vector(BOOST_RV_REF(stable_vector) x)
      : internal_data(lslboost::move(x.node_alloc())), impl(lslboost::move(x.impl))
   {
   ...
   stable_vector& operator=(BOOST_RV_REF(stable_vector) x)
   {
      if (&x != this){
         ...
         if(this_alloc == x_alloc){
            ...
            this->clear();
            this->impl = lslboost::move(x.impl);
            this->priv_swap_members(x);
   ...
   {
      assign_dispatch(first, last, lslboost::is_integral<InputIterator>());
   }
   ...
   void assign(size_type n,const T& t)
   {
      ...
      return assign_dispatch(cvalue_iterator(t, n), cvalue_iterator(), lslboost::mpl::false_());
   }
   ...
   void push_back(const U &u, typename container_detail::enable_if_c
                  <container_detail::is_same<T, U>::value && !::lslboost::has_move_emulation_enabled<U>::value >::type* =0)
   ...
   {  this->insert(end(), lslboost::move(t));  }
   ...
   void insert(const_iterator position,InputIterator first, InputIterator last)
   {
      ...
      this->insert_iter(position,first,last,
                        lslboost::mpl::not_<lslboost::is_integral<InputIterator> >());
   }
            ...
            ::value>::type * = 0)
   {
      if(!impl.empty() && impl.back()){
         ...
         node_alloc().deallocate_individual(lslboost::move(holder));
         pool_first_ref = pool_last_ref = 0;
         this->internal_data.pool_size = 0;
      }
   }
            ...
            ::value>::type * = 0)
   {
      ...
      multiallocation_chain holder;
      holder.incorporate_after(holder.before_begin(), pool_first_ref, pool_last_ref, internal_data.pool_size);
      //BOOST_STATIC_ASSERT((::lslboost::has_move_emulation_enabled<multiallocation_chain>::value == true));
   ...
   template<typename InputIterator>
   void assign_dispatch(InputIterator first, InputIterator last, lslboost::mpl::false_)
   {
      STABLE_VECTOR_CHECK_INVARIANT;
   ...
   void assign_dispatch(Integer n, Integer t, lslboost::mpl::true_)
   {
      typedef constant_iterator<value_type, difference_type> cvalue_iterator;
      this->assign_dispatch(cvalue_iterator(t, n), cvalue_iterator(), lslboost::mpl::false_());
   }
   ...
   void_ptr new_node(const void_ptr &up, Iter it)
   {
      ...
      try{
         lslboost::container::construct_in_place(this->node_alloc(), container_detail::addressof(p->value), it);
         //This does not throw
   ...
   template <class InputIterator>
   void insert_iter(const_iterator position,InputIterator first,InputIterator last, lslboost::mpl::true_)
   {
   ...
   void insert_iter_fwd_alloc(const impl_iterator it, FwdIterator first, FwdIterator last, difference_type n, allocator_v2)
   {
      ...
      try{
         while(first != last){
            ...
            mem.pop_front();
            //This can throw
            lslboost::container::construct_in_place(this->node_alloc(), container_detail::addressof(p->value), first);
            //This does not throw
            ::new(static_cast<node_type_base_t*>(container_detail::to_raw_pointer(p))) node_type_base_t;
         ...
         node_alloc().deallocate_one(p);
         node_alloc().deallocate_many(lslboost::move(mem));
         impl_iterator e = impl.erase(it+i, it+n);
         this->align_nodes(e, get_last_align());
         throw;
      }
   ...
   template <class InputIterator>
   void insert_iter(const_iterator position, InputIterator first, InputIterator last, lslboost::mpl::false_)
   {
...
/*
      explicit ebo_holder(BOOST_RV_REF(ebo_holder) x)
         : node_allocator_type(lslboost::move(static_cast<node_allocator_type&>(x)))
         , pool_size(0)
         , end_node()
      {}
*/
