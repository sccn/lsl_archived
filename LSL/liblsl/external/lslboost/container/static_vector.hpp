...
class static_storage_allocator
{
   ...
   typedef lslboost::container::container_detail::version_type<static_storage_allocator, 0>   version;
   ...
   typename lslboost::aligned_storage
      <sizeof(T)*N, lslboost::alignment_of<T>::value>::type storage;
};
 ...
 *
 * static_vector is a sequence container like lslboost::container::vector with contiguous storage that can
 * change in size, along with the static allocation, low overhead, and fixed capacity of lslboost::array.
 *
...
// TEMPORARY WORKAROUND
    ...
    static_vector & operator=(::lslboost::rv< static_vector<value_type, C> > const& other)
#else
    ...
    //!
    //! @par Throws
    //!   @li If \c lslboost::has_nothrow_move<Value>::value is \c true and Value's move constructor throws.
    //!   @li If \c lslboost::has_nothrow_move<Value>::value is \c false and Value's copy constructor throws.
    //!
    //! @par Complexity
    //!   Linear O(N).
    static_vector(BOOST_RV_REF(static_vector) other)
        : base_t(lslboost::move(static_cast<base_t&>(other)))
    {}
    ...
    static_vector(BOOST_RV_REF_BEG static_vector<value_type, C> BOOST_RV_REF_END other)
        : base_t(lslboost::move(static_cast<typename static_vector<value_type, C>::base_t&>(other)))
    {}
    ...
    //!
    //! @par Throws
    //!   @li If \c lslboost::has_nothrow_move<Value>::value is \c true and Value's move constructor or move assignment throws.
    //!   @li If \c lslboost::has_nothrow_move<Value>::value is \c false and Value's copy constructor or copy assignment throws.
    //!
    //! @par Complexity
    //!   Linear O(N).
    static_vector & operator=(BOOST_RV_REF(static_vector) other)
    {
        base_t::operator=(lslboost::move(static_cast<base_t&>(other)));
        return *this;
    }
    ...
    {
        base_t::operator=(lslboost::move(static_cast<typename static_vector<value_type, C>::base_t&>(other)));
        return *this;
    }
