...
#include <lslboost/ptr_container/ptr_map_adapter.hpp>
#include <lslboost/ptr_container/detail/serialize_xml_names.hpp>
...
void save(Archive& ar, const ptr_container_detail::ptr_map_adapter_base<T, VoidPtrMap, CloneAllocator,Ordered>& c, unsigned int /*version*/)
{
    ...
    ar << lslboost::serialization::make_nvp( ptr_container_detail::count(), 
                                          ptr_container_detail::serialize_as_const(c.size()) );

    const_iterator i = c.begin(), e = c.end();
    for(; i != e; ++i)
    {
        ar << lslboost::serialization::make_nvp( ptr_container_detail::first(), i->first );
        ar << lslboost::serialization::make_nvp( ptr_container_detail::second(),
...
void load(Archive& ar, ptr_map_adapter<T, VoidPtrMap, CloneAllocator,Ordered>& c, unsigned int /*version*/)
{
    ...
    c.clear();
    size_type n;
    ar >> lslboost::serialization::make_nvp( ptr_container_detail::count(), n );

    for(size_type i = 0u; i != n; ++i)
    {
        key_type key;
        T* value;
        ar >> lslboost::serialization::make_nvp( ptr_container_detail::first(), key );
        ar >> lslboost::serialization::make_nvp( ptr_container_detail::second(), value );
