...
#include <lslboost/ptr_container/detail/reversible_ptr_container.hpp>
#include <lslboost/ptr_container/indirect_fun.hpp>
#include <lslboost/ptr_container/detail/void_ptr_iterator.hpp>
...
{    
    ...
    struct sequence_config
    {
        ...
        BOOST_STATIC_CONSTANT(bool, allow_null = lslboost::is_nullable<T>::value );
    };
    ...
    public:
        ...
        { 
            ...
            BOOST_ASSERT( !::lslboost::is_null( this->begin() ) );
            return *this->begin(); 
        }
        ...
        reference back()
        {
            ...
            BOOST_ASSERT( !::lslboost::is_null( --this->end() ) );
            return *--this->end(); 
        }
    ...
    public: // assign, insert, transfer
        ...
        {
            assign( lslboost::begin(r), lslboost::end(r ) );
        }
    ...
    public:
        ...
        lslboost::disable_if< ptr_container_detail::is_pointer_or_integral<Range> >::type
        insert( iterator before, const Range& r )
        {
            insert( before, lslboost::begin(r), lslboost::end(r) );
        }
        ...
        BOOST_DEDUCED_TYPENAME lslboost::disable_if< lslboost::is_same< Range,
                      BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator > >::type
        transfer( iterator before, const Range& r, PtrSeqAdapter& from ) // strong
        {
            transfer( before, lslboost::begin(r), lslboost::end(r), from );
        }
    ...
    public: // resize
        ...
        void resize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( lslboost::next( this->begin(), size ), this->end() );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
                    this->push_back( new BOOST_DEDUCED_TYPENAME 
                                     lslboost::remove_pointer<value_type>::type() ); 
            }
        ...
        void rresize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( this->begin(), 
                             lslboost::next( this->begin(), old_size - size ) );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
    ...
    public:
        ...
        void unique( iterator first, iterator last, Compare comp )
        {
            ...
            for( ; next != last; ++next )
            {
                BOOST_ASSERT( !::lslboost::is_null(prev) );
                BOOST_ASSERT( !::lslboost::is_null(next) );
                if( comp( *prev, *next ) )
                {
