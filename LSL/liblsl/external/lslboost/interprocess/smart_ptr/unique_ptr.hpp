...
#include <lslboost/interprocess/detail/pointer_type.hpp>
#include <lslboost/move/move.hpp>
#include <lslboost/compressed_pair.hpp>
...
class unique_ptr
{
   ...
   unique_ptr(BOOST_RV_REF(unique_ptr) u)
      : ptr_(u.release(), lslboost::forward<D>(u.get_deleter()))
   {}
   ...
   unique_ptr(BOOST_RV_REF_2_TEMPL_ARGS(unique_ptr, U, E) u,
      typename ipcdetail::enable_if_c<
            ...
            nat
            >::type = nat())
      : ptr_(const_cast<unique_ptr<U,E>&>(u).release(), lslboost::move<D>(u.get_deleter()))
   {}
   ...
   {
      reset(u.release());
      ptr_.second() = lslboost::move(u.get_deleter());
      return *this;
   }
   ...
   /// @cond
   private:
   lslboost::compressed_pair<pointer, D> ptr_;
...
private:
    ...
    template <class U, class E> unique_ptr(U p, E,
        typename lslboost::enable_if<lslboost::is_convertible<U, pointer> >::type* = 0);
