...
// interprocess
#include <lslboost/interprocess/containers/allocation_type.hpp>
...
// interprocess/detail
#include <lslboost/interprocess/detail/min_max.hpp>
...
// container
#include <lslboost/container/detail/multiallocation_chain.hpp>
// container/detail
#include <lslboost/container/detail/placement_new.hpp>
// move/detail
#include <lslboost/move/detail/type_traits.hpp> //make_unsigned, alignment_of
// intrusive
...
class rbtree_best_fit
{
   ...
   typedef typename lslboost::intrusive::pointer_traits<char_ptr>::difference_type difference_type;
   typedef typename lslboost::container::container_detail::make_unsigned<difference_type>::type     size_type;
   ...
   {
      //-----------------------
      lslboost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
   ...
   template<class T>
   T * allocation_command  (lslboost::interprocess::allocation_type command, size_type limit_size,
                           size_type &prefer_in_recvd_out_size, T *&reuse);

   void * raw_allocation_command (lslboost::interprocess::allocation_type command,   size_type limit_object,
   ...
   void* priv_allocation_command(lslboost::interprocess::allocation_type command,   size_type limit_size,
                        size_type &prefer_in_recvd_out_size, void *&reuse_ptr, size_type sizeof_object);


   //!Real allocation algorithm with min allocation option
   void * priv_allocate( lslboost::interprocess::allocation_type command
   ...
   void* priv_expand_both_sides(lslboost::interprocess::allocation_type command
                               ,size_type min_size
                               ,size_type &prefer_in_recvd_out_size
                               ,void *reuse_ptr
   ...
   static const size_type Alignment = !MemAlignment
      ? size_type(::lslboost::container::container_detail::alignment_of
                  < ::lslboost::container::container_detail::max_align_t>::value)
   ...
   BOOST_STATIC_ASSERT((Alignment >= ::lslboost::container::container_detail::alignment_of<void_pointer>::value));
   ...
   priv_add_segment(void *addr, size_type segment_size)
{
   ...
   block_ctrl *first_big_block = ::new(addr, lslboost_container_new_t())block_ctrl;
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::shrink_to_fit()
{
   ...
   if(priv_next_block(first_block) == old_end_block){
      ...
      void *ignore_reuse = 0;
      unique_buffer = priv_allocate(lslboost::interprocess::allocate_new, 0, ignore_recvd, ignore_reuse);
   ...
   allocate(size_type nbytes)
{
   ...
   void *ignore_reuse = 0;
   return priv_allocate(lslboost::interprocess::allocate_new, nbytes, ignore_recvd, ignore_reuse);
}
...
inline T* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   allocation_command  (lslboost::interprocess::allocation_type command,   size_type limit_size,
                        size_type &prefer_in_recvd_out_size, T *&reuse)
{
   ...
   BOOST_ASSERT(0 == ((std::size_t)ret % ::lslboost::container::container_detail::alignment_of<T>::value));
   return static_cast<T*>(ret);
}

template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   raw_allocation_command  (lslboost::interprocess::allocation_type command,   size_type limit_objects,
                        size_type &prefer_in_recvd_out_objects, void *&reuse_ptr, size_type sizeof_object)
{
   ...
   if(command & lslboost::interprocess::try_shrink_in_place){
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_allocation_command (lslboost::interprocess::allocation_type command,   size_type limit_size,
                       size_type &prefer_in_recvd_out_size,
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_expand_both_sides(lslboost::interprocess::allocation_type command
                         ,size_type min_size
                         ,size_type &prefer_in_recvd_out_size
                         ,void *reuse_ptr
                         ,bool only_preferred_backwards
                         ,size_type backwards_multiple)
{
   ...
   if(command & lslboost::interprocess::expand_fwd){
   ...
   if(command & lslboost::interprocess::expand_bwd){
      //Obtain the real size of the block
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void * rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_allocate(lslboost::interprocess::allocation_type command
...
{
   size_type const preferred_size = prefer_in_recvd_out_size;
   if(command & lslboost::interprocess::shrink_in_place){
      if(!reuse_ptr)  return static_cast<void*>(0);
      bool success =
   ...
   if(reuse_ptr && (command & (lslboost::interprocess::expand_fwd | lslboost::interprocess::expand_bwd))){
   ...
   if(command & lslboost::interprocess::allocate_new){
   ...
   priv_expand (void *ptr, const size_type min_size, size_type &prefer_in_recvd_out_size)
{
   ...
   if((merged_units - intended_units) >=  BlockCtrlUnits){
      ...
      block_ctrl *rem_block = ::new(reinterpret_cast<block_ctrl*>
                     (reinterpret_cast<char*>(block) + intended_units*Alignment), lslboost_container_new_t())block_ctrl;
   ...
   ,size_type &received_size)
{
   ...
   if (block->m_size >= upper_nunits){
      ...
      //This is the remaining block
      block_ctrl *rem_block = ::new(reinterpret_cast<block_ctrl*>
                     (reinterpret_cast<char*>(block) + Alignment*nunits), lslboost_container_new_t())block_ctrl;
      algo_impl_t::assert_alignment(rem_block);
