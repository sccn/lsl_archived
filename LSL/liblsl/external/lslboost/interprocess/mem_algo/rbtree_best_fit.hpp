...
class rbtree_best_fit
{
   ...
   typedef typename lslboost::make_unsigned<difference_type>::type     size_type;
   ...
   std::pair<void *, bool>
     raw_allocation_command  (lslboost::interprocess::allocation_type command,   size_type limit_object,
   ...
   std::pair<void *, bool> priv_allocate(lslboost::interprocess::allocation_type command
   ...
   static const size_type Alignment = !MemAlignment
      ? size_type(::lslboost::alignment_of< ::lslboost::detail::max_align>::value)
   ...
   BOOST_STATIC_ASSERT((Alignment >= ::lslboost::alignment_of<void_pointer>::value));
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::shrink_to_fit()
{
   ...
   if(priv_next_block(first_block) == old_end_block){
      ...
      size_type ignore;
      unique_buffer = priv_allocate(lslboost::interprocess::allocate_new, 0, 0, ignore).first;
...
{ 
   ...
   //-----------------------
   size_type ignore;
   void * ret = priv_allocate(lslboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
   return ret;
}
                        ...
                        T *reuse_ptr)
{
   ...
   BOOST_ASSERT(0 == ((std::size_t)ret.first % ::lslboost::alignment_of<T>::value));
   ...
   deallocate_many(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::multiallocation_chain chain)
{
   ...
   lslboost::interprocess::scoped_lock<mutex_type> guard(m_header);
   //-----------------------
   algo_impl_t::deallocate_many(this, lslboost::move(chain));
}
