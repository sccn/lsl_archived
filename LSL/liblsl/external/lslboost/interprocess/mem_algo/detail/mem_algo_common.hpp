...
class memory_algorithm_common
{
   ...
   {
      return this_type::priv_deallocate_many(memory_algo, lslboost::move(chain));
   }
      ...
      (MemoryAlgorithm *memory_algo, size_type nbytes, size_type alignment)
   {
      ...
      if(alignment <= Alignment){
         return memory_algo->priv_allocate
            (lslboost::interprocess::allocate_new, nbytes, nbytes, real_size).first;
      }
      ...
      void *buffer = memory_algo->priv_allocate
         (lslboost::interprocess::allocate_new, request, request, real_size).first;
      if(!buffer){
         return 0;
      }
      ...
      , size_type sizeof_element)
   {
      ...
      while(low_idx < n_elements){
         ...
         std::pair<void *, bool> ret = memory_algo->priv_allocate
            (lslboost::interprocess::allocate_new, min_allocation, total_bytes, received_size, 0);
         if(!ret.first){
            break;
         }
...
//         block_ctrl *prev_block = 0;
      ...
      if(low_idx != n_elements){
         priv_deallocate_many(memory_algo, lslboost::move(chain));
      }
