      ...
      basic_multiallocation_chain<VoidPointer>
{
   ...
   {
      this->base_t::operator=(::lslboost::move(static_cast<base_t&>(other)));
      return *this;
   }

   void *pop_front()
   {
      return lslboost::interprocess::ipcdetail::to_raw_pointer(this->base_t::pop_front());
   }
};
      ...
      (MemoryAlgorithm *memory_algo, size_type nbytes, size_type alignment)
   {
      ...
      if(alignment <= Alignment){
         ...
         return memory_algo->priv_allocate
            (lslboost::interprocess::allocate_new, nbytes, real_size, ignore_reuse);
      }
      ...
      real_size = request;
      void *ignore_reuse = 0;
      void *buffer = memory_algo->priv_allocate(lslboost::interprocess::allocate_new, request, real_size, ignore_reuse);
      if(!buffer){
         return 0;
      }
      ...
      if(total_request_units && !multiplication_overflows(total_request_units, Alignment)){
         ...
         while(low_idx < n_elements){
            ...
            void *ret = memory_algo->priv_allocate
               (lslboost::interprocess::allocate_new, min_allocation, received_size, ignore_reuse);
            if(!ret){
               break;
            }
            ...
            while(total_used_units < received_units){
               ...
               void_pointer p = ::new(memory_algo->priv_get_user_buffer(new_block), lslboost_container_new_t())void_pointer(0);
               chain.push_back(p);
               ++low_idx;
            }
            //Sanity check
