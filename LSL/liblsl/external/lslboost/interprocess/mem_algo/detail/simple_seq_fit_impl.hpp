...
class simple_seq_fit_impl
{
   ...
   std::pair<void *, bool>
      raw_allocation_command  (lslboost::interprocess::allocation_type command,   size_type limit_size,
                               size_type preferred_size,size_type &received_size,
   ...
   std::pair<void *, bool> priv_allocation_command(lslboost::interprocess::allocation_type command
                                        ,size_type min_size
                                        ,size_type preferred_size
                                        ,size_type &received_size
                                        ,void *reuse_ptr
   ...
   public:
	static const size_type Alignment      = ::lslboost::alignment_of< ::lslboost::detail::max_align>::value;
   private:
...
void simple_seq_fit_impl<MutexFamily, VoidPointer>::shrink_to_fit()
{
   ...
   if(!m_header.m_allocated){
      ...
      size_type ignore;
      unique_block = priv_allocate(lslboost::interprocess::allocate_new, 0, 0, ignore).first;
      if(!unique_block)
         return;
   ...
   allocate(size_type nbytes)
{
   ...
   //-----------------------
   size_type ignore;
   return priv_allocate(lslboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
}
