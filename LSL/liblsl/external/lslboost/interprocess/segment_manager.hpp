   ...
   :  private MemoryAlgorithm
{
   ...
   {  MemoryAlgorithm::deallocate_many(lslboost::move(chain)); }
                           ...
                           T *reuse_ptr = 0)
   {
      std::pair<T *, bool> ret = MemoryAlgorithm::allocation_command
         ( command | lslboost::interprocess::nothrow_allocation, limit_size, preferred_size, received_size
         , reuse_ptr);
      if(!(command & lslboost::interprocess::nothrow_allocation) && !ret.first)
         throw bad_alloc();
      return ret;
   }
                           ...
                           void *reuse_ptr = 0, size_type sizeof_object = 1)
   {
      std::pair<void *, bool> ret = MemoryAlgorithm::raw_allocation_command
         ( command | lslboost::interprocess::nothrow_allocation, limit_objects, preferred_objects, received_objects
