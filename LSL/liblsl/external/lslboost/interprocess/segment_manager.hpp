...
#include <lslboost/interprocess/detail/transform_iterator.hpp>

#include <lslboost/interprocess/detail/mpl.hpp>
#include <lslboost/interprocess/detail/nothrow.hpp>
#include <lslboost/interprocess/detail/segment_manager_helper.hpp>
#include <lslboost/interprocess/detail/named_proxy.hpp>
#include <lslboost/interprocess/detail/utilities.hpp>
#include <lslboost/interprocess/offset_ptr.hpp>
#include <lslboost/interprocess/indexes/iset_index.hpp>
#include <lslboost/interprocess/exceptions.hpp>
#include <lslboost/interprocess/allocators/allocator.hpp>
#include <lslboost/interprocess/smart_ptr/deleter.hpp>
   ...
   :  private MemoryAlgorithm
{
   ...
   //!Allocates nbytes bytes. Throws lslboost::interprocess::bad_alloc
   //!on failure
                           ...
                           size_type &prefer_in_recvd_out_size, T *&reuse)
   {
      T *ret = MemoryAlgorithm::allocation_command
         (command | lslboost::interprocess::nothrow_allocation, limit_size, prefer_in_recvd_out_size, reuse);
      if(!(command & lslboost::interprocess::nothrow_allocation) && !ret)
         throw bad_alloc();
      return ret;
   }

   void *raw_allocation_command  (lslboost::interprocess::allocation_type command,   size_type limit_objects,
                           size_type &prefer_in_recvd_out_size, void *&reuse, size_type sizeof_object = 1)
   {
      void *ret = MemoryAlgorithm::raw_allocation_command
         ( command | lslboost::interprocess::nothrow_allocation, limit_objects,
      ...
      (size_type num, bool dothrow, ipcdetail::in_place_interface &table)
   {
      ...
      block_header_t * hdr = ::new(ptr_struct, lslboost_container_new_t()) block_header_t(block_info);
   ...
   :  public segment_manager_base<MemoryAlgorithm>
{
   ...
   //!Can throw lslboost::interprocess::bad_alloc if there is no enough memory.
   ...
   struct allocator
   {
      typedef lslboost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef lslboost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      lslboost::interprocess::allocation_type type = ctrl_data->alloc_type();
      ...
      IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::true_ is_intrusive)
   {
     ...
     std::size_t namelen  = std::char_traits<CharT>::length(name);
      ...
      intrusive_value_type * intrusive_hdr = ::new(buffer_ptr, lslboost_container_new_t()) intrusive_value_type();
      block_header_t * hdr = ::new(intrusive_hdr->get_block_header(), lslboost_container_new_t())block_header_t(block_info);
      void *ptr = 0; //avoid gcc warning
      ptr = hdr->value();
      ...
      IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm> > &index, ipcdetail::false_ is_intrusive)
   {
      ...
      if(is_node_index_t::value){
         ...
         index_it *idr = ::new(buffer_ptr, lslboost_container_new_t()) index_it(it);
      ...
      hdr = ::new(hdr, lslboost_container_new_t())block_header_t(block_info);
      void *ptr = 0; //avoid gcc warning
      ptr = hdr->value();
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(lslboost::move(local));
   }
