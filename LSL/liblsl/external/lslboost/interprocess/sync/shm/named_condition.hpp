...
#include <lslboost/interprocess/sync/named_mutex.hpp>
#include <lslboost/interprocess/permissions.hpp>
   ...
   #include <lslboost/interprocess/sync/detail/condition_any_algorithm.hpp>
#else
...
class shm_named_condition
{
   ...
   friend class lslboost::interprocess::ipcdetail::interprocess_tester;
   void dont_close_on_destruction();

   typedef ipcdetail::managed_open_or_create_impl<shared_memory_object, 0, true, false> open_create_impl_t;
   open_create_impl_t m_shmem;

   template <class T, class Arg> friend class lslboost::interprocess::ipcdetail::named_creation_functor;
   typedef lslboost::interprocess::ipcdetail::named_creation_functor<internal_condition> construct_func_t;
   #endif   //#ifndef BOOST_INTERPROCESS_DOXYGEN_INVOKED
};
...
inline bool shm_named_condition::timed_wait
   (L& lock, const lslboost::posix_time::ptime &abs_time)
{  return this->internal_cond().timed_wait(lock, abs_time); }

template <typename L, typename Pr>
inline bool shm_named_condition::timed_wait
   (L& lock, const lslboost::posix_time::ptime &abs_time, Pr pred)
