...
class intermodule_singleton_common
{
   ...
   static void initialize_shm()
   {
      ...
      while(1){
         ...
         ::lslboost::uint32_t tmp = atomic_cas32(&this_module_shm_initialized, Initializing, Uninitialized);
         if(tmp >= Initialized){
            break;
         }
   ...
   static volatile lslboost::uint32_t this_module_shm_initialized;

   //Raw memory to construct the shared memory manager  
   static struct mem_holder_t
   {
      ::lslboost::detail::max_align aligner;
      char shm_mem [sizeof(ManagedGlobalMemory)];
   } mem_holder;
};

template<class ManagedGlobalMemory>
volatile lslboost::uint32_t intermodule_singleton_common<ManagedGlobalMemory>::this_module_singleton_count;

template<class ManagedGlobalMemory>
volatile lslboost::uint32_t intermodule_singleton_common<ManagedGlobalMemory>::this_module_shm_initialized;
...
class intermodule_singleton_impl
{
   ...
   {
      ref_count_ptr(C *p, lslboost::uint32_t count)
...
template <typename C, bool L, bool P, class ManagedGlobalMemory>
volatile lslboost::uint32_t intermodule_singleton_impl<C, L, P, ManagedGlobalMemory>::this_module_singleton_initialized = 0;
