...
#include <lslboost/interprocess/mapped_region.hpp>
   ...
   : public managed_open_or_create_impl_device_holder<StoreDevice, DeviceAbstraction>
{
   ...
   static const std::size_t
      ManagedOpenOrCreateUserOffset =
         ct_rounded_size
            < sizeof(lslboost::uint32_t)
            , MemAlignment ? (MemAlignment) :
               (::lslboost::container::container_detail::alignment_of
   ...
   {
      managed_open_or_create_impl tmp(lslboost::move(moved));
      this->swap(tmp);
      return *this;
   }
       ...
       ConstructFunc construct_func)
   {
      ...
      if(created){
         try{
            ...
            lslboost::uint32_t *patomic_word = 0;  //avoid gcc warning
            patomic_word = static_cast<lslboost::uint32_t*>(region.get_address());
            lslboost::uint32_t previous = atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
         ...
         lslboost::uint32_t *patomic_word = static_cast<lslboost::uint32_t*>(region.get_address());
         lslboost::uint32_t value = atomic_read32(patomic_word);
      ...
      if(StoreDevice){
         this->DevHolder::get_device() = lslboost::move(dev);
      }
   }
