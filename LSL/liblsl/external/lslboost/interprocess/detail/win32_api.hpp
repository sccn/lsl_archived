...
#include <lslboost/date_time/filetime_functions.hpp>
#include <cstddef>
#include <cstring>
#include <cstdlib>
#include <cstdio>
...
union system_timeofday_information
{
   struct data_t
   {
      ...
      ::lslboost::ulong_long_type ullBootTimeBias;
      ::lslboost::ulong_long_type ullSleepTimeBias;
   } data;
...
inline void * create_file_mapping (void * handle, unsigned long access, ::lslboost::ulong_long_type file_offset, const char * name, interprocess_security_attributes *psec)
{
   const unsigned long high_size(file_offset >> 32), low_size((lslboost::uint32_t)file_offset);
...
inline void *map_view_of_file_ex(void *handle, unsigned long file_access, ::lslboost::ulong_long_type offset, std::size_t numbytes, void *base_addr)
{
   const unsigned long offset_low  = (unsigned long)(offset & ((::lslboost::ulong_long_type)0xFFFFFFFF));
...
class nt_query_mem_deleter
{
   ...
   static const std::size_t rename_suffix =
      (SystemTimeOfDayInfoLength + sizeof(unsigned long) + sizeof(lslboost::uint32_t))*2;
...
inline bool unlink_file(const char *filename)
{
   ...
   try{
      ...
      handle_closer h_closer(fh);
      {
         ...
         static volatile lslboost::uint32_t u32_count = 0;
         interlocked_decrement(reinterpret_cast<volatile long*>(&u32_count));
         buffer_to_wide_str(const_cast<const lslboost::uint32_t *>(&u32_count), sizeof(lslboost::uint32_t), &pfri->FileName[filename_string_length]);
         filename_string_length += sizeof(lslboost::uint32_t)*2;
