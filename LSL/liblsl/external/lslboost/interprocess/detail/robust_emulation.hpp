...
class robust_spin_mutex
{
   ...
   bool is_owner_dead(lslboost::uint32_t owner);
   void owner_to_filename(lslboost::uint32_t owner, std::string &s);
   //The real mutex
   Mutex mtx;
...
template<class Mutex>
inline void robust_spin_mutex<Mutex>::owner_to_filename(lslboost::uint32_t owner, std::string &s)
{
...
template<class Mutex>
inline bool robust_spin_mutex<Mutex>::is_owner_dead(lslboost::uint32_t owner)
{
   //If owner is an invalid id, then it's clear it's dead
   if(owner == (lslboost::uint32_t)get_invalid_process_id()){
      return true;
   }
