...
#include <lslboost/interprocess/segment_manager.hpp>
#include <lslboost/interprocess/sync/scoped_lock.hpp>
#include <lslboost/interprocess/detail/nothrow.hpp>
#include <lslboost/interprocess/detail/simple_swap.hpp>
//
...
class basic_managed_memory_impl
{
   ...
   bool  create_impl   (void *addr, size_type size)
   {
      ...
      BOOST_TRY{
         ...
         mp_header       = ::new(addr, lslboost_container_new_t()) segment_manager(size);
      }
      BOOST_CATCH(...){
         return false;
      }
   ...
   //!memory is available throws a lslboost::interprocess::bad_alloc exception
   ...
   //!
   //!-> Throws lslboost::interprocess::bad_alloc if there is no available memory
   //!
