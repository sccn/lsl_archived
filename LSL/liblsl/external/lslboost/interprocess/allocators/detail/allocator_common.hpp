...
#include <lslboost/interprocess/detail/utilities.hpp> //to_raw_pointer
#include <lslboost/utility/addressof.hpp> //lslboost::addressof
#include <lslboost/assert.hpp>   //BOOST_ASSERT
#include <lslboost/interprocess/exceptions.hpp> //bad_alloc
#include <lslboost/interprocess/sync/scoped_lock.hpp> //scoped_lock
#include <lslboost/interprocess/containers/allocation_type.hpp> //lslboost::interprocess::allocation_type
...
struct get_or_create_node_pool_func
{
   ...
   //!Can throw lslboost::interprocess::bad_alloc
   void operator()()
   {
      //Find or create the node_pool_t
      mp_node_pool =    mp_segment_manager->template find_or_construct
                        <NodePool>(lslboost::interprocess::unique_instance)(mp_segment_manager);
...
struct destroy_if_last_link_func
{
   ...
   void operator()()
   {
      ...
      mp_node_pool->get_segment_manager()->template destroy<NodePool>(lslboost::interprocess::unique_instance);
   } 
...
class cache_impl
{
   ...
   multiallocation_chain cached_allocation(size_type n)
   {
      ...
      BOOST_TRY{
         ...
         return lslboost::move(chain);
      }
      BOOST_CATCH(...){
         this->cached_deallocation(lslboost::move(chain));
         BOOST_RETHROW
      }
      BOOST_CATCH_END
   }
   ...
   void deallocate_all_cached_nodes()
   {
      ...
      mp_node_pool->deallocate_nodes(lslboost::move(m_cached_nodes));
   }
   ...
   void priv_deallocate_n_nodes(size_type n)
   {
      ...
      mp_node_pool->deallocate_nodes(lslboost::move(chain));
   }
...
class array_allocation_impl
{
   ...
   typedef lslboost::container::container_detail::transform_multiallocation_chain
   ...
   std::pair<pointer, bool>
      allocation_command(lslboost::interprocess::allocation_type command,
   ...
   {  return this->derived()->get_segment_manager()->deallocate_many(lslboost::move(chain)); }
   ...
   {  return pointer(lslboost::addressof(value));  }

   //!Returns address of non mutable object.
   //!Never throws
   const_pointer address(const_reference value) const
   {  return const_pointer(lslboost::addressof(value));  }
   ...
   {  ::new((void*)ipcdetail::to_raw_pointer(ptr)) value_type(::lslboost::forward<P>(p));  }
      ...
      , SegmentManager>
{
   ...
   //!Throws lslboost::interprocess::bad_alloc if there is no enough memory
         ...
         <cached_allocator_impl<T, NodePool, Version>, T, typename NodePool::segment_manager>
{
   ...
   void deallocate_individual(multiallocation_chain chain)
   {
      typename node_pool_t::multiallocation_chain mem
         (chain.extract_multiallocation_chain());
      m_cache.cached_deallocation(lslboost::move(mem));
   }
   ...
   : public private_node_allocator_t
{
 ...
 public:
   ...
   //!Allocates array of count elements. Can throw lslboost::interprocess::bad_alloc
   void *allocate_node()
   {
      //-----------------------
...
*/
   ...
   void deallocate_nodes(multiallocation_chain chain)
   {
      ...
      lslboost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
      private_node_allocator_t::deallocate_nodes(lslboost::move(chain));
   }
