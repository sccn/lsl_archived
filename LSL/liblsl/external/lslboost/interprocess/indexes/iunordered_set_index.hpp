...
#include <lslboost/interprocess/containers/vector.hpp>
#include <lslboost/intrusive/unordered_set.hpp>
...
#include <lslboost/container/detail/minimal_char_traits_header.hpp>  //std::char_traits
#include <lslboost/container/detail/placement_new.hpp>

//!\file
//!Describes index adaptor of lslboost::intrusive::unordered_set container, to use it
   ...
   struct equal_function
   {
      ...
      : std::unary_function<value_type, std::size_t>
    {
        std::size_t operator()(const value_type &val) const
        {
            ...
            return lslboost::hash_range(beg, end);
        }
...
//!Index type based in lslboost::intrusive::set.
//!Just derives from lslboost::intrusive::set
   ...
   ,  public iunordered_set_index_aux<MapConfig>::index_t
{
   ...
   static bucket_ptr create_buckets(allocator_type &alloc, size_type num)
   {
      ...
      for(size_type i = 0; i < num; ++i){
         ::new(to_raw_pointer(buckets_init++), lslboost_container_new_t())bucket_type();
      }
      return buckets;
   }
      ...
      , allocator_type &alloc, size_type new_size)
   {
      ...
      if(!alloc.allocation_command
         (lslboost::interprocess::try_shrink_in_place | lslboost::interprocess::nothrow_allocation, old_size, received_size, buckets)){
         return old_size;
      }
      ...
      bucket_ptr shunk_p = alloc.allocation_command
         (lslboost::interprocess::shrink_in_place | lslboost::interprocess::nothrow_allocation, received_size, received_size, buckets);
      ...
      , allocator_type &alloc,  const size_type new_num)
   {
      ...
      bucket_ptr ret = alloc.allocation_command
            (lslboost::interprocess::expand_fwd | lslboost::interprocess::allocate_new, new_num, received_size, reuse);
      if(ret == old_buckets){
...
struct is_intrusive_index
   <lslboost::interprocess::iunordered_set_index<MapConfig> >
{
