...
#include <lslboost/archive/iterators/remove_whitespace.hpp>
#include <lslboost/archive/iterators/istream_iterator.hpp>
#include <lslboost/archive/iterators/binary_from_base64.hpp>
    ...
    locale_saver(* is_.rdbuf())
{
    if(! no_codecvt){
        archive_locale.reset(
            add_facet(
                std::locale::classic(),
                new lslboost::archive::codecvt_null<typename IStream::char_type>
            )
        );
