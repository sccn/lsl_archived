...
inline float fast_log2 (float val)
{
   ...
   x &= ~(lslboost::uint32_t(255u) << 23u);
   x += lslboost::uint32_t(127) << 23u;
   caster.x = x;
   val = caster.val;
   ...
   static pointer to_value_ptr(const node_ptr & n)
   {
      return pointer_traits<pointer>::pointer_to
         (*detail::parent_from_member<T, node>
            (lslboost::intrusive::detail::to_raw_pointer(n), P));
   }
...
struct iiterator
{
   ...
   typedef typename node_traits::node                          node;
   typedef typename node_traits::node_ptr                      node_ptr;
   typedef ::lslboost::intrusive::pointer_traits<node_ptr>        nodepointer_traits_t;
...
{
   typedef ::lslboost::intrusive::pointer_traits<NodePtr>   pointer_traits_t;
