...
//      great as lslboost::intmax_t can cause some differences to be
...
// 06 Feb 2001 - Factored if_true out into lslboost/detail/select_type.hpp
//               (David Abrahams)
...
# ifndef BOOST_NO_LIMITS_COMPILE_TIME_CONSTANTS
  ...
  // This should probably be rolled into lslboost::integer_traits one day, but I
      ...
      if_true<(int(x::is_signed)
              && (!int(x::is_bounded)
                 // digits is the number of no-sign bits
                  || (int(x::digits) + 1 >= digit_traits<lslboost::intmax_t>::digits)))>::template then<
        Integer,
...
#else
      BOOST_STATIC_ASSERT(lslboost::is_integral<Integer>::value);
