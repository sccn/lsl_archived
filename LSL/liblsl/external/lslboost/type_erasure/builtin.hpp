...
#include <lslboost/type_erasure/constructible.hpp>
...
struct copy_constructible :
    ::lslboost::mpl::vector<constructible<T(const T&)>, destructible<T> >
{};
...
{
    using Base::_lslboost_type_erasure_deduce_assign;
    assignable<T, U>* _lslboost_type_erasure_deduce_assign(
        typename ::lslboost::type_erasure::rebind_any<Base, const U&>::type)
    {
        return 0;
    }
};
...
template<class T>
struct get_null_vtable_entry< ::lslboost::type_erasure::typeid_<T> >
{
    typedef typeid_<void> type;
};

struct null_destroy {
    static void value(::lslboost::type_erasure::detail::storage&) {}
};

template<class T>
struct get_null_vtable_entry< ::lslboost::type_erasure::destructible<T> >
{
    typedef ::lslboost::type_erasure::detail::null_destroy type;
};
