...
{
    return *static_cast<typename ::lslboost::remove_reference<T>::type*>(arg.data);
}

template<class T>
T extract(const storage& arg)
{
    return *static_cast<const typename ::lslboost::remove_reference<T>::type*>(arg.data);
}
...
T extract(storage&& arg)
{
    return std::move(*static_cast<typename ::lslboost::remove_reference<T>::type*>(arg.data));
}
