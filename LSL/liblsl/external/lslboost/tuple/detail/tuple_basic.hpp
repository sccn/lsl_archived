...
#include "lslboost/detail/workaround.hpp" // needed for BOOST_WORKAROUND
...
template<int N, class T>
struct element:
  public detail::element_impl<N, T, ::lslboost::is_const<T>::value>
{
};
...
template <class T> struct access_traits {
  ...
  typedef const typename lslboost::remove_cv<T>::type& parameter_type;
...
template <class T> struct wrap_non_storeable_type {
  typedef typename IF<
    ::lslboost::is_function<T>::value, non_storeable_type<T>, T
  >::RET type;
};
...
struct cons {
  ...
  typename access_traits<
           ...
           >::non_const_type
  get() {
    return lslboost::tuples::get<N>(*this); // delegate to non-member get
  }
...
struct cons<HT, null_type> {
  ...
  get(BOOST_EXPLICIT_TEMPLATE_NON_TYPE(int, N)) {
    return lslboost::tuples::get<N>(*this);
  }
...
inline void swap(cons<HH, null_type>& lhs, cons<HH, null_type>& rhs) {
  ::lslboost::swap(lhs.head, rhs.head);
}
template<class HH, class TT>
inline void swap(cons<HH, TT>& lhs, cons<HH, TT>& rhs) {
  ::lslboost::swap(lhs.head, rhs.head);
  ::lslboost::tuples::swap(lhs.tail, rhs.tail);
}
template <class T0, class T1, class T2, class T3, class T4,
          class T5, class T6, class T7, class T8, class T9>
inline void swap(tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>& lhs,
  ...
  ::lslboost::tuples::swap(static_cast<base&>(lhs), static_cast<base&>(rhs));
}
