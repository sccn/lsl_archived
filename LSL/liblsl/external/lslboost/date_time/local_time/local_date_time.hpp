 ...
 * (See accompanying file LICENSE_1_0.txt or http://www.lslboost.org/LICENSE_1_0.txt)
...
#include <lslboost/date_time/time.hpp>
#include <lslboost/date_time/posix_time/posix_time.hpp> //todo remove?
#include <lslboost/date_time/dst_rules.hpp>
#include <lslboost/date_time/time_zone_base.hpp>
...
namespace local_time {
   ...
   * required to be in the form of a lslboost::shared_ptr<time_zone_base>.
   */
  ...
  class local_date_time_base :  public date_time::base_time<utc_time_,
                                                            lslboost::posix_time::posix_time_system> {
  public:
    ...
    local_date_time_base(utc_time_type t,
                         lslboost::shared_ptr<tz_type> tz) :
    ...
    local_date_time_base(date_type d,
      ...
      zone_(tz)
    {
      if(tz != lslboost::shared_ptr<tz_type>() && tz->has_dst()){
        ...
        if(result == invalid_time_label){
          ...
          ss << "time given: " << d << ' ' << td;
          lslboost::throw_exception(time_label_invalid(ss.str()));
        }
          ...
          lslboost::throw_exception(dst_not_valid(ss.str()));
        }
      ...
      zone_(tz)
    {
      ...
      if(result == ambiguous) {
        if(calc_option == EXCEPTION_ON_ERROR){
          std::ostringstream ss;
          ss << "time given: " << d << ' ' << td;
          lslboost::throw_exception(ambiguous_result(ss.str()));
        }
        else{ // NADT on error
    ...
    explicit local_date_time_base(const lslboost::date_time::special_values sv,
                                  lslboost::shared_ptr<tz_type> tz = lslboost::shared_ptr<tz_type>()) :
      date_time::base_time<utc_time_type, time_system_type>(utc_time_type(sv)),
      zone_(tz)
    {}

    //! returns time zone associated with calling instance
    lslboost::shared_ptr<tz_type> zone() const
    {
      return zone_;
    }
    ...
    bool is_dst() const
    {
      if(zone_ != lslboost::shared_ptr<tz_type>() && zone_->has_dst() && !this->is_special()) {
    ...
    {
      if(zone_ != lslboost::shared_ptr<tz_type>()){
    ...
    local_date_time_base local_time_in(lslboost::shared_ptr<tz_type> new_tz,
    ...
    /*bool operator==(const date_time::base_time<lslboost::posix_time::ptime,lslboost::posix_time::posix_time_system>& rhs) const
    { // fails due to rhs.time_ being protected
      return date_time::base_time<lslboost::posix_time::ptime,lslboost::posix_time::posix_time_system>::operator==(rhs);
  ...
  private:
    lslboost::shared_ptr<tz_type> zone_;
    //bool is_dst_;

    /*! Adjust the passed in time to UTC?
     */
    utc_time_type construction_adjustment(utc_time_type t,
                                          lslboost::shared_ptr<tz_type> z,
                                          bool dst_flag)
    {
      if(z != lslboost::shared_ptr<tz_type>()) {
