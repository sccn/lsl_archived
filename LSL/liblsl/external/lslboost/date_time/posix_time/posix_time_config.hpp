...
#include <lslboost/date_time/time_resolution_traits.hpp>
#include <lslboost/date_time/gregorian/gregorian_types.hpp>
#include <lslboost/date_time/wrapping_int.hpp>
...
//                                              10, 0 > time_res_traits;
...
#else
    BOOST_STATIC_CONSTANT(lslboost::int64_t, tick_per_second = 1000000000);
#endif
  };
  ...
  {
   public:
    typedef lslboost::int64_t time_rep_type;
...
#if (defined(BOOST_DATE_TIME_NO_MEMBER_INIT)) //help bad compilers
#else
    BOOST_STATIC_CONSTANT(lslboost::int64_t, tick_per_second = 1000000);
#endif
  };
