...
#include <lslboost/date_time/c_time.hpp>
#include <lslboost/date_time/time_resolution_traits.hpp> // absolute_value
#include <lslboost/date_time/gregorian/conversion.hpp>
...
namespace posix_time {
  ...
  inline
  std::tm to_tm(const lslboost::posix_time::ptime& t) {
    std::tm timetm = lslboost::gregorian::to_tm(t.date());
    lslboost::posix_time::time_duration td = t.time_of_day();
  ...
  inline
  std::tm to_tm(const lslboost::posix_time::time_duration& td) {
    std::tm timetm;
  ...
  ptime ptime_from_tm(const std::tm& timetm) {
    lslboost::gregorian::date d = lslboost::gregorian::date_from_tm(timetm);
   ...
   *       lslboost::date_time::winapi::file_time type.
   */
  template< typename TimeT, typename FileTimeT >
  inline
  TimeT from_ftime(const FileTimeT& ft)
  {
    return lslboost::date_time::time_from_ftime<TimeT>(ft);
  }

#endif // BOOST_HAS_FTIME

} } //namespace lslboost::posix_time
