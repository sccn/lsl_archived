...
#include "lslboost/type_traits/object_traits.hpp"
...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename lslboost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    lslboost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     lslboost::tuples::access_traits<
       typename lslboost::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<lslboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const lslboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile lslboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile lslboost::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<lslboost::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  BOOST_STATIC_CONSTANT(int, nof_elems = lslboost::tuples::length<args_t>::value);
