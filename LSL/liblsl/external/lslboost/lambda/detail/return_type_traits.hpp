...
using ::lslboost::type_traits::ice_and;
using ::lslboost::type_traits::ice_or;
using ::lslboost::type_traits::ice_not;
...
namespace detail {
  ...
  class protect_conversion {
      typedef typename lslboost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in 
  // the args tuple
    typedef typename detail::IF_type<
//      lslboost::is_reference<T>::value && !lslboost::is_const<non_ref_T>::value,
      ice_and<lslboost::is_reference<T>::value,
              ice_not<lslboost::is_const<non_ref_T>::value>::value>::value,
...
// experimental feature
  ...
  typedef typename lslboost::remove_reference<A>::type non_ref_A;
  typedef typename lslboost::remove_reference<B>::type non_ref_B;
