...
#include "lslboost/lambda/detail/is_instance_of.hpp"
...
struct return_type_1<other_action<addressof_action>, A> { 
  ...
  typedef typename detail::IF<
    lslboost::is_same<type1, detail::unspecified>::value, 
...
template <class A> struct contentsof_type {
  typedef typename lslboost::indirect_reference<A>::type type; 
};
...
struct pointer_arithmetic_traits<plus_action, A, B> { 
  ...
  typedef typename 
    array_to_pointer<typename lslboost::remove_reference<A>::type>::type AP;
  typedef typename 
    array_to_pointer<typename lslboost::remove_reference<B>::type>::type BP;

  static const bool is_pointer_A = lslboost::is_pointer<AP>::value;
  static const bool is_pointer_B = lslboost::is_pointer<BP>::value;  
    ...
    lslboost::is_same<
      typename lslboost::remove_const<
        typename lslboost::remove_pointer<
          typename lslboost::remove_const<AP>::type
        >::type
      >::type,
      typename lslboost::remove_const<
        typename lslboost::remove_pointer<
          typename lslboost::remove_const<BP>::type
        >::type
      >::type
    >::value;
...
struct return_type_2<bitwise_action<Act>, A, B>
{
  ...
  // the apply function
  BOOST_STATIC_ASSERT(!(lslboost::is_float<plain_A>::value && lslboost::is_float<plain_B>::value));
...
struct leftshift_type {
private:
  typedef typename lslboost::remove_reference<A>::type plainA;
public:
...
struct return_type_2<arithmetic_assignment_action<Act>, A, B> { 
  ...
  typedef typename 
    detail::IF<
      lslboost::is_same<type1, detail::unspecified>::value, 
      typename lslboost::add_reference<A>::type,
      type1
    >::RET type;
};
...
struct return_type_2<other_action<subscript_action>, A, B> {
  ...
  typedef typename lslboost::remove_reference<A>::type nonref_A;
  typedef typename lslboost::remove_reference<B>::type nonref_B;
