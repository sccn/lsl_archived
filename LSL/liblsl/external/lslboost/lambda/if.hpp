...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(lslboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(lslboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
      ...
      detail::select(lslboost::tuples::get<2>(args), CALL_ACTUAL_ARGS);
  }
};
...
struct return_type_2_ifthenelsereturn<1, false, false, false, A, B> {
  ...
  typedef const typename lslboost::remove_reference<A>::type plainA; 
  typedef const typename lslboost::remove_reference<B>::type plainB; 
  // TODO: Add support for volatile ?

  typedef typename
       return_type_2_ifthenelsereturn<
         2,
         lslboost::is_convertible<plainA,plainB>::value, 
         lslboost::is_convertible<plainB,plainA>::value,
         lslboost::is_same<plainA,plainB>::value,
         plainA, 
         plainB>::type type;
};
...
struct return_type_2<other_action<ifthenelsereturn_action>, A, B> { 
  ...
  typedef typename 
    lslboost::add_const<typename detail::same_or_not<A1, B1>::type>::type type;
};
