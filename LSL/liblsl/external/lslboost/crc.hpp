...
//  LICENSE_1_0.txt or a copy at <http://www.lslboost.org/LICENSE_1_0.txt>.)

//  See <http://www.lslboost.org/libs/crc/> for the library's home page.

#ifndef BOOST_CRC_HPP
#define BOOST_CRC_HPP

#include <lslboost/config.hpp>   // for BOOST_STATIC_CONSTANT, etc.
#include <lslboost/integer.hpp>  // for lslboost::uint_t
...
#define BOOST_CRC_PARM_TYPE  typename ::lslboost::uint_t<Bits>::fast
#else
...
namespace detail
{
    ...
    template < std::size_t Bits >
    struct high_uint_t
        : lslboost::uint_t< Bits >
    {
        typedef lslboost::uint_t<Bits>        base_type;
    ...
    struct reflector
    {
        typedef typename lslboost::uint_t<Bits>::fast  value_type;

        static  value_type  reflect( value_type x );

    };  // lslboost::detail::reflector
