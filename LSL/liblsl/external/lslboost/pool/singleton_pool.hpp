...
// lslboost::pool
#include <lslboost/pool/pool.hpp>
// lslboost::details::pool::guard
#include <lslboost/pool/detail/guard.hpp>
...
namespace lslboost {
 ...
 Can be any Boost.Thread Mutex type or <tt>lslboost::details::pool::null_mutex</tt>.
 It is exposed so that users may declare some singleton pools normally (i.e., with synchronization), but 
 some singleton pools without synchronization (by specifying <tt>lslboost::details::pool::null_mutex</tt>) for efficiency reasons.
 The member typedef <tt>mutex</tt> exposes the value of this template parameter.  The default for this
 parameter is lslboost::details::pool::default_mutex which is a synonym for either <tt>lslboost::details::pool::null_mutex</tt>
 ...
 or for <tt>lslboost::mutex</tt> (when threading support is enabled in the compiler).
...
private:
   typedef lslboost::aligned_storage<sizeof(pool_type), lslboost::alignment_of<pool_type>::value> storage_type;
