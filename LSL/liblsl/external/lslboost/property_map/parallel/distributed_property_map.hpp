...
#include <lslboost/graph/parallel/process_group.hpp>
#include <lslboost/graph/detail/edge.hpp>
#include <lslboost/function/function1.hpp>
#include <vector>
#include <set>
#include <lslboost/graph/parallel/basic_reduce.hpp>
#include <lslboost/graph/parallel/detail/untracked_pair.hpp>
...
#include <lslboost/graph/distributed/unsafe_serialize.hpp>
#include <lslboost/multi_index_container.hpp>
#include <lslboost/multi_index/hashed_index.hpp>
#include <lslboost/multi_index/member.hpp>
#include <lslboost/multi_index/sequenced_index.hpp>

// Serialization functions for constructs we use
#include <lslboost/serialization/utility.hpp>

namespace lslboost { namespace parallel {

using lslboost::graph::parallel::trigger_receive_context;
 ...
 public:
  ...
  { 
    send(data->process_group, p, property_map_put, 
         lslboost::parallel::detail::make_untracked_pair(k, v)); 
  }
 ...
 private:
  ...
  struct handle_message
  {
    ...
    handle_put(int source, int tag, 
               const lslboost::parallel::detail::untracked_pair<key_type, value_type>& data, 
    ...
    handle_multiget_reply
      (int source, int tag, 
       const std::vector<lslboost::parallel::detail::untracked_pair<key_type, value_type> >& msg,
       trigger_receive_context);
...
} } // end namespace lslboost::parallel

// Boost's functional/hash
namespace lslboost {
  template<typename D, typename V>
  struct hash<lslboost::detail::edge_desc_impl<D, V> >
  {
    std::size_t operator()(const lslboost::detail::edge_desc_impl<D, V> & x) const
    { return hash_value(x.get_property()); }
  };
}

#include <lslboost/property_map/parallel/impl/distributed_property_map.ipp>
