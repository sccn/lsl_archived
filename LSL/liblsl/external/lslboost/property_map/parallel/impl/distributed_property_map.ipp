...
#include <lslboost/property_map/parallel/simple_trigger.hpp>
...
void PBGL_DISTRIB_PMAP::prune_ghost_cells() const
{
  ...
  while (data->ghost_cells->size() > data->max_ghost_cells) {
    ...
    if (data->model & cm_flush) {
      ...
      lslboost::parallel::detail::untracked_pair<key_type, value_type> const& victim
...
handle_put(int /*source*/, int /*tag*/, 
           const lslboost::parallel::detail::untracked_pair<key_type, value_type>& req, trigger_receive_context)
{
  using lslboost::get;
                ...
                trigger_receive_context)
{
  ...
  typedef lslboost::parallel::detail::untracked_pair<key_type, value_type> key_value;
...
{
  using lslboost::parallel::simple_trigger;
...
PBGL_DISTRIB_PMAP::set_max_ghost_cells(std::size_t max_ghost_cells)
{
  if ((data->model & cm_backward) && max_ghost_cells > 0)
      lslboost::throw_exception(std::runtime_error("distributed_property_map::set_max_ghost_cells: "
