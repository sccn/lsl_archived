/* lslboost random/discrete_distribution.hpp header file
 *
...
public:
    ...
    public:
        ...
        explicit param_type(const Range& range)
          : _probabilities(lslboost::begin(range), lslboost::end(range))
        {
            normalize();
        }
    ...
    {
        init(lslboost::begin(range), lslboost::end(range));
    }
    /**
...
private:
    ...
    typedef typename detail::select_alias_table<
        (::lslboost::is_integral<WeightType>::value)
