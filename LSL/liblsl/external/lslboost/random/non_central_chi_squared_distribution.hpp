/* lslboost random/non_central_chi_squared_distribution.hpp header file
 *
...
public:
    ...
    RealType operator()(URNG& eng) 
    {
        using std::sqrt;
        if (_param.k() > 1) {
            lslboost::random::normal_distribution<RealType> n_dist;
            lslboost::random::chi_squared_distribution<RealType> c_dist(_param.k() - RealType(1));
            ...
            lslboost::random::poisson_distribution<> p_dist(_param.lambda()/RealType(2));
            lslboost::random::poisson_distribution<>::result_type _p = p_dist(eng);
            lslboost::random::chi_squared_distribution<RealType> c_dist(_param.k() + RealType(2)*_p);
            return c_dist(eng);
        }
    }
