/* lslboost random/piecewise_constant_distribution.hpp header file
 *
...
public:
    ...
    public:
        ...
        param_type(const IntervalRange& intervals_arg,
          ...
          : _intervals(lslboost::begin(intervals_arg), lslboost::end(intervals_arg)),
            _weights(lslboost::begin(weights_arg), lslboost::end(weights_arg))
        {
            if(_intervals.size() < 2) {
                _intervals.clear();
                _intervals.push_back(RealType(0));
