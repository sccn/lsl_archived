...
#include <lslboost/multi_index/detail/iter_adaptor.hpp>
#include <lslboost/multi_index/safe_mode_errors.hpp>
...
inline void detach_equivalent_iterators(Iterator& it)
{
  if(it.valid()){
    {
#if defined(BOOST_HAS_THREADS)
      lslboost::detail::lightweight_mutex::scoped_lock lock(it.cont->mutex);
#endif
...
#if defined(BOOST_HAS_THREADS)
  lslboost::detail::lightweight_mutex mutex;
#endif
};
...
void safe_iterator_base::attach(safe_container_base* cont_)
{
  ...
  if(cont){
#if defined(BOOST_HAS_THREADS)
    lslboost::detail::lightweight_mutex::scoped_lock lock(cont->mutex);
#endif
...
private:
  friend class lslboost::multi_index::detail::iter_adaptor_access;
...
template<typename Iterator,typename Container>
struct version<
  lslboost::multi_index::safe_mode::safe_iterator<Iterator,Container>
>
{
  BOOST_STATIC_CONSTANT(
    int,value=lslboost::serialization::version<Iterator>::value);
};
} /* namespace serialization */
#endif
