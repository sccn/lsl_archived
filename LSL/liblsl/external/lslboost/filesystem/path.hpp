...
#include <lslboost/filesystem/path_traits.hpp>  // includes <cwchar>
...
#include <lslboost/io/detail/quoted_manip.hpp>
...
# endif
    ...
    path(Source const& source,
      typename lslboost::enable_if<path_traits::is_pathable<
        typename lslboost::decay<Source>::type> >::type* =0)
    {
    ...
    template <class Source>
      typename lslboost::enable_if<path_traits::is_pathable<
        typename lslboost::decay<Source>::type>, path&>::type
    ...
    template <class CharT>
      typename lslboost::enable_if<is_integral<CharT>, path&>::type
    operator+=(CharT c)
    {
      CharT tmp[2];
      tmp[0] = c;
...
# endif
  ...
  private:
    ...
    friend class lslboost::filesystem::path;
...
# endif
  ...
  //    use lslboost::io::quoted() to handle spaces in paths
  ...
  {
    return os
      << lslboost::io::quoted(p.template string<std::basic_string<Char> >(), static_cast<Char>('&'));
  }
  ...
  operator>>(std::basic_istream<Char, Traits>& is, path& p)
  {
    ...
    is >> lslboost::io::quoted(str, static_cast<Char>('&'));
    p = str;
    return is;
  }
