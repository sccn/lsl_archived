...
namespace detail{
//
// Workaround for missing abs(lslboost::long_long_type) and abs(__int128) on some compilers:
//
template <class T>
...
#define BOOST_MP_USING_ABS using lslboost::multiprecision::detail::abs;
...
template <class S>
void format_float_string(S& str, lslboost::intmax_t my_exp, lslboost::intmax_t digits, std::ios_base::fmtflags f, bool iszero)
{
   ...
   else if(!fixed || (my_exp >= 0))
   {
      ...
      //
      lslboost::intmax_t chars = str.size();
      chars = digits - chars;
      if(scientific)
         ++chars;
      if(chars > 0)
   ...
   {
      if(1 + my_exp > static_cast<lslboost::intmax_t>(str.size()))
      {
      ...
      else if(my_exp + 1 < static_cast<lslboost::intmax_t>(str.size()))
      {
         if(my_exp < 0)
         {
      ...
      if(fixed)
      {
         ...
         lslboost::intmax_t l = str.find('.') + 1;
         l = digits - (str.size() - l);
         if(l > 0)
   ...
   else
   {
      ...
      S e = lslboost::lexical_cast<S>(abs(my_exp));
...
template <class tag, class A1, class A2, class A3, class A4>
struct promote_arg<lslboost::multiprecision::detail::expression<tag, A1, A2, A3, A4> >
{
   typedef typename lslboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type type;
};

template <class R, class B, lslboost::multiprecision::expression_template_option ET>
inline R real_cast(const lslboost::multiprecision::number<B, ET>& val)
{
   return val.template convert_to<R>();
}

template <class R, class tag, class A1, class A2, class A3, class A4>
inline R real_cast(const lslboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>& val)
{
   typedef typename lslboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type val_type;
...
namespace constants{
   ...
   template <class B, lslboost::multiprecision::expression_template_option ET>
   struct is_explicitly_convertible_from_string<lslboost::multiprecision::number<B, ET> >
   {
