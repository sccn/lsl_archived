...
#include <lslboost/multiprecision/detail/bitscan.hpp> // lsb etc
#include <lslboost/integer/common_factor_rt.hpp> // gcd/lcm
...
{
   typedef typename lslboost::multiprecision::detail::canonical<R, CppInt>::type cast_type;
   if(val.sign())
   {
   ...
   eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& backend) BOOST_MP_NOEXCEPT_IF((is_non_throwing_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value))
{
   ...
   *result = static_cast<R>(backend.limbs()[0]);
   ...
   if(backend.sign())
   {
      check_is_negative(lslboost::is_signed<R>());
      *result = negate_integer(*result, lslboost::is_signed<R>());
   }
}
   ...
   eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   unsigned result = lslboost::multiprecision::detail::find_lsb(a.limbs()[index]);
   ...
   eval_msb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   return (a.size() - 1) * cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::limb_bits + lslboost::multiprecision::detail::find_msb(a.limbs()[a.size() - 1]);
}
...
inline limb_type integer_gcd_reduce(limb_type u, limb_type v)
{
   do
   {
      ...
      if(u == v)
         break;
      v -= u;
      v >>= lslboost::multiprecision::detail::find_lsb(v);
   } while(true);
   return u;
}
      ...
      limb_type v)
{
   ...
   unsigned vs = lslboost::multiprecision::detail::find_lsb(v);
   shift = (std::min)(us, vs);
...
{
   *result.limbs() = lslboost::integer::gcd(*a.limbs(), *b.limbs());
}
...
{
   *result.limbs() = lslboost::integer::lcm(*a.limbs(), *b.limbs());
   ...
   eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& val)
{
   ...
   if(val.isneg())
   {
      check_is_negative(mpl::bool_<lslboost::is_signed<R>::value || lslboost::is_floating_point<R>::value>());
      *result = negate_integer(*result, mpl::bool_<lslboost::is_signed<R>::value || lslboost::is_floating_point<R>::value>());
   }
}
   ...
   eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return lslboost::multiprecision::detail::find_lsb(*a.limbs());
}
   ...
   eval_msb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return lslboost::multiprecision::detail::find_msb(*a.limbs());
}
