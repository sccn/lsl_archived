...
namespace lslboost{ namespace multiprecision{ namespace backends{ namespace detail{

//
// Simple routines for performing checked arithmetic with a builtin arithmetic type.
// Note that this is not a complete header, it must be included as part of lslboost/multiprecision/cpp_int.hpp.
//
...
{
   return checked_add_imp(a, b, lslboost::is_signed<A>());
}
template <class A>
...
{
   return checked_subtract_imp(a, b, lslboost::is_signed<A>());
}
template <class A>
...
template <class A>
inline A checked_left_shift(A a, lslboost::ulong_long_type shift, const mpl::int_<checked>&)
{
   if(a && shift)
   {
...
template <class A>
inline A checked_left_shift(A a, lslboost::ulong_long_type shift, const mpl::int_<unchecked>&)
{
