...
//    the way for the lslboost::optional and future std::tr2::optional deployment;
...
#include <lslboost/convert/detail/is_fun.hpp>
...
namespace lslboost
{
    ...
    /// @details The lslboost::throw_on_failure is the name of an object of the
    /// lslboost::detail::throw_on_failure type that is used to indicate
    ...
    /// @return lslboost::optional<TypeOut> result of conversion together with the indication of
    ///         success or failure of the conversion request.
    /// @details For example,
    /// @code
    ///    lslboost::cnv::cstream cnv;
    ///
    ///    lslboost::optional<int>    i = lslboost::convert<int>("12", cnv);
    ///    lslboost::optional<string> s = lslboost::convert<string>(123.456, cnv);
    /// @endcode

    template<typename TypeOut, typename TypeIn, typename Converter>
    lslboost::optional<TypeOut>
    convert(TypeIn const& value_in, Converter const& converter)
    {
        optional<TypeOut> result;
        lslboost::unwrap_ref(converter)(value_in, result);
        return result;
    }

    namespace cnv { namespace detail
    {
        template<typename TypeOut, typename TypeIn, typename Converter =lslboost::cnv::by_default>
        struct delayed_resolution
        {
            static optional<TypeOut> convert(TypeIn const& value_in)
            {
                return lslboost::convert<TypeOut>(value_in, Converter());
            }
        };
    }}
    ...
    /// @details For example,
    /// @code
    ///    struct lslboost::cnv::by_default : public lslboost::cnv::cstream {};
    ///
    ///    // lslboost::cnv::cstream (through lslboost::cnv::by_default) is deployed
    ///    // as the default converter when no converter is provided explicitly.
    ///    lslboost::optional<int>    i = lslboost::convert<int>("12");
    ///    lslboost::optional<string> s = lslboost::convert<string>(123.456);
    /// @endcode
...
namespace lslboost
{
    ...
    template<typename TypeOut, typename TypeIn, typename Converter>
    TypeOut
    convert(TypeIn const& value_in, Converter const& converter, lslboost::detail::throw_on_failure)
    {
...
namespace lslboost { namespace cnv
{
    ...
    /// @details For example,
    /// @code
    ///    lslboost::array<char const*, 3> strs = {{ " 5", "0XF", "not an int" }};
    ///    std::vector<int>             ints;
    ///    lslboost::cnv::cstream           cnv;
    ///
    ...
    ///        lslboost::cnv::apply<int>(lslboost::cref(cnv)).value_or(-1));
    /// @endcode
