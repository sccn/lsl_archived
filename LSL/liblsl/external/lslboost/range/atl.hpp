...
#include <atldef.h> // _ATL_VER
    ...
    #if (_MSC_VER < 1310)   // from <lslboost/regex/mfc.hpp>, but dubious
...
#include <lslboost/range/detail/microsoft.hpp>
...
namespace lslboost { namespace range_detail_microsoft {
...
#endif
    {
        ...
        Iterator begin(X& x)
        {
            return Iterator(lslboost::make_tuple(x.m_aKey, x.m_aVal));
        }

        template< class Iterator, class X >
        Iterator end(X& x)
        {
            return Iterator(lslboost::make_tuple(x.m_aKey + x.GetSize(), x.m_aVal + x.GetSize()));
        }
    };
...
//
    ...
    BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE(
        lslboost::range_detail_microsoft::using_type_as_tag,
    ...
    BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE(
        lslboost::range_detail_microsoft::atl_rb_tree_tag,
    ...
    #if !defined(BOOST_RANGE_ATL_HAS_OLD_CSIMPLESTRING)
        BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE(
            lslboost::range_detail_microsoft::atl_string_tag,
