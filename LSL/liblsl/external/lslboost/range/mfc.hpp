...
#include <lslboost/range/atl.hpp>
...
namespace lslboost { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit mfc_ptr_array_iterator(ArrayT& arr, INT_PTR index) :
            super_t(index), m_parr(lslboost::addressof(arr))
        { }
    ...
    public:
        ...
        explicit mfc_map_iterator(MapT const& map, POSITION pos) :
            m_pmap(lslboost::addressof(map)), m_posNext(pos)
        {
            increment();
        }
    ...
    public:
        ...
        explicit mfc_cpair_map_iterator(MapT& map, PairT *pp) :
            m_pmap(lslboost::addressof(map)), m_pp(pp)
        { }
    ...
    struct customization< ::CTypedPtrArray<BaseClass, PtrType> >
    {
        ...
        Iterator begin(X& x)
        {
            return Iterator(lslboost::begin<BaseClass>(x), fun<X>());
        }

        template< class Iterator, class X >
        Iterator end(X& x)
        {
            return Iterator(lslboost::end<BaseClass>(x), fun<X>());
        }
    };
