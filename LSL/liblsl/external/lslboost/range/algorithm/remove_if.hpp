...
inline BOOST_DEDUCED_TYPENAME lslboost::range_iterator<ForwardRange>::type
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return std::remove_if(lslboost::begin(rng), lslboost::end(rng), pred);
}

/// \overload
template< class ForwardRange, class UnaryPredicate >
inline BOOST_DEDUCED_TYPENAME lslboost::range_iterator<const ForwardRange>::type
...
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    ...
    return range_return<ForwardRange,re>::pack(
        std::remove_if(lslboost::begin(rng), lslboost::end(rng), pred),
        rng);
}
