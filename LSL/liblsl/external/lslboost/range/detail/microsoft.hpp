...
#include <lslboost/iterator/iterator_traits.hpp> // iterator_difference
...
#include <lslboost/preprocessor/list/for_each.hpp>
...
#include <lslboost/utility/enable_if.hpp> // disable_if

#if !defined(BOOST_RANGE_DETAIL_MICROSOFT_RANGE_VERSION_1)
    #include <lslboost/range/mutable_iterator.hpp>
#else
...
namespace lslboost { namespace range_detail_microsoft {
    ...
    size_of(T const& x)
    {
        return std::distance(lslboost::begin(x), lslboost::end(x));
    }
...
} } // namespace lslboost::range_detail_microsoft
...
/**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_begin(Fullname) \
        inline \
        lslboost::range_detail_microsoft::mutable_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname& x) \
        { \
            return lslboost::range_detail_microsoft::begin_of(x); \
        } \
    /**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_begin_const(Fullname) \
        inline \
        lslboost::range_detail_microsoft::const_iterator_of< Fullname >::type \
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_end(Fullname) \
        ...
        { \
            return lslboost::range_detail_microsoft::end_of(x); \
        } \
    /**/
    ...
    /**/
        ...
        #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_size(Fullname) \
            inline \
            lslboost::range_detail_microsoft::size_type_of< Fullname >::type \
            lslboost_range_size(Fullname const& x) \
            { \
                return lslboost::range_detail_microsoft::size_of(x); \
            } \
        /**/
...
/**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_begin(Params, Fullname) \
        ...
        typename lslboost::range_detail_microsoft::mutable_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname& x) \
        { \
            return lslboost::range_detail_microsoft::begin_of(x); \
        } \
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_begin_const(Params, Fullname) \
        template< Params > inline \
        typename lslboost::range_detail_microsoft::const_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname const& x) \
        { \
            return lslboost::range_detail_microsoft::begin_of(x); \
        } \
    ...
    /**/
        ...
        #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_size(Params, Fullname) \
            template< Params > inline \
            typename lslboost::range_detail_microsoft::size_type_of< Fullname >::type \
            lslboost_range_size(Fullname const& x) \
            { \
                return lslboost::range_detail_microsoft::size_of(x); \
            } \
...
namespace lslboost { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit list_iterator(ListT& lst, POSITION pos) :
            m_plst(lslboost::addressof(lst)), m_pos(pos)
        { }
...
#include <lslboost/range/empty.hpp>
...
namespace lslboost { namespace range_detail_microsoft {
    ...
    {
        return
            lslboost::distance(rng1) == lslboost::distance(rng2) &&
            std::equal(lslboost::begin(rng1), lslboost::end(rng1), lslboost::begin(rng2))
        ;
    }
    ...
    {
        typedef typename lslboost::range_const_iterator<AssocContainer>::type iter_t;
        for (iter_t it = lslboost::const_begin(ac), last = lslboost::const_end(ac); it != last; ++it) {
    ...
    bool test_emptiness(Range& )
    {
        ...
        Range emptyRng;
        result = result && lslboost::empty(emptyRng);
    ...
    bool test_trivial(Range& rng)
    {
        ...
        citer_t cit = lslboost::begin(rng);
        (void)cit; // unused

        // mutability check
        typedef typename range_value<Range>::type val_t;
        val_t v = *lslboost::begin(rng);
        *lslboost::begin(rng) = v;
        result = result && *lslboost::begin(rng) == v;
    ...
    {
        lslboost::function_requires< ForwardRangeConcept<Range> >();
        ...
        std::copy(lslboost::begin(rng), lslboost::end(rng), std::back_inserter(saved));
        std::rotate(lslboost::begin(saved), lslboost::next(lslboost::begin(saved)), lslboost::end(saved));

        std::rotate(lslboost::begin(rng), lslboost::next(lslboost::begin(rng)), lslboost::end(rng));
    ...
    {
        lslboost::function_requires< BidirectionalRangeConcept<Range> >();
        ...
        result = result && (test_equals)(
            lslboost::make_iterator_range(lslboost::rbegin(saved), lslboost::rend(saved)),
            lslboost::make_iterator_range(lslboost::rbegin(rng), lslboost::rend(rng))
        );
    ...
    {
        lslboost::function_requires< RandomAccessRangeConcept<Range> >();
        ...
        std::vector<val_t> saved;
        std::copy(lslboost::begin(rng), lslboost::end(rng), std::back_inserter(saved));
        std::sort(lslboost::begin(saved), lslboost::end(saved));
        ...
        std::random_shuffle(lslboost::begin(rng), lslboost::end(rng));
        std::partial_sort(lslboost::begin(rng), lslboost::end(rng), lslboost::end(rng));
        result = result && (test_equals)(rng, saved);
    ...
    bool test_init_array(ArrayT& arr, SampleRange const& sample)
    {
        ...
        for (iter_t it = lslboost::const_begin(sample), last = lslboost::const_end(sample); it != last; ++it) {
    ...
    bool test_init_map(MapT& map, SampleMap const& sample)
    {
        ...
        return lslboost::distance(map) == lslboost::distance(sample);
    }
    ...
    struct test_mutable_iter :
        lslboost::is_same< typename lslboost::BOOST_RANGE_DETAIL_MICROSOFT_range_mutable_iterator<Range>::type, Iter >
    { };


    template< class Range, class Iter >
    struct test_const_iter :
        lslboost::is_same< typename lslboost::range_const_iterator<Range>::type, Iter >
    { };
