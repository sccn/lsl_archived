...
#include <lslboost/detail/is_sorted.hpp>
#include <algorithm>
...
inline bool is_sorted(const SinglePassRange& rng)
{
    ...
    return ::lslboost::detail::is_sorted(lslboost::begin(rng), lslboost::end(rng));
}
...
inline bool is_sorted(const SinglePassRange& rng, BinaryPredicate pred)
{
    ...
    BOOST_RANGE_CONCEPT_ASSERT((BinaryPredicateConcept<BinaryPredicate,
      ...
      BOOST_DEDUCED_TYPENAME range_value<const SinglePassRange>::type>));
    return ::lslboost::detail::is_sorted(lslboost::begin(rng), lslboost::end(rng), pred);
}
