...
#include <lslboost/typeof/int_encoding.hpp>

#include <lslboost/typeof/type_template_param.hpp>
#include <lslboost/typeof/integral_template_param.hpp>
#include <lslboost/typeof/template_template_param.hpp>
...
#define BOOST_TYPEOF_REGISTER_TEMPLATE_IMPL(Name, Params, Size, ID)\
    ...
    {\
        typedef typename lslboost::type_of::push_back<V, lslboost::mpl::size_t<ID> >::type V0;\
        BOOST_PP_SEQ_FOR_EACH_I(BOOST_TYPEOF_REGISTER_TEMPLATE_ENCODE_PARAM, ~, Params)\
        typedef BOOST_PP_CAT(V, Size) type;\
    };\
    template<class Iter>\
    struct decode_type_impl<lslboost::mpl::size_t<ID>, Iter>\
    {\
        typedef decode_type_impl<lslboost::mpl::size_t<ID>, Iter> self_t;\
        typedef lslboost::mpl::size_t<ID> self_id;\
        typedef Iter iter0;\
