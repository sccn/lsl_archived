...
/// \brief Contains lslboost::typeindex::stl_type_index class.
///
/// lslboost::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined lslboost::typeindex::ctti is usually used instead of lslboost::typeindex::stl_type_index.

#include <lslboost/type_index/type_index_facade.hpp>

// MSVC is capable of calling typeid(T) even when RTTI is off
#if defined(BOOST_NO_RTTI) && !defined(BOOST_MSVC)
#error "File lslboost/type_index/stl_type_index.ipp is not usable when typeid() is not available."
#endif
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "lslboost::typeindex::detail::cvr_saver<";
    ...
    const lslboost::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        lslboost::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return lslboost::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME lslboost::remove_reference<T>::type no_ref_t;
    typedef BOOST_DEDUCED_TYPENAME lslboost::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME lslboost::mpl::if_<
            lslboost::is_signed<no_cvr_prefinal_t>,
            lslboost::make_signed<no_cvr_prefinal_t>,
            lslboost::mpl::identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME lslboost::mpl::if_<
        lslboost::mpl::or_<lslboost::is_reference<T>, lslboost::is_const<T>, lslboost::is_volatile<T> >,
        detail::cvr_saver<T>,
        T
    >::type type;
...
#ifdef BOOST_NO_RTTI
    return value.lslboost_type_index_type_id_runtime_();
#else
    return typeid(value);
#endif
}
