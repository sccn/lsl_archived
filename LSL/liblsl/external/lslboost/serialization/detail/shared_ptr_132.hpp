...
#include <lslboost/serialization/detail/shared_ptr_nmt_132.hpp>
#else
...
#include <lslboost/serialization/detail/shared_count_132.hpp>
...
public:
    ...
    template<class Y>
    explicit shared_ptr(Y * p): px(p), pn(p, lslboost::checked_deleter<Y>()) // Y must be complete
#else
    template<class Y>
    explicit shared_ptr(Y * p): px(p), pn(p, lslboost::checked_deleter<Y>()) // Y must be complete
#endif
...
//  except that Borland C++ has a bug, and g++ with -Wsynth warns
    ...
    shared_ptr(shared_ptr<Y> const & r, detail::polymorphic_cast_tag): px(dynamic_cast<element_type *>(r.px)), pn(r.pn)
    {
        if(px == 0)
        {
            lslboost::serialization::throw_exception(std::bad_cast());
        }
    }
