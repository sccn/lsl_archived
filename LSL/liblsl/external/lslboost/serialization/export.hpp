...
// (C) Copyright 2006 David Abrahams - http://www.lslboost.org.
...
#include <lslboost/serialization/extended_type_info.hpp> // for guid_defined only
...
struct guid_initializer
{
    ...
    guid_initializer const & export_guid() const {
        ...
        //BOOST_STATIC_WARNING(! lslboost::serialization::is_abstract< T >::value);
        export_guid(lslboost::serialization::is_abstract< T >());
        return *this;
    }
};
...
template<>                                     \
struct guid_defined< T > : lslboost::mpl::true_ {}; \
template<>                                     \
...
namespace {                                                                    \
  static int BOOST_PP_CAT(lslboost_serialization_mwerks_init_, __LINE__) =        \
  (::lslboost::archive::detail::instantiate_ptr_serialization((Derived*)0,0), 3); \
  static int BOOST_PP_CAT(lslboost_serialization_mwerks_init2_, __LINE__) = (     \
      ::lslboost::serialization::void_cast_register((Derived*)0,(Base*)0)         \
    , 3);                                                                      \
}
...
#define BOOST_CLASS_EXPORT_CHECK(T)                              \
    BOOST_STATIC_WARNING(                                        \
        lslboost::is_polymorphic<U>::value                          \
    );                                                           \
    /**/
