...
// This might happen when previously including some lslboost headers that overload the coma operator.
#if defined(_MSC_VER)
#  pragma warning(push)
...
struct assign_range {
    ...
    template <class Pointer>
    void operator () (Pointer p) const {
        lslboost::cb_details::uninitialized_copy(m_first, m_last, p, m_alloc);
    }
};
...
inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    ...
    BOOST_TRY {
        for (; first != last; ++first, ++dest)
            lslboost::container::allocator_traits<Alloc>::construct(a, lslboost::addressof(*dest), *first);
    } BOOST_CATCH(...) {
        for (; next != dest; ++next)
            lslboost::container::allocator_traits<Alloc>::destroy(a, lslboost::addressof(*next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
    return dest;
...
ForwardIterator uninitialized_move_if_noexcept_impl(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a,
    true_type) {
    for (; first != last; ++first, ++dest)
        lslboost::container::allocator_traits<Alloc>::construct(a, lslboost::addressof(*dest), lslboost::move(*first));
    return dest;
}
...
template<class InputIterator, class ForwardIterator, class Alloc>
ForwardIterator uninitialized_move_if_noexcept(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    typedef typename lslboost::is_nothrow_move_constructible<typename lslboost::container::allocator_traits<Alloc>::value_type>::type tag_t;
...
inline void uninitialized_fill_n_with_alloc(ForwardIterator first, Diff n, const T& item, Alloc& alloc) {
    ...
    BOOST_TRY {
        for (; n > 0; ++first, --n)
            lslboost::container::allocator_traits<Alloc>::construct(alloc, lslboost::addressof(*first), item);
    } BOOST_CATCH(...) {
        for (; next != first; ++next)
            lslboost::container::allocator_traits<Alloc>::destroy(alloc, lslboost::addressof(*next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
}
