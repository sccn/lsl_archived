...
/*!
    ...
    http://www.lslboost.org/libs/circular_buffer/doc/circular_buffer.html
*/
...
/*! \endcond */
{
  ...
  // Requirements
    //BOOST_CLASS_REQUIRE(T, lslboost, SGIAssignableConcept);
...
// Basic types
    ...
    typedef typename lslboost::container::allocator_traits<Alloc>::value_type value_type;

    //! A pointer to an element.
    typedef typename lslboost::container::allocator_traits<Alloc>::pointer pointer;

    //! A const pointer to the element.
    typedef typename lslboost::container::allocator_traits<Alloc>::const_pointer const_pointer;

    //! A reference to an element.
    typedef typename lslboost::container::allocator_traits<Alloc>::reference reference;

    //! A const reference to an element.
    typedef typename lslboost::container::allocator_traits<Alloc>::const_reference const_reference;

    //! The distance type.
    /*!
        (A signed integral type used to represent the distance between two iterators.)
    */
    typedef typename lslboost::container::allocator_traits<Alloc>::difference_type difference_type;

    //! The size type.
    /*!
        (An unsigned integral type that can represent any non-negative value of the container's distance type.)
    */
    typedef typename lslboost::container::allocator_traits<Alloc>::size_type size_type;
...
// Iterators
    ...
    typedef cb_details::iterator< circular_buffer<T, Alloc>, cb_details::const_traits<lslboost::container::allocator_traits<Alloc> > > const_iterator;

    //! A (random access) iterator used to iterate through the <code>circular_buffer</code>.
    typedef cb_details::iterator< circular_buffer<T, Alloc>, cb_details::nonconst_traits<lslboost::container::allocator_traits<Alloc> > > iterator;

    //! A const iterator used to iterate backwards through a <code>circular_buffer</code>.
    typedef lslboost::reverse_iterator<const_iterator> const_reverse_iterator;

    //! An iterator used to iterate backwards through a <code>circular_buffer</code>.
    typedef lslboost::reverse_iterator<iterator> reverse_iterator;
            ...
            for (pointer first = m_first; dest < src; src = first) {
                for (size_type ii = 0; src < m_end; ++src, ++dest, ++moved, ++ii) {
                    ...
                    if (is_uninitialized(dest)) {
                        lslboost::container::allocator_traits<Alloc>::construct(m_alloc, lslboost::addressof(*dest), lslboost::move_if_noexcept(*src));
                        ++constructed;
                    } else {
                        value_type tmp = lslboost::move_if_noexcept(*src); 
                        replace(src, lslboost::move_if_noexcept(*dest));
                        replace(dest, lslboost::move(tmp));
                    }
                }
            }
        } BOOST_CATCH(...) {
    ...
    */
    void rotate(const_iterator new_begin) {
        ...
        if (full()) {
            ...
            if (m < n) {
                for (; m > 0; --m) {
                    push_front(lslboost::move_if_noexcept(back()));
                    pop_back();
                }
            } else {
                for (; n > 0; --n) {
                    push_back(lslboost::move_if_noexcept(front()));
                    pop_front();
                }
            }
        }
...
// Size and capacity
    ...
    */
    size_type max_size() const BOOST_NOEXCEPT {
        return (std::min<size_type>)(lslboost::container::allocator_traits<Alloc>::max_size(m_alloc), (std::numeric_limits<difference_type>::max)());
    }
...
private:
    ...
    void push_back_impl(ValT item) {
        if (full()) {
            ...
            lslboost::container::allocator_traits<Alloc>::construct(m_alloc, lslboost::addressof(*m_last), static_cast<ValT>(item));
            increment(m_last);
            ++m_size;
        }        
    }
    ...
    void push_front_impl(ValT item) {
        BOOST_TRY {
            if (full()) {
                ...
                decrement(m_first);
                lslboost::container::allocator_traits<Alloc>::construct(m_alloc, lslboost::addressof(*m_first), static_cast<ValT>(item));
                ++m_size;
            }
        } BOOST_CATCH(...) {
            increment(m_first);
...
public:
    ...
    */
    void push_back(rvalue_type item) {
        push_back_impl<rvalue_type>(lslboost::move(item));
    }
    ...
    */
    void push_front(rvalue_type item) {
        push_front_impl<rvalue_type>(lslboost::move(item));
    }
...
// Insert
    ...
    */
    iterator insert(iterator pos, rvalue_type item) {
        return insert_impl<rvalue_type>(pos, lslboost::move(item));
    }
    ...
    */
    iterator insert(iterator pos) {
        value_type temp;
        return insert(pos, lslboost::move(temp));
    }
...
private:
    ...
    iterator rinsert_impl(iterator pos, ValT item) {
        ...
        if (pos == begin()) {
            ...
            BOOST_TRY {
                while (src != pos.m_it) {
                    construct_or_replace(construct, dest, lslboost::move_if_noexcept(*src));
                    increment(src);
                    increment(dest);
                    construct = false;
                }
...
public:
    ...
    */
    iterator rinsert(iterator pos, rvalue_type item) {
        return rinsert_impl<rvalue_type>(pos, lslboost::move(item));
    }
    ...
    */
    iterator rinsert(iterator pos) {
        value_type temp;
        return rinsert(pos, lslboost::move(temp));
    }
...
// Erase
    ...
    */
    iterator erase(iterator pos) {
        ...
        for (pointer p = pos.m_it; next != m_last; p = next, increment(next))
            replace(p, lslboost::move_if_noexcept(*next));
        decrement(m_last);
        destroy_item(m_last);
        --m_size;
#if BOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator erase(iterator first, iterator last) {
        ...
        while (last.m_it != 0)
            replace((first++).m_it, lslboost::move_if_noexcept(*last++));
        do {
            decrement(m_last);
            destroy_item(m_last);
            --m_size;
    ...
    */
    iterator rerase(iterator pos) {
        ...
        for (decrement(prev); p != m_first; p = prev, decrement(prev))
            replace(p, lslboost::move_if_noexcept(*prev));
        destroy_item(m_first);
        increment(m_first);
        --m_size;
#if BOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator rerase(iterator first, iterator last) {
        ...
        while (first.m_it != m_first) {
            ...
            decrement(p);
            replace(p, lslboost::move_if_noexcept(*first.m_it));
        }
        do {
            destroy_item(m_first);
            increment(m_first);
...
// Helper methods
    ...
    */
    void construct_or_replace(bool construct, pointer pos, param_value_type item) {
        if (construct)
            lslboost::container::allocator_traits<Alloc>::construct(m_alloc, lslboost::addressof(*pos), item);
        else
            replace(pos, item);
    }
    ...
    */
    void construct_or_replace(bool construct, pointer pos, rvalue_type item) {
        if (construct)
            lslboost::container::allocator_traits<Alloc>::construct(m_alloc, lslboost::addressof(*pos), lslboost::move(item));
        else
            replace(pos, lslboost::move(item));
    }

    //! Destroy an item.
    void destroy_item(pointer p) {
        lslboost::container::allocator_traits<Alloc>::destroy(m_alloc, lslboost::addressof(*p));
#if BOOST_CB_ENABLE_DEBUG
    ...
    void initialize(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        initialize(distance, lslboost::make_move_iterator(tmp.begin()), lslboost::make_move_iterator(tmp.end()), distance);
    }
    ...
    void initialize(capacity_type buffer_capacity,
        ...
        while (first != last && !full()) {
            lslboost::container::allocator_traits<Alloc>::construct(m_alloc, lslboost::addressof(*m_last), *first++);
            increment(m_last);
            ++m_size;
        }
        while (first != last) {
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::make_assign_range
                (lslboost::make_move_iterator(tmp.begin()), lslboost::make_move_iterator(tmp.end()), m_alloc));
    }
        ...
        if (pos.m_it == 0) {
            ...
            BOOST_TRY {
                for (; ii < construct; ++ii, increment(p))
                    lslboost::container::allocator_traits<Alloc>::construct(m_alloc, lslboost::addressof(*p), *wrapper());
