...
private:
    ...
    /*! \cond */
    template <class ValT> 
    static inline typename lslboost::conditional<
        ((lslboost::is_nothrow_move_constructible<ValT>::value && lslboost::is_nothrow_move_assignable<ValT>::value) || !lslboost::is_copy_constructible<ValT>::value)
...
// Helper methods
    ...
    */
    void construct_or_replace(bool construct, pointer pos, rvalue_type item) {
        if (construct)
            ::new (pos) value_type(lslboost::move(item));
        else
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::make_assign_range<value_type>
                (lslboost::make_move_iterator(tmp.begin()), lslboost::make_move_iterator(tmp.end())));
    }
