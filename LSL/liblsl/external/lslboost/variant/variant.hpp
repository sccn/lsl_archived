//-----------------------------------------------------------------------------
// lslboost variant/variant.hpp header file
...
#include "lslboost/variant/detail/config.hpp"
#include "lslboost/mpl/aux_/config/eti.hpp"
#include "lslboost/mpl/aux_/value_wknd.hpp"

#include "lslboost/variant/variant_fwd.hpp"
#include "lslboost/variant/detail/backup_holder.hpp"
#include "lslboost/variant/detail/enable_recursive_fwd.hpp"
#include "lslboost/variant/detail/forced_return.hpp"
#include "lslboost/variant/detail/initializer.hpp"
#include "lslboost/variant/detail/make_variant_list.hpp"
#include "lslboost/variant/detail/over_sequence.hpp"
#include "lslboost/variant/detail/visitation_impl.hpp"
#include "lslboost/variant/detail/hash_variant.hpp"

#include "lslboost/variant/detail/generic_result_type.hpp"
#include "lslboost/variant/detail/has_nothrow_move.hpp"
#include "lslboost/variant/detail/move.hpp"
...
#include "lslboost/variant/recursive_wrapper_fwd.hpp"
...
#include "lslboost/mpl/max_element.hpp"
...
#include "lslboost/mpl/sizeof.hpp"
...
//   Defined in lslboost/variant/detail/visitation_impl.hpp.
//
...
struct add_alignment
{
    ...
    struct apply
        : mpl::size_t<
              ::lslboost::math::static_lcm<
                  BOOST_MPL_AUX_VALUE_WKND(State)::value
                , ::lslboost::alignment_of<Item>::value
                >::value
            >
    {};
};
...
//
// This implementation is designed to prefer lslboost::blank over other potential
...
private: // helpers, for metafunction result (below)
    ...
    // [...now search the rest of the sequence for lslboost::blank...]

    typedef typename mpl::iter_fold_if<
          mpl::iterator_range< first_result_it,end_it >
        , first_result_index, mpl::protect< mpl::next<> >
        , mpl::protect< mpl::not_same_as<lslboost::blank> >
        >::type second_result_;
...
public: // metafunction result
    ...
    typedef ::lslboost::aligned_storage<
...
public: // visitor interface
    ...
    T& operator()(U&) const
    {
        ...
        BOOST_ASSERT(false);
        return ::lslboost::detail::variant::forced_return< T& >();
    }
...
public: // internal visitor interface
    ...
    template <typename T>
        BOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(lslboost::detail::variant::backup_holder<T>& operand, long) const
    {
    ...
    template <typename T>
        BOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(const lslboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( operand.get() );
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
...
private: // helpers, for visitor interface (below)
        ...
        )
    {
        ...
        LhsT backup_lhs_content(
              ::lslboost::detail::variant::move(lhs_content)
            ); // nothrow
        ...
        catch (...)
        {
            ...
            new(lhs_.storage_.address())
                LhsT(
                      ::lslboost::detail::variant::move(backup_lhs_content)
                    ); // nothrow
...
public: // internal visitor interfaces
    ...
    void operator()(T& operand) const
    {
        ...
        // ...and swap:
        ::lslboost::detail::variant::move_swap( operand, other );
    }
...
public: // internal visitor interfaces, cont.
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(lslboost::recursive_wrapper<T>& operand, long)
    {
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const lslboost::recursive_wrapper<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(lslboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const lslboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(lslboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const lslboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
...
private: // helpers, for typedefs (below)
    ...
    BOOST_STATIC_ASSERT((
          ::lslboost::mpl::not_< mpl::empty<specified_types> >::value
        ));
...
#   endif
        ...
        template <typename T>
        int internal_visit(lslboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const lslboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(lslboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const lslboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(lslboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const lslboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }
...
private: // helpers, for structors, below 
    ...
    void convert_construct(
          lslboost::variant<BOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
    ...
    template <BOOST_VARIANT_ENUM_PARAMS(typename U)>
    void convert_construct(
          const lslboost::variant<BOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
        convert_construct_variant(operand);    
...
#include "lslboost/variant/detail/variant_io.hpp"
