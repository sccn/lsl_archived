...
{
public:
    typedef lslboost::uint16_t tag_t;
    typedef lslboost::uint16_t index_t;
...
struct compiletime_sized_freelist_storage
{
    ...
    lslboost::array<char, size * sizeof(T)> data;
    ...
    Alloc
{
    ...
    {
        if (count > 65535)
            lslboost::throw_exception(std::runtime_error("lslboost.lockfree: freelist size is limited to a maximum of 65535 objects"));
