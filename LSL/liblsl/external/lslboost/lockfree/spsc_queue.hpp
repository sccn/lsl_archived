...
#include <lslboost/lockfree/detail/atomic.hpp>
#include <lslboost/lockfree/detail/branch_hints.hpp>
#include <lslboost/lockfree/detail/copy_payload.hpp>
#include <lslboost/lockfree/detail/parameter.hpp>
#include <lslboost/lockfree/detail/prefix.hpp>
...
typedef parameter::parameters<lslboost::parameter::optional<tag::capacity>,
...
protected:
    ...
    ConstIterator push(ConstIterator begin, ConstIterator end, T * internal_buffer, size_t max_size)
    {
        ...
        const ConstIterator last = lslboost::next(begin, input_count);

        if (write_index + input_count > max_size) {
            /* copy data in two sections */
            const size_t count0 = max_size - write_index;
            const ConstIterator midpoint = lslboost::next(begin, count0);
...
public:
    ...
    void reset(void)
    {
        if ( !lslboost::has_trivial_destructor<T>::value ) {
    ...
    public ringbuffer_base<T>
{
    ...
    typedef typename lslboost::aligned_storage<max_size * sizeof(T),
                                            lslboost::alignment_of<T>::value
                                           >::type storage_type;
 ...
 *
 *  \b Policies:
 *  - \c lslboost::lockfree::capacity<>, optional <br>
 *    If this template argument is passed to the options, the size of the ringbuffer is set at compile-time.
 *
 *  - \c lslboost::lockfree::allocator<>, defaults to \c lslboost::lockfree::allocator<std::allocator<T>> <br>
...
public:
     ...
     */
    template <typename U>
    typename lslboost::enable_if<typename is_convertible<T, U>::type, bool>::type
    pop (U & ret)
    {
     ...
     * */
    template <typename OutputIterator>
    typename lslboost::disable_if<typename is_convertible<T, OutputIterator>::type, size_type>::type
    ...
    /// \copydoc lslboost::lockfree::spsc_queue::consume_one(Functor & rhs)
    template <typename Functor>
    bool consume_one(Functor const & f)
    {
        return base_type::consume_one(f);
    ...
    /// \copydoc lslboost::lockfree::spsc_queue::consume_all(Functor & rhs)
    template <typename Functor>
    size_type consume_all(Functor const & f)
    {
        return base_type::consume_all(f);
    ...
    /// \copydoc lslboost::lockfree::spsc_queue::front() const
    T& front()
    {
        BOOST_ASSERT(read_available() > 0);
        return base_type::front();
