...
#include <lslboost/move/detail/unique_ptr_meta_utils.hpp>
#include <lslboost/move/default_delete.hpp>
...
class is_noncopyable
{
   ...
   template<class U> static true_t  dispatch(typename U::lslboost_move_no_copy_constructor_or_assign*);
   public:
...
struct deleter_types
{
   ...
   typedef ::lslboost::rv<D> &                                       deleter_arg_type2;
   #endif
};
...
class is_rvalue_convertible
{
   ...
   #else
   typedef typename bmupmu::if_c
      < ::lslboost::has_move_emulation_enabled<T>::value && !bmupmu::is_reference<T>::value
      , ::lslboost::rv<T>&
...
class unique_ptr
{
      ...
      : m_data(p)
   {
      ...
      BOOST_STATIC_ASSERT(( !::lslboost::move_upmu::missing_virtual_destructor
   ...
   unique_ptr(BOOST_MOVE_DOC0PTR(bmupd::nullptr_type), BOOST_MOVE_SEEDOC(deleter_arg_type2) d2) BOOST_NOEXCEPT
      : m_data(pointer(), ::lslboost::move(d2))
   {}
   ...
   unique_ptr(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
      : m_data(u.release(), ::lslboost::move_if_not_lvalue_reference<D>(u.get_deleter()))
   {}
   ...
   unique_ptr( BOOST_RV_REF_BEG_IF_CXX11 unique_ptr<U, E> BOOST_RV_REF_END_IF_CXX11 u
      ...
      : m_data(u.release(), ::lslboost::move_if_not_lvalue_reference<E>(u.get_deleter()))
   {
   ...
   unique_ptr& operator=(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
   {
      ...
      m_data.deleter() = ::lslboost::move_if_not_lvalue_reference<D>(u.get_deleter());
      return *this;
   }
   ...
   {
      this->reset(u.release());
      m_data.deleter() = ::lslboost::move_if_not_lvalue_reference<E>(u.get_deleter());
      return *this;
   }
   ...
   {
      ::lslboost::adl_move_swap(m_data.m_p, u.m_data.m_p);
      ::lslboost::adl_move_swap(m_data.deleter(), u.m_data.deleter());
   }
};
