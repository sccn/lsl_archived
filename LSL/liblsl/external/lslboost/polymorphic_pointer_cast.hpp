//  lslboost polymorphic_pointer_cast.hpp header file  ----------------------------------------------//
...
//  polymorphic_pointer_downcast  --------------------------------------------//
    ...
    //  built-in pointers, std::shared_ptr, lslboost::shared_ptr, lslboost::intrusive_ptr, etc.
    ...
    namespace detail
    {
        ...
        {
#ifdef BOOST_NO_CXX11_DECLTYPE
            BOOST_TYPEOF_NESTED_TYPEDEF_TPL(nested, dynamic_pointer_cast<Target>(lslboost::declval<Source>()))
            typedef typename nested::type type;
#else
            typedef decltype(dynamic_pointer_cast<Target>(lslboost::declval<Source>())) type;
#endif
        };
    }
    ...
    polymorphic_pointer_cast (const Source& x)
    {
        ...
        if ( !tmp ) lslboost::throw_exception( std::bad_cast() );
