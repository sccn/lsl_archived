...
// Home at http://www.lslboost.org/libs/scope_exit

#ifndef FILE_lslboost_scope_exit_hpp_INCLUDED
#define FILE_lslboost_scope_exit_hpp_INCLUDED

#ifndef DOXYGEN

#include <lslboost/local_function/detail/preprocessor/line_counter.hpp>
#include <lslboost/local_function/detail/preprocessor/void_list.hpp>
#include <lslboost/local_function/detail/preprocessor/keyword/thisunderscore.hpp>
...
#include <lslboost/preprocessor/logical/bitor.hpp>
...
#include <lslboost/preprocessor/seq/cat.hpp>
...
#include <lslboost/preprocessor/list/enum.hpp>
#include <lslboost/preprocessor/list/adt.hpp>
#include <lslboost/preprocessor/list/for_each_i.hpp>
...
namespace lslboost { namespace scope_exit { namespace detail {
...
BOOST_TYPEOF_REGISTER_TEMPLATE(lslboost::scope_exit::detail::wrapper, 1)

#define BOOST_SCOPE_EXIT_AUX_ARGS lslboost_scope_exit_aux_args
extern lslboost::scope_exit::detail::undeclared BOOST_SCOPE_EXIT_AUX_ARGS;

#define BOOST_SCOPE_EXIT_AUX_GUARD(id)   \
    BOOST_PP_CAT(lslboost_se_guard_, id)

#define BOOST_SCOPE_EXIT_AUX_GUARD_T(id) \
    BOOST_PP_CAT(lslboost_se_guard_t_, id)

#define BOOST_SCOPE_EXIT_AUX_PARAMS(id)  \
    BOOST_PP_CAT(lslboost_se_params_, id)

#define BOOST_SCOPE_EXIT_AUX_THIS_T(id)  \
    BOOST_PP_CAT(lslboost_se_this_t_,  id)

#define BOOST_SCOPE_EXIT_AUX_THIS_CAPTURE_T(id) \
    BOOST_PP_CAT(lslboost_se_this_capture_t_,  id)

#define BOOST_SCOPE_EXIT_DETAIL_PARAMS_T(id) \
    BOOST_PP_CAT(lslboost_se_params_t_, id)

#define BOOST_SCOPE_EXIT_DETAIL_TAG(id, i) \
    BOOST_PP_SEQ_CAT( (lslboost_se_tag_)(i)(_)(id) )

#define BOOST_SCOPE_EXIT_DETAIL_PARAM_THIS(id) \
    BOOST_PP_SEQ_CAT( (lslboost_se_param_this_)(id) )

#define BOOST_SCOPE_EXIT_DETAIL_PARAM(id, i, var) \
    BOOST_PP_SEQ_CAT( (lslboost_se_param_)(i)(_)(id) )

#define BOOST_SCOPE_EXIT_DETAIL_PARAM_T(id, i, var) \
    BOOST_PP_SEQ_CAT( (lslboost_se_param_t_)(i)(_)(id) )

#define BOOST_SCOPE_EXIT_DETAIL_CAPTURE_T(id, i, var) \
    BOOST_PP_SEQ_CAT( (lslboost_se_capture_t_)(i)(_)(id) )

#define BOOST_SCOPE_EXIT_AUX_WRAPPED(id, i) \
    BOOST_PP_SEQ_CAT( (lslboost_se_wrapped_t_)(i)(_)(id) )

#define BOOST_SCOPE_EXIT_AUX_DEREF(id, i, var) \
    ::lslboost::scope_exit::detail::deref(var, \
            static_cast<BOOST_SCOPE_EXIT_DETAIL_TAG(id, i)>(0))

#define BOOST_SCOPE_EXIT_AUX_MEMBER(r, id, i, var) \
    ::lslboost::scope_exit::detail::member< \
...
#define BOOST_SCOPE_EXIT_AUX_ARG(r, id, i, var) \
    BOOST_PP_COMMA_IF(i) \
    lslboost_se_params_->BOOST_SCOPE_EXIT_DETAIL_PARAM(id, i, var).value
...
// lslboost/typeof/msvc/typeof_impl.hpp). However, this code cannot be integrated
...
namespace lslboost { namespace scope_exit { namespace aux {
...
struct msvc_typeid_wrapper {
    typedef typename msvc_extract_type<lslboost::mpl::int_<Id>
            >::id2type id2type;
...
struct encode_type {
    ...
    typedef typename msvc_register_type<T,
            lslboost::mpl::int_<value> >::id2type type;
...
typename lslboost::disable_if<
      typename lslboost::is_function<T>::type
...
#define BOOST_SCOPE_EXIT_AUX_TYPEDEF_TYPEOF_THIS_INDEX_(id) \
    BOOST_PP_CAT(lslboost_se_thistype_index_, id)
...
#define BOOST_SCOPE_EXIT_DETAIL_TYPEDEF_TYPEOF_THIS(id, ty, new_type) \
    ...
    enum { \
        BOOST_SCOPE_EXIT_AUX_TYPEDEF_TYPEOF_THIS_INDEX_(id) = sizeof( \
            *::lslboost::scope_exit::aux::msvc_typeof_this::encode_start(this)) \
    }; \
    typedef \
        ty ::lslboost::scope_exit::aux::msvc_typeof_this::msvc_typeid_wrapper< \
...
#define BOOST_SCOPE_EXIT_AUX_CTOR_ARG_THIS_NAME(id) \
    BOOST_PP_CAT(lslboost_se_this_arg_, id)
...
#define BOOST_SCOPE_EXIT_DETAIL_CAPTURE_DECL(r, id_ty, i, var) \
    struct BOOST_SCOPE_EXIT_AUX_WRAPPED(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), i) \
        ...
        : BOOST_TYPEOF(::lslboost::scope_exit::detail::wrap( \
...
private:
    This this_;
    lslboost::function<void (This)> f_;
};
...
struct guard<void> { // Without object `this_` (could capture `this` directly).
    ...
    void operator=(Lambda f) { f_ = f; }
private:
    lslboost::function<void (void)> f_;
};

} } } // namespace
    
#define BOOST_SCOPE_EXIT_AUX_LAMBDA_PARAMS(id) \
    BOOST_PP_CAT(lslboost_se_lambda_params_, id)

#define BOOST_SCOPE_EXIT_AUX_LAMBDA_THIS_CAPTURE_TYPE(id) \
    BOOST_PP_CAT(lslboost_se_lambda_this_t_, id)

#define BOOST_SCOPE_EXIT_AUX_LAMBDA_THIS_PARAM_TYPE(id) \
    BOOST_PP_CAT(lslboost_se_lambda_this_capture_t_, id)
...
#define BOOST_SCOPE_EXIT_AUX_IMPL(id, ty, traits) \
    ...
    struct BOOST_SCOPE_EXIT_DETAIL_PARAMS_T(id) { \
        ...
        BOOST_SCOPE_EXIT_AUX_PARAMS_INIT(id, \
                ...
                BOOST_SCOPE_EXIT_AUX_TRAITS_HAS_THIS(traits)) \
    ; \
    ::lslboost::scope_exit::detail::declared< \
        ::lslboost::scope_exit::detail::resolve< \
    ...
    struct BOOST_SCOPE_EXIT_AUX_GUARD_T(id) { \
        BOOST_SCOPE_EXIT_DETAIL_PARAMS_T(id)* lslboost_se_params_; \
        BOOST_SCOPE_EXIT_AUX_GUARD_T(id) (void* lslboost_se_params) \
            : lslboost_se_params_( \
                    (BOOST_SCOPE_EXIT_DETAIL_PARAMS_T(id)*)lslboost_se_params) \
        {} \
        ~BOOST_SCOPE_EXIT_AUX_GUARD_T(id)() { \
            lslboost_se_body( \
                ...
                BOOST_PP_EXPR_IIF(BOOST_SCOPE_EXIT_AUX_TRAITS_HAS_THIS( \
                        traits), \
                    lslboost_se_params_->BOOST_SCOPE_EXIT_DETAIL_PARAM_THIS(id) \
                ) \
            ); \
        } \
        static void lslboost_se_body( \
...
#endif // #ifndef FILE_lslboost_scope_exit_hpp_INCLUDED
