        ...
        namespace
        {
          ...
          lslboost::once_flag current_thread_tls_init_flag;
            ...
            extern "C"
            {
                ...
                {
                    //lslboost::detail::thread_data_base* thread_info=static_cast<lslboost::detail::thread_data_base*>(data);
                    lslboost::detail::thread_data_ptr thread_info = static_cast<lslboost::detail::thread_data_base*>(data)->shared_from_this();
            ...
            struct  delete_current_thread_tls_key_on_dlclose_t
            {
                ...
                ~delete_current_thread_tls_key_on_dlclose_t()
                {
                    const lslboost::once_flag uninitialized = BOOST_ONCE_INIT;
                    if (memcmp(&current_thread_tls_init_flag, &uninitialized, sizeof(lslboost::once_flag)))
                    {
    ...
    namespace
    {
        extern "C"
        {
            static void* thread_proxy(void* param)
            {
                ...
                lslboost::detail::thread_data_ptr thread_info = static_cast<lslboost::detail::thread_data_base*>(param)->shared_from_this();
                thread_info->self.reset();
...
//                 }
    ...
    #   endif
                    timespec now2 = lslboost::detail::timespec_now_realtime();
      ...
      namespace hidden
      {
        void BOOST_THREAD_DECL sleep_for(const timespec& ts)
        {
            ...
            {
              lslboost::this_thread::no_interruption_point::hidden::sleep_for(ts);
            }
        }
...
#   endif
    ...
    unsigned thread::physical_concurrency() BOOST_NOEXCEPT
    {
        ...
        try {
            ...
            while ( getline(proc_cpuinfo, line) ) {
                ...
                lslboost::split(key_val, line, lslboost::is_any_of(":"));
                ...
                if (key == physical_id) {
                    current_core_entry.first = lslboost::lexical_cast<unsigned>(value);
                    continue;
                }

                if (key == core_id) {
                    current_core_entry.second = lslboost::lexical_cast<unsigned>(value);
    ...
    void thread::interrupt()
    {
        ...
        if(local_thread_info)
        {
            ...
            {
                lslboost::pthread::pthread_mutex_scoped_lock internal_lock(local_thread_info->cond_mutex);
