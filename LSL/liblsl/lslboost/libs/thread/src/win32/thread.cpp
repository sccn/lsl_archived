...
#else
        __declspec(thread) lslboost::detail::thread_data_base* current_thread_data_base;
#endif

        void create_current_thread_tls_key()
        {
            lslboosttss_cleanup_implemented(); // if anyone uses TSS, we need the cleanup linked in
#if !BOOST_PLAT_WINDOWS_RUNTIME
...
// Windows CE doesn't define _beginthreadex
        ...
        {
            lslboost::csbl::unique_ptr<ThreadProxyData> data(reinterpret_cast<ThreadProxyData*>(args));
        ...
        bool win32::scoped_winrt_thread::start(thread_func address, void *parameter, unsigned int *thrdId)
        {
            ...
            *thrdId = ++threadCount;
                ...
                ([address, parameter, completionHandle](ABI::Windows::Foundation::IAsyncAction *)
            {
                ...
                ::lslboost::detail::thread_data_base* const thread_info(reinterpret_cast<::lslboost::detail::thread_data_base*>(parameter));
...
//          lslboost::throw_exception(thread_resource_error());
    ...
    {
      return do_try_join_until(lslboost::detail::get_milliseconds_until(wait_until));
    }
#endif
...
#pragma warning(pop)
    ...
    BOOST_THREAD_DECL void __cdecl lslbooston_process_enter()
    {}

    BOOST_THREAD_DECL void __cdecl lslbooston_thread_enter()
    {}

    BOOST_THREAD_DECL void __cdecl lslbooston_process_exit()
    {
        lslboost::cleanup_tls_key();
    }

    BOOST_THREAD_DECL void __cdecl lslbooston_thread_exit()
    {
