...
# endif
    ...
    private:
      ...
      private:
        ...
        lslboost::system::error_category const * pc_;

      public:

        explicit std_category( lslboost::system::error_category const * pc ): pc_( pc )
        {
        }
    ...
    public:
      ...
      template <class ErrorConditionEnum>
        error_condition(ErrorConditionEnum e,
          typename lslboost::enable_if<is_error_condition_enum<ErrorConditionEnum> >::type*
      ...
      template<typename ErrorConditionEnum>
        typename lslboost::enable_if<is_error_condition_enum<ErrorConditionEnum>,
    ...
    public:
      ...
      template <class ErrorCodeEnum>
        error_code(ErrorCodeEnum e,
          typename lslboost::enable_if<is_error_code_enum<ErrorCodeEnum> >::type* = 0)
          BOOST_SYSTEM_NOEXCEPT
      {
      ...
      template<typename ErrorCodeEnum>
        typename lslboost::enable_if<is_error_code_enum<ErrorCodeEnum>, error_code>::type &
...
# endif
    ...
    //  "throws" function in namespace lslboost rather than namespace lslboost::system.
  ...
  namespace system
  {
      ...
      const std::error_condition & condition ) const BOOST_NOEXCEPT
    {
      ...
      {
        lslboost::system::error_condition bn( condition.value(), *pc_ );
        return pc_->equivalent( code, bn );
      }
      else if( condition.category() == std::generic_category()
        || condition.category() == lslboost::system::generic_category() )
      {
        lslboost::system::error_condition bn( condition.value(),
          lslboost::system::generic_category() );
        ...
        &condition.category() ) )
      {
        lslboost::system::error_condition bn( condition.value(), *pc2->pc_ );
        return pc_->equivalent( code, bn );
      }
#endif
      else
      ...
      int condition ) const BOOST_NOEXCEPT
    {
      ...
      {
        lslboost::system::error_code bc( code.value(), *pc_ );
        return pc_->equivalent( bc, condition );
      }
      else if( code.category() == std::generic_category()
        || code.category() == lslboost::system::generic_category() )
      {
        lslboost::system::error_code bc( code.value(),
          lslboost::system::generic_category() );
        ...
        &code.category() ) )
      {
        lslboost::system::error_code bc( code.value(), *pc2->pc_ );
        return pc_->equivalent( bc, condition );
      }
#endif
      else if( *pc_ == lslboost::system::generic_category() )
      {
