...
class optional_base : public optional_tag
{
  ...
  protected:
    ...
    {
      if ( rhs.is_initialized() )
        construct( lslboost::move(rhs.get_impl()) );
    }
#endif
    ...
    explicit optional_base ( Expr&& expr, PtrExpr const* tag )
      ...
      m_initialized(false)
    {
      construct(lslboost::forward<Expr>(expr),tag);
    }
    ...
    void assign ( optional_base&& rhs )
    {
      ...
      {
        if ( rhs.is_initialized() )
             assign_value( lslboost::move(rhs.get_impl()) );
        else destroy();
      }
      else
      {
    ...
    {
      if (is_initialized())
           assign_value( lslboost::move(val) );
      else construct( lslboost::move(val) );
    }
#endif
    ...
    {
      if (is_initialized())
        assign_expr_to_initialized(lslboost::forward<Expr>(expr),tag);
      else construct(lslboost::forward<Expr>(expr),tag);
    }
#else
    template<class Expr>
  ...
  protected :
    ...
    void construct ( rval_reference_type val )
     {
       ::new (m_storage.address()) value_type( lslboost::move(val) ) ;
       m_initialized = true ;
     }
#endif
    ...
    {
      ::new (m_storage.address()) value_type( lslboost::forward<Args>(args)... ) ;
      m_initialized = true ;
    }
    ...
    {
      destroy();
      construct(in_place_init, lslboost::forward<Args>(args)...);
    }
    ...
    void construct ( in_place_init_t, Arg&& arg )
     {
       ::new (m_storage.address()) value_type( lslboost::forward<Arg>(arg) );
       m_initialized = true ;
     }
    ...
    void emplace_assign ( Arg&& arg )
     {
       destroy();
       construct(in_place_init, lslboost::forward<Arg>(arg)) ;
     }
    ...
    void construct ( Expr&& factory, in_place_factory_base const* )
     {
       lslboost_optional_detail::construct<value_type>(factory, m_storage.address());
       m_initialized = true ;
     }
    ...
    {
      new (m_storage.address()) value_type(lslboost::forward<Expr>(expr)) ;
      m_initialized = true ;
    }
    ...
    {
      assign_value( lslboost::forward<Expr>(expr) );
    }
#else
    // Constructs using any expression implicitly convertible to the single argument
    // of a one-argument T constructor.
    ...
    void construct ( Expr&& expr, optional_tag const* )
     {
       if ( expr.is_initialized() )
       {
         ...
         new (m_storage.address()) value_type(lslboost::move(expr.get())) ;
         m_initialized = true ;
       }
     }
#else
...
struct is_optional_related
  : lslboost::conditional< lslboost::is_base_of<optional_detail::optional_tag, BOOST_DEDUCED_TYPENAME lslboost::decay<U>::type>::value
                     || lslboost::is_same<BOOST_DEDUCED_TYPENAME lslboost::decay<U>::type, none_t>::value
                     || lslboost::is_same<BOOST_DEDUCED_TYPENAME lslboost::decay<U>::type, in_place_init_t>::value
                     || lslboost::is_same<BOOST_DEDUCED_TYPENAME lslboost::decay<U>::type, in_place_init_if_t>::value,
    lslboost::true_type, lslboost::false_type>::type
{};
...
struct is_convertible_to_T_or_factory
  : lslboost::conditional< lslboost::is_base_of<lslboost::in_place_factory_base, BOOST_DEDUCED_TYPENAME lslboost::decay<U>::type>::value
                     || lslboost::is_base_of<lslboost::typed_in_place_factory_base, BOOST_DEDUCED_TYPENAME lslboost::decay<U>::type>::value
                     || (lslboost::is_constructible<T, U&&>::value && !lslboost::is_same<T, BOOST_DEDUCED_TYPENAME lslboost::decay<U>::type>::value)
                      , lslboost::true_type, lslboost::false_type>::type
{};

template <typename T, typename U>
struct is_optional_constructible : lslboost::is_constructible<T, U>
{};

#else

template <typename, typename>
struct is_convertible_to_T_or_factory : lslboost::true_type
{};

template <typename T, typename U>
struct is_optional_constructible : lslboost::true_type
{};
...
struct is_optional_val_init_candidate
  : lslboost::conditional< !is_optional_related<U>::value && is_convertible_to_T_or_factory<T, U>::value
                      , lslboost::true_type, lslboost::false_type>::type
{};
...
class optional : public optional_detail::optional_base<T>
{
  ...
  public :
    ...
    // Can throw if T::T(T &&) does
    optional ( rval_reference_type val ) : base( lslboost::forward<T>(val) ) 
      {}
#endif
    ...
    explicit optional ( optional<U> const& rhs
                        ...
                        ,typename lslboost::enable_if< optional_detail::is_optional_constructible<T, U const&> >::type* = 0
#endif
                      )
      :
      base()
...
#ifndef BOOST_OPTIONAL_DETAIL_NO_SFINAE_FRIENDLY_CONSTRUCTORS
                        ,typename lslboost::enable_if< optional_detail::is_optional_constructible<T, U> >::type* = 0
#endif
                      )
      :
      base()
    {
      if ( rhs.is_initialized() )
        this->construct( lslboost::move(rhs.get()) );
    }
#endif
  ...
  explicit optional ( Expr&& expr, 
                      BOOST_DEDUCED_TYPENAME lslboost::enable_if< optional_detail::is_optional_val_init_candidate<T, Expr> >::type* = 0 
  ) 
    : base(lslboost::forward<Expr>(expr),lslboost::addressof(expr)) 
    {}

#else
    template<class Expr>
    explicit optional ( Expr const& expr ) : base(expr,lslboost::addressof(expr)) {}
	...
	optional ( optional && rhs ) 
	  BOOST_NOEXCEPT_IF(::lslboost::is_nothrow_move_constructible<T>::value)
	  : base( lslboost::move(rhs) ) 
	{}
    ...
    template<class Expr>
    BOOST_DEDUCED_TYPENAME lslboost::enable_if<optional_detail::is_optional_val_init_candidate<T, Expr>, optional&>::type 
    operator= ( Expr&& expr )
      {
        this->assign_expr(lslboost::forward<Expr>(expr),lslboost::addressof(expr));
        return *this ;
      }
    ...
    optional& operator= ( Expr const& expr )
      {
        this->assign_expr(expr,lslboost::addressof(expr));
        return *this ;
      }
#endif // !defined  BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES
    ...
    optional& operator= ( optional && rhs ) 
	  BOOST_NOEXCEPT_IF(::lslboost::is_nothrow_move_constructible<T>::value && ::lslboost::is_nothrow_move_assignable<T>::value)
      {
    ...
    BOOST_DEDUCED_TYPENAME lslboost::enable_if<lslboost::is_same<T, BOOST_DEDUCED_TYPENAME lslboost::decay<T_>::type>, optional&>::type
    operator= ( T_&& val )
      {
        this->assign( lslboost::forward<T_>(val) ) ;
        return *this ;
      }
    ...
    {
      this->emplace_assign( lslboost::forward<Args>(args)... );
    }
     
    template<class... Args>
    explicit optional ( in_place_init_t, Args&&... args )
    : base( in_place_init, lslboost::forward<Args>(args)... )
    {}
    
    template<class... Args>
    explicit optional ( in_place_init_if_t, bool cond, Args&&... args )
    : base( in_place_init_if, cond, lslboost::forward<Args>(args)... )
    {}
    ...
    void emplace ( Arg&& arg )
     {
       this->emplace_assign( lslboost::forward<Arg>(arg) );
     }
    ...
    reference_type_of_temporary_wrapper operator *() && { return lslboost::move(this->get()) ; }
#else
    reference_const_type operator *() const { return this->get() ; }
    reference_type       operator *()       { return this->get() ; }
...
{
  BOOST_STATIC_ASSERT_MSG(sizeof(CharType) == 0, "If you want to output lslboost::optional, include header <boost/optional/optional_io.hpp>");
  return os;  
}
