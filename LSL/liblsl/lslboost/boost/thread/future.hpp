  ...
  shared_ptr<T const> static_shared_from_this(T const* that)
  {
    ...
    namespace detail
    {
        ...
        struct shared_state_base : enable_shared_from_this<shared_state_base>
        {
            ...
            void set_executor_policy(executor_ptr_type aex, lslboost::lock_guard<lslboost::mutex>&)
            {
              set_executor();
              ex = aex;
            }
            void set_executor_policy(executor_ptr_type aex, lslboost::unique_lock<lslboost::mutex>&)
            {
              set_executor();
              ex = aex;
            }

            bool valid(lslboost::unique_lock<lslboost::mutex>&) { return is_valid_; }
            bool valid() {
              lslboost::unique_lock<lslboost::mutex> lk(this->mutex);
              return valid(lk);
            }
            void invalidate(lslboost::unique_lock<lslboost::mutex>&) { is_valid_ = false; }
            void invalidate() {
              lslboost::unique_lock<lslboost::mutex> lk(this->mutex);
              invalidate(lk);
            }
            void validate(lslboost::unique_lock<lslboost::mutex>&) { is_valid_ = true; }
            void validate() {
              lslboost::unique_lock<lslboost::mutex> lk(this->mutex);
              validate(lk);
            }
...
#endif
            notify_when_ready_handle notify_when_ready(lslboost::condition_variable_any& cv)
            {
                lslboost::unique_lock<lslboost::mutex> lock(this->mutex);
                do_callback(lock);
            ...
            {
                lslboost::lock_guard<lslboost::mutex> lock(this->mutex);
...
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            virtual void set_continuation_ptr(continuation_ptr_type continuation, lslboost::unique_lock<lslboost::mutex>& lock)
            {
            ...
            virtual void wait(lslboost::unique_lock<lslboost::mutex>& lock, bool rethrow=true)
            {
            ...
            future_state::state get_state(lslboost::unique_lock<lslboost::mutex>&) const
            {
                if(!done)
                {
            ...
            {
                lslboost::lock_guard<lslboost::mutex> guard(this->mutex);
                if(!done)
                {
                    return future_state::waiting;
                }
        ...
        {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
              typedef lslboost::optional<T> storage_type;
#else
              typedef lslboost::csbl::unique_ptr<T> storage_type;
#endif
            ...
            typedef typename conditional<lslboost::is_fundamental<T>::value,T,T const&>::type source_reference_type;
            typedef BOOST_THREAD_RV_REF(T) rvalue_source_type;
            typedef T move_dest_type;
#else
            typedef T& source_reference_type;
            typedef typename conditional<lslboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename conditional<lslboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#endif
            ...
            template <class ...Args>
            void mark_finished_with_result_internal(lslboost::unique_lock<lslboost::mutex>& lock, BOOST_THREAD_FWD_REF(Args)... args)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result.emplace(lslboost::forward<Args>(args)...);
#else
                result.reset(new T(lslboost::forward<Args>(args)...));
#endif
                this->mark_finished_internal(lock);
            }
#endif
            ...
            void mark_finished_with_result(rvalue_source_type result_)
            {
                ...
                mark_finished_with_result_internal(lslboost::move(result_), lock);
#else
                mark_finished_with_result_internal(static_cast<rvalue_source_type>(result_), lock);
#endif
            }

            storage_type& get_storage(lslboost::unique_lock<lslboost::mutex>& lk)
            {
                wait_internal(lk);
                return result;
            }
            virtual move_dest_type get(lslboost::unique_lock<lslboost::mutex>& lk)
            {
                return lslboost::move(*get_storage(lk));
            }
            move_dest_type get()
            {
                lslboost::unique_lock<lslboost::mutex> lk(this->mutex);
            ...
            virtual shared_future_get_result_type get_sh(lslboost::unique_lock<lslboost::mutex>& lk)
            {
            ...
            detail::shared_state_base
        {
            ...
            virtual T& get(lslboost::unique_lock<lslboost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            virtual T& get_sh(lslboost::unique_lock<lslboost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            detail::shared_state_base
        {
            ...
            virtual void get(lslboost::unique_lock<lslboost::mutex>& lock)
            {
            ...
            virtual void get_sh(lslboost::unique_lock<lslboost::mutex>& lock)
            {
                this->wait_internal(lock);
            }
            void get_sh()
        ...
        public:
          ...
          virtual void wait(lslboost::unique_lock<lslboost::mutex>& lk, bool rethrow)
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
              {
                relocker rlk(lk);
        ...
        struct future_async_shared_state: future_async_shared_state_base<Rp>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = lslboost::thread(&future_async_shared_state::run, static_shared_from_this(this), lslboost::forward<Fp>(f));
#else
            lslboost::thread(&future_async_shared_state::run, static_shared_from_this(this), lslboost::forward<Fp>(f)).detach();
#endif
          }
        ...
        struct future_async_shared_state<void, Fp>: public future_async_shared_state_base<void>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = lslboost::thread(&future_async_shared_state::run, static_shared_from_this(this), lslboost::move(f));
#else
            lslboost::thread(&future_async_shared_state::run, static_shared_from_this(this), lslboost::move(f)).detach();
#endif
          }
        ...
        private:
            ...
            struct all_futures_lock
            {
                    ...
                    count(futures.size()),locks(new lslboost::unique_lock<lslboost::mutex>[count])
                {
                    ...
                    {
                        locks[i]=BOOST_THREAD_MAKE_RV_REF(lslboost::unique_lock<lslboost::mutex>(futures[i].future_->mutex));
                    }
                }
...
//    }
    ...
    typename lslboost::enable_if<is_future_type<F1>,typename detail::future_waiter::count_type>::type wait_for_any(F1& f1,F2& f2)
    {
        detail::future_waiter waiter;
        waiter.add(f1);
        waiter.add(f2);
...
#else
    template<typename F1, typename... Fs>
    typename lslboost::enable_if<is_future_type<F1>, typename detail::future_waiter::count_type>::type
    ...
    namespace detail
    {
      ...
      public:
        ...
        state get_state(lslboost::unique_lock<lslboost::mutex>& lk) const
        {
            if(!future_)
            {
        ...
        bool is_ready(lslboost::unique_lock<lslboost::mutex>& lk) const
        {
        ...
        {
          if ( future_ ) {
            lslboost::unique_lock<lslboost::mutex> lk(this->future_->mutex);
    ...
    namespace detail
    {
        ...
        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_sync_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_deferred_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_async_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_sync_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);


  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_executor_continuation_shared_state(Ex& ex, lslboost::unique_lock<lslboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_executor_continuation_shared_state(Ex& ex, lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
      ...
      typename lslboost::disable_if<is_future_type<InputIterator>,
    ...
    private:
        ...
        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_sync_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_deferred_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_async_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_sync_continuation_shared_state(lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_continuation_shared_state(Ex& ex, lslboost::unique_lock<lslboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_executor_continuation_shared_state(Ex& ex, lslboost::unique_lock<lslboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
...
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      friend typename lslboost::disable_if<is_future_type<InputIterator>,
        BOOST_THREAD_FUTURE<csbl::vector<typename InputIterator::value_type>  >
      >::type
      when_all(InputIterator first, InputIterator last);
    ...
    public:
        ...
        move_dest_type get()
        {
            ...
            unique_lock<lslboost::mutex> lk(this->future_->mutex);
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    public:
        ...
        void set_executor(executor_ptr_type aex)
        {
          ...
          lslboost::lock_guard<lslboost::mutex> lk(future_->mutex);
...
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename lslboost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value(TR const &  r)
        {
            lazy_init();
            lslboost::unique_lock<lslboost::mutex> lock(future_->mutex);
            if(future_->done)
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->mark_finished_with_result_internal(lslboost::move(r), lock);
#else
        ...
        void emplace(BOOST_THREAD_FWD_REF(Args) ...args)
        {
            ...
            future_->mark_finished_with_result_internal(lock, lslboost::forward<Args>(args)...);
        }
        ...
        {
          set_exception(lslboost::copy_exception(ex));
        }
        // setting the result with deferred notification
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename lslboost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value_at_thread_exit(TR const& r)
        {
          if (future_.get()==0)
          {
              lslboost::throw_exception(promise_moved());
        ...
        {
          set_exception_at_thread_exit(lslboost::copy_exception(ex));
        }
...
namespace lslboost { namespace container {
    ...
    struct uses_allocator< ::lslboost::promise<R> , Alloc> : true_type
    {
    };
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
...
namespace lslboost
{
      ...
      struct task_base_shared_state:
        ...
        public:
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->set_value_at_thread_exit(f(lslboost::move(args)...));
                }
#else
            void do_apply()
            {
            ...
            void do_run(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->mark_finished_with_result(f(lslboost::move(args)...));
                }
#else
            void do_run()
            {
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_shared_state_type(f, lslboost::move(args)...));
            future_obtained=false;
        }
  #else
...
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
            typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(lslboost::move(f)));
#else
            typedef detail::task_shared_state<F,R()> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(lslboost::move(f)));
#endif
        ...
        packaged_task(lslboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(lslboost::move(f)), D(a2, 1) );
          future_obtained = false;
        }
        ...
        void set_executor(executor_ptr_type aex)
        {
          ...
          lslboost::lock_guard<lslboost::mutex> lk(task->mutex);
          task->set_executor_policy(aex, lk);
        }
#endif
        void reset() {
            if (!valid())
              lslboost::throw_exception(future_error(system::make_error_code(future_errc::no_state)));
        ...
        void make_ready_at_thread_exit() {
          ...
          if (task->has_value()) lslboost::throw_exception(promise_already_satisfied());
          task->apply();
        }
#endif
        template<typename F>
...
namespace lslboost { namespace container {
    template <class R, class Alloc>
    struct uses_allocator< ::lslboost::packaged_task<R> , Alloc> : true_type
    {};
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
namespace std {
...
namespace detail {
    ...
    public:
      ...
      : that(x.that), f_(lslboost::move(x.f_))
      {
        x.that.reset();
      }
...
namespace detail {
    ...
    public:
      ...
      void init(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f)
      {
        ...
        shared_state_nullary_task<Rp,Cont> t(this->shared_from_this(), lslboost::forward<Fp>(f));
        ex.submit(lslboost::move(t));
      }
    ...
    make_future_executor_shared_state(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f) {
      ...
      h->init(ex, lslboost::forward<Fp>(f));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
...
//#if ! defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(lslboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(lslboost::forward<ArgTypes>(args))...
        )
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(A1)), BOOST_THREAD_FWD_REF(A1) a1) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(lslboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(lslboost::forward<A1>(a1))
        )
    ));
  }
#endif // defined BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f)  {
    ...
    return lslboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(lslboost::forward<F>(f))
        )
    );
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(lslboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(lslboost::forward<F>(f))
          , thread_detail::decay_copy(lslboost::forward<A1>(a1))
          , thread_detail::decay_copy(lslboost::forward<A2>(a2))
        )
    ));
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future() {
    promise<T> p;
    p.set_exception(lslboost::current_exception());
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  template <typename T>
...
namespace detail
{
  ...
  public:
    ...
    void init(lslboost::unique_lock<lslboost::mutex> &lock)
    {
    ...
    void call() {
      try {
        this->mark_finished_with_result(this->continuation(lslboost::move(this->parent)));
      } catch(...) {
        this->mark_exceptional_finish();
      }
    ...
    static void run(shared_ptr<lslboost::detail::shared_state_base> that_)
    {
  ...
  public:
    ...
    {
      try {
        this->continuation(lslboost::move(this->parent));
        this->mark_finished_with_result();
      } catch(...) {
        this->mark_exceptional_finish();
      }
  ...
  public:
    ...
    : base_type(lslboost::move(f), lslboost::forward<Fp>(c))
    {    }

    void launch_continuation() {
#if defined BOOST_THREAD_FUTURE_BLOCKING
      lslboost::lock_guard<lslboost::mutex> lk(this->mutex);
      this->thr_ = lslboost::thread(&future_async_continuation_shared_state::run, static_shared_from_this(this));
#else
      lslboost::thread(&base_type::run, static_shared_from_this(this)).detach();
#endif
    }
  };
...
namespace detail {
  ...
  public:
    ...
    template <class Ex>
    void init(lslboost::unique_lock<lslboost::mutex> &lk, Ex& ex)
    {
    ...
    void launch_continuation() {
      ...
      this->get_executor()->submit(lslboost::move(fct));
    }
...
#if defined BOOST_THREAD_FUTURE_BLOCKING
      lslboost::lock_guard<lslboost::mutex> lk(this->mutex);
      this->thr_ = lslboost::thread(&base_type::run, static_shared_from_this(this));
#else
      lslboost::thread(&base_type::run, static_shared_from_this(this)).detach();
#endif
    }
  ...
  public:
    ...
    virtual void execute(lslboost::unique_lock<lslboost::mutex>& lk) {
      this->parent.wait();
      this->call(lk);
    }
  ...
  make_future_deferred_continuation_shared_state(
    ...
    shared_ptr<future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new future_deferred_continuation_shared_state<F, Rp, Cont>(lslboost::move(f), lslboost::forward<Fp>(c)));
    h->init(lock);
    return BOOST_THREAD_FUTURE<Rp>(h);
  }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_async_continuation_shared_state(
      lslboost::unique_lock<lslboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_async_continuation_shared_state<F,Rp, Cont> >
        h(new future_async_continuation_shared_state<F,Rp, Cont>(lslboost::move(f), lslboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_sync_continuation_shared_state(
    ...
    shared_ptr<future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new future_sync_continuation_shared_state<F,Rp, Cont>(lslboost::move(f), lslboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<future_executor_continuation_shared_state<F,Rp, Cont> >
        h(new future_executor_continuation_shared_state<F,Rp, Cont>(lslboost::move(f), lslboost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  make_shared_future_deferred_continuation_shared_state(
    ...
    shared_ptr<shared_future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_deferred_continuation_shared_state<F, Rp, Cont>(f, lslboost::forward<Fp>(c)));
    h->init(lock);
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_shared_future_async_continuation_shared_state(
      lslboost::unique_lock<lslboost::mutex> &lock, F f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<shared_future_async_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_async_continuation_shared_state<F,Rp, Cont>(f, lslboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_sync_continuation_shared_state(
    ...
    shared_ptr<shared_future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_sync_continuation_shared_state<F,Rp, Cont>(f, lslboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<shared_future_executor_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_executor_continuation_shared_state<F, Rp, Cont>(f, lslboost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    ...
    lslboost::unique_lock<lslboost::mutex> lock(sentinel->mutex);

    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_future_async_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, lslboost::move(*this), lslboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_future_deferred_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, lslboost::move(*this), lslboost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
      ...
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_future_executor_continuation_shared_state<Ex, BOOST_THREAD_FUTURE<R>, future_type>(ex,
                    lock, lslboost::move(*this), lslboost::forward<F>(func)
                )));
#endif
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)  {
    ...
    return this->then(this->launch_policy(), lslboost::forward<F>(func));
#else
    typedef typename lslboost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_.get()!=0, future_uninitialized());
  ...
  template <typename R2>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename lslboost::result_of<F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >)>::type>
  BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_future_sync_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, lslboost::move(*this), lslboost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
    } else if (underlying_cast<int>(policy) & int(launch::executor)) {
  ...
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)  const
  {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, lslboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_shared_future_deferred_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, lslboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::sync)) {
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_shared_future_sync_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, lslboost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
    } else if (underlying_cast<int>(policy) & int(launch::executor)) {
      typedef executor Ex;
      Ex& ex = *(this->future_->get_executor());
      return BOOST_THREAD_MAKE_RV_REF((lslboost::detail::make_shared_future_executor_continuation_shared_state<Ex, shared_future<R>, future_type>(ex,
                    lock, *this, lslboost::forward<F>(func)
                )));
#endif
    } else if (underlying_cast<int>(policy) & int(launch::inherit)) {
    ...
    void launch_continuation()
    {
      ...
      if (! unwrapped.valid() )
      {
        if (wrapped.has_exception()) {
          ...
          {
            lk.unlock();
            lslboost::unique_lock<lslboost::mutex> lk2(unwrapped.future_->mutex);
            unwrapped.future_->set_continuation_ptr(this->shared_from_this(), lk2);
          } else {
            this->mark_exceptional_finish_internal(lslboost::copy_exception(future_uninitialized()), lk);
          }
        }
      } else {
...
namespace detail
{
  ...
  struct future_when_all_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<lslboost::detail::shared_state_base> that_) {
      ...
      try {
        lslboost::wait_for_all(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(lslboost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = lslboost::thread(&future_when_all_vector_shared_state::run, this->shared_from_this());
#else
      lslboost::thread(&future_when_all_vector_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  public:
    ...
    future_when_all_vector_shared_state(values_tag, BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
      vec_.push_back(lslboost::forward<T0>(f));
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(lslboost::forward<T>(futures)),'0'
          )..., '0'
  ...
  struct future_when_any_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<lslboost::detail::shared_state_base> that_)
    {
      ...
      try {
        lslboost::wait_for_any(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(lslboost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = lslboost::thread(&future_when_any_vector_shared_state::run, this->shared_from_this());
#else
      lslboost::thread(&future_when_any_vector_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  public:
    ...
    future_when_any_vector_shared_state(values_tag,
      ...
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(lslboost::forward<T>(futures))
          ,'0'
          )...,
          '0'
      }; //second part of magic unpacker
  ...
  struct wait_for_all_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      lslboost::wait_for_all(lslboost::forward<T>(v)...);
    }
  };
  ...
  struct wait_for_any_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      lslboost::wait_for_any(lslboost::forward<T>(v)...);
    }
  };
  ...
  struct future_when_all_tuple_shared_state: future_async_shared_state_base<Tuple>
  {
    ...
    static void run(shared_ptr<lslboost::detail::shared_state_base> that_) {
      ...
      try {
        // TODO make use of apply(that->tup_, lslboost::detail::wait_for_all_fctor());
        that->wait_for_all(Index());

        that->mark_finished_with_result(lslboost::move(that->tup_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = lslboost::thread(&future_when_all_tuple_shared_state::run, this->shared_from_this());
#else
      lslboost::thread(&future_when_all_tuple_shared_state::run, this->shared_from_this()).detach();
#endif
  ...
  public:
    ...
    future_when_all_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Fs) ... futures) :
      tup_(lslboost::csbl::make_tuple(lslboost::forward<F>(f), lslboost::forward<Fs>(futures)...))
    {
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = lslboost::thread(&future_when_any_tuple_shared_state::run, this->shared_from_this());
#else
      lslboost::thread(&future_when_any_tuple_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  when_all(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
    ...
    shared_ptr<factory_type>
        h(new factory_type(detail::values_tag_value, lslboost::forward<T0>(f), lslboost::forward<T>(futures)...));
    h->init();
    return BOOST_THREAD_FUTURE<container_type>(h);
  }
#endif
