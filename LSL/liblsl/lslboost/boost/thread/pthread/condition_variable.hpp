...
namespace lslboost
{
    ...
    inline void condition_variable::wait(unique_lock<mutex>& m)
    {
        ...
        if(! m.owns_lock())
        {
            lslboost::throw_exception(condition_error(-1, "lslboost::condition_variable::wait() failed precondition mutex not owned"));
        }
#endif
        int res=0;
        {
                ...
                struct timespec const &timeout)
    {
        ...
        if (!m.owns_lock())
        {
            lslboost::throw_exception(condition_error(EPERM, "lslboost::condition_variable::do_wait_until() failed precondition mutex not owned"));
        }
#endif
        int cond_res;
        {
        ...
        if(cond_res)
        {
            lslboost::throw_exception(condition_error(cond_res, "lslboost::condition_variable::do_wait_until failed in pthread_cond_timedwait"));
        }
        return true;
    }
    ...
    public:
        ...
        condition_variable_any()
        {
            ...
            if(res2)
            {
                ...
                lslboost::throw_exception(thread_resource_error(res2, "lslboost::condition_variable_any::condition_variable_any() failed in detail::monotonic_pthread_cond_init"));
            }
        }
        ~condition_variable_any()
        {
        ...
        void wait(lock_type& m)
        {
            ...
            if(res)
            {
                lslboost::throw_exception(condition_error(res, "lslboost::condition_variable_any::wait() failed in pthread_cond_wait"));
            }
        }
    ...
    private: // used by lslboost::thread::try_join_until
          ...
          struct timespec const &timeout)
        {
          ...
          if(res)
          {
              lslboost::throw_exception(condition_error(res, "lslboost::condition_variable_any::do_wait_until() failed in pthread_cond_timedwait"));
          }
          return true;
        }
    };
