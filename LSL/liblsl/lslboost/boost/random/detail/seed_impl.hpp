...
void generate_from_int(Engine& eng, Iter begin, Iter end)
{
    ...
    typedef typename lslboost::random::traits::make_unsigned<IntType>::type unsigned_type;
    int remaining_bits = 0;
    lslboost::uint_least32_t saved_bits = 0;
...
{
    return detail::generate_impl(eng, first, last, lslboost::random::traits::is_integral<typename Engine::result_type>());
}
...
IntType get_one_int(Iter& first, Iter last)
{
    ...
    for(int j = 0; j < k; ++j) {
        if(first == last) {
            lslboost::throw_exception(::std::invalid_argument("Not enough elements in call to seed."));
        }
...
{
    BOOST_STATIC_ASSERT_MSG(lslboost::is_integral<IntType>::value, "Sorry but this routine has not been ported to non built-in integers as it relies on a reinterpret_cast.");
...
{
    seed_array_int_impl<w>(seq, x, lslboost::random::traits::is_signed<IntType>());
}
...
{
    fill_array_int_impl<w>(first, last, x, lslboost::random::traits::is_signed<IntType>());
}
...
void fill_array_real(Iter& first, Iter last, RealType (&x)[n])
{
    ...
    for(j = 0; j < n; ++j) {
        ...
        for(int k = 0; k < w/32; ++k, ++first) {
            if(first == last) lslboost::throw_exception(std::invalid_argument("Not enough elements in call to seed."));
            val += *first * mult;
            mult *= two32;
        }
        if(mask != 0) {
