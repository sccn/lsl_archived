 ...
 *
 * Much of the code here taken by lslboost::math::hyperexponential_distribution.
...
class hyperexponential_distribution
{
    ...
    public: class param_type
    {
        ...
        public: template <typename RateIterT>
                param_type(RateIterT rate_first, 
                           RateIterT rate_last,  
                           typename lslboost::enable_if_c<lslboost::has_pre_increment<RateIterT>::value>::type* = 0)
    ...
    : dd_(std::vector<RealT>(lslboost::size(rate_range), 1)),
      rates_(lslboost::begin(rate_range), lslboost::end(rate_range))
    {
        assert( hyperexp_detail::check_params(dd_.probabilities(), rates_) );
    }
            ...
            RealT operator()(URNG& urng) const
    {
        ...
        return lslboost::random::exponential_distribution<RealT>(rates_[i])(urng);
    }
    ...
    {
        dd_.param(typename lslboost::random::discrete_distribution<int,RealT>::param_type(param.probabilities()));
        rates_ = param.rates();
    }
    ...
    private: lslboost::random::discrete_distribution<int,RealT> dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
    private: std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
