...
public:
   ...
   lslboost::intmax_t toi(const char_type*& p1, const char_type* p2, int radix)const
   {
...
private:
   ...
   lslboost::shared_ptr< ::lslboost::BOOST_REGEX_DETAIL_NS::icu_regex_traits_implementation> m_pimpl;
};
...
inline u32regex make_u32regex(InputIterator i, 
                              ...
                              lslboost::regex_constants::syntax_option_type opt)
{
   return BOOST_REGEX_DETAIL_NS::do_make_u32regex(i, j, opt, static_cast<lslboost::mpl::int_<sizeof(*i)> const*>(0));
}
//
...
{
   return BOOST_REGEX_DETAIL_NS::do_make_u32regex(p, p + std::strlen(p), opt, static_cast<lslboost::mpl::int_<1> const*>(0));
}
...
{
   return BOOST_REGEX_DETAIL_NS::do_make_u32regex(p, p + std::strlen(reinterpret_cast<const char*>(p)), opt, static_cast<lslboost::mpl::int_<1> const*>(0));
}
//
...
{
   return BOOST_REGEX_DETAIL_NS::do_make_u32regex(p, p + std::wcslen(p), opt, static_cast<lslboost::mpl::int_<sizeof(wchar_t)> const*>(0));
}
#endif
...
{
   return BOOST_REGEX_DETAIL_NS::do_make_u32regex(p, p + u_strlen(p), opt, static_cast<lslboost::mpl::int_<2> const*>(0));
}
#endif
//
...
{
   return BOOST_REGEX_DETAIL_NS::do_make_u32regex(s.begin(), s.end(), opt, static_cast<lslboost::mpl::int_<sizeof(C)> const*>(0));
}
//
...
{
   return BOOST_REGEX_DETAIL_NS::do_make_u32regex(s.getBuffer(), s.getBuffer() + s.length(), opt, static_cast<lslboost::mpl::int_<2> const*>(0));
}
                                 ...
                                 )
{
   ...
   else
   {
      ...
      while(i != j)
      {
         ...
         if(f.size())
            out = ::lslboost::BOOST_REGEX_DETAIL_NS::regex_format_imp(out, *i, &*f.begin(), &*f.begin() + f.size(), flags, e.get_traits());
         else
            out = ::lslboost::BOOST_REGEX_DETAIL_NS::regex_format_imp(out, *i, static_cast<UChar32 const*>(0), static_cast<UChar32 const*>(0), flags, e.get_traits());
         last_m = (*i)[0].second;
