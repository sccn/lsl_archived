...
public: 
   ...
   difference_type position(size_type sub = 0) const
   {
      ...
      if(sub < m_subs.size())
      {
         ...
         if(s.matched || (sub == 2))
         {
            return ::lslboost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)(m_base), (BidiIterator)(s.first));
         }
      }
...
void BOOST_REGEX_CALL match_results<BidiIterator, Allocator>::maybe_assign(const match_results<BidiIterator, Allocator>& m)
{
   ...
   for(i = 0; i < size(); ++i, ++p1, ++p2)
   {
      ...
      base1 = ::lslboost::BOOST_REGEX_DETAIL_NS::distance(l_base, p1->first);
      base2 = ::lslboost::BOOST_REGEX_DETAIL_NS::distance(l_base, p2->first);
      BOOST_ASSERT(base1 >= 0);
      BOOST_ASSERT(base2 >= 0);
      if(base1 < base2) return;
      if(base2 < base1) break;

      len1 = ::lslboost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)p1->first, (BidiIterator)p1->second);
      len2 = ::lslboost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)p2->first, (BidiIterator)p2->second);
      BOOST_ASSERT(len1 >= 0);
      BOOST_ASSERT(len2 >= 0);
