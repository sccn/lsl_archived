...
public:
   ...
   */
  explicit strand(lslboost::asio::io_context& io_context)
    : service_(lslboost::asio::use_service<
        lslboost::asio::detail::strand_service>(io_context))
  {
  ...
  lslboost::asio::io_context& context() const BOOST_ASIO_NOEXCEPT
  {
    return service_.get_io_context();
  }
...
#if !defined(BOOST_ASIO_NO_DEPRECATED)
  /// (Deprecated: Use lslboost::asio::dispatch().) Request the strand to invoke
  /// the given handler.
  /**
...
#if !defined(BOOST_ASIO_NO_DEPRECATED)
  /// (Deprecated: Use lslboost::asio::post().) Request the strand to invoke the
   ...
   * @code strand.dispatch(lslboost::bind(f, a1, ... an)); @endcode
   */
...
private:
  lslboost::asio::detail::strand_service& service_;
  mutable lslboost::asio::detail::strand_service::implementation_type impl_;
};
