...
kqueue_reactor::kqueue_reactor(lslboost::asio::io_service& io_service)
  : lslboost::asio::detail::service_base<kqueue_reactor>(io_service),
    ...
    shutdown_(false)
{
  ...
  {
    lslboost::system::error_code error(errno,
        lslboost::asio::error::get_system_category());
    lslboost::asio::detail::throw_error(error);
  }
}
...
void kqueue_reactor::fork_service(lslboost::asio::io_service::fork_event fork_ev)
{
  if (fork_ev == lslboost::asio::io_service::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      lslboost::system::error_code ec(errno,
          lslboost::asio::error::get_system_category());
      lslboost::asio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      if (state->num_kevents_ > 0)
      {
        ...
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          lslboost::system::error_code ec(errno,
              lslboost::asio::error::get_system_category());
          lslboost::asio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        {
          op->ec_ = lslboost::system::error_code(errno,
              lslboost::asio::error::get_system_category());
          io_service_.post_immediate_completion(op, is_continuation);
          return;
        }
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    lslboost::system::error_code ec(errno,
        lslboost::asio::error::get_system_category());
    lslboost::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
