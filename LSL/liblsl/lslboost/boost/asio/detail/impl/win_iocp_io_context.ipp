...
win_iocp_io_context::win_iocp_io_context(
    lslboost::asio::execution_context& ctx, int concurrency_hint)
    ...
    concurrency_hint_(concurrency_hint)
{
  ...
  if (!iocp_.handle)
  {
    ...
    lslboost::asio::detail::throw_error(ec, "iocp");
  }
}
...
lslboost::system::error_code win_iocp_io_context::register_handle(
    HANDLE handle, lslboost::system::error_code& ec)
{
...
size_t win_iocp_io_context::run(lslboost::system::error_code& ec)
{
...
size_t win_iocp_io_context::run_one(lslboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::wait_one(long usec, lslboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll(lslboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll_one(lslboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
void win_iocp_io_context::on_completion(win_iocp_operation* op,
    const lslboost::system::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_context::do_one(DWORD msec, lslboost::system::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      lslboost::system::error_code result_ec(last_error,
          lslboost::asio::error::get_system_category());
      ...
      {
        result_ec = lslboost::system::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<lslboost::system::error_category*>(op->Internal));
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      lslboost::system::error_code ec(last_error,
          lslboost::asio::error::get_system_category());
      lslboost::asio::detail::throw_error(ec, "timer");
    }
