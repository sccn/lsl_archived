...
/// \brief Contains helper macros and implementation details of lslboost::typeindex::ctti_type_index.
...
#   include <boost/preprocessor/facilities/expand.hpp>
    ...
    // sizeof("const char *__cdecl lslboost::detail::ctti<") - 1, sizeof(">::n(void)") - 1
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(40, 10, false, "")
#elif defined(_MSC_VER) && !defined (BOOST_NO_CXX11_NOEXCEPT)
    // sizeof("const char *__cdecl lslboost::detail::ctti<") - 1, sizeof(">::n(void) noexcept") - 1
    ...
    // sizeof("static const char *lslboost::detail::ctti<") - 1, sizeof("]") - 1, true, "???????????>::n() [T = int"
    ...
    // sizeof("static const char *lslboost::detail::ctti<") - 1, sizeof(">::n()") - 1
    // note: checked on 3.0
    ...
    // sizeof("static const char *lslboost::detail::ctti<") - 1, sizeof("]") - 1, true, "int>::n() [T = int"
    // note: checked on 3.1, 3.4
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(39, 1, true, "T = ")
#elif defined(__GNUC__) && (__GNUC__ < 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static constexpr char lslboost::detail::ctti<T>::s() [with unsigned int I = 0u; T = ") - 1, sizeof("]") - 1
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(81, 1, false, "")
#elif defined(__GNUC__) && (__GNUC__ >= 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static constexpr char lslboost::detail::ctti<T>::s() [with unsigned int I = 0; T = ") - 1, sizeof("]") - 1
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(80, 1, false, "")
#elif defined(__GNUC__) && defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static const char* lslboost::detail::ctti<T>::n() [with T = ") - 1, sizeof("]") - 1
...
namespace lslboost { namespace typeindex { namespace detail {
    ...
    BOOST_CXX14_CONSTEXPR inline const char* skip_begining_runtime(const char* begin, lslboost::mpl::false_) BOOST_NOEXCEPT {
        return begin;
    }
    ...
    template <unsigned int ArrayLength>
    BOOST_CXX14_CONSTEXPR inline const char* skip_begining_runtime(const char* begin, lslboost::mpl::true_) BOOST_NOEXCEPT {
    ...
    BOOST_CXX14_CONSTEXPR inline const char* skip_begining(const char* begin) BOOST_NOEXCEPT {
        ...
        return skip_begining_runtime<ArrayLength - ctti_skip_size_at_begin>(
            ...
            lslboost::mpl::bool_<ctti_skip_more_at_runtime>()
        );
    }
...
struct ctti {
    ...
    constexpr static const char* impl(::lslboost::typeindex::detail::index_seq<Indexes...> ) BOOST_NOEXCEPT {
        return ::lslboost::typeindex::detail::cstring<s<Indexes>()...>::data_;
    }
    ...
    #else
        lslboost::typeindex::detail::failed_to_get_function_name();
    #endif

        lslboost::typeindex::detail::assert_compile_time_legths<
            (size > lslboost::typeindex::detail::ctti_skip_size_at_begin + lslboost::typeindex::detail::ctti_skip_size_at_end + sizeof("const *") - 1)
        >();
        static_assert(!lslboost::typeindex::detail::ctti_skip_more_at_runtime, "Skipping for GCC in C++14 mode is unsupported");

        typedef typename lslboost::typeindex::detail::make_index_seq_impl<
            lslboost::typeindex::detail::ctti_skip_size_at_begin,
            size - sizeof("const *") + 1 - lslboost::typeindex::detail::ctti_skip_size_at_begin
        >::type idx_seq;
        return impl(idx_seq());
    }
#else
    ...
    #if defined(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE)
        return lslboost::typeindex::detail::skip_begining< sizeof(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE) >(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE);
    #elif defined(__FUNCSIG__)
        return lslboost::typeindex::detail::skip_begining< sizeof(__FUNCSIG__) >(__FUNCSIG__);
    #elif defined(__PRETTY_FUNCTION__) \
                || defined(__GNUC__) \
                || (defined(__SUNPRO_CC) && (__SUNPRO_CC >= 0x5130)) \
                || (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) \
                ...
                || defined(__ghs__) \
                || defined(__DMC__)
        return lslboost::typeindex::detail::skip_begining< sizeof(__PRETTY_FUNCTION__) >(__PRETTY_FUNCTION__);
    #else
        lslboost::typeindex::detail::failed_to_get_function_name();
        return "";
    #endif
