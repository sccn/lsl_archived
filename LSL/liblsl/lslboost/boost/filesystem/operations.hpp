...
namespace lslboost
{
  namespace filesystem
  {
    ...
    private:
      ...
      lslboost::shared_ptr<m_imp> m_imp_ptr;
    };
...
# endif
  ...
  struct space_info
  {
    ...
    lslboost::uintmax_t capacity;
    lslboost::uintmax_t free;      // <= capacity
    lslboost::uintmax_t available; // <= free
  };
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    BOOST_FILESYSTEM_DECL
    lslboost::uintmax_t file_size(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    lslboost::uintmax_t hard_link_count(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    ...
    BOOST_FILESYSTEM_DECL
    lslboost::uintmax_t remove_all(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
...
# endif
  ...
  inline
  lslboost::uintmax_t file_size(const path& p) {return detail::file_size(p);}

  inline
  lslboost::uintmax_t file_size(const path& p, system::error_code& ec) BOOST_NOEXCEPT
                                       {return detail::file_size(p, &ec);}
  inline
  lslboost::uintmax_t hard_link_count(const path& p) {return detail::hard_link_count(p);}

  inline
  lslboost::uintmax_t hard_link_count(const path& p, system::error_code& ec) BOOST_NOEXCEPT
  ...
  inline
  lslboost::uintmax_t remove_all(const path& p) {return detail::remove_all(p);}
    
  inline
  lslboost::uintmax_t remove_all(const path& p, system::error_code& ec) BOOST_NOEXCEPT
...
{
public:
  typedef lslboost::filesystem::path::value_type value_type;   // enables class path ctor taking directory_entry

  directory_entry() BOOST_NOEXCEPT {}
  explicit directory_entry(const lslboost::filesystem::path& p)
    : m_path(p), m_status(file_status()), m_symlink_status(file_status())
    {}
  directory_entry(const lslboost::filesystem::path& p,
  ...
  void assign(const lslboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
    { m_path = p; m_status = st; m_symlink_status = symlink_st; }

  void replace_filename(const lslboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
  {
...
# ifndef BOOST_FILESYSTEM_NO_DEPRECATED
  void replace_leaf(const lslboost::filesystem::path& p,
    file_status st, file_status symlink_st)
      { replace_filename(p, st, symlink_st); }
# endif

  const lslboost::filesystem::path&  path() const BOOST_NOEXCEPT {return m_path;}
  operator const lslboost::filesystem::path&() const BOOST_NOEXCEPT
                                                              {return m_path;}
...
private:
  lslboost::filesystem::path   m_path;
...
//--------------------------------------------------------------------------------------//
  ...
  class directory_iterator
    : public lslboost::iterator_facade< directory_iterator,
                                     directory_entry,
                                     lslboost::single_pass_traversal_tag >
  {
  public:
  ...
  private:
    ...
    lslboost::shared_ptr< detail::dir_itr_imp >  m_imp;

    friend class lslboost::iterator_core_access;

    lslboost::iterator_facade<
      directory_iterator,
      directory_entry,
      lslboost::single_pass_traversal_tag >::reference dereference() const 
    {
  ...
  //  namespace lslboost template specializations
  template<>
  struct range_mutable_iterator<lslboost::filesystem::directory_iterator>
    { typedef lslboost::filesystem::directory_iterator type; };
  template<>
  struct range_const_iterator <lslboost::filesystem::directory_iterator>
    { typedef lslboost::filesystem::directory_iterator type; };
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    //  clients of struct 'lslboost::filesystem::detail::recur_dir_itr_imp'
...
#   endif
  ...
  private:
    ...
    lslboost::shared_ptr< detail::recur_dir_itr_imp >  m_imp;
    ...
    lslboost::iterator_facade< 
      recursive_directory_iterator,
      directory_entry,
      lslboost::single_pass_traversal_tag >::reference
    dereference() const 
    {
      BOOST_ASSERT_MSG(m_imp.get(),
  ...
  //  namespace lslboost template specializations
  template<>
  struct range_mutable_iterator<lslboost::filesystem::recursive_directory_iterator>
                        { typedef lslboost::filesystem::recursive_directory_iterator type; };
  template<>
  struct range_const_iterator <lslboost::filesystem::recursive_directory_iterator>
                        { typedef lslboost::filesystem::recursive_directory_iterator type; };
