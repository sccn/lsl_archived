...
namespace lslboost {  namespace movelib{
...
{
   typedef typename lslboost::movelib::iterator_traits<BirdirectionalIterator>::value_type value_type;
   if (first != last){
      BirdirectionalIterator i = first;
      for (++i; i != last; ++i){
         BirdirectionalIterator j = i;
         if (comp(*i,  *--j)) {
            value_type tmp(::lslboost::move(*i));
            *i = ::lslboost::move(*j);
