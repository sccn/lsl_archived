...
BOOST_STATIC_ASSERT((AdaptiveSortInsertionSortThreshold&(AdaptiveSortInsertionSortThreshold-1)) == 0);
   ...
   typedef ::lslboost::uintptr_t uintptr_t;
#else
...
class adaptive_xbuf
{
   ...
   {
      if(n <= m_size){
         lslboost::move(first, first+n, m_ptr);
         ...
         RandRawIt result = lslboost::move(first, first+m_size, m_ptr);
         lslboost::uninitialized_move(first+m_size, first+n, result);
         m_size = n;
      }
   }
   ...
   void push_back(RandIt first, std::size_t n)
   {
      ...
      lslboost::uninitialized_move(first, first+n, m_ptr+m_size);
      m_size += n;
   }
   ...
   void insert(iterator pos, RandIt it)
   {
      if(pos == (m_ptr + m_size)){
         ...
         //m_size updated
         lslboost::move_backward(pos, m_ptr+m_size-2, m_ptr+m_size-1);
         *pos = lslboost::move(*it);
      }
   }
   ...
   void initialize_until(std::size_t const size, T &t)
   {
      BOOST_ASSERT(m_size < m_capacity);
      if(m_size < size){
         ::new((void*)&m_ptr[m_size]) T(::lslboost::move(t));
         ++m_size;
         for(; m_size != size; ++m_size){
            ::new((void*)&m_ptr[m_size]) T(::lslboost::move(m_ptr[m_size-1]));
         }
         t = ::lslboost::move(m_ptr[m_size-1]);
      }
   }
...
{
   if(is_next_far_away){
      ::lslboost::adl_move_swap_ranges(begin, end, with);
      ::lslboost::adl_move_swap(*key_next, *key_range2);
      if(key_next == key_mid){
         key_mid = key_range2;
      }
   ...
   (RandIt first1, RandIt last1, RandIt const last2, bool *const pis_range1_A, Compare comp)
{
   ...
   if(first1 != last1 && comp(*last1, last1[-1])){
      do{
         ...
         last1  = lslboost::movelib::lower_bound(last1, last2, *first1, comp);
      ...
      , Buf &xbuf)
{
   if(first != middle && middle != last && comp(*middle, middle[-1])){
      ...
      if(len1 <= len2){
         first = lslboost::movelib::upper_bound(first, middle, *middle, comp);
         xbuf.move_assign(first, size_type(middle-first));
         op_merge_with_right_placed
            (xbuf.data(), xbuf.end(), first, middle, last, comp, op);
      }
      else{
         last = lslboost::movelib::lower_bound(middle, last, middle[-1], comp);
   ...
   if(max_collected){
      ...
      if(xbuf.capacity() >= max_collected){
         ...
         while(u != last && h < max_collected){
            typename XBuf::iterator const r = lslboost::movelib::lower_bound(ph0, xbuf.end(), *u, comp);
            //If key not found add it to [h, h+h0)
            if(r == xbuf.end() || comp(*u, *r) ){
               RandIt const new_h0 = lslboost::move(search_end, u, h0);
               search_end = u;
               ++search_end;
               ++h;
               xbuf.insert(r, u);
         ...
         lslboost::move_backward(first, h0, h0+h);
         lslboost::move(xbuf.data(), xbuf.end(), first);
      }
      else{
         while(u != last && h < max_collected){
            RandIt const r = lslboost::movelib::lower_bound(h0, search_end, *u, comp);
            //If key not found add it to [h, h+h0)
...
{
   lslboost::movelib::inplace_stable_sort(first, last, comp);
}
              ...
              , bool const xbuf_used)
{
   ...
   if(xbuf_used){
      lslboost::move_backward(cur_pos, cur_pos+l_data, new_pos+l_data);      
   }
   else{
      lslboost::adl_move_swap_ranges_backward(cur_pos, cur_pos+l_data, new_pos+l_data);      
              ...
              , bool const xbuf_used)
{
   //Move buffer to the total combination right
   if(xbuf_used){
      lslboost::move(cur_pos, cur_pos+l_data, new_pos);
   }
   else{
      lslboost::adl_move_swap_ranges(cur_pos, cur_pos+l_data, new_pos);
      //Rotate does less moves but it seems slower due to cache issues
      //rotate_gcd(first-l_block, first+len-l_block, first+len);
   }
}
   ...
   , Compare comp, bool const is_stable, Op op)
{
   ...
   for(; n_block_left; --n_block_left, ++key_first, min_check -= min_check != 0, max_check -= max_check != 0){
      ...
      if(first_reg == dest){
         dest = next_key_idx ? ::lslboost::adl_move_swap_ranges(first_min, last_min, first_reg)
                             : last_reg;
      }
      else{
      ...
      , Compare comp)
{
   ...
   if(kbuf){
      ...
      if(kbuf != l_build_buf){
         lslboost::move(xbuf.data()+kbuf-l_merged, xbuf.data() + kbuf, first_block-l_merged+elements_in_blocks);
      }
   }
   else{
   ...
   if(kbuf && kbuf == l_build_buf){
      ...
      lslboost::move(xbuf.data(), xbuf.data() + kbuf, first);
   }
   else{
   ...
   , Compare comp)
{
   ...
   if(common_xbuf){
      if(buffer_right){
         lslboost::move(xbuf.data(), xbuf.data() + l_intbuf, buffer+l_data);
      }
      else{
         lslboost::move(xbuf.data(), xbuf.data() + l_intbuf, buffer);
      }
   }
   return buffer_right;
}
