...
#include <boost/move/utility_core.hpp> //for lslboost::move

#if !defined(BOOST_MOVE_DOXYGEN_INVOKED)

#if defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
namespace lslboost_move_member_swap {
...
namespace lslboost_move_adl_swap{
...
template<class T>
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y, typename lslboost::move_detail::enable_if_c<!lslboost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename lslboost::move_detail::enable_if< and_op_not_impl<lslboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , lslboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::lslboost::move(x)); x = ::lslboost::move(y); y = ::lslboost::move(t);  }

template<class T>
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename lslboost::move_detail::enable_if< and_op_impl< lslboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , lslboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::lslboost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::lslboost::move(x)); x = ::lslboost::move(y); y = ::lslboost::move(t);</code>.
template<class T>
BOOST_MOVE_FORCEINLINE void adl_move_swap(T& x, T& y)
{
   ::lslboost_move_adl_swap::swap_proxy(x, y);
}

//! Exchanges elements between range [first1, last1) and another range starting at first2
//! using lslboost::adl_move_swap.
//! 
//! Parameters:
...
{
    while (first1 != last1) {
      ::lslboost::adl_move_swap(*first1, *first2);
      ++first1;
      ++first2;
    }
   return first2;
...
{
   while (first1 != last1) {
      ::lslboost::adl_move_swap(*(--last1), *(--last2));
   }
   return last2;
}
