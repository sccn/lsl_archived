...
namespace lslboost { namespace spirit
{
  namespace detail
  {
    ...
    struct unrolled_byte_loops
    {
      ...
      static typename lslboost::make_unsigned<T>::type load_big(const unsigned char* bytes)
        { return *(bytes - 1) | (next::load_big(bytes - 1) << 8); }
      static typename lslboost::make_unsigned<T>::type load_little(const unsigned char* bytes)
