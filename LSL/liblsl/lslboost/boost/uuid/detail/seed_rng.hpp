...
//#include <boost/nondet_random.hpp> //forward declare lslboost::random::random_device
...
public:
    ...
    ~seed_rng() BOOST_NOEXCEPT
    {
        if (random_) {
#if defined(BOOST_WINDOWS)
            lslboost::winapi::CryptReleaseContext(random_, 0);
#else
            std::fclose(random_);
#endif
        }
...
#if defined(BOOST_WINDOWS)
            lslboost::winapi::CryptGenRandom(random_, sizeof(state), state);
#else
...
#if defined(BOOST_WINDOWS)
            lslboost::winapi::DWORD_ procid = lslboost::winapi::GetCurrentProcessId();
            sha.process_bytes( (unsigned char const*)&procid, sizeof( procid ) );

            lslboost::winapi::DWORD_ threadid = lslboost::winapi::GetCurrentThreadId();
            sha.process_bytes( (unsigned char const*)&threadid, sizeof( threadid ) );

            lslboost::winapi::LARGE_INTEGER_ ts;
            ts.QuadPart = 0;
            lslboost::winapi::QueryPerformanceCounter( &ts );
...
#if defined(BOOST_WINDOWS)
    lslboost::winapi::HCRYPTPROV_ random_;
#else
    std::FILE * random_;
#endif
};
