...
//
...
#endif
    BOOST_SP_NOEXCEPT : px( r.lock().get() ), pn( static_cast< lslboost::detail::weak_count && >( r.pn ) )
    {
        lslboost::detail::sp_assert_convertible< Y, T >();
        r.px = 0;
    }
    ...
    weak_ptr( weak_ptr && r )
    BOOST_SP_NOEXCEPT : px( r.px ), pn( static_cast< lslboost::detail::weak_count && >( r.pn ) )
    {
        r.px = 0;
    }
