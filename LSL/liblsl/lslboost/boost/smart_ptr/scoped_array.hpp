...
template<class T> inline bool operator==( scoped_array<T> const & p, lslboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( lslboost::detail::sp_nullptr_t, scoped_array<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
