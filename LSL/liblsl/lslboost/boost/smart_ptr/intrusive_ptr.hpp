...
// Move support
    ...
    intrusive_ptr(intrusive_ptr<U> && rhs, typename lslboost::detail::sp_enable_if_convertible<U,T>::type = lslboost::detail::sp_empty())
...
template<class T> inline bool operator==( intrusive_ptr<T> const & p, lslboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( lslboost::detail::sp_nullptr_t, intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
template< class T > std::size_t hash_value( lslboost::intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
