...
template <
  ...
  class T6, class T7, class T8,
  class T9>
struct is_boost_tuple< lslboost::tuples::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> >
{
...
struct disable_if_boost_tuple
   : lslboost::container::container_detail::disable_if< is_boost_tuple<Tuple> >
{};
...
template<>
struct is_tuple_null<lslboost::tuples::null_type>
{
   static const bool value = true;
};
...
struct piecewise_construct_use
{
   ...
   {  (void)&::lslboost::container::piecewise_construct;   }
};
...
struct pair
{
   ...
   pair(try_emplace_t, BOOST_FWD_REF(KeyType) k, Args && ...args)
      : first(lslboost::forward<KeyType>(k)), second(::lslboost::forward<Args>(args)...)\
   {}
   #else

   //piecewise construction from lslboost::tuple
   ...
   pair( try_emplace_t, BOOST_FWD_REF(KeyType) k BOOST_MOVE_I##N BOOST_MOVE_UREF##N )\
      : first(lslboost::forward<KeyType>(k)), second(BOOST_MOVE_FWD##N)\
   {}\
   //
   ...
   pair( piecewise_construct_t\
       , BoostTuple<BOOST_MOVE_TARG##N  BOOST_MOVE_I##N BOOST_MOVE_REPEAT(BOOST_MOVE_SUB(10,N),::lslboost::tuples::null_type)> p\
       , BoostTuple<BOOST_MOVE_TARGQ##M BOOST_MOVE_I##M BOOST_MOVE_REPEAT(BOOST_MOVE_SUB(10,M),::lslboost::tuples::null_type)> q\
       , typename container_detail::enable_if_c\
         < pair_impl::is_boost_tuple< BoostTuple<BOOST_MOVE_TARG##N  BOOST_MOVE_I##N BOOST_MOVE_REPEAT(BOOST_MOVE_SUB(10,N),::lslboost::tuples::null_type)> >::value &&\
   ...
   #  if !defined(BOOST_CONTAINER_NO_CXX11_DELEGATING_CONSTRUCTORS)
      ...
      pair(Tuple<Args1...>& t1, Tuple<Args2...>& t2, index_tuple<Indexes1...>, index_tuple<Indexes2...>)
         : first (::lslboost::forward<Args1>(get<Indexes1>(t1))...)
         , second(::lslboost::forward<Args2>(get<Indexes2>(t2))...)
      {  (void) t1; (void)t2; }
   ...
   #  else
      ...
      {  return do_build_from_args<T>(::lslboost::move(t), typename build_number_seq<sizeof...(Args)>::type());   }
      ...
      {  (void)t; return T(::lslboost::forward<Args>(get<Indexes>(t))...);  }

      public:
      template< template<class ...> class Tuple, class... Args1, class... Args2
              , class = typename pair_impl::disable_if_boost_tuple< Tuple<Args1...> >::type>
      pair(piecewise_construct_t, Tuple<Args1...> t1, Tuple<Args2...> t2)
         : first  (build_from_args<first_type> (::lslboost::move(t1)))
         , second (build_from_args<second_type>(::lslboost::move(t2)))
      {}
   #  endif   //BOOST_NO_CXX11_VARIADIC_TEMPLATES
   ...
   template <class D, class S>
   typename ::lslboost::container::container_detail::disable_if_or
      < pair &
      , ::lslboost::container::container_detail::is_same<T1, D>
      , ::lslboost::container::container_detail::is_same<T2, S>
      >::type
...
//std::pair copy assignment
   ...
   void swap(pair& p)
   {
      ::lslboost::adl_move_swap(this->first, p.first);
      ::lslboost::adl_move_swap(this->second, p.second);
   }
};
...
template<class T1, class T2>
struct has_move_emulation_enabled< ::lslboost::container::container_detail::pair<T1, T2> >
{
   static const bool value = true;
};
...
template <class T1, class T2>
struct is_class_or_union< ::lslboost::container::container_detail::pair<T1, T2> >
...
template <class T1, class T2>
struct is_union< ::lslboost::container::container_detail::pair<T1, T2> >
//This specialization is needed to avoid instantiation of pair in
//is_class, and allow recursive maps.
{
   static const bool value = false;
...
template <class T1, class T2>
struct is_class< ::lslboost::container::container_detail::pair<T1, T2> >
//This specialization is needed to avoid instantiation of pair in
//is_class, and allow recursive maps.
{
   static const bool value = true;
