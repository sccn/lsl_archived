...
struct get_flat_tree_iterators
{
   ...
   typedef typename lslboost::container::container_detail::
...
{
   public:
   typedef lslboost::container::vector<Value, Allocator>    sequence_type;
      ...
      : public value_compare
   {
      ...
      Data(BOOST_RV_REF(Data) d)
         : value_compare(lslboost::move(static_cast<value_compare&>(d))), m_seq(lslboost::move(d.m_seq))
      {}
      ...
      Data(BOOST_RV_REF(Data) d, const Allocator &a)
         : value_compare(lslboost::move(static_cast<value_compare&>(d))), m_seq(lslboost::move(d.m_seq), a)
      {}
      ...
      {
         this->value_compare::operator=(lslboost::move(static_cast<value_compare &>(d)));
         m_seq = lslboost::move(d.m_seq);
         return *this;
      }
      ...
      void swap(Data &d)
      {
         ...
         lslboost::adl_move_swap(mycomp, othercomp);
   ...
   BOOST_CONTAINER_FORCEINLINE flat_tree(BOOST_RV_REF(flat_tree) x, const allocator_type &a)
      :  m_data(lslboost::move(x.m_data), a)
   { }
   ...
   {  m_data = lslboost::move(x.m_data); return *this;  }
   ...
   std::pair<iterator,bool> insert_unique(BOOST_RV_REF(value_type) val)
   {
      ...
      insert_commit_data data;
      ret.second = this->priv_insert_unique_prepare(KeyOfValue()(val), data);
      ret.first = ret.second ? this->priv_insert_commit(data, lslboost::move(val))
                             : iterator(vector_iterator_get_ptr(data.position));
      return ret;
   }
   ...
   iterator insert_equal(BOOST_RV_REF(value_type) mval)
   {
      ...
      i = this->m_data.m_seq.insert(i, lslboost::move(mval));
      return i;
   }
   ...
   iterator insert_unique(const_iterator hint, BOOST_RV_REF(value_type) val)
   {
      ...
      insert_commit_data data;
      return this->priv_insert_unique_prepare(hint, KeyOfValue()(val), data)
         ? this->priv_insert_commit(data, lslboost::move(val))
         : iterator(vector_iterator_get_ptr(data.position));
   }
   ...
   iterator insert_equal(const_iterator hint, BOOST_RV_REF(value_type) mval)
   {
      ...
      return this->priv_insert_commit(data, lslboost::move(mval));
   }
   ...
   {
      const size_type len = static_cast<size_type>(lslboost::container::iterator_distance(first, last));
      ...
      (const_iterator hint, BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(Args)... args)
   {
      ...
      if(!ret.second){
         ...
         func_t func(try_emplace_t(), ::lslboost::forward<KeyType>(key), ::lslboost::forward<Args>(args)...);
   ...
   {\
      ...
      if(!ret.second){\
         ...
         typedef emplace_iterator<value_type, func_t, difference_type> it_t;\
         func_t func(try_emplace_t(), ::lslboost::forward<KeyType>(key) BOOST_MOVE_I##N BOOST_MOVE_FWD##N);\
         ret.first = this->m_data.m_seq.insert(data.position, it_t(func), it_t());\
      }\
      return ret;\
   }\
   ...
   std::pair<iterator, bool> insert_or_assign(const_iterator hint, BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(M) obj)
   {
      ...
      if(!ret.second){
         ret.first  = this->nth(data.position - this->cbegin());
         ret.first->second = lslboost::forward<M>(obj);
      }
      else{
         typedef typename emplace_functor_type<KeyType, M>::type func_t;
         typedef emplace_iterator<value_type, func_t, difference_type> it_t;
         func_t func(lslboost::forward<KeyType>(key), lslboost::forward<M>(obj));
         ret.first = this->m_data.m_seq.insert(data.position, it_t(func), it_t());
      }
      return ret;
   }
   ...
   {
      this->insert( lslboost::make_move_iterator(source.begin())
                  , lslboost::make_move_iterator(source.end()));
   }
   ...
   void merge_unique(flat_tree& source)
   {
      this->m_data.m_seq.merge_unique
         ( lslboost::make_move_iterator(source.begin())
         , lslboost::make_move_iterator(source.end())
   ...
   {
      return lslboost::move(m_data.m_seq);
   }
   ...
   void adopt_sequence_equal(BOOST_RV_REF(sequence_type) seq)
   {
      ...
      lslboost::movelib::adaptive_sort
         ( lslboost::movelib::iterator_to_raw_pointer(seq.begin())
         , lslboost::movelib::iterator_to_raw_pointer(seq.end())
         , this->priv_value_comp()
         , lslboost::movelib::iterator_to_raw_pointer(tseq.begin() + tseq.size())
         , tseq.capacity() - tseq.size());
      tseq = lslboost::move(seq);
   }
   ...
   void adopt_sequence_unique(BOOST_RV_REF(sequence_type) seq)
   {
      ...
      lslboost::movelib::adaptive_sort
         ...
         , lslboost::movelib::iterator_to_raw_pointer(tseq.begin() + tseq.size())
         , tseq.capacity() - tseq.size());
      seq.erase( lslboost::movelib::unique
                  (seq.begin(), seq.end(), lslboost::movelib::negate<value_compare>(this->m_data.get_comp()))
               , seq.cend());
      tseq = lslboost::move(seq);
   }
...
class Compare, class Allocator>
struct has_trivial_destructor_after_move<lslboost::container::container_detail::flat_tree<T, KeyOfValue, Compare, Allocator> >
{
