...
namespace lslboost {
   ...
   namespace detail
   {
      ...
      template <class T>          struct has_nothrow_assign_imp<T, false, true>{ static const bool value = noexcept(lslboost::declval<typename add_reference<T>::type>() = lslboost::declval<typename add_reference<T const>::type>()); };
   ...
   struct has_nothrow_assign : public integral_constant < bool,
      ...
      >::value
#else
      ::lslboost::has_trivial_assign<T>::value
#endif
#else
