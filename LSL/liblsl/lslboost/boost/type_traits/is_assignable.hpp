...
namespace lslboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(lslboost::declval<T>() = lslboost::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(lslboost::type_traits::yes_type)>{};
