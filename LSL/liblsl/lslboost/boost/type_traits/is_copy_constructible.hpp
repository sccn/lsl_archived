...
template <class T> struct is_copy_constructible : public lslboost::is_constructible<T, const T&>{};
...
namespace lslboost {
   ...
   namespace detail
   {
      ...
      template <class T, bool b> struct is_copy_constructible_imp : public lslboost::is_constructible<T, const T&>{};
...
namespace lslboost {
   ...
   namespace detail{
      ...
      struct is_copy_constructible_impl2 {
         ...
         template <class T1>
         static lslboost::type_traits::yes_type test(const T1&, lslboost::mpl::int_<sizeof(T1(lslboost::declval<const T1&>()))>* = 0);
#else
         template <class T1>
         static lslboost::type_traits::yes_type test(const T1&, decltype(T1(lslboost::declval<const T1&>()))* = 0);
#endif

         static lslboost::type_traits::no_type test(...);
#else
         template <class T1>
         static lslboost::type_traits::no_type test(const T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(
            lslboost::declval<BOOST_DEDUCED_TYPENAME lslboost::add_reference<T const>::type>()
   ...
   template <class T> struct is_copy_constructible : public integral_constant<bool, ::lslboost::detail::is_copy_constructible_impl<T>::value>{};
   template <> struct is_copy_constructible<void> : public false_type{};
