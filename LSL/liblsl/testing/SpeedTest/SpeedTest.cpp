...
// generate a random sample
void rand_sample(int numchan, vector<double> &sample) {
	...
	try {
		...
		for (double endtime = now()+duration;now()<endtime;) {
			// run a single execution of the outlet
			{
				...
				vector<double> sample(numchan);
				vector<vector<double> > chunk(max(0,chunk_len),vector<double>(numchan));
				if (chunk_len<=0) {
					rand_sample(numchan,sample);
				} else {
					for (unsigned s=0;s<chunk_len;s++)
						rand_sample(numchan,chunk[s]);
				}
				for (double failtime = now()+seconds_between_failures;seconds_between_failures<0 || now()<failtime;) {
					if (chunk_len<=0) {
...
int main(int argc, char* argv[]) {
	//boost::thread highrate_outlet_thread(boost::bind(&run_outlet,1000000,"HighRateStream","Audio",1,lsl::cf_int8,100000,-1,64));
	//boost::thread highrate_inlet_thread(boost::bind(&run_inlet,1000000,"HighRateStream","Audio",true,false,false,-1));
