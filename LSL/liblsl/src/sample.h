	...
	public:
		...
		template<class StreamBuf, typename T> static void save_value(StreamBuf &sb, const T &v, int use_byte_order) {
			if (sizeof(T)>1 && use_byte_order != BOOST_BYTE_ORDER) {
				T temp = lsllslboost::endian::reverse_value(v);
		...
		void convert_endian(void *data) const {
			switch (format_sizes[format_]) {
				case 1: break;
				case sizeof(lslboost::int16_t): for (lslboost::int16_t *p=(lslboost::int16_t*)data,*e=p+num_channels_; p<e; lsllslboost::endian::reverse(*p++)); break;
				case sizeof(lslboost::int32_t): for (lslboost::int32_t *p=(lslboost::int32_t*)data,*e=p+num_channels_; p<e; lsllslboost::endian::reverse(*p++)); break;
#ifndef BOOST_NO_INT64_T
				case sizeof(lslboost::int64_t): for (lslboost::int64_t *p=(lslboost::int64_t*)data,*e=p+num_channels_; p<e; lsllslboost::endian::reverse(*p++)); break;
#else
				case sizeof(double): for (double *p=(double*)data,*e=p+num_channels_; p<e; lsllslboost::endian::reverse(*p++)); break;
#endif
