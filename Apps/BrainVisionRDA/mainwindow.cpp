...
void MainWindow::read_thread(QString serverIP) {
	...
	try {
		...
		boost::scoped_ptr<MovingRegression> drift_correction(new MovingRegression());
		while(!stop_) {
						...
						while(!stop_ && !rda_client->messages().empty())
						{
							...
							case RDA_message::DATA32:
								if(lsl_outlet)
								{
									...
									long blockNumber = msg.decode_data32(rdaInfo.channelResolutions, samples, markers);
									...
									else
									{
										...
										{
											// blocks lost, reset timestamp adjustment and send new segment marker
											lostBlocks += blockNumber - blockCounter;
											blockCounter = blockNumber;
											firstTimestamp = -1.0;
										}
									}

									// adjust timestamps to the nominal sampling rate
									now = lsl::local_clock(); 
									if(firstTimestamp < 0)
									{
										sampleCounter = samples.size();
										firstTimestamp = now - (double)(sampleCounter-1) / rdaInfo.sampleRate;
										drift_correction->reset((long)(200.0 / (double)samples.size() * rdaInfo.sampleRate)); // 200s window size
										timeDrift = 0.0;
										timeOffset = -0.02; // buffer lag
										lsl_event_outlet->push_sample(&std::string("new segment"));
										OutputDebugStringA("LSL_RDA start new segment\n");
									}
									timestamp = firstTimestamp + (double)(sampleCounter-1) / rdaInfo.sampleRate;
									timeDrift = drift_correction->get(timestamp, now - timestamp);
									if(blockCounter % 1000 == 0)
										OutputDebugStringA(QString("LSL_RDA time drift: %1s offset: %2s time: %3s\n").arg(timeDrift).arg(timeOffset).arg(now).toStdString().c_str());
									timestamp += timeDrift + timeOffset;

									// send samples
									lsl_outlet->push_chunk(samples, timestamp);
									if(lostBlocks)
										emit sendMessage(QString("samples received: %1, packets lost: %2").arg(sampleCounter).arg(lostBlocks));
									else
										emit sendMessage(QString("samples received: %1").arg(sampleCounter));
									// send markers
									timestamp = timestamp - (double)(samples.size()-1) / rdaInfo.sampleRate;
									for(size_t mkr = 0; mkr < markers.size(); mkr++)
									{
										double tm = timestamp + (double)markers[mkr].samplePosition / rdaInfo.sampleRate;
										if(markers[mkr].description.size())
											lsl_event_outlet->push_sample(&markers[mkr].description, tm, false);
										else
											lsl_event_outlet->push_sample(&markers[mkr].type, tm, false);
									}
								}
								break;
							...
							case RDA_message::STOP:
								...
								emit sendMessage(QString("acquisition stopped, samples received: %1").arg(sampleCounter));
								break;

							case RDA_message::NEWSTATE:
								// recorder state changed, reset timestamp and drift correction
								firstTimestamp = -1;
								blockCounter = -15;
								break;
							}
