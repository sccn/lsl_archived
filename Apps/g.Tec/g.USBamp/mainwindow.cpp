...
void MainWindow::link() {
	...
	if (reader_thread_) {
		...
		try {
			...
			hDevice = GT_OpenDevice(deviceNumber);
			if (!hDevice)
				throw std::runtime_error("Could not open device. Please make sure that the device is plugged in, turned on, and the driver is installed correctly.");
...
void MainWindow::read_thread(int deviceNumber, int chunkSize, int samplingRate, bool isSlave, std::string serialNumber, int channelCount, std::vector<std::string> channelLabels) {
	...
	try {
		...
		lsl::stream_info data_info("g.USBamp"+boost::lexical_cast<std::string>(deviceNumber),"EEG",channelCount,samplingRate,lsl::cf_float32,"gUSBamp_" + boost::lexical_cast<std::string>(deviceNumber) + "_" + boost::lexical_cast<std::string>(serialNumber));
		...
		lsl::stream_info marker_info("gUSBamp-Markers","Markers",1,0,lsl::cf_string,"gUSBamp_" + boost::lexical_cast<std::string>(deviceNumber) + "_" + boost::lexical_cast<std::string>(serialNumber) + "_markers");
					...
					if (GetOverlappedResult(hDevice, pOverlapped, &bytes_read, FALSE) && bytes_read == chunk_bytes) {
						...
						for (int s=0;s<chunkSize;s++)
							for (int c=0;c<channelCount;c++)
								send_buffer[s][c] = scale*src_buffer[s + c*(channelCount+1)];
