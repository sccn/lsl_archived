...
lsl::channel_format_t fmts[] = {lsl::cf_int8,    lsl::cf_int16,    lsl::cf_int32,
                                lsl::cf_float32, lsl::cf_double64, lsl::cf_string};
...
void run_outlet(const double duration_ = 0.0, const string& name_ = "", const string& type_ = "",
                const int numchan_ = 0, const lsl::channel_format_t fmt_ = lsl::cf_undefined,
                const double srate_ = 0.0, const double seconds_between_failures_ = 0.0,
                const int chunk_len_ = 0) {
	num_outlets++;
	...
	try {
		...
		for (double endtime = lsl::local_clock() + duration; lsl::local_clock() < endtime;) {
			...
			cout << "new outlet(" << name << "," << type << "," << numchan << "," << fmt << ","
			     << srate << ")...";
			...
			cout << "del outlet(" << name << "," << type << "," << numchan << "," << fmt << ","
			     << srate << ")" << endl;
			// downtime
...
void run_inlet(const double duration_ = 0.0, const string& name_ = "", const string& type_ = "",
	...
	try {
		...
		for (double endtime = lsl::local_clock() + duration; lsl::local_clock() < endtime;) {
			...
			init_sample(numchans * (int)ceil(max_chunk_len_ms * result.nominal_srate() / 1000 *
			            ...
			            chunk);
			if (request_info) { cout << "  info = " << inlet.info(1.0).name() << endl; }
			...
			if (request_time) cout << "  tcorr = " << t << endl;
			// downtime
			sleep(.1 * (rand() % 50));
		}
...
void random_inlets(double spawn_every = 0.0, double duration = 0.0, string name = "",
                   string type = "", int in_chunks = -1, int request_info = -1,
...
void random_outlets(double spawn_every = 0.0, double duration = 0.0, string name = "",
                    string type = "", int numchan = 0,
