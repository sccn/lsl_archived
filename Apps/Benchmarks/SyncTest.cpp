...
enum processing_options_t {
	post_none = 0, // No automatic post-processing; return the ground-truth time stamps for manual
	               // post-processing
	// (this is the default behavior of the inlet).
	post_clocksync = 1, // Perform automatic clock synchronization; equivalent to manually adding
	                    // the time_correction() value
	// to the received time stamps.
	post_dejitter = 2, // Remove jitter from time stamps. This will apply a smoothing algorithm to
	                   // the received time stamps;
	// the smoothing needs to see a minimum number of samples (1-2 minutes) until the remaining
	// jitter is consistently below 1ms.
	post_monotonize = 4, // Force the time-stamps to be monotonically ascending (only makes sense if
	                     // timestamps are dejittered).
	post_threadsafe = 8, // Post-processing is thread-safe (same inlet can be read from by multiple
...
/// (This is a version of time_postprocessor.cpp with the callback functions/hooks removed for
/// easier testing)
...
public:
	...
	time_postprocessor(double nominal_rate)
	    : nominal_rate_(nominal_rate), next_query_time_(0.0), last_offset_(0.0), samples_seen_(0.0),
	      options_(post_none), halftime_(90.0), smoothing_initialized_(false),
	...
	/**
	 * Set post-processing options to use. By default, this class performs NO post-processing and
	 * returns the ground-truth time stamps, which can then be manually synchronized using
	 * time_correction(), and then smoothed/dejittered if desired. This function allows automating
	 * these two and possibly more operations.
	 * @param flags An integer that is the result of bitwise OR'ing one or more options from
	 * processing_options_t together (e.g., post_clocksync|post_dejitter); the default is to enable
	 * all options.
	 */
...
private:
	...
	double process_internal(double value) {
		...
		if (options_ & post_clocksync) {
			// update last correction value if needed (we do this every 100 samples and at most once
			// per second)
	...
	double
	    baseline_value_; // first observed time-stamp value, used as a baseline to improve numerics
	...
	bool smoothing_applicable_;    // whether smoothing can be applied to these data (false if
	                               // irregular srate)
	...
	std::mutex processing_mut_; // a mutex that protects the runtime data structures
};

// the scripts that regenerate these buffers (and evaluate results) can be found at:
// ftp://sccn.ucsd.edu/pub/christian/SyncTest
const int rates[] = {50, 100, 200, 500, 1000, 2000};
const int n_chunks = 100;

int main(int argc, char** argv) {
	if (argc < 2)
		throw std::runtime_error("Data path not provided as first argument.\nSee "
		                         "ftp://sccn.ucsd.edu/pub/christian/SyncTest");
	...
	for (double rate : rates) {
		...
		loadpath += ("/NR" + std::to_string(rate));
		// run test
		double sum_errors = 0.0;
		int n_samples = 0;
		for (int chunk = 1; chunk <= n_chunks; chunk++) {
			std::string true_data = loadpath + "_YMatrix_" + std::to_string(chunk) + ".bin";
			std::string jittered_data = loadpath + "_ZMatrix_" + std::to_string(chunk) + ".bin";
			std::string estimated_data = loadpath + "_WMatrix_" + std::to_string(chunk) + ".bin";
			std::ifstream true_stream(true_data, std::ifstream::binary);
			std::ifstream jittered_stream(jittered_data, std::ifstream::binary);
			std::ofstream estimated_stream(estimated_data,
			...
			while (true) {
				...
				double jittered_value = 0.0;
				jittered_stream.read((char*)&jittered_value, sizeof(jittered_value));
				if (!jittered_stream) break;
				double estimated_value = processor.process_timestamp(jittered_value);
				double true_value = 0.0;
				true_stream.read((char*)&true_value, sizeof(true_value));
