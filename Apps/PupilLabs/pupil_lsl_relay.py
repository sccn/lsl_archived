...
class Pupil_LSL_Relay(Plugin):
    ...
    """
    def __init__(self, g_pool, relay_pupil=True,relay_gaze=True,relay_notifications=True):
        """Summary
    ...
    def init_gui(self):
        ...
        help_str = 'Pupil LSL Relay subscribes to the Pupil ZMQ Backbone and relays the incoming data using the lab streaming layer.'
        ...
        self.menu.append(ui.Switch(
            'relay_notifications',self,
            label='Relay notifications',
            setter=make_setter(NOTIFY_SUB_TOPIC,'relay_notifications')))

    def get_init_dict(self):
        """Store settings"""
        return {'relay_pupil':self.relay_pupil,'relay_gaze':self.relay_gaze,'relay_notifications':self.relay_notifications}
    ...
    def thread_loop(self,context,pipe):
        ...
        try:
            ...
            inlet = zmq_tools.Msg_Receiver(context,self.g_pool.ipc_sub_url,block_until_connected=False)
            poller = zmq.Poller()
            ...
            while True:
                ...
                if inlet.socket in items:
                    ...
                    elif topic.startswith(NOTIFY_SUB_TOPIC) and notification_outlet:
                        sample = (payload['subject'], repr(payload))
                        notification_outlet.push_sample(sample)

                    else: logger.debug('Did not handle topic "%s"'%topic)
                ...
                if pipe in items:
                    ...
                    if cmd == 'Exit': break
                        ...
                        if topic == PUPIL_SUB_TOPIC and not pupil_outlets:
                            ...
                            gaze_outlets = self._create_gaze_lsl_outlets()
                        elif topic == NOTIFY_SUB_TOPIC and not notification_outlet:
                            notification_outlet = self._create_notify_lsl_outlet()
            ...
            logger.error('Error during relaying data to LSL. Unloading the plugin...')
    ...
    def _create_gaze_lsl_outlets(self):
        ...
        return (self._create_primitive_lsl_outlet('Gaze Primitive Data'),
            self._create_python_repr_lsl_outlet('Gaze Python Representation'))
    ...
    def _create_notify_lsl_outlet(self):
        ...
        self._append_channel_info(notification_info, ("subject", "Python Representation"))
