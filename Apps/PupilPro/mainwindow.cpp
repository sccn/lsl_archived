...
void MainWindow::listen(QString serverIP, QString port, int fps){
	...
	try {
		...
		while(!stop_)
		{
			...
			//smp = 1.0;
			zmq_client_.socket_pull();
			MainWindow::parse(zmq_client_.msg_string(), sample, markers, marker_cnt);
...
void MainWindow::connect(QString serverIP, QString port){
	
	zmq_client_.connect("tcp://"+serverIP.toStdString()+":"+port.toStdString());
	zmq_client_.set_timeout(5000);
}
...
void MainWindow::link_pupilpro() {
	...
	if (listen_thread_) {
		...
		try {
			...
			connect_thread_.reset(new boost::thread(&MainWindow::connect,this,serverIP, port));
			if(!connect_thread_->try_join_for(boost::chrono::seconds(10)))
				QMessageBox::critical(this,"Error", (std::string("Could not connect to server tcp://") + serverIP.toStdString() + std::string(":") + port.toStdString()).c_str(),QMessageBox::Ok);
			else
			{
				...
				// done, all successful
				ui->connectionStatus->setText((std::string("Receiving on tcp://") + serverIP.toStdString() + std::string(":") + port.toStdString()).c_str());
