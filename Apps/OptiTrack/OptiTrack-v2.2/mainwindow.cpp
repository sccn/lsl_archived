...
const bool use_multicast = false;						// we're not using multicast since it may be dysfunctional on some routers / OSs
...
void MainWindow::load_config(const string &filename) {
	...
	try {
		...
		string camtypes = pt.get<string>("cameras.knowntypes","[S250e, Flex 13, V100:R2, V120:Duo, V120:Trio, V120:SLIM]");
